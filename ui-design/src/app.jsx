...
const App = () => {
  ...
  const T = React.useMemo(() => {
    const themeKey = window.EI_THEMES[tweaks.theme] ? tweaks.theme : "warm";
    const isDark = !!tweaks.dark;
    return { ...window.EI_THEMES[themeKey][isDark ? "dark" : "light"] };
  }, [tweaks.dark, tweaks.theme]);
  ...
  return (
    <div style={{ minHeight: "100vh", background: T.bg, color: T.ink, fontFamily: "var(--ei-sans)" }} data-screen-label={route.name}>
      {!hideTopBar && <TopBar T={T} route={{ ...route, name: activeRouteName }} nav={nav} lang={lang} setLang={setLang} dark={tweaks.dark} setDark={(v) => updateTweak("dark", v)} theme={tweaks.theme} setTheme={(v) => updateTweak("theme", v)} signedIn={signedIn} signOut={() => nav("auth_logout")} />}
...
const TopBar = ({ T, route, nav, lang, setLang, dark, setDark, theme, setTheme, signedIn, signOut }) => {
    ...
    <div style={{
      ...
      <TweakRow T={T} label="Theme">
        <div style={{ display: "flex", gap: 6 }}>
          {(window.EI_THEME_LIST || []).map((t) => {
            const selected = tweaks.theme === t.key;
            return (
              <button key={t.key} onClick={() => updateTweak("theme", t.key)} title={t.labelEn} style={{
