// P2 screens: Voice practice in-progress & Multi-round plan orchestrator
...
// ───────────── MULTI-ROUND PLAN ORCHESTRATOR ─────────────
const PlanScreen = ({ T, lang, nav }) => {
  const [activeIdx, setActiveIdx] = React.useState(1);

  const rounds = [
    {
      id: "hr",
      name: lang === "en" ? "HR screen" : "HR 初筛",
      role: lang === "en" ? "People operations" : "HR 面试官",
      mins: 20,
      status: "done", // done / active / locked
      date: "4/20 · 10:00",
      focus: lang === "en" ? "Motivation · timing · compensation" : "动机 · 节奏 · 薪资期望",
      questions: 5,
      readiness: 3,
      readinessLabel: lang === "en" ? "Well-prepared" : "较为充分",
      carryOver: [
        lang === "en" ? "Confirmed: can start June 1 — mention up front in tech round." : "已确认：6/1 可到岗——技术面要主动提。",
        lang === "en" ? "Salary band accepted — no need to revisit." : "薪资区间已对齐——不再复述。",
      ],
    },
    {
      id: "tech-1",
      name: lang === "en" ? "Tech round 1" : "技术一面",
      role: lang === "en" ? "Senior engineer" : "资深工程师",
      mins: 45,
      status: "active",
      date: lang === "en" ? "today · 15:00" : "今天 · 15:00",
      focus: lang === "en" ? "React internals · perf · TS design" : "React 底层 · 性能 · TS 类型设计",
      questions: 8,
      readiness: 2,
      readinessLabel: lang === "en" ? "Practice more" : "建议再练",
      carryOver: [
        lang === "en" ? "⚠ Quantify the perf story (LCP from 3.2s to 1.4s)." : "⚠ 性能优化故事要带量化（LCP 3.2s → 1.4s）。",
        lang === "en" ? "Prep a trade-off narrative for Design System migration." : "准备 Design System 迁移的「替代方案权衡」叙事。",
      ],
      mistakesTagged: 3,
    },
    {
      id: "tech-2",
      name: lang === "en" ? "Tech round 2" : "技术二面",
      role: lang === "en" ? "Tech lead" : "技术负责人",
      mins: 60,
      status: "locked",
      date: "4/23 · TBD",
      focus: lang === "en" ? "Architecture · cross-team influence" : "架构决策 · 跨团队影响",
      questions: 6,
      readiness: 1,
      readinessLabel: lang === "en" ? "Minimum viable" : "基本可面",
      prep: [
        lang === "en" ? "Tech 1 feedback auto-imports here on completion." : "技术一面的反馈会自动导入到这一轮。",
      ],
    },
    {
      id: "manager",
      name: lang === "en" ? "Hiring manager" : "经理面",
      role: lang === "en" ? "Engineering manager" : "用人经理",
      mins: 40,
      status: "locked",
      date: "4/24 · 14:00",
      focus: lang === "en" ? "Influence · conflict · alignment" : "影响力 · 冲突处理 · 目标一致性",
      questions: 5,
      readiness: 0,
      readinessLabel: lang === "en" ? "Not ready" : "未就绪",
      prep: [
        lang === "en" ? "Must: one end-to-end ownership story, one conflict you resolved." : "必备：一个 end-to-end owner 故事 + 一次你解决的冲突。",
      ],
    },
  ];

  const active = rounds[activeIdx];

  const statusDot = (s) => ({
    done: { bg: T.ok, fg: "#fff", label: lang === "en" ? "Done" : "已完成" },
    active: { bg: T.accent, fg: "#fff", label: lang === "en" ? "Now" : "进行中" },
    locked: { bg: T.bgSoft, fg: T.ink3, label: lang === "en" ? "Upcoming" : "待进行" },
  }[s]);

  return (
    <div className="ei-fadein" style={{ maxWidth: 1280, margin: "0 auto", padding: "40px 48px 96px" }}>
      <button onClick={() => nav("workspace", { jobId: "tj-1" })} style={{ background: "transparent", border: "none", color: T.ink3, cursor: "pointer", display: "flex", gap: 6, alignItems: "center", fontSize: 13, padding: 0, marginBottom: 18 }}>
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-end", marginBottom: 36, gap: 32 }}>
        <div>
          <div className="ei-label" style={{ color: T.ink3, marginBottom: 10 }}>
            {lang === "en" ? "MULTI-ROUND PLAN · 星环科技 · 资深前端" : "多轮计划 · 星环科技 · 资深前端"}
          </div>
          <h1 className="ei-serif" style={{ fontSize: 40, margin: 0, color: T.ink, letterSpacing: "-0.025em", lineHeight: 1.15, maxWidth: 780 }}>
            {lang === "en"
              ? "Four rounds, one chain of evidence."
              : "四轮串成一条链——每一轮都吃上一轮的观察。"}
          </h1>
          <div style={{ fontSize: 14, color: T.ink3, marginTop: 10, maxWidth: 640, lineHeight: 1.5 }}>
            {lang === "en"
              ? "Each round inherits what the previous one exposed: unresolved mistakes, strong stories, and explicit carry-overs. Nothing gets re-asked for no reason."
              : "每一轮都会继承上一轮暴露的弱点、已经稳下来的故事、和明确标注的延续项。不做重复提问，也不会凭空加题。"}
          </div>
        </div>
      ...
      <div style={{ position: "relative", marginBottom: 48 }}>
        {/* baseline */}
        <div style={{ position: "absolute", top: 28, left: 44, right: 44, height: 2, background: T.rule }} />
        <div style={{
          position: "absolute", top: 28, left: 44,
          width: `calc(${(activeIdx / (rounds.length - 1)) * 100}% - 0px)`,
          height: 2, background: T.accent,
        }} />

        <div style={{ display: "grid", gridTemplateColumns: `repeat(${rounds.length}, 1fr)`, gap: 0, position: "relative" }}>
          {rounds.map((r, i) => {
            const s = statusDot(r.status);
            const isActive = i === activeIdx;
            return (
              <div key={r.id} style={{ display: "flex", flexDirection: "column", alignItems: "center", padding: "0 12px" }}>
                {/* node */}
                <button onClick={() => setActiveIdx(i)} style={{
                  width: 56, height: 56, borderRadius: 28, background: s.bg, color: s.fg,
                  border: `3px solid ${isActive ? T.accent : T.bg}`,
                  cursor: "pointer", fontFamily: "var(--ei-mono)", fontSize: 13, fontWeight: 600,
                  display: "flex", alignItems: "center", justifyContent: "center",
                  boxShadow: isActive ? `0 0 0 4px ${T.accentSoft}` : "none",
                  position: "relative", zIndex: 2, transition: "box-shadow .15s",
                }}>
                  {r.status === "done" ? <Icon name="check" size={18} /> : r.status === "locked" ? <Icon name="clock" size={14} /> : String(i + 1).padStart(2, "0")}
                </button>
                <div style={{ marginTop: 14, textAlign: "center", cursor: "pointer" }} onClick={() => setActiveIdx(i)}>
                  <div className="ei-serif" style={{ fontSize: 16, color: isActive ? T.ink : T.ink2, fontWeight: 500 }}>{r.name}</div>
                  <div style={{ fontSize: 11, color: T.ink3, fontFamily: "var(--ei-mono)", marginTop: 3, letterSpacing: "0.04em" }}>{r.date} · {r.mins}min</div>
                  <div style={{ marginTop: 8 }}>
                    <Tag T={T} tone={r.status === "done" ? "ok" : r.status === "active" ? "accent" : "muted"}>
                      {s.label}
                    </Tag>
                  </div>
                </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1.5fr 1fr", gap: 24 }}>
        ...
        <Card T={T} pad={28}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 22 }}>
            <div>
              <div className="ei-label" style={{ color: T.accent, marginBottom: 8 }}>
                {active.status === "active" ? (lang === "en" ? "CURRENT ROUND" : "当前轮次") : active.status === "done" ? (lang === "en" ? "COMPLETED" : "已完成") : (lang === "en" ? "UPCOMING" : "待进行")}
              </div>
              <div className="ei-serif" style={{ fontSize: 26, color: T.ink, letterSpacing: "-0.015em" }}>{active.name}</div>
              <div style={{ fontSize: 13.5, color: T.ink3, marginTop: 6 }}>{active.role} · {active.date}</div>
            </div>
            <ReadinessDial level={active.readiness} T={T} size={54} />
          </div>

          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14, marginBottom: 22 }}>
            <div style={{ background: T.bgSoft, padding: "12px 14px", borderRadius: 2 }}>
              <div className="ei-label" style={{ color: T.ink3, marginBottom: 5 }}>{lang === "en" ? "FOCUS" : "本轮焦点"}</div>
              <div style={{ fontSize: 13.5, color: T.ink, lineHeight: 1.5 }}>{active.focus}</div>
            </div>
            <div style={{ background: T.bgSoft, padding: "12px 14px", borderRadius: 2 }}>
              <div className="ei-label" style={{ color: T.ink3, marginBottom: 5 }}>{lang === "en" ? "READINESS" : "准备度"}</div>
              <div style={{ fontSize: 13.5, color: T.ink, lineHeight: 1.5 }}>
                {active.readinessLabel}
                <span style={{ color: T.ink3, marginLeft: 8, fontSize: 12 }}>· {active.questions} {lang === "en" ? "questions planned" : "题已规划"}</span>
              </div>
            </div>
          </div>
          ...
          {active.carryOver && (
            <div style={{ marginBottom: 22 }}>
              <div className="ei-label" style={{ color: T.ink3, marginBottom: 10, display: "flex", gap: 8, alignItems: "center" }}>
                <Icon name="layers" size={12} />
                {lang === "en" ? "CARRY-OVER FROM PREVIOUS ROUND" : "从上一轮继承"}
              </div>
              <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                {active.carryOver.map((c, i) => (
                  <div key={i} style={{
                    display: "flex", gap: 10, padding: "10px 14px", background: T.accentSoft,
                    borderLeft: `2px solid ${T.accent}`, fontSize: 13.5, lineHeight: 1.5, color: T.ink,
                  }}>
                    <Icon name="arrow_right" size={13} color={T.accent} style={{ marginTop: 3, flexShrink: 0 }} />
                    <span>{c}</span>
                  </div>
                ))}
              </div>
          ...
          {active.prep && (
            <div style={{ marginBottom: 22 }}>
              <div className="ei-label" style={{ color: T.ink3, marginBottom: 10 }}>
                {lang === "en" ? "PREP CHECKLIST" : "准备清单"}
              </div>
              <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                {active.prep.map((p, i) => (
                  <div key={i} style={{ display: "flex", gap: 10, padding: "8px 0", borderBottom: `1px dotted ${T.rule}`, fontSize: 13.5, color: T.ink2 }}>
                    <Icon name="dot" size={10} color={T.ink3} style={{ marginTop: 5 }} />
          ...
          {active.mistakesTagged != null && (
            <div style={{ padding: "12px 14px", background: T.warnSoft, border: `1px solid ${T.warn}`, borderRadius: 2, fontSize: 13, color: T.warn, display: "flex", gap: 10, alignItems: "center" }}>
              <Icon name="book" size={14} />
              <span>
                {lang === "en" ? `${active.mistakesTagged} open mistakes tagged to this round — practice them before you start.` : `本轮关联 ${active.mistakesTagged} 道未攻克错题——开始前建议先复练。`}
              </span>
              <button onClick={() => nav("mistakes")} style={{ marginLeft: "auto", background: "transparent", border: "none", color: T.warn, cursor: "pointer", fontSize: 12.5, fontWeight: 500, display: "flex", gap: 4, alignItems: "center" }}>
                {lang === "en" ? "Open" : "去复练"} <Icon name="arrow_right" size={12} />
              </button>
            </div>
          )}
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
          <Card T={T} pad={20}>
            <div className="ei-label" style={{ color: T.ink3, marginBottom: 14 }}>{lang === "en" ? "CHAIN SIGNAL" : "轮次间信号"}</div>
            <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
              {[
                { l: lang === "en" ? "Stories consolidated" : "已稳住的故事", v: "2 / 5", tone: "ok", hint: lang === "en" ? "自我介绍、离职原因" : "自我介绍、离职原因" },
                { l: lang === "en" ? "Open mistakes" : "未攻克错题", v: "3", tone: "warn", hint: lang === "en" ? "performance quant · design-system rollout" : "性能量化 · Design System 落地" },
                { l: lang === "en" ? "Evidence coverage" : "证据覆盖率", v: "62%", tone: "warn", hint: lang === "en" ? "still missing: cross-team stories" : "仍缺：跨团队协作案例" },
                { l: lang === "en" ? "Predicted fatigue" : "预计疲劳度", v: lang === "en" ? "low" : "低", tone: "ok", hint: lang === "en" ? "rounds spaced out well" : "轮次间隔合理" },
              ].map((m) => {
                const c = { ok: T.ok, warn: T.warn, danger: T.danger }[m.tone];
                return (
                  <div key={m.l} style={{ display: "flex", justifyContent: "space-between", alignItems: "baseline", paddingBottom: 12, borderBottom: `1px dotted ${T.rule}` }}>
                    <div>
                      <div style={{ fontSize: 13, color: T.ink2 }}>{m.l}</div>
                      <div style={{ fontSize: 11.5, color: T.ink3, marginTop: 3, fontFamily: "var(--ei-mono)" }}>{m.hint}</div>
                    </div>
                    <div style={{ fontFamily: "var(--ei-mono)", fontSize: 15, color: c, fontWeight: 500 }}>{m.v}</div>
                  </div>
                );
              })}
            </div>
          ...
          <Card T={T} pad={20}>
            <div className="ei-label" style={{ color: T.ink3, marginBottom: 12 }}>{lang === "en" ? "NARRATIVE DEPENDENCIES" : "叙事依赖图"}</div>
            <div style={{ display: "flex", flexDirection: "column", gap: 10, fontSize: 12.5, color: T.ink2 }}>
              {[
                { from: "HR", to: lang === "en" ? "Tech 1" : "技术一面", what: lang === "en" ? "start date" : "到岗时间" },
                { from: lang === "en" ? "Tech 1" : "技术一面", to: lang === "en" ? "Tech 2" : "技术二面", what: lang === "en" ? "perf & TS depth" : "性能 & TS 深度" },
                { from: lang === "en" ? "Tech 1" : "技术一面", to: lang === "en" ? "Manager" : "经理面", what: lang === "en" ? "cross-team case" : "跨团队案例" },
                { from: lang === "en" ? "Tech 2" : "技术二面", to: lang === "en" ? "Manager" : "经理面", what: lang === "en" ? "architecture taste" : "架构判断" },
              ].map((d, i) => (
                <div key={i} style={{ display: "flex", gap: 8, alignItems: "center", lineHeight: 1.4 }}>
                  <span style={{ color: T.ink3, fontFamily: "var(--ei-mono)", fontSize: 11, minWidth: 68 }}>{d.from}</span>
                  <Icon name="arrow_right" size={11} color={T.ink4} />
                  <span style={{ color: T.ink3, fontFamily: "var(--ei-mono)", fontSize: 11, minWidth: 72 }}>{d.to}</span>
