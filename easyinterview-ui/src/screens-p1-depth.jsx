// P1 depth: Real-interview debrief + thank-you letter, Experience library, Resume versions + diff

// ═══════════════════════════════════════════════════════════════════
// #9 REAL-INTERVIEW DEBRIEF (full version) + THANK-YOU LETTER
// ═══════════════════════════════════════════════════════════════════
const DebriefFullScreen = ({ T, lang, nav }) => {
  const [step, setStep] = React.useState(0); // 0 record, 1 compare, 2 letter

  const steps = lang === "en"
    ? ["Record what happened", "Compare to simulation", "Thank-you & follow-up"]
    : ["记录真实经过", "对比模拟表现", "感谢信 & 跟进"];

  // Mock real Q&A entries
  const [entries, setEntries] = React.useState([
    { id: "e1", q: lang === "en" ? "Walk me through the checkout perf work — what exactly did YOU drive?" : "跟我讲讲结账性能那个项目——具体哪些是你推动的？", a: lang === "en" ? "I talked about RSC migration, but fumbled on the 'you specifically' part." : "讲了 RSC 迁移，但「具体你做了什么」那里卡住了。", reaction: "probed", flagged: true, tag: lang === "en" ? "ownership" : "Ownership" },
    { id: "e2", q: lang === "en" ? "How would you roll out a Design System across 8 teams?" : "8 个团队的 Design System 你会怎么推？", a: lang === "en" ? "Started with tokens, pilot team, workshops. They nodded. Moved on fast." : "讲了 tokens、先挑一个团队试点、办推广会。对方点头，很快切下一题。", reaction: "positive", flagged: false, tag: lang === "en" ? "influence" : "影响力" },
    { id: "e3", q: lang === "en" ? "Any questions for me?" : "你有什么想问我的吗？", a: lang === "en" ? "Asked about engineering culture. Didn't ask about the team's top pain or his personal take." : "问了工程文化。没问团队眼下最头疼的事、也没问他个人看法。", reaction: "neutral", flagged: true, tag: lang === "en" ? "reverse-Q" : "反问" },
  ]);
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-end", marginBottom: 32, gap: 32 }}>
        <div>
          ...
          <h1 className="ei-serif" style={{ fontSize: 38, margin: 0, color: T.ink, letterSpacing: "-0.022em", lineHeight: 1.15, maxWidth: 780 }}>
            {lang === "en" ? "Fresh out of the room — turn it into ammo for the next round." : "刚从会议室出来——把它变成下一轮的弹药。"}
          </h1>
          <div style={{ fontSize: 14, color: T.ink3, marginTop: 10, maxWidth: 640, lineHeight: 1.5 }}>
            {lang === "en" ? "The faster you write it down, the sharper it stays. We'll diff it against your simulation and draft a thank-you note from the same facts." : "越早写下来越锋利。我们会把它和你的模拟表现对比，再从同一组事实里起草一封感谢信。"}
          </div>
        </div>
      ...
      {step === 0 && (
        <div>
          <div style={{ display: "grid", gridTemplateColumns: "1fr 320px", gap: 28 }}>
            <div>
              <div className="ei-label" style={{ color: T.ink3, marginBottom: 12 }}>{lang === "en" ? "QUESTIONS THEY ASKED" : "他们问了什么"}</div>
              <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
                {entries.map((e, idx) => (
                  <div key={e.id} style={{ padding: "18px 20px", background: T.bgCard, border: `1px solid ${T.rule}`, borderLeft: `3px solid ${e.flagged ? T.warn : T.rule}`, borderRadius: 2 }}>
                    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: 12, marginBottom: 10 }}>
                      <div style={{ fontFamily: "var(--ei-mono)", fontSize: 10.5, color: T.ink4, letterSpacing: "0.08em" }}>Q{idx + 1} · {e.tag.toUpperCase()}</div>
                      <div style={{ display: "flex", gap: 6 }}>
                        {Object.keys(reactions).map((r) => (
                          <button key={r} onClick={() => setEntries(entries.map(x => x.id === e.id ? { ...x, reaction: r } : x))} style={{
                            padding: "3px 8px", fontSize: 10.5, borderRadius: 2, cursor: "pointer",
                            border: `1px solid ${e.reaction === r ? T.ink2 : T.rule}`,
                            background: e.reaction === r ? T.bgSoft : "transparent",
                            color: e.reaction === r ? T.ink : T.ink3, fontFamily: "var(--ei-mono)",
                          }}>
                            {reactions[r]}
                          </button>
                        ))}
                    ...
                    <div className="ei-serif" style={{ fontSize: 17, color: T.ink, letterSpacing: "-0.01em", marginBottom: 10, lineHeight: 1.4 }}>{e.q}</div>
                    <textarea defaultValue={e.a} rows={3} style={{ width: "100%", padding: "10px 12px", fontSize: 13.5, color: T.ink2, background: T.bgSoft, border: `1px solid ${T.rule}`, borderRadius: 2, resize: "vertical", lineHeight: 1.5, outline: "none", boxSizing: "border-box", fontFamily: "var(--ei-sans)" }} placeholder={lang === "en" ? "What you actually said — rough notes are fine" : "你当时是怎么回的——草稿即可"} />
                    <div style={{ marginTop: 10, display: "flex", gap: 8, alignItems: "center" }}>
                      <label style={{ display: "flex", gap: 6, alignItems: "center", fontSize: 12, color: T.ink3, cursor: "pointer" }}>
                        <input type="checkbox" checked={e.flagged} onChange={() => setEntries(entries.map(x => x.id === e.id ? { ...x, flagged: !x.flagged } : x))} style={{ accentColor: T.warn }} />
                        {lang === "en" ? "Flag for next round" : "带到下一轮"}
                      </label>
                      <div style={{ flex: 1 }} />
                      <button style={{ background: "transparent", border: "none", color: T.ink3, fontSize: 12, cursor: "pointer" }}>
                        <Icon name="trash" size={11} /> {lang === "en" ? "Remove" : "删除"}
                      </button>
                    </div>
                  </div>
                ))}
                <button style={{ padding: "14px 20px", background: "transparent", border: `1px dashed ${T.rule}`, borderRadius: 2, color: T.ink3, cursor: "pointer", fontSize: 13 }}>
                  <Icon name="plus" size={12} style={{ marginRight: 6 }} /> {lang === "en" ? "Add another question" : "再记一题"}
                </button>
              </div>
            </div>
            ...
            <div>
              <Card T={T} pad={18} style={{ position: "sticky", top: 20 }}>
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: 12, fontSize: 13 }}>
                  ...
                  <div>
                    <div style={{ color: T.ink2, marginBottom: 5 }}>{lang === "en" ? "What they seemed to like" : "他们似乎欣赏的"}</div>
          ...
          <div style={{ display: "flex", justifyContent: "flex-end", marginTop: 28 }}>
            <Btn T={T} variant="accent" iconRight="arrow_right" onClick={() => setStep(1)}>{lang === "en" ? "Compare to simulation" : "与模拟对比"}</Btn>
          </div>
        </div>
      )}
      ...
      {step === 1 && (
        <div>
          <div className="ei-label" style={{ color: T.ink3, marginBottom: 14 }}>
            {lang === "en" ? "REAL vs SIMULATION" : "真实 vs 模拟"}
          </div>
          ...
          <div className="ei-label" style={{ color: T.ink3, marginBottom: 10 }}>{lang === "en" ? "CARRIED INTO MISTAKE BOOK" : "已写入错题本"}</div>
          <div style={{ display: "flex", flexDirection: "column", gap: 10, marginBottom: 28 }}>
            {[
              lang === "en" ? "Claiming ownership explicitly — practice 'I drove', 'I proposed', 'I decided'." : "明确声明 Ownership —— 练习「我推动」「我提出」「我决定」的表达。",
              lang === "en" ? "Reverse-Q discipline — 3 questions minimum, one about the person across from you." : "反问纪律 —— 至少 3 题，其中一题要问坐在对面的这个人。",
            ].map((m, i) => (
              <div key={i} style={{ display: "flex", gap: 10, padding: "10px 14px", background: T.warnSoft, borderLeft: `2px solid ${T.warn}`, fontSize: 13.5, lineHeight: 1.5, color: T.ink }}>
                <Icon name="book" size={13} color={T.warn} style={{ marginTop: 3, flexShrink: 0 }} />
                <span style={{ flex: 1 }}>{m}</span>
                <button onClick={() => nav("mistakes")} style={{ background: "transparent", border: "none", color: T.warn, fontSize: 12, cursor: "pointer" }}>
                  {lang === "en" ? "Open" : "去复练"} →
                </button>
              </div>
            ))}
          ...
          <div style={{ display: "flex", justifyContent: "space-between" }}>
            ...
            <Btn T={T} variant="accent" iconRight="arrow_right" onClick={() => setStep(2)}>{lang === "en" ? "Draft thank-you note" : "起草感谢信"}</Btn>
          </div>
        </div>
      )}

      {/* Step 2: Thank-you letter */}
      {step === 2 && <ThankYouLetter T={T} lang={lang} nav={nav} back={() => setStep(1)} entries={entries} />}
    </div>
  );
};
...
// ═══════════════════════════════════════════════════════════════════
const ResumeVersionsScreen = ({ T, lang, nav }) => {
  const [active, setActive] = React.useState("v2");
  const [selectedBullet, setSelectedBullet] = React.useState("b1");

  const versions = [
    { id: "master", name: lang === "en" ? "Master" : "主版本", tag: "MASTER", date: "2026 · 04 · 18", target: lang === "en" ? "General" : "通用", bullets: 24, tone: "neutral" },
    { id: "v1", name: lang === "en" ? "Star-Ring · Senior FE" : "星环 · 资深前端", tag: "TARGETED", date: "2026 · 04 · 20", target: "星环科技", bullets: 22, tone: "accent", active: true },
    { id: "v2", name: lang === "en" ? "Lumen · Platform" : "Lumen · 平台", tag: "TARGETED", date: "2026 · 04 · 19", target: "Lumen Labs", bullets: 18, tone: "accent" },
    { id: "draft", name: lang === "en" ? "Cloud-Yun · draft" : "云栖 · 草稿", tag: "DRAFT", date: "2026 · 04 · 21", target: "云栖集团", bullets: 20, tone: "muted" },
  ];
  ...
  return (
    <div className="ei-fadein" style={{ maxWidth: 1320, margin: "0 auto", padding: "40px 48px 96px" }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-end", marginBottom: 28, gap: 32 }}>
        ...
        <div style={{ display: "flex", gap: 10 }}>
          <Btn T={T} variant="secondary" size="sm" icon="plus">{lang === "en" ? "New version" : "新版本"}</Btn>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 10, marginBottom: 24 }}>
        {[
          { k: lang === "en" ? "Target" : "目标岗位", v: versions.find((v) => v.id === active)?.target },
          ...
          { k: lang === "en" ? "Reading time" : "阅读时长", v: "1m 40s", sub: lang === "en" ? "recruiter scan · 7s to qualify" : "HR 首次扫读 · 7s 过滤" },
        ].map((m) => (
