...
P0 主闭环：`JD 导入 → 目标岗位工作台 → 模拟面试 → 证据化报告 → 题目回顾 / 本轮复练 → 真实面试复盘`。
...
| [`test/`](./test/README.md) | 跨服务测试根目录与场景测试框架 |
...
| `make codegen-check` | B1 conventions + B2 OpenAPI generator drift gate（`git diff --exit-code`） |
