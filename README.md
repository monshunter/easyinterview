...
围绕具体目标岗位、JD、简历资产和模拟面试训练设计的 AI 面试训练产品。把「看到岗位 → 导入 JD / 简历 → 完成模拟面试 → 获得证据化报告 → 复练当前轮 / 进入下一轮」标准化、重复化、可视化，帮助高意图求职者在 24-72 小时内围绕一份真实 JD 完成高质量准备。

P0 主闭环：`JD 导入 → 目标面试规划 → 模拟面试 → 证据化报告 → 复练当前轮 / 进入下一轮`。
...
| [`scripts/`](./scripts/README.md) | 跨语言运维脚本、git hooks、codegen 与 lint 工具入口 |
...
| 入口 | 说明 |
|------|------|
| `make lint` | B1 conventions + A4 config + A3/F3/E1/runtime-topology local gates + Go lint + frontend typecheck-backed lint |
| `make test` | UI prototype Node contract + Python tooling/skill contracts + 后端 Go + 前端 TS 单元测试；AI 测试走 stub/fixture，**不读取真实 secret** |
...
| `make fmt` | 使用 `gofmt` 格式化 backend Go 源码 |
...
工具链版本由 [.tool-versions](./.tool-versions) 锁定（asdf / mise 兼容）。首次运行 Python gate 前，在隔离环境中执行 `python3 -m pip install -r requirements-dev.txt`。编辑器格式由 [.editorconfig](./.editorconfig) 锁定。
