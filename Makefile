...
lint-go-mod-tidy: ## Reject backend Go module metadata drift
	@locked_version="$$(awk '$$1 == "golang" { print $$2; exit }' "$(ROOT_DIR)/.tool-versions")"; \
	workspace_version="$$(awk '$$1 == "go" { print $$2; exit }' "$(ROOT_DIR)/go.work")"; \
	module_version="$$(awk '$$1 == "go" { print $$2; exit }' "$(ROOT_DIR)/backend/go.mod")"; \
	if [[ -z "$$locked_version" || "$$locked_version" != "$$workspace_version" || "$$locked_version" != "$$module_version" ]]; then \
		printf 'Go version drift: .tool-versions=%s go.work=%s backend/go.mod=%s\n' "$$locked_version" "$$workspace_version" "$$module_version" >&2; \
		exit 1; \
	fi; \
	cd "$(ROOT_DIR)/backend"; \
	go mod tidy -diff
...
lint-ai-provider-terminology: ## Reject out-of-scope AI gateway terminology in active AI provider surfaces
...
lint-mock-contract: validate-fixtures lint-openapi ## Validate fixture coverage, registry metadata, mock runtime boundaries, and out-of-scope mock/API tokens
...
lint-runtime-topology: ## Reject out-of-scope standalone backend worker process terminology in active code/docs
	@python3 "$(ROOT_DIR)/scripts/lint/runtime_topology.py" --repo-root "$(ROOT_DIR)"

test: ## A5 test aggregator: Python tooling/skill + backend Go + frontend tests; AI uses stub/fixture only
...
eval-offline: ## F3 offline eval (NOT in make test): single-source drift gate + exact 28 cases + Promptfoo runner over recorded fixtures; EVAL_LIVE=1 opts into real provider/judge calls
...
docs-check: ## A5 docs gate aggregator: Header/INDEX drift + links/fragments + spec contract IDs
	...
	@python3 "$(ROOT_DIR)/scripts/lint/check_spec_contract_ids.py" "$(ROOT_DIR)/docs/spec"
