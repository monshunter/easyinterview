...

{
  ...
  echo "DB state: provisional version, queued resume_tailor_run, queued resume_tailor async_job, status transitions, concurrent claim, rollback"
  echo "privacy: async dispatch and tailor endpoint evidence contains IDs and status only"
} | tee "$OUT/verify.log"
