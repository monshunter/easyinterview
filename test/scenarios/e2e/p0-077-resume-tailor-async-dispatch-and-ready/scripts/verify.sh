...

{
  ...
  echo "fixture parity: branchResumeVersion ai-select-202-with-job"
  echo "DB state: provisional version, queued resume_tailor_run, queued resume_tailor async_job, rollback"
  echo "privacy: async dispatch evidence contains IDs and status only"
} | tee "$OUT/verify.log"
