...

{
  ...
  go test ./internal/upload/service -run 'Test(RegisterFileObject|DeleteFileObjectsForUser)' -count=1 -v
  go test ./internal/upload/store -run 'TestRepository(Create|Mark|Lock|HardDelete|DeleteFileObjectsForUser|InsertAuditTombstone)' -count=1 -v
