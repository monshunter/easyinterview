...
- `sourceType=upload` requires `fileObjectId`; `paste` requires `rawText`; `guided` requires `guidedAnswers`; invalid combinations return 422.
...
- `GET /api/v1/resumes/{resumeAssetId}` hides cross-user assets as 404.
...
- Scenario logs do not contain raw resume text, guided answer values, object keys, or retired module terms.
