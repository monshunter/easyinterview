...

{
  ...
  go test ./internal/resume -run 'Test(RegisterResumePassesConfiguredActiveLimitToStore|RegisterPasteUsesConfiguredUTF8ByteLimitBeforeStore|ListResumesDoesNotReuseFullDetailMappers|ListResumesMapsNullHeadlineAndUnreadableContent|ListResumesRejectsInvalidSourceType)' -count=1 -v
  go test ./internal/resume/store -run 'Test(CreateWithParseJobInsertsResumeAndJobAtomically|CreateWithParseJobRollsBackWhenJobInsertFails|ParseStatusTransition|ListCursorPagination|ResumeSummaryProjectionKeepsOnlyScalarResultFields|ResumeSummaryProjectionSQLLocksHeadlinePriorityAndReadablePredicates|CompleteParseSuccessWritesReadyStateProfileAndCompletedOutboxAtomically|CompleteParseFailureMarksFailedWithoutCompletedOutbox)' -count=1 -v
