...

{
  ...
  grep -q 'TestListResumesDoesNotReuseFullDetailMappers' "$OUT/trigger.log"
  grep -q 'TestRegisterResumePassesConfiguredActiveLimitToStore' "$OUT/trigger.log"
  grep -q 'TestRegisterPasteUsesConfiguredUTF8ByteLimitBeforeStore' "$OUT/trigger.log"
  grep -q 'TestResumeSummaryProjectionKeepsOnlyScalarResultFields' "$OUT/trigger.log"
  grep -q 'TestResumeSummaryProjectionSQLLocksHeadlinePriorityAndReadablePredicates' "$OUT/trigger.log"
  ...
  echo "list projection: exact 9-field ResumeSummary, scalar SQL only, no detail mapper or N+1"
  ...
  echo "resume boundaries: configured active/paste limits use in-memory exact and limit+1 inputs before store"
} | tee "$OUT/verify.log"
