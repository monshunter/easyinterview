...
- `topbar-user-profile`, `topbar-user-settings`, and `topbar-user-logout` are
  absent until `topbar-user-chip` opens the dropdown.
- Dropdown header includes `Alice Example` and `ali***@example.com`.
- Profile and settings actions route to `route-profile` and `route-settings`.
...
- Evidence log contains `dev mock unauthenticated login avatar dropdown profile settings logout`.
