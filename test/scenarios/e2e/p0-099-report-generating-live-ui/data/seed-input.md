...
captures the three live HTTP and read-only PostgreSQL projections before the
environment readiness recheck, then removes the cookie and database URL from
its general child-process environment. It reads the default `DATABASE_URL`
from the local mode-`0600` dev-stack env; `P0_099_DATABASE_URL` is only a
temporary override.
