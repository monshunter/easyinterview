# E2E.P0.084 Retired Branch Flow + Flat Resume UI Regression
...
- Fixture-backed mock-first client：`Resumes/listResumes.json default` +
  ...
  `PreviewStage`。
- 用户：未登录 → 登录态，lang 默认。
- D-20 flatten: `ResumeBranchFlow` / `branchResumeVersion` /
  `seedStrategy` / suggestion accept-reject routes are retired.
...
- Source grep 检查 branch/version operation 旧 token 不回流。

## 3 Then

- pendingAction 不携带 retired branch form draft 或 old wire 字段。
...
- Retired tailor mode `(inline|rewrite|mirror)` 0 命中；prototype import
...
- `make codegen-check` 已通过的 generated client, with retired version
  operations absent.
...
`method=fixture-backed-frontend`。Backend retired-route evidence is covered by
P0.074/P0.079.
