...
grep -Fq "rejects runtime limit+1 UTF-8 bytes before import and keeps the JD" "$LOG_FILE"
...
grep -Fq "processing target job response keeps the loading demo free of internal metadata" "$LOG_FILE"
grep -Fq "parse loading matches the UI truth at desktop and mobile" "$LOG_FILE"
grep -Fq "E2E.P0.015 processing-response loading browser gate project=desktop viewport=1440x900 screenshotBytes=" "$LOG_FILE"
grep -Fq "E2E.P0.015 processing-response loading browser gate project=mobile viewport=390x844 screenshotBytes=" "$LOG_FILE"
