...
- Home omits the non-current hero sub copy and `解析并确认面试` CTA; the main CTA renders as `立即面试` / `Start interview now`.
- Home integrates paste textarea with upload/URL source actions inside the JD input card, exposes existing ready resumes through a compact dropdown row with the create-resume CTA, keeps `立即面试` below resume selection, keeps import disabled before explicit resume selection, and never creates `importTargetJob` or pending import without a real selected `resumeId`.
