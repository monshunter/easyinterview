...
- D-20: suggestion accept/reject/updateVersion routes are non-current; Rewrites
...
- Disallowed non-current operations are searched out of runtime source.
...
- `.test-output/e2e/p0-086-resume-rewrites-edit-save/trigger.log` Vitest pass.
- verify.sh asserts vitest summary + spec presence + non-current-grep gates.

## 6 Baseline

`make codegen-check` clean: non-current accept/reject/updateVersion operations stay absent; flat `updateResume` / `duplicateResume` / tailor operations remain valid.
