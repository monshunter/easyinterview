...
- 切换 `initialRoute` 渲染 `auth_login` / `settings`，并输入
  非当前 standalone insight route 验证其 fallback 到 home，断言
...
- Auth / settings / placeholder shell 全部渲染 ui-design 原生卡片
  scaffold；non-current 入口（`welcome` / `growth` / `mistakes` / `drill` / 独立
  `voice`）以及非当前 module 文案（错题本 / 成长中心 / 经历库 /
  目标角色 / 技能标签）均不可达。
...
- 负向断言：non-current 路由 / 非当前 testid / 非当前文案均无回流。
...
本场景目前在 vitest + jsdom 范围内就足以阻止 non-current 模块回流、视觉
