...
- 点击 `topbar-theme-button` / `topbar-theme-custom-option` 触发 `customAccent` overlay，断言
  ...
  没有被覆盖。
- 切换 `initialRoute` 渲染 `auth_login` / `profile` / `settings`，并输入
  retired `company_intel` route 验证其归一到 workspace，断言
  `ei-auth-shell` / `ei-screen-shell` 双层结构与 `ei-screen-card` 卡片骨架。

## 3 Then

- TopBar 四入口、显示控制、custom accent picker、用户区按 `ei-shell-topbar`
...
- DOM 锚点：`app-shell-topbar` / `topbar-{primary-nav,display-controls,
  ...
  `route-{home,auth_login,profile,settings}` 与 workspace fallback 等全部存在，
  `route-company_intel` 不得 materialize。
