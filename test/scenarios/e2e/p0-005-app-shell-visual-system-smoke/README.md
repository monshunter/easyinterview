...
- 切换 `topbar-dark-toggle` 与 `topbar-theme-select` 验证 `<html>` 的
...
- 点击 `topbar-custom-accent-button` 触发 `customAccent` overlay，断言
  `<html>` 内联 `--ei-color-accent` / `--ei-color-accent-soft` 出现 oklch 值，
  且 base palette token（`--ei-color-bg-canvas` / `--ei-color-fg-primary`）
  没有被覆盖。
- 切换 `initialRoute` 渲染 `auth_login` / `profile` / `settings` /
  `company_intel`，断言 `ei-auth-shell` / `ei-screen-shell` 双层结构与
  `ei-screen-card` 卡片骨架。

## 3 Then

- TopBar 五入口、显示控制、custom accent picker、用户区按 `ei-shell-topbar`
...
- `:root[data-theme][data-mode]` selector 切换后，关键 CSS variable
  ...
  两条 inline style。
- Auth / profile / settings / placeholder shell 全部渲染 ui-design 原生卡片
  scaffold；retired 入口（`welcome` / `growth` / `mistakes` / `drill` / 独立
  `voice`）以及旧 retired-module 文案（错题本 / 成长中心 / 经历库 /
  目标角色 / 技能标签）均不可达。
- `ui-design/src/app.jsx` / `screen-auth.jsx` / `screen-profile.jsx` 的字面量
...
证据，`cleanup.sh` 移除 setup marker，保留日志。
...
- DOM 锚点：`app-shell-topbar` / `topbar-{primary-nav,display-controls,
  user-area}` / `topbar-nav-*` / `topbar-{theme-select,dark-toggle,
  lang-select,custom-accent-{button,swatch,picker,hue,chroma,clear}}` /
  `route-{home,auth_login,profile,settings,company_intel}` 等全部存在。
- className 契约：`ei-shell-topbar` / `ei-topbar-{nav,nav-button,controls,
  ...
  side-panel-pending}` / `ei-screen-{shell,card,card--placeholder}` /
...
- 负向断言：retired 路由 / 旧 testid / 旧文案均无回流。
- ui-design 源追溯：`app.jsx` / `screen-auth.jsx` / `screen-profile.jsx`
...
本场景目前在 vitest + jsdom 范围内就足以阻止 retired 模块回流、视觉
