...
copy_screenshot() {
  local name="$1"
  local source
  source="$(find "$OUTPUT_DIR/playwright" -type f -name "$name" -print -quit)"
  if [ -z "$source" ]; then
...
python3 "$REPO_ROOT/test/scenarios/_shared/scripts/capture-source-fingerprint.py" \
  ...
  --path test/scenarios/e2e/p0-014-home-default-render
...
for screenshot in \
  ...
  home-formal-viewport-mobile.png; do
  copy_screenshot "$screenshot"
done
echo 'HOME_P0014_VIEWPORT_SCREENSHOTS_CAPTURED css=1440x900,390x844 png=1440x900,1170x2532' \
