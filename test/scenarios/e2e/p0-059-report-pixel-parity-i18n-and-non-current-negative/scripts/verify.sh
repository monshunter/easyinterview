...
OUTPUT_DIR="$REPO_ROOT/.test-output/e2e/p0-059-report-pixel-parity-i18n-and-non-current-negative"
LOG_FILE="$OUTPUT_DIR/trigger.log"

test -s "$LOG_FILE"
"$REPO_ROOT/test/scenarios/_shared/scripts/frontend-real-backend-verify.sh" "$LOG_FILE" "${SCENARIO_ID:-$(basename "$OUTPUT_DIR")}"
grep -Eq 'Test Files +[0-9]+ passed' "$LOG_FILE" || { echo "E2E.P0.059: i18n + non-current negative tests did not pass" >&2; exit 1; }
grep -Fq 'frontend-report-dashboard non-current lint OK' "$LOG_FILE" || { echo "E2E.P0.059: non-current lint script did not succeed" >&2; exit 1; }
