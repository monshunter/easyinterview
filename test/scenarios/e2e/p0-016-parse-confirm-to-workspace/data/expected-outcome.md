...
- Parse preview reads ready resumes and disables Save/Start when no ready resume exists.
- Authenticated Save plan navigates to workspace with interview context fields and a real ready `resumeId`.
- Browser-level Save plan reaches `/workspace` with all context query params:
  ...
  is not rendered.
- Browser-level Start interview uses workspace `autoStartPractice=1` and reaches `practice`.
