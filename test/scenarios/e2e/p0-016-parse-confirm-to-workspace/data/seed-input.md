...
- Parse terminal inputs: queued/processing followed by ready, and an initial ready response; both ready paths must replace to Workspace.
- Hostile URLs: `/parse?targetJobId=<uuid>&section=reports` and `/workspace?targetJobId=<uuid>&section=reports`; `section=reports` must be discarded.
