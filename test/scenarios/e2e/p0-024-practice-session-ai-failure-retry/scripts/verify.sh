...
RUN_ID="${TEST_RUN_ID:-practice-ai-retry-$(date -u '+%Y%m%dT%H%M%SZ')}"
RUN_DIR="${TEST_OUTPUT_DIR:-$REPO_ROOT/.test-output}/runs/$RUN_ID/e2e/E2E.P0.024"

test -s "$LOG_FILE"
grep -Fq -- '--- PASS: TestE2EP0024PracticeSessionAIFailureRetry' "$LOG_FILE"
...
printf '{"scenario":"E2E.P0.024","status":"passed","method":"cmd-api-http","validBddEvidence":true,"verifiedAt":"%s","evidence":"%s","snapshot":"in-process practice store asserted failed_retryable retry state machine"}\n' "$(date -u '+%Y-%m-%dT%H:%M:%SZ')" "$RUN_DIR/trigger.log" > "$RUN_DIR/result.json"
