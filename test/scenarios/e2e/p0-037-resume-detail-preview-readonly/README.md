...
- `verify.sh` 校验 6 tests passed + pending PDF upload 轮询到 source page stack + failed-with-snapshot PDF upload 单次请求 + non-current testid 字面量 negative + 不出现 `TARGET_JOB_NOT_FOUND` 字符串。
