...
grep -Fq 'stops active TTS playback and reports barge_in_detected before recording the next user speech' "$PRACTICE_TEST" || { echo "E2E.P0.008: frontend barge-in assertion source missing" >&2; exit 1; }
grep -Fq 'headers.get("Idempotency-Key")).toBeNull()' "$PRACTICE_TEST" || { echo "E2E.P0.008: frontend idempotency boundary assertion missing" >&2; exit 1; }
