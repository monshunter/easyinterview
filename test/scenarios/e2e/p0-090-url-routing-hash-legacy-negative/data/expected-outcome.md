...
| 12 + 1 个退役 alias | 全部 hash 启动后 URL 落到对应保留 canonical path（见 plan §4.2）；`debrief` / `debrief_full` / `profile` 均落到 `/` |
...
| 反向断言 | 含义 |
|----------|------|
| `routeUrl.ROUTE_TO_PATH` 文件中不出现 `"/voice"` / `"/welcome"` / `"/debrief"` / `"/profile"` 等 retired path 字面值 | 禁止 alias 通过 typed table 复活 |
| `frontend/src/app/screens/` 下不存在 welcome / growth / mistakes / drill / followup / experiences / star / onboarding / debrief / profile 目录 | retired 模块零 materialize |
