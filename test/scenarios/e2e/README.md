...
- 环境类型：本地 runner（Go / Vitest / Playwright / browser smoke / hybrid real-provider UAT 等，按具体场景 README 声明）
...
当顶层 env 脚本不可用时，手动引导顺序是：`make dev-up` → `make dev-doctor` → 按目标场景 README 启动 repo-tracked runner。需要本地前后端联调或真实 provider hybrid UAT 时，先用 `test/scenarios/env-setup.sh` 构建共享依赖环境，再按目标场景 README 在宿主机启动 backend/frontend；真实 secret 只放在未跟踪本地文件中。
