...
当前脚本使用 `cmd/api` HTTP 场景测试，覆盖 auth middleware、generated route、TargetJob handler/service、in-process drainer runtime 与可注入 A3/F3 失败路径；`cleanup.sh` 只清理 setup marker，保留 trigger log 与 result evidence。`result.json.validBddEvidence=true`。
