...

{
  ...
  grep -q 'TestAiTaskRunsPrivacyForTailorHandler' "$LOG"
  grep -q 'TestAuditPrivacyForTailorHandler' "$LOG"
  ...
  grep -q 'RUNNER go test cmd/api resume tailor runner kernel privacy' "$LOG"
