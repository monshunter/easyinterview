...
printf '{"scenario":"E2E.P0.026","status":"passed","method":"cmd-api-http","validBddEvidence":true,"verifiedAt":"%s","evidence":"%s","snapshot":"observed AI typed columns metric allowlist privacy redlines and backend-practice out-of-scope gate"}\n' "$(date -u '+%Y-%m-%dT%H:%M:%SZ')" "$RUN_DIR/trigger.log" > "$RUN_DIR/result.json"
