...
场景使用 Go focused tests、httptest/fake store 与 sqlmock，不写共享数据库，不启动 Kind cluster；`cleanup.sh` 只清理 setup marker，保留 `.test-output/e2e/p0-010-targetjob-text-import-parse-ready/trigger.log` 与 `result.json` 作为证据。
