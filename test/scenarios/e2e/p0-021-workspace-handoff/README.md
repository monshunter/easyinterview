...
## 2 When
运行 workspace source negative、共享 start-practice structured-round budget、report replay handoff regression、隐私反查和 out-of-scope 入口负向 grep。
...
- C: workspace/report 共用的 start-practice 只接受 TargetJob 当前结构化轮次，并把轮次时长写入 PracticePlan；旧 plan 预算不匹配时不复用
