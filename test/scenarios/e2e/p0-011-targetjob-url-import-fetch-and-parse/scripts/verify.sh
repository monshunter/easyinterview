...
RUN_DIR="${TEST_OUTPUT_DIR:-$REPO_ROOT/.test-output}/runs/$RUN_ID/e2e/E2E.P0.011"

test -s "$LOG_FILE"
grep -Fq -- '--- PASS: TestE2EP0011URLImportFetchAndParse' "$LOG_FILE"
grep -Eq 'ok[[:space:]]+github.com/monshunter/easyinterview/backend/internal/targetjob' "$LOG_FILE"
grep -Eq 'ok[[:space:]]+github.com/monshunter/easyinterview/backend/internal/targetjob/urlfetch' "$LOG_FILE"

for forbidden in \
  'token=secret' \
  'latest/meta-data' \
  'Fetched public JD text' \
  'Authorization:'; do
...
printf '{"scenario":"E2E.P0.011","status":"passed","method":"go-test","verifiedAt":"%s","evidence":"%s"}\n' "$(date -u '+%Y-%m-%dT%H:%M:%SZ')" "$RUN_DIR/trigger.log" > "$RUN_DIR/result.json"
