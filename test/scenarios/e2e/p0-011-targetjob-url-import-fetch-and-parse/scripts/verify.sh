...
test -s "$LOG_FILE"
grep -Fq -- '--- PASS: TestE2EP0011HTTPURLImportFetchAndParse' "$LOG_FILE"
...
printf '{"scenario":"E2E.P0.011","status":"passed","method":"cmd-api-http","validBddEvidence":true,"verifiedAt":"%s","evidence":"%s"}\n' "$(date -u '+%Y-%m-%dT%H:%M:%SZ')" "$RUN_DIR/trigger.log" > "$RUN_DIR/result.json"
