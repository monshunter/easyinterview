...
| E2E.P0.093 | backend-profile C-10, C-14 | `p0-093-profile-privacy-delete-lifecycle/` | candidate_profile + experience cards privacy delete chain + audit tombstone PII redline + re-seed | automated | Ready |
| E2E.P0.094 | backend-jobs-recommendations C-1, C-2, C-3, C-4, C-5, C-13, C-14, C-17, C-19 | `p0-094-jd-match-profile-and-recommendations-list/` | getJobMatchProfile 7-cross-owner aggregation + spec D-18 sparse baseline + D-19 structural parity + cursor pagination + dismiss + cross-user 404 | automated | Ready |
| E2E.P0.095 | backend-jobs-recommendations C-6, C-7, C-8, C-10, C-13, C-14, C-17 | `p0-095-jd-match-watchlist-and-saved-search-lifecycle/` | watchlist UNIQUE + IK replay + tone derivation Q-4 + saved-searches required-field validation + redaction red lines | automated | Ready |
| E2E.P0.096 | backend-jobs-recommendations C-9, C-11, C-13, C-14, C-17 | `p0-096-jd-match-search-and-market-signals/` | searchJobs sync response + jd_match_search_runs provenance + getMarketSignals 4-signal fixture parity | automated | Ready |
| E2E.P0.097 | backend-jobs-recommendations C-12, C-15, C-16, C-18 | `p0-097-jd-match-agent-scan-and-privacy-delete/` | jd_match_agent_scan job lifecycle + recommendation generator + outbox completed event + DeleteJobMatchDataForUser cascade | automated | Ready |
...
| E2E.P0.101 | backend-auth C-8; frontend-shell C-12; local-dev-stack C-10/C-15 | `p0-101-auth-mail-link-login-register/` | Playwright host-run real-mode auth：登录/注册提交邮箱 → Mailpit frontend callback → 自动验证 session → URL token 清理 → console/http failures 为 0 | automated | Ready |
