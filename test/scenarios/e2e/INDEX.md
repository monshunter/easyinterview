...
| E2E.P0.018 | frontend-workspace-and-practice C-2, C-7, C-8, C-9 | `p0-018-workspace-default-render/` | 面试入口规划列表 + Workspace 统一面试规划详情：plan list、统一详情母版、简历选择器、旧独立详情负向锚点 | automated | Ready |
...
| E2E.P0.037 | frontend-resume-workshop C-3, C-10, C-11 | `p0-037-resume-detail-preview-readonly/` | Resume Workshop detail：只读简历正文、旧 tab/query 忽略、无 export/copy/original/edit/rewrite surface、404 fallback 不回显 fixture error.code | automated | Ready |
...
| E2E.P0.079 | backend-resume C-16 | `p0-079-resume-rewrites-accept-only-save/` | Flat save fixtures + read-only detail boundary：backend save fixture parity、frontend detail Rewrites/Edit absent 与非当前 route/catalog gate | automated | Ready |
...
| E2E.P0.081 | frontend-resume-workshop C-10, C-7, C-8, C-9 | `p0-081-resume-create-flow-upload-paste-guided-happy/` | ResumeCreateFlow upload/paste happy + presign + register + direct detail navigation + IK + privacy + retired parser negative | automated | Ready |
| E2E.P0.082 | frontend-resume-workshop C-10, C-8 | `p0-082-resume-create-flow-parsing-failure-and-retry/` | Retired parsing/preview surfaces negative gate + direct register-to-detail behavior | automated | Ready |
| E2E.P0.083 | frontend-resume-workshop C-10, C-8, C-9 | `p0-083-resume-create-flow-preview-confirm-and-cta-handoff/` | Home/Workspace CTA handoff + auth pendingAction + create register direct detail, no PreviewConfirm/updateResume | automated | Ready |
| E2E.P0.084 | frontend-resume-workshop C-11, C-3 | `p0-084-resume-flat-ui-regression/` | Flat Resume UI regression：route/auth/create/read-only original-content detail smoke + non-current operation / prototype import negative | automated | Ready |
