...
> **Status**: Ready
> **Owner plan**: [e2e-scenarios-p0/001 Phase 5](../../../../docs/spec/e2e-scenarios-p0/plans/001-full-funnel-happy-journey/plan.md)
...
Practice UI behavior is owned by P0.044-P0.047; P0.099 is the minimal real
report/generating visual acceptance required by frontend-report-dashboard C-7.
...
Trigger also runs exactly these three Vitest files through a directly focused
`pnpm exec vitest run` invocation:
...
`validate_vitest_log.py` requires one real Vitest `RUN` segment, exactly those
three executed result rows, `Test Files  3 passed (3)`, and a positive all-pass
`Tests` count. A removed runner segment, extra file, missing file, failure, or
zero-test summary fails closed.
...
agent-browser --session p0-099-report screenshot --full .test-output/e2e/p0-099-full-funnel-fullstack-ui-journey/screenshots/<canonical-desktop-name>.png
agent-browser --session p0-099-report set viewport 390 844
agent-browser --session p0-099-report screenshot --full .test-output/e2e/p0-099-full-funnel-fullstack-ui-journey/screenshots/<canonical-mobile-name>.png
```
...
within the language limit; exact 24-word/64-code-point boundaries belong to the
deterministic fixture parity gate. This acceptance evidence does not add a
product API field.
...
captures immediately after setup/run identity checks and before environment
verification or focused Vitest, so an honest generating state cannot advance
during the runner and then be described from stale state.
...
After the automated PNG, current-run DB/API, privacy, raw-debug, and Vitest
gates pass, trigger writes result `MANUAL_REQUIRED` with reason
...
```bash
bash test/scenarios/e2e/p0-099-full-funnel-fullstack-ui-journey/scripts/setup.sh
...
bash test/scenarios/e2e/p0-099-full-funnel-fullstack-ui-journey/scripts/trigger.sh
unset P0_099_SESSION_COOKIE
# inspect the exact six PNGs without OCR, then write manual-visual-audit.json
bash test/scenarios/e2e/p0-099-full-funnel-fullstack-ui-journey/scripts/verify.sh
bash test/scenarios/e2e/p0-099-full-funnel-fullstack-ui-journey/scripts/cleanup.sh
```
