...
d22_out_of_scope_ui = re.compile(
...
for rel in scan_paths:
    ...
    for file_path in files:
        ...
        if pattern.search(text):
            ...
            sys.exit(1)
        if d22_out_of_scope_ui.search(text):
            print(f"verify: D-22 out-of-scope debrief/profile UI token found in {file_path.relative_to(root)}", file=sys.stderr)
            sys.exit(1)
PY
