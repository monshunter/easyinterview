...
LIVE_CAPTURE_RUNNER="$ROOT/test/scenarios/e2e/p0-099-full-funnel-fullstack-ui-journey/scripts/capture_live_evidence.py"
MANUAL_AUDIT="$OUT/manual-visual-audit.json"
VITEST_VALIDATOR="$ROOT/test/scenarios/e2e/p0-099-full-funnel-fullstack-ui-journey/scripts/validate_vitest_log.py"
...
VITEST_EVIDENCE="$(mktemp "$OUT/.vitest-evidence.XXXXXX")"
(
  cd "$ROOT/frontend"
  pnpm exec vitest run \
    ...
    --reporter=default
) 2>&1 | tee "$VITEST_EVIDENCE"
python3 "$VITEST_VALIDATOR" "$VITEST_EVIDENCE"
rm -f "$VITEST_EVIDENCE"
