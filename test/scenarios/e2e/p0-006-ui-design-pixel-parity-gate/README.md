...
- `tests/pixel-parity/screenshot.spec.ts` — 默认 ocean/light screenshot smoke
  （非空截图 buffer）+ dark / customAccent 状态变更
...
- dark toggle 把 `<html data-mode>` 翻到 `dark`、`--ei-color-bg-canvas`
  resolves 为 `#0c0f17`；`document.body.background-color` 切到 `rgb(12,
  15, 23)`。
...
- `setup.sh` 仅写一个 marker；`cleanup.sh` 删除 marker，保留 trigger.log
