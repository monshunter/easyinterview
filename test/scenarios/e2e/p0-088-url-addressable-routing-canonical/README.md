...
- workspace 丢弃 `targetJobId` / `resumeId` / `planId` / `autoStartPractice` 并规范化为 query-free list route；generating/report 保留各自的 `reportId` / `reportStatus`；简历工作台过滤 `tab` / `tailorRunId`。
