...
| Non-current debrief/profile direct-open | `/debrief?...` 与 `/profile` 折回 `/`；不保留 `debriefId` / `debriefJobId` / `targetJobId`；不渲染 `debrief-screen` 或 `route-profile` |
...
| 反向断言 | 含义 |
|----------|------|
| 不能出现 `route-welcome` / `route-voice` | 非当前 route 名禁止 materialize |
| 不能出现 `topbar-nav-mistakes` / `growth` / `drill` / `voice` | TopBar 不应承接非当前入口 |
| URL 不能出现 `/voice?` / `/welcome?` / `/growth?` / `/mistakes?` / `/drill?` / `/debrief?` / `/profile?` | canonical path 不可接入非当前入口 |
