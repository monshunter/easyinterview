...
## 使用方式与边界(EasyInterview 项目内)

本文档是 Anthropic Claude.com 品牌设计系统的只读参考快照(来自 [getdesign.md](https://getdesign.md/)),是 EasyInterview **前端实施必须参考的 UI 设计体系**。`ui-design/` 与 `docs/ui-design/` 是项目当前 UI 真理源;本文档作为创建新页面、新组件时的品牌与语义命名参考,与真理源协同使用。

### 做什么

- **语义命名优先复用**:新增页面或组件时,遵循本文档的语义键(如 `feature-card`、`code-window-card`、`callout-card-coral`、`pricing-tier-card-featured`、`cta-band-coral`、`cta-band-dark`、`product-mockup-card-dark`、`hero-illustration-card`、`badge-coral` 等),让 EasyInterview 与 Claude 品牌族保持结构对齐。
- **章节节奏**:页面分段的默认参考是 cream → cream-card → dark-mockup → coral-callout → dark-footer 的交替。
- **token 参考语言**:颜色/排版/间距/圆角/响应式分级作为 token 设计的参考语言,新增 token 时优先复用语义键名。
- **品牌出处**:讨论"为什么 EasyInterview 是 warm cream + coral 而不是冷蓝"时,引用本文档作为出处。

### 不要做

- **不要机械同步 token**:不要把 [ui-design/src/primitives.jsx](ui-design/src/primitives.jsx) 的 `EI_THEMES.warm` 色值改成本文档的精确 hex(例如不要把 `#c96442` 改成 `#cc785c`,不要把 `#fdfcf8` 改成 `#faf9f5`)。`primitives.jsx` 的多主题(warm/forest/ocean)+ 双模式(light/dark)系统已经做过二次设计,机械对齐会破坏既有协调。
- **不要引入私有授权字体**:Copernicus 与 StyreneB 是 Anthropic 私有授权字体,公网不可用;当前项目以系统字体栈和开源替代为准(详见下方 *Note on Font Substitutes*)。
- **不要按本文档数值重写已实现组件**:不要按本文档的 `padding: 32px` / `height: 40px` 等数值重写 [primitives.jsx](ui-design/src/primitives.jsx) 中已实现的组件。
- **不要作为 spec/plan 完成判据**:`docs/spec/` 下的 spec、plan、checklist 不应把"DESIGN.md 第 X 节"作为完成判据;只有 `docs/ui-design/` 与 `ui-design/` 才是 UI 验收源。

### 维护

本文档不跟随 ui-design/ 演进。若 Claude.com 官网更新设计系统,可整体重新下载替换本文档,无需逐项 diff。当本文档与 `ui-design/` 出现冲突时,以 `ui-design/` 为准;本文档保持原貌,不为对齐项目而修改。

---

以下为原始 DESIGN.md 完整内容(Claude.com 营销站设计系统,作为只读参考)。
