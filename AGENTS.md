...
easyinterview 是一款围绕具体目标岗位、JD 与真实面试流程设计的 AI 面试训练产品。它把“看到岗位 -> 开始准备 -> 模拟练习 -> 获得证据化反馈 -> 修正表达 -> 复练 -> 面后复盘”的求职准备过程标准化、重复化、可视化，重点帮助高意图求职者在 24 到 72 小时内围绕一份真实 JD 完成高质量准备。

当前项目处于开发阶段，不需要考虑线上兼容性；所有实现应优先服务规格中的 P0 MVP 闭环：`JD 导入 -> 目标岗位工作台 -> 模拟面试 -> 证据化报告 -> 错题本 -> 针对性复练`。P0 的核心判断是：用户带着一份具体 JD 来到产品后，能否在最短时间内完成一轮有效练习，并明确知道下一次该怎么练。

产品完整能力分为五个模块：

- **M1 · Progressive Profile（渐进式画像）**：通过简历、JD、练习与复盘逐步沉淀候选人经历、偏好、优势和短板，避免冗长前置画像。
- **M2 · Target Job Workspace（目标岗位工作台）**：围绕 JD 管理岗位要点、轮次假设、风险提示、练习入口、简历定制入口和历史上下文。
- **M3 · Mock Interview Orchestrator（模拟面试编排器）**：根据岗位、轮次和用户回答生成问题、追问与会话状态，优先保证练习贴合具体岗位而非泛题库。
- **M4 · Evidence-based Review（证据化复盘）**：提供可回溯到题目和回答片段的反馈、报告、错题本与复练任务，前台优先展示准备度档位和维度状态，不输出无依据的精确通过率。
- **M5 · Growth System（成长系统）**：跨时间、岗位和轮次沉淀能力趋势、准备度与后续行动计划。

工程实现必须遵守规格中的边界：不做真实面试中的隐形实时辅助或作弊能力；不在 MVP 阶段实现全球搜岗、视频情绪识别、社区、Team 版或企业端候选人评估；任何生成结果都应携带 prompt / rubric / model / language / feature flag / data source 等版本或来源信息，便于质量评估、回归检测与问题追踪。隐私默认保守，音视频、简历、画像和面试内容等敏感数据必须可解释、可关闭、可删除。
