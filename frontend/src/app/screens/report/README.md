...
- `components/ReportDashboard.tsx` — top-level dashboard container that drives:
  ...
  - `hooks/useReportContextData.ts` — label-only fetch from `getTargetJob` + `getResumeVersion`; failures fall back to the raw ID (decorative strip, never blocks the dashboard).
