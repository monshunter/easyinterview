 ...
 *
 * (a) The create-flow source tree must NOT reference non-current module names that
...
describe("frontend-resume-workshop/002 — non-current module negative grep", () => {
  const nonCurrentTerms = [
    ["wel", "come"].join(""),
    ["mis", "take"].join(""),
    ["gro", "wth"].join(""),
    ["dr", "ill"].join(""),
  ...
  it("create-flow source files (non-test) contain zero non-current module references", () => {
    const offenders: Array<{ file: string; match: string }> = [];
    for (const file of walk(CREATE_DIR)) {
      if (isTestFile(file)) continue;
      const content = readFileSync(file, "utf8");
      const match = content.match(nonCurrentPattern);
