...
旧 alias（`welcome` / `growth` / `plan` / `mistakes` / `drill` / `followup` / `experiences` / `star` / `resume` / `onboarding` / `auth_register` / `auth_reset`）在 [`src/app/normalizeRoute.ts`](./src/app/normalizeRoute.ts) 集中映射到当前 route，**不允许在新代码中作为 live route name 出现**；`src/app/scope.test.ts` 自动负向 grep 阻止回流。独立 `voice` route alias 已删除，输入 `voice` 会按未知 route fallback 到 `home`；`practice?mode=voice&modality=voice` 是语音面试唯一显式入口。旧 `/auth/reset` path 由 `routeUrl.LEGACY_PATH_TO_ROUTE` 与 `scripts/spaFallback.mjs` 的 `FRONTEND_LEGACY_PATHS` 共同承接，加载 App 后归一回 `auth_login`。
