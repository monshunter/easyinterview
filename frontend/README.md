...
- TopBar 语言切换必须保持为与 `ui-design/src/app.jsx` 一致的可访问 icon toggle（当前为 `button[data-testid="topbar-lang-toggle"]`），不要改成 native select、按钮组或只切状态的占位控件。
...
- TopBar 主题 menu、暗色 toggle、custom accent 控件、语言 toggle 的 testid / aria 契约见 §2.5 与 [`src/app/topbar/TopBar.tsx`](./src/app/topbar/TopBar.tsx)。
...
2. **Playwright + chromium pixel parity gate（E2E.P0.006，秒级）**：在 desktop (1440×900) 与 mobile (390×844) 两个 chromium project 下加载 `frontend/dist/index.html` 与 `ui-design/index.html` golden preview，断言 DOM 锚点 + computed style + bounding box + 截图差异。CI / 主线合并前必跑。
   ...
   # 2. 跑 4 个 spec × 2 viewport = 46 项 Playwright gate
