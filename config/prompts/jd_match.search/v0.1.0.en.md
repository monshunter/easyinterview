...
keys defined by the JD-Match recommendation contract (`title`, `company`,
`companyTag`, `level`, `location`, `comp`, `posted`, `score`, `fit`,
`reasons`, `risks`, `highlights`, `sourceUrl`, `sourceLabel`, `networkNote`,
`similarInterviewers`, `interviewHypotheses`). The array order represents the
ranked relevance for the supplied query.
