"""Self-test for frontend_report_dashboard_non_current.py.
...
SCRIPT = (Path(__file__).resolve().parent / "frontend_report_dashboard_non_current.py").resolve()
...
def test_frontend_report_dashboard_non_current_includes_terms() -> None:
    text = SCRIPT.read_text(encoding="utf-8")
    for term in REQUIRED_TERMS:
        assert term in text, f"lint script missing forbidden term: {term}"


def test_frontend_report_dashboard_non_current_passes_on_clean_repo() -> None:
...
def test_frontend_report_dashboard_non_current_allows_negative_docs() -> None:
    # Stand up a throwaway frontend skeleton and confirm allowed files are not
    # flagged even when they enumerate non-current vocabulary.
    with tempfile.TemporaryDirectory() as tmpdir:
        root = Path(tmpdir)
        target = root / "frontend" / "src" / "app" / "screens" / "report" / "__tests__"
        target.mkdir(parents=True, exist_ok=True)
        non_current_negative = target / "nonCurrentNegative.test.ts"
        non_current_negative.write_text(
        ...
        copy_target = scripts_dir / "frontend_report_dashboard_non_current.py"
