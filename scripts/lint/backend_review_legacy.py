#!/usr/bin/env python3
"""Legacy-negative gate for backend-review report generation.

The owner plan intentionally names retired terms in markdown gates. This script
...
SCAN_PREFIXES = (
    ...
    ("test", "scenarios", "e2e", "p0-055-report-privacy-and-legacy"),
)
    ...
    for path in paths:
        ...
        for lineno, line in enumerate(lines, start=1):
            for term in RETIRED_TERMS:
                if term in line:
                    problems.append(f"{path}:{lineno}: retired backend-review term {term!r}")
            if "worker_id" in line and "slog.String(\"worker_id\"" not in line:
                problems.append(f"{path}:{lineno}: retired async worker_id persistence vocabulary")
...
def main() -> int:
    ...
    if problems:
        ...
        return 1
    print("OK: backend-review legacy terms absent from runtime report surfaces")
    return 0
