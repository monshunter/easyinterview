#!/usr/bin/env python3
"""Non-current gate for backend-review report generation.

The owner plan intentionally names non-current terms in markdown gates. This script
...
def scan_paths(paths: list[Path], repo_root: Path) -> list[str]:
    ...
    for path in paths:
        ...
        for lineno, line in enumerate(lines, start=1):
            for term in NON_CURRENT_TERMS:
                if term in line:
                    problems.append(f"{path}:{lineno}: non-current backend-review term {term!r}")
            if "worker_id" in line and "slog.String(\"worker_id\"" not in line:
                problems.append(f"{path}:{lineno}: non-current async worker_id persistence vocabulary")
...
def main() -> int:
    ...
    if problems:
        ...
        return 1
    print("OK: backend-review non-current terms absent from runtime report surfaces")
    return 0
