#!/usr/bin/env python3
"""Non-current gates for backend-practice plans.

The non-current practice-mode literal must not remain in active code, specs,
...
Phase 3 also pins the non-current module vocabulary on implementation/runtime
surfaces. The owner plan docs intentionally name the non-current terms as gate
...
def scan_paths(paths: list[Path], repo_root: Path) -> list[str]:
    ...
    for path in paths:
        ...
        for lineno, line in enumerate(lines, start=1):
            if NON_CURRENT_MODE in line and not is_allowed_line(line):
                problems.append(f"{path}:{lineno}: non-current practice mode literal in active context")
    return problems
...
def scan_phase3_paths(paths: list[Path], repo_root: Path) -> list[str]:
    ...
    for path in paths:
        ...
        try:
            ...
            continue
        for lineno, line in enumerate(lines, start=1):
            for term in PHASE3_NON_CURRENT_TERMS:
                if term in line:
                    if term == "lega" "cy debrief replay value" and path.name.endswith("_test.go"):
                        continue
                    problems.append(f"{path}:{lineno}: non-current backend-practice term {term!r}")
            if STANDALONE_VOICE_ROUTE.search(line) and "practice-voice-mvp" not in line:
                problems.append(f"{path}:{lineno}: non-current standalone voice route")
    return problems
...
def main(argv: list[str] | None = None) -> int:
    ...
    if problems:
        ...
        return 1
    print(f"backend_practice_non_current {args.phase}: OK")
    return 0
