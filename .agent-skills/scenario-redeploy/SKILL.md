...
`make dev-container-up`, then `make dev-container-doctor`; resolve the effective
frontend/backend endpoints from the current README, environment configuration,
or lifecycle command output. Use `make dev-container-logs` for diagnostics and
`make dev-container-down` for a volume-preserving stop. Do not route this
explicit topology through the host-run `scenario-env-redeploy` path.
