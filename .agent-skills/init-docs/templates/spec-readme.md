...
4. **Context 必填**：每个 plan 目录必须有 `context.yaml`，作为 `/implement`、`/plan-review`、`/plan-code-review` 的入口。schema 只承载 target 与一等文档链接；single-plan/loop 模式允许 `plan` 与 `checklist` 同指 `./plan.md`。
