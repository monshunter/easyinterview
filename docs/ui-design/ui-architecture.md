...
17. 报告必须显式区分 `复练当前轮` 与 `进入下一轮`：前者直接进入当前轮复练 session 并带入报告中的问题，后者直接进入下一轮面试 session。
...
23. `screens` 映射不得重新注册 `welcome`、`mistakes`、`growth`、`plan`、`experiences`、`drill`、`followup`、`star`、`resume`、`onboarding` 等旧页面 key。
24. `ResumeScreen`、旧 `ResumeVersionsScreen`、`OnboardingScreen`、`ReportEditorial`、`ReportTimeline`、`PlanScreen`、`ExperienceLibraryScreen` 和旧 `DebriefScreen` 不应进入当前源码或目标文档的页面结构。
