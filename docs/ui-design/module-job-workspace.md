...
- 通过删除图标先打开可访问的二次确认；确认前不调用 `archiveTargetJob`，取消、遮罩或 Escape 零副作用。用户确认后才持久归档，并在成功后从当前列表中移除卡片。
- 引导用户回首页导入新的 JD。
...
4. 列表卡片不展示可见的 `进入规划` / `Open plan` 按钮；点击卡片主体进入 `/workspace?targetJobId=...` 详情且不得触发 import/poll/Parse animation，点击 `立即面试` 启动 practice。点击右上角删除图标必须阻止卡片导航并只打开确认对话框：显示“请谨慎操作”、面试规划级问题和当前无法撤销说明，“取消”为初始焦点；取消、遮罩或 Escape 关闭并把焦点还给原删除按钮，`archiveTargetJob` 请求数保持 0。用户确认后才调用 generated `archiveTargetJob`；pending 禁止关闭和重复提交，成功隐藏当前卡片且刷新后不回灌，失败保留卡片与对话框并允许使用同一 idempotency key 重试。
...
| 日期 | 版本 | 变更 |
|------|------|------|
| 2026-07-20 | 1.50 | 面试规划卡片删除增加可访问的二次确认：确认前零请求，取消/遮罩/Escape 零副作用，pending 禁止重复提交，失败在对话框内重试。 |
