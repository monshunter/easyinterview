...
原始文件会作为只读来源保存。详情页直接展示从 PDF / DOCX / Markdown / TXT 提取的可读原始文本 / 解析文本快照；文件名不得作为简历内容或完成态名称。
...
注册成功即创建简历资产。后台解析可以继续更新结构化字段和 LLM-derived `displayName`，但前端不展示解析动画页、结构化草稿确认页或确认保存页；解析前不得把原文第一行或文件名当作列表 / 详情名称。
