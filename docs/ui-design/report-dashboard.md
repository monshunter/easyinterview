...
- Header 下的目标岗位摘要卡只消费现有 TargetJob 事实：公司、岗位、可选 location、canonical round 数量和 `createdAt`；`createdAt` 只能标注为“规划创建日期 / Plan created”，不得标注为面试日期。缺失字段留空或降级为已有本地化文案，不伪造部门、面试日期或业务状态。轮次时间线的时长只来自 canonical round `durationMinutes`，报告日期只来自 `currentReport.generatedAt` / latest attempt 真实时间。

## 3 诚实生成态

Generating 与其它长耗时流程共用 `AsyncTransitionScene`：页面保留共享 TopBar，报告插画、真实 `queued/generating` 状态标签、衬线标题、说明、indeterminate rule 和返回动作直接位于共享蓝白氛围画布的中心轴线上。等待内容外不得再包裹白色面板、卡片边框、卡片阴影或局部毛玻璃；画布装饰与其它异步等待态保持一致。视觉上的局部高亮只表达“仍在处理”，不携带 `aria-valuenow`，不映射 provider attempt、重试次数、固定百分比或 SLA。返回目标已由可信 report context 解析为报告列表时，控件使用专用文案“返回面试报告 / Back to interview reports”；尚无可信 TargetJob identity、只能安全回到 Workspace 时保留通用“返回 / Back”，避免标签与实际目标不一致。ready 自动进入 Report；timeout/network/failed/invalid 的 typed error 仍使用既有恢复状态，不被动画覆盖。
...
| R-11 | trusted target context / no trusted identity | 从 ready/pending/failed/recoverable generating 点击 Back，或在 missing/first-load failure 点击 Back | 有 trusted target 时控件显示“返回面试报告 / Back to interview reports”并进入 `/reports?targetJobId=...`；否则显示通用“返回 / Back”并进入 workspace；report/generating route 仍只含 reportId |
...
| 日期 | 版本 | 变更 |
|------|------|------|
| 2026-07-20 | 1.50 | 明确 ReportsScreen 的 TargetJob `createdAt` 只标注为规划创建日期，报告/面试时间不得由该字段伪造。 |
| 2026-07-20 | 1.49 | 将 Generating 的 trusted-report 返回动作恢复为“返回面试报告 / Back to interview reports”，无可信目标的 Workspace fallback 继续使用通用“返回 / Back”。 |
| 2026-07-20 | 1.48 | 按修订后的报告生成 UI 去除中心白色卡片，使插画、状态、标题、说明、indeterminate rule 与返回动作直接位于共享氛围画布上。 |
