...
│  ├─ round
│  └─ resume
├─ 查看本次面试记录 -> ReportConversation(reportId)
├─ SummaryMetrics
│  ├─ 准备度 + summary
│  ├─ 能力维度数量
│  └─ 会话证据数量
...
├─ Back -> exact Report / Generating state for the same reportId
├─ Frozen report context summary
└─ Readonly ordered transcript
...
- `currentReport` 行在“查看报告”旁可提供低强调度的“查看面试记录”快捷入口；它仍以同一个 `reportId` 打开报告附属页，不生成会话列表、第二套记录索引或 `sessionId` 路由。queued/generating/failed 最新尝试的记录入口由对应 Generating/失败页承担，不把 latest attempt 伪装为 current report。
...
- 完整 transcript 只在 `ReportConversation(reportId)` 附属页呈现，报告 Evidence Surface 不复制正文。
...
- 主入口是 ReportDashboard 的 Context Strip 下方低强调度链接“查看本次面试记录”，不进入 Header CTA 组，不改变 D-19 的唯一一对开练 CTA。
- 规划范围的快捷入口只允许出现在 ReportsScreen 同轮 `currentReport` 行；Workspace 仍只提供“面试报告”入口，不直接请求会话、不显示会话行，也不新增一级导航。
- canonical route 为 `/report-conversation?reportId=<uuid>`。页面只消费 `getReportConversation(reportId)`；`sessionId`、`targetJobId`、status 或任何消息 ID 不进入 URL，也不成为客户端 authority。
- 报告资源一旦存在，queued / generating / ready / failed 都可查看其完成会话记录。Back 根据同一响应中的 `reportStatus` 返回 `/report?reportId=...` 或 `/generating?reportId=...`；缺失/404/首读网络失败没有可信父级事实时回 `/workspace`。
- 页面按服务端 `sequence` 升序显示 frozen report context 与 ordered user/assistant messages；复用 Practice 的安全 Markdown/GFM renderer、role styling、长代码/表格 mobile overflow 约束，但不渲染 Composer、thinking、retry、结束、暂停、计时、电话或继续面试动作。
- loading / network error / invalid payload / hidden 404 状态必须可访问。跨用户、空消息、重复/乱序 sequence、非 user/assistant role 或额外内部 locator 均 fail closed，不保留 stale transcript。
- 页面是报告 read-side，不读取 `getPracticeSession`，不恢复实时 `replyStatus/clientMessageId`，也不把 transcript 写入 localStorage、sessionStorage、URL、日志或 metric label。
...
- ReportConversation 使用单列可读 transcript；desktop 保持与 Practice message content width 一致，390px code/table 仅在 message 容器内滚动，document 不横向溢出。
...
- Session History 一级模块、会话列表、`listPracticeSessions` consumer、`sessionId` 深链、Workspace 会话数据源或 Header 第三枚主 CTA。
...
| R-13 | owned report 处于 queued/generating/ready/failed | 从 Report/Generating 或 ReportsScreen current report 行打开记录 | `/report-conversation?reportId=...` 显示该报告唯一 ordered Markdown transcript；无 Composer/实时动作/内部 ID，并能返回同一父报告状态 |
| R-14 | missing/cross-user/invalid/乱序 transcript | 深链或切换 reportId | hidden 404 或 typed error fail closed；旧 transcript 首帧清空，不泄漏其他报告消息 |
| R-15 | desktop/390 prototype 与 formal frontend | 执行 source structure、computed style、bounding box 与截图 parity | `ui-design/src/screen-report.jsx::ReportConversationScreen` 和 `ui-design/src/screen-practice.jsx` 的消息 renderer 锚点可追溯，单列 transcript 无溢出且没有第三 Header CTA |

## 11 修订记录

| 日期 | 版本 | 变更 |
|------|------|------|
| 2026-07-15 | 1.34 | 锁定 report-owned 只读会话记录：reportId-only、报告存在即展示、Report 主入口 + ReportsScreen 同轮快捷入口、安全 Markdown、父报告 Back、无 session 列表/listPracticeSessions/第三 Header CTA。 |
