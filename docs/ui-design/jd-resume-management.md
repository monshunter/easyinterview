...
│  ├─ JD 输入卡
│  │  ├─ 粘贴 JD 输入框
│  │  └─ 上传 JD 文件 / URL 导入（卡片底部 source actions）
...
| 用户在首页新建面试规划 | 在首页同一个 JD 输入卡内粘贴 JD，或从卡片底部选择上传 JD 文件 / URL 导入；通过适度宽度下拉框选择已有 ready `Resume`，创建简历入口在下拉框右侧同排；未选择简历或未提供 JD 前「立即面试」禁用，按钮位于简历选择下方，成功导入后 route 必须携带真实 `resumeId` |
...
| 版本 | 日期 | 修订内容 |
|------|------|----------|
| 2.4 | 2026-07-06 | 首页 JD source actions 整合回输入卡底部：上传 JD 文件与 URL 导入不再作为独立 panel，保留简历下拉框与立即面试布局。 |
