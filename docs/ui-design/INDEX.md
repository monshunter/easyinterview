...
| 文档 | 版本 | 状态 | 更新日期 | 说明 |
|------|------|------|----------|------|
| [目标总体架构](./ui-architecture.md) | 2.22 | active | 2026-07-08 | 三入口信息架构、TopBar、用户菜单、核心 JD / 简历 -> 面试 -> 报告 -> 复练 / 下一轮闭环、面试入口默认展示面试规划列表，首页新建规划把上传 JD / URL source actions 整合进 JD 输入卡底部，简历下拉框与创建入口同排，简历详情只读原始正文，非当前 route 输入归一和当前边界 |
...
| [当前面试规划目标模块](./module-job-workspace.md) | 1.21 | active | 2026-07-08 | 面试一级模块作为规划列表和当前规划详情回访枢纽：列表项必须是可感知且简洁的卡片，包含背景、边框、轻阴影、body/footer 分区和主题 accent 进入按钮，不展示来源类型、目标语言或手动输入等导入元信息；JD、简历、轮次、InterviewContext、公司情报嵌入卡片、当前规划记录与立即面试入口；Home 最近模拟面试更多入口进入规划列表；首次导入主路径在 Home 输入卡内选择 JD source，再下拉选择已有简历后进入 parse 核对 |
