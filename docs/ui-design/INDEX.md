...
| [目标用户流程](./user-flow.md) | 2.16 | active | 2026-07-07 | 首页启动、解析确认、模拟面试回访、面试与报告、简历创建后直接打开详情、只读原始简历详情、认证设置和非当前流程边界 |
| [目标模块地图](./module-map.md) | 2.9 | active | 2026-07-06 | 三入口模块、会话级页面、设置认证、非当前 route 输入归一和当前目标数据依赖 |
...
| [简历一级模块](./resume-module.md) | 2.6 | active | 2026-07-07 | 平铺简历列表、上传/粘贴创建后直接打开详情、LLM-derived displayName 与前端兜底、只读原始正文、无导出/复制/编辑/改写/原件弹层和非当前入口边界（D-20） |
...
| [首次无简历引导](./resume-onboarding.md) | 1.9 | active | 2026-07-07 | 上传/粘贴简历、注册成功后直接打开只读详情、通用粘贴名称负向和非当前 onboarding / 轻量问答边界（D-20） |
