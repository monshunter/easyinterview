...
| [当前面试规划目标模块](./module-job-workspace.md) | 1.24 | active | 2026-07-09 | 面试一级 `workspace` 是纯规划列表页：只展示 ready 且标题非空的 TargetJob 卡片，包含背景、边框、轻阴影、body/footer 分区和主题 accent 进入按钮，不读取 `targetJobId/planId/resumeId` 等详情上下文；卡片进入 `parse` 统一面试规划详情 / 面试上下文确认；解析失败、非 ready 或空标题 JD 不得进入列表；Home 最近模拟面试更多入口进入规划列表 |
