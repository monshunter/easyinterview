...
- desktop 设置页使用约 `1372px` 内容列：顶部左侧为 eyebrow、标题与说明，右侧为低对比度账号/安全插画；下方依次是外观、账号、隐私与数据三张横向功能卡。
- 每张设置卡均使用统一的左侧图标轨道、中央信息区和右侧操作区。外观卡保留真实主题草稿与保存，账号卡保留真实账号字段与退出，隐私卡保留不可用导出说明和删除账号状态机；不得因视觉重排复制 mutation、增加静态伪功能或改变 owner。
