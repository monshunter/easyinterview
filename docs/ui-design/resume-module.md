...
3. 简历首页是 `Resume Workshop` 平铺卡片列表，无树形分组、无视图切换、无“选为底稿”；桌面端使用固定最大列宽的响应式多列，移动端使用同序单列。
...
1. 简历卡片列表和简历选择弹窗都必须展示简历名称、来源和最近编辑时间；列表卡片还可以展示 closed `ResumeSummary.summaryHeadline` 与语言标签，但不得读取详情正文。
2. 简历列表是单层卡片网格，不得出现表头、表格行、树形分组、主版本 / 定制版本标签或“选为底稿”动作；桌面卡片使用固定最大列宽并从左开始排列，单卡不得拉伸整行，移动端保持同一 DOM/阅读顺序并收敛为单列。
