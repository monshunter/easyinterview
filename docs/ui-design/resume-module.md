...
5. 简历详情页是只读正文页：上传 PDF 使用同源 source endpoint 渲染为从上到下平铺的 PDF 页面栈；粘贴、Markdown 文件和 TXT 文件使用 Markdown 渲染引擎展示正文；Markdown body 区只渲染简历正文本身，不额外注入 `displayName`、header 名称、summary 或来源元数据。Desktop `1916×821` 下详情内容面约 `1512px`，Back、蓝色 eyebrow、名称 kicker、主标题和 meta 共享左边界；正文不再包裹额外的共享浅色背景板，PDF 页面与 Markdown 白色 page surface 共用 `794px` A4 纸宽并直接位于页面画布居中。PDF 的每一页保持 `210:297` 纵向纸张比例；Markdown 是一张连续长页面，只采用 A4 宽度，高度完全由整份正文内容决定，不按 A4 分页、不设置 A4 比例或固定/最小纸高。Markdown 白色纸张拥有独立的深色墨水基色，正文、列表、强调文本和代码不得继承夜间应用壳的浅色 foreground；应用切换 light/dark 时纸张内容都保持可读。窄屏按可用宽度收敛，不产生横向滚动。详情不提供导出、复制、编辑、预览标签、改写建议、结构化草稿确认、浏览器 PDF 工具栏或查看原始简历弹层。
...
3. 简历详情必须只展示简历内容本身；Header 使用 Back、蓝色 eyebrow、名称 kicker、主标题与来源/日期 meta 的参考稿层级；desktop 内容面约 `1512px`，不再添加约 `1310px` 的共享阅读背景板，PDF 页面栈与 Markdown 白色页面统一为 `794px` A4 纸宽并直接居中；PDF 每页保持 `210:297` 纵向比例，Markdown 作为整份正文的一张连续长页面，只锁定宽度并由内容自然撑高，不分页、不设置 A4 比例或固定/最小纸高；Markdown body 不得额外注入 `displayName`、header 名称、summary 或来源元数据，且白色纸张必须用自身深色墨水色覆盖正文、列表、强调文本和代码，不能在夜间应用模式下继承浅色壳层 foreground；mobile 在可用宽度内收敛且不得横向溢出，也不得出现预览 / 改写 / 编辑 tabs、导出、复制、原件弹层、浏览器 PDF 工具栏或二次编辑动作。
