...
当前运行时入口是 `resume_versions`，目标实现以 `ui-design/src/screen-resume-workshop.jsx` 为准。按 product-scope D-20，简历资产是扁平 IA：不存在原始简历树、结构化主版本、岗位定制版本、分叉流程或版本管理；范围外画板或范围外组件不得作为树形实现依据。
