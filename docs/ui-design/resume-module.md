...
5. 简历详情页是只读正文页：上传 PDF 使用同源 source endpoint 渲染为从上到下平铺的 PDF 页面栈；粘贴、Markdown 文件和 TXT 文件使用 Markdown 渲染引擎展示正文；Markdown body 区只渲染简历正文本身，不额外注入 `displayName`、header 名称、summary 或来源元数据。Desktop `1916×821` 下详情内容面约 `1512px`，Back、蓝色 eyebrow、名称 kicker、主标题和 meta 共享左边界；正文不再包裹额外的共享浅色背景板，PDF 页面栈与约 `1150px` 的 Markdown 白色 page surface 直接位于页面画布并居中。详情不提供导出、复制、编辑、预览标签、改写建议、结构化草稿确认、浏览器 PDF 工具栏或查看原始简历弹层。
...
9. 用户最多可保留默认 10 份 active 简历，上传文件默认最大 10MiB；用户可以从列表删除简历以释放数量上限。点击卡片右上角删除图标只打开二次确认对话框，确认前不得调用 `archiveResume`；对话框明确说明简历会从列表移除且当前无法撤销，并以“取消”为初始焦点。取消、点击遮罩或按 Escape 均关闭对话框且零请求，确认后才执行软归档；pending 期间禁止关闭和重复提交，失败保留对话框并提供重试。
...
原始内容只读保存。详情页根据来源格式自动选择渲染方式：PDF 上传使用原始文件的同源 source endpoint 渲染纵向页面栈；粘贴、Markdown 文件和 TXT 文件优先展示 `parsedTextSnapshot` 形成的 Markdown 正文，`parsedTextSnapshot` 为空时才回退到 `originalText`。正文 wrapper 只承担语义分组，不设置背景、边框、圆角、阴影、固定高度或额外内边距；PDF 页面与 Markdown 白色页面直接位于详情画布。Markdown 页面内不得额外 prepend `displayName`、详情 header 名称、summary 或来源元数据。它不提供结构化草稿确认、二次编辑、改写采纳、复制、导出、浏览器 PDF 工具栏或原件弹层。结构化内容只作为无原文时的降级兜底。
...
3. 简历详情必须只展示简历内容本身；Header 使用 Back、蓝色 eyebrow、名称 kicker、主标题与来源/日期 meta 的参考稿层级；desktop 内容面约 `1512px`，不再添加约 `1310px` 的共享阅读背景板，PDF 页面栈与约 `1150px` 的 Markdown 白色纸张直接居中；PDF 上传用从上到下平铺的 PDF 页面栈，粘贴 / Markdown / TXT 用 Markdown 渲染；Markdown body 不得额外注入 `displayName`、header 名称、summary 或来源元数据；mobile 收敛为满宽可读页面，且不得出现预览 / 改写 / 编辑 tabs、导出、复制、原件弹层、浏览器 PDF 工具栏或二次编辑动作。
...
7. 简历列表不得出现重复创建 CTA；每张卡片底部保留明确“打开”动作，删除图标固定在右上角。首次点击删除只打开与账号危险操作同层级的确认对话框：显示“请谨慎操作”、实体级问题和不可撤销说明，“取消”为安全默认动作；确认前 `archiveResume` 请求数必须为 0。取消、遮罩和 Escape 关闭后焦点回到原删除按钮；确认后才执行软归档，pending 禁止关闭/重复提交，成功从 active 列表隐藏简历并释放默认 10 份数量上限，失败保留原卡片与对话框并显示可恢复错误，同一对话框内重试复用同一 idempotency key。
...
| 4.4 | 2026-07-20 | Home 简历下拉选项收敛为仅显示简历名称，移除语言、来源、更新时间与摘要拼接；简历资产列表和详情继续承接元信息。 |
| 4.3 | 2026-07-20 | 移除只读详情正文外层的共享浅色背景板；PDF 页面栈与 Markdown page surface 直接位于详情画布，保留 Header、来源格式 renderer 与只读数据合同。 |
