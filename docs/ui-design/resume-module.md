...
8. `拒绝 / 编辑 / 采纳` 只作用于当前岗位定制版本，不改变原始简历、主版本或其它兄弟版本。
9. 用户上传、粘贴或问答生成的简历都必须有可识别名称。
10. 创建新原始简历必须经历 `输入 -> Agent 解析 -> 预览确认 -> 保存 v1`，不能在未确认前写入正式版本。
11. 系统必须保留原始文件或原始文本；解析、编辑和岗位定制不能覆盖原件。
12. 模拟面试规划页只选择当前面试使用哪份简历，不承担完整简历管理职责。
13. 更换绑定简历应在模拟面试规划页打开简历选择弹窗。
...
8. 从已有树分叉岗位定制版本时，必须记录目标岗位 / 公司、侧重方向和 bullet 初始化方式。
9. 模拟面试规划页更换简历时不应直接跳转到简历模块。
