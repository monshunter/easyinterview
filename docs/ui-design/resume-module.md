...
7. 用户上传或粘贴生成的简历都必须有可识别名称；最终 `displayName` 由 backend parse 根据 LLM 结构化结果生成，不以“上传的简历”“粘贴的简历”、文件名或 raw resume 第一行作为完成态名称。
8. 系统必须保留原始文件或原始文本；上传 PDF / DOCX / Markdown / text 的详情正文来自文件可读正文提取，解析结果不能覆盖原始来源快照。
...
5. parse 成功后的完成态名称必须优先使用 LLM 根据简历内容生成的 `displayName`；通用上传 / 粘贴标题、文件名和 raw resume 第一行只能作为来源信息或正文内容，前端不得展示为列表或详情名称。
...
| 版本 | 日期 | 修订内容 |
|------|------|----------|
| 2.7 | 2026-07-07 | 修订命名与上传原文展示回归：禁止从 raw 第一行或文件名派生可见名称，上传 PDF/DOCX/Markdown/text 详情正文必须来自文件可读正文提取。 |
