...
`easyinterview-ui/easyinterview-canvas.html` 是设计画板总览，仍保留若干历史分区标题和旧 artboard 标签。判断目标模块时，以 `src/app.jsx` 的一级导航、`routeAliases` 和实际 screen 返回内容为准；画板标签只能作为截图索引，不能覆盖运行时交互。
