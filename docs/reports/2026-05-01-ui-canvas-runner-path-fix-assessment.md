...
- 本次交付范围：修复 `ui-design/run.sh` 启动后 `canvas.html` 空白的问题，覆盖画板包装脚本路径、iframe 原型入口路径、默认启动入口文件名。
- 成功证据：
  - `curl -I http://127.0.0.1:5173/canvas.html` 返回 `HTTP/1.0 200 OK`。
  - `curl -I http://127.0.0.1:5173/design-canvas.jsx` 返回 `HTTP/1.0 200 OK`，覆盖截图中 404 的画板脚本。
  - `curl -I http://127.0.0.1:5173/index.html` 返回 `HTTP/1.0 200 OK`，覆盖画板 iframe 的原型入口。
  - `./run.sh -f canvas.html --no-open` 在 5174 端口启动后，`canvas.html`、`design-canvas.jsx`、`index.html` 均返回 200。
  - `./run.sh --no-open` 使用默认入口 `index.html` 启动并返回 200。
  - `git diff --check -- ui-design/canvas.html ui-design/run.sh` 通过。

## 2 会话中的主要阻点/痛点

- 画板 HTML 使用了与 `run.sh` 服务根目录不一致的资源路径。
  - **证据**：`canvas.html` 曾引用 `ui-design/design-canvas.jsx` 与 `ui-design/index.html`，但 `run.sh` 会先 `cd` 到 `ui-design/` 再启动静态服务器。
  - **影响**：浏览器请求多出一层 `ui-design/`，导致画板脚本 404，页面只剩背景空白。
- 默认入口文件名需要跟随 UI 目录迁移收敛。
  - **证据**：当前 `run.sh` 默认入口已统一为 `index.html`，与仓库真实入口一致。
  - **影响**：若入口命名再次漂移，默认启动脚本会找不到入口，用户需要手动传 `-f` 才能绕过。
- 当前 change-intake 无法稳定命中 UI 原型运行入口。
  ...
  - **影响**：问题定位需要手动回到 `ui-design` 文件路径，无法由 plan/context 自动承接。
- 浏览器级验证环境有额外限制。
...
- 为 `ui-design` 增加一个轻量 smoke 验证脚本，至少检查 `index.html`、`canvas.html`、`design-canvas.jsx` 和 iframe 入口均可访问。
  - **落点**：README / test
  - **优先级**：medium
