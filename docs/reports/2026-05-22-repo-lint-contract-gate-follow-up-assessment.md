...
- Lint gate 修复：`backend_practice_legacy` 和 `mock_runtime_boundary` 不再把当前 practice voice MVP 的 `createPracticeVoiceTurn`、`/voice-turns`、`PracticeVoiceTurn*` 误判为退役独立 Voice 入口；runtime topology active docs 去除旧 shorthand；Go 内部命名满足 revive。Review follow-up 确认 JD Match F3 维度原本已在 `rubric_lint` allowlist 中，本分支仅需移除重复 no-op stanza 并校正证据记录。
...
- `make lint` 的 fail-fast 行为使多个历史 gate drift 串行暴露。
  - **证据**：practice voice retired-route false positive 修复后，才继续暴露 mock runtime boundary、runtime topology wording 和 Go revive 命名问题；review follow-up 反查 merge base 后确认 JD Match rubric allowlist 记录属于 no-op 重复项证据漂移。
...
- Follow-up 证据记录没有先对照 merge base。
  - **证据**：JD Match F3 rubric 文件已经存在，`rubric_lint.py` 与 `config/rubrics/README.md` 也已经允许当前六个维度；本分支重复添加同一 allowlist stanza，并把它误写成 lint blocker。
  - **影响**：BUG、report、owner plan/history 会误导后续 reviewer，以为存在已修复的 rubric 行为缺口。
...
- Retired-token lint 缺少“当前 owner 复用旧词根”的正向 fixture。
  - **类别**：spec-plan / test
- Follow-up 证据记录缺少 merge-base/current-truth 反查。
  - **类别**：spec-plan
...
- 在涉及退役模块清理的 lint owner checklist 中加入正向 fixture 要求：凡当前 owner contract 合法包含旧词根，必须新增 allow-case test，不能只写 negative search。
  - **落点**：spec-plan / test
  - **优先级**：high
- 对 repo lint follow-up 增加“先对照 merge base / current truth source，再写 BUG/report/plan 证据”的检查；如果 diff 只是重复项，应记录为 evidence correction 而不是 behavior fix。
  - **落点**：spec-plan / skill
  - **优先级**：high
...
- 同步可做：在 mock-contract-suite、practice-voice-mvp、backend-jobs-recommendations 的后续计划模板中保留 lint precision / current-truth diff / regression fixture 的显式 gate。
