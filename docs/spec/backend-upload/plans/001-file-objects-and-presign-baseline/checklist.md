...
- [ ] 1.5 handler 返回 200 + `UploadPresign{fileObjectId, uploadUrl, method, headers, expiresAt}` 与 B2 fixture `default` scenario 字节一致（验证：fixture parity test）
...
- [ ] 4.5 privacy_delete job 顺序：file_objects 先于 resume_assets / target_jobs（按 B4 §3.1.2 matrix）（验证：手工或 scenario 验证顺序）
...
- [ ] 5.4 在 `test/scenarios/e2e/INDEX.md` 追加 E2E.P0.033 行（关联需求 `backend-upload C-1, C-2, C-6, C-8`，状态 Ready，automated）
