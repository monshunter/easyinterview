...
**关联 Spec**: [Harness 工程框架收敛](../../spec.md) 2.3
**工作模式**: `loop`
...
- `context.yaml`、独立 checklist/BDD/test plan、`history.md`、checked-in template、`docs/ui-design/` 和已完成 plan 的语义承接与退出；
...
- 保留无期限旧名 alias、双轨 workflow、通用阻塞状态、`context.yaml` 兼容层或等价 manifest/cache；
...
- 目标执行路径对旧 Skill 名称、Skill-to-Skill 调用、`context.yaml`、独立 checklist/BDD/test plan、checked-in templates 和通用阻塞机制的可执行依赖减少；
...
4. 新路径不读取、写入、导入、转换、兼容或回放 `context.yaml`，也不建立替代 manifest/cache。
5. 删除前先证明有效语义和当前调用方已被替代 owner 承接；`context.yaml` 只按路径盘点后删除，不从内容迁移事实。
...
- 不调用依赖 `context.yaml`/独立 checklist 的旧 `/implement` 主路径，不为本计划创建临时 manifest、转换器或兼容 reader；
...
**目标**：在继续 Phase 1.2 前，执行用户于 2026-07-19 授权的单一 migration Change，把仍被旧 workflow 消费的 `context.yaml` 收缩为最小文档链接清单，停止 discovery/references/metadata 扩展继续产生错误 owner 置信度和重复事实。该 Phase 不改变 Phase 2 删除全部清单与 reader/writer 的最终目标。
...
- [ ] 2.8 用全量负向搜索证明目标执行路径不存在 `context.yaml` 支持、独立 checklist/BDD/test plan reader/writer、`history.md`、checked-in templates 或 `docs/ui-design/` owner。
...
| A7、A19、A23、A28、A29、A30、A31 | Cross-layer contract / regression | 2 | docs transaction、INDEX rebuild、current-truth lookup、资产族迁移审计 | context reader/writer/importer/converter、独立 checklist/BDD/test/history/template/ui-design owner |
...
- **最近完成**: 完成 Phase 0A：49 份现有清单全部迁移为最小 schema，保留原 9 类 target identity 和一等链接；validator/generator/matcher/candidate、AGENTS、Skill、共享合同和模板完成 hard cut；无 context 的当前 plan 可被发现并直接读取 owner 文件；post-pass reconcile 与 retrospective 已完成。
- **当前 Phase**: Phase 1。
- **当前未完成**: 实现 Phase 1.2 的 versioned Blueprint、bundled templates、adapter schema、四模式原子 CLI 与 `make harness-test` 聚合入口，使 Phase 1.1 owner tests 转绿。
- **当前阻塞**: 无；当前两个 Harness 红项属于 Phase 2 owner lookup 迁移输入，不通过旧标题或临时入口制造假绿。
- **验证摘要**: Phase 0A 最终 Skill tests `155 passed`；4 个生产脚本 `py_compile` PASS；49 份 manifest schema audit `invalid=0`、batch validation `49/49`、generator `updated=0, reconciled=49`、target/一等链接 `semantic_link_drift=0`；完整 Harness matcher query medium 命中本 plan，practice recovery query 保持正确；`sync-doc-index --check`、`make docs-check`、字段生产读取负向搜索和 `git diff --check` PASS。Phase 1.1 的 12 个 Project Arch Red 与 Phase 2 两个既知红项保持不变。
- **下一动作**: 恢复 Phase 1.2，实现 `scripts/harness_arch.py` 最小 Green，并只运行 focused Project Arch tests、目标 `make harness-test`、`make docs-check` 与 `git diff --check`。
- **恢复入口**: 读取 Spec 2.3、本节 checkpoint、`git status/diff/log`、focused Harness/Skill tests、`make docs-check` 和 fixture Env status；不把 `context.yaml` 内容当成新 Harness owner 事实，不机械运行无关产品 gate。
