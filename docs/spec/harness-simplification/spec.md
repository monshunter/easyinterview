...
### 10.1A 过渡期 `context.yaml` 最小合同

在 Phase 2 删除全部 `context.yaml` 与调用方之前，仍由旧 workflow 消费的清单必须先硬收缩为只表达文档链接的临时合同，避免把已经证明会漂移的人工检索词和执行元数据继续当成 owner 事实。该收缩不是目标架构兼容层，也不改变 A23 的最终退出条件。
...
- generator 必须确定性删除上述退出字段，不得读取、合并或保留旧值；validator 必须对任何额外字段 fail closed；owner 匹配不得再以这些字段计算置信度；
- 全仓现有清单、模板、Skill 说明和调用方必须在同一个 Change 中迁移，并以 focused contract tests、全量 manifest 校验和字段 zero-reference 搜索作为退出证据。

Phase 2 的最终删除仍优先于长期维护该临时格式；不得新增依赖、alias、converter 或第二份 manifest 延长其生命周期。
