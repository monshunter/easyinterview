...
- 移除通用阻塞与通行状态抽象，把 `context.yaml` 收敛为当前工作文档的一等链接投影；
- 不同风险任务应承担的最小充分流程成本；
...
Project Arch 是由 `init-arch` 内置、安装和升级的版本化工程骨架合同。它定义稳定角色、固定入口、模板参数、生命周期、扩展点和确定性检查，不硬编码任何产品业务实例。业务实例不是外部手工输入，而是 Harness 读取仓库事实与项目 Spec 后生成、实施并验证的 Project Arch 实例。安装版本由 `docs/README.md` 中唯一的 `<!-- project-arch: v1 -->` 标记持有；每个当前 plan 只保留一份可由目录重建的最小 `context.yaml` 文档链接投影，不复制 Project Arch 版本或业务事实。
...
└── spec/
    ...
    └── <subject>/
        ...
        └── plans/
            ├── INDEX.md
            └── <work>/
                ├── context.yaml     # 最小一等文档链接投影
                └── plan.md          # 仅在存在有限或长时工作时
test/
├── README.md
└── scenarios/
    ├── README.md
...
| 当前工作层 | 为达到当前 Spec，当前有限交付或长时循环具体做什么、如何恢复与退出 | 可选的唯一 `docs/spec/<subject>/plans/<work>/plan.md`；同目录最小 `context.yaml` 只投影一等文档链接 |
...
新 Harness 保留最小 `context.yaml` 作为 plan-context Skill 的统一入口和一等文档链接投影。它只允许 `metadata.name`、`spec.defaultTarget`、`spec.targets`，以及 target 内的 `plan`、`checklist`、`spec`、`testPlan`、`testChecklist`、`bddPlan`、`bddChecklist`；不得承载 discovery、references、分支、版本、命令、状态或业务事实。精确 owner 路由和候选解释仍由 Project Arch tooling 从 manifest 所指正文、路径、Header、代码/API/route 标识、Git 和当前任务输入实时确认；manifest 只负责定位，不是 Spec、plan 或实现事实源，也不得派生第二份 manifest/cache。单文件 plan 可让 `plan` 与 `checklist` 指向同一个 `plan.md`，由正文行内 checkbox 承接执行进度。
...
1. 通过 `docs/spec/INDEX.md`、subject `plans/INDEX.md` 与最小 `context.yaml` 定位当前 Spec 和适用的单一 `plan.md`，再由正文与仓库事实确认 owner；不得把 manifest 字段扩展为 owner 语义或跨任务状态；
...
| `history.md`、`checklist.md`、`bdd-plan.md`、`bdd-checklist.md`、`test-plan.md`、`test-checklist.md` | 不属于目标架构；实施迁移时按第 10 节承接唯一有效语义后删除 | 任何当前 owner、输入、输出、缓存或完成依据 |
| 最小 `context.yaml` | 当前 plan 的 target 与一等文档链接投影；单文件 plan 的 `plan`/`checklist` 可同指 `plan.md` | 业务事实、owner 置信度、分支/版本/命令、执行状态或跨任务缓存 |
...
| owner 路由与引用解释 | Project Arch tooling 的 INDEX + 最小 `context.yaml` + current-truth lookup contract | Skill 读取并校验链接投影，再以正文和仓库事实确认 owner；不消费扩展语义或持久化第二份 manifest/cache |
...
3. 建立确定性 INDEX、current-truth owner lookup 和 Project Arch docs transaction/check；按“当前事实 / 当前工作 / 独立知识 / 可删除投影”重分类现有 docs；让最小 `context.yaml` 的 generator、validator 和所有 consumer 原子适配单文件/多文件 plan，并禁止扩展字段或第二份 manifest/cache；把旧 checklist、BDD/test plan/checklist 中仍属当前工作的内容折叠进唯一 `plan.md`；把旧 `history.md` 中仍有效的当前约束合并回 Spec、独立知识迁到 Bug/report/Decision 后删除全部 `history.md`；把旧 `docs/ui-design/` 的有效合同合并到相关 subject Spec 后删除该文档域；把模板迁入 bundled assets，重建各文档集合 INDEX 并删除空扩展；调用方切换后删除 `/create-doc` 与 `/sync-doc-index`；
...
### 10.1A `context.yaml` 最小合同

所有当前 plan 必须使用只表达 target 与一等文档链接的最小合同，避免把已经证明会漂移的人工检索词和执行元数据当成 owner 事实。该 manifest 是 plan-context Skill 的稳定入口和可重建投影，不是业务 owner、执行状态或兼容缓存。

临时合同固定为：

- `metadata` 只允许 `name`，不得保留 `subspec`、`sequence`、`specVersion`、`baseBranch`、`branch` 或其他字段；subject、plan 顺序和分支信息必须从路径、当前 Spec Header、Git 与 `AGENTS.md` 推导；
- `spec` 只允许 `defaultTarget` 和 `targets`，不得存在顶层 `discovery`；
- `spec.targets.<target>` 只允许当前文档链接字段 `plan`、`checklist`、`spec`、`testPlan`、`testChecklist`、`bddPlan`、`bddChecklist`，不得存在 target `discovery`、`references` 或其他人工扩展字段；
- generator 必须确定性删除上述退出字段，不得读取、合并或保留旧值，并在一等文档新增后补齐对应链接；validator 必须对任何额外字段 fail closed；owner 匹配不得再以这些字段计算置信度；
- 单文件 plan 允许 `plan` 与 `checklist` 同指 `./plan.md`，多文件 plan 仍分别指向 `plan.md` 与 `checklist.md`；consumer 必须按绝对路径去重读取，但保留两个角色语义；
- 全仓现有清单、模板、Skill 说明和调用方必须在同一个 Change 中迁移，并以 focused contract tests、全量 manifest 校验和字段 zero-reference 搜索作为退出证据。

暂不规划删除 `context.yaml`。后续只有在所有 consumer 已有更小且可验证的统一入口、用户重新确认迁移方向时，才能另行设计删除；当前 Phase 不以 zero-reference 或无 reader/writer 为退出条件。

### 10.2 迁移期保留

1. 当前 workflow Skill 与 docs 目录在对应阶段完成前继续可用，但不得为新目标增加旧机制依赖。
2. Git 历史和已冻结的 work journal 保留；Bug、report、Decision 只在具有独立知识价值时保留。`history.md` 不因“历史”名义继续存在，必须按第 10.1 节完成当前事实/独立知识承接后删除。`context.yaml` 只保留文档定位语义，任何 owner 事实仍必须从其引用正文与当前仓库推导。
...
5. 任何删除必须先证明唯一语义已被 docs/README 或可执行验证完整承接，并有负向搜索和回退证据；最小 `context.yaml` 当前不在删除范围内。
...
Loop Engineering 是编排与执行方式，不是新文档类型、顶层 Skill、后台常驻服务或无限授权。进入 `loop` 模式时，`plan.md` 必须以正文持有以下最小合同；除标准最小 `context.yaml` 链接投影外不新增 YAML/manifest：
...
长时工作可以跨多次 Agent turn、上下文压缩或进程重启。恢复可通过最小 `context.yaml` 定位当前 Spec/plan，但状态只读取 `plan.md` 的最新 checkpoint、确定性 INDEX、Git status/diff/log、测试结果和 Env status；不得把 manifest 扩展为 session state。checkpoint 只保存恢复所需的当前状态：最近完成的 Iteration 摘要、当前未完成项、阻塞、验证结果摘要和下一动作；详细过程进入 Git/work journal/靠近执行 owner 的日志，plan 不追加为无限增长的运行日志。
...
| A23 | 每个当前 plan 都有且只有一份最小 `context.yaml`；validator 对未知字段 fail closed，generator 能补齐新增一等文档链接且保持 target identity，所有 consumer 只用它定位正文并从当前仓库实时确认 owner，不建立第二份 manifest/cache |
...
- 任一代码、脚本、Skill 或文档流程把 `context.yaml` 扩展为 discovery、references、分支/版本/命令、状态或业务 owner，或允许缺失 manifest 绕过统一入口；
...
恢复动作应优先回退单个 Project Arch/Skill Change、恢复升级 checkpoint、原编排入口或原仓库验证入口。长时工作优先用最小 `context.yaml` 定位当前 Spec/plan，再从 plan checkpoint、INDEX、Git、测试与 Env 状态恢复。不得通过要求 caller 每次重传 Arch、复制业务规则回 Skill、扩展 manifest 语义或建立长期双轨掩盖职责不清，也不得通过拆分 checklist/BDD 文档或重建通用阻塞机制来恢复。
...
- 2026-07-19：用户授权先收缩仍在运行的旧 `context.yaml` 合同：删除顶层与 target `discovery`、target `references`，并让 `metadata` 仅保留 `name`；该 Change 必须同步迁移 generator、validator、matcher、Skill/模板与全仓清单，不构成对 A23 最终删除目标的兼容承诺。
- 2026-07-19：用户进一步确认以收缩后的最小 `context.yaml` 为当前标准，要求其他 Skill 与 script 适配该合同，暂不考虑移除 `context.yaml`；本决策取代此前 A23 的删除方向，但不恢复 discovery、references 或其他退出字段。
