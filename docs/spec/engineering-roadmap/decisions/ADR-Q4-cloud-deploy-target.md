...
### 选项 A · Docker Compose 外部依赖 + 宿主机 app runtime + 本地 scenario runner

**Pros**：

- 与当前代码事实一致：`make dev-up` 管理外部依赖，backend/frontend 可在宿主机直接运行。
- 场景脚本复用现有 Go / Vitest / Playwright / browser runner，证据链短、反馈快。
...
**P0 锁定选项 A：Docker Compose 外部依赖 + 宿主机 app runtime + repo-tracked 本地 scenario runner。**
...
- **AGENTS.md / CLAUDE.md** —— Agent 执行门禁必须区分 Docker Compose 外部依赖、宿主机 app runtime 和本地 scenario runner，不得默认引入 Kind / K8s / Helm。

## 5 失效与修订条件

触发推翻或升级本 ADR 的具体阈值：

- 第二名长期贡献者加入，且本地 runner 无法提供一致验证环境 → E4 / A5 评估远端 CI 和容器化 app service。
