...
`engineering-roadmap decisions` §2 把「PostHog / Segment（产品分析）」并列为外部依赖；`F1 observability-stack` §1 把「产品分析」分类设为 `PostHog / Segment / Warehouse`。当前产品漏斗和事件范围以 `docs/spec/product-scope/spec.md` 与 F2 `analytics-funnel` 后续 contract 为准：导入→规划→练习→报告→复练当前轮 / 下一轮；题目回顾信号只嵌入报告，不形成独立模块。`README.md` §「待评审的 5 个决策点」第 3 项只作为决策输入。

业务背景：

- Readiness / trend signals 强依赖事件流（funnel / cohort / retention），但只嵌入报告和面试规划，不生成独立成长中心
...
P0 阶段采用与 product-scope / 当前 UI scope 对齐的产品分析事件集 + 3 漏斗，事件量级预期 ≤ 100k/day，无 ETL 重塑需求；具体事件名和 props 由 F2 原地锁定，只覆盖当前模块事件。
...
- 含 feature flags；灰度字段以 A4/F2 当前 contract 为准，`F1 observability-stack` §「灰度发布」只作背景输入
- 不引入 warehouse 维护成本
...
- PostHog Kubernetes / Helm 路径已不再是官方支持主线（见 PostHog `charts-clickhouse` README）；不得把当前 gate 建在 out-of-scope chart 假设上
...
- Readiness / trend signals 失去基础数据
...
1. **事件契约**：以 product-scope / 当前 UI scope 对齐后的 F2 产品分析事件集为唯一产品分析来源；若需要复用内部 envelope / trace 语义，必须引用 B3 `event-and-outbox-contract` 当前 contract，不再直接以 `B3 event-and-outbox-contract` 为准。前后端双发由 F2 `analytics-funnel` 提供 `idempotency_key` 去重
...
5. **feature flags**：自托管 PostHog feature flags 是唯一灰度源；具体 flag 字典由 A4/F2 当前 contract 决定，`engineering-roadmap decisions` §「ai_fallback_model_enabled」只作背景输入
...
8. **rate limit**：客户端 SDK 默认开启 batch；服务端事件经 backend async runner outbox 异步发送，避免阻塞主请求；`analytics_dispatch` 若落入 `async_jobs`，必须先由 B3 / B4 additive 加入 public `jobType` 契约
...
- **`backend-async-runner`** —— `analytics_dispatch` job_type 异步推送 server-side 事件；新增前必须由 B3 / B4 更新 public job 契约
...
- **当前 P0 backend / frontend workstream 与 future candidates** —— 通过 F2 接口埋点，不引入新 vendor；readiness / trends 只能作为嵌入式信号进入报告或面试规划，不恢复独立 growth overview
...
修订流程：如需推翻本决策，新增修订 ADR 并同步 roadmap Q-3 与相关 owner spec。如果 F2 / E4 发现 self-host path 不可运维，不得静默降级到 Cloud，必须走新 ADR。

## 6 关联

- `engineering-roadmap/spec.md` §3.2 Q-3
- `engineering-roadmap/plans/001-decompose-subspecs/plan.md` checklist 1.1
- 参考背景：`engineering-roadmap decisions` §2、`F1 observability-stack` §1 §4 §15、`B3 event-and-outbox-contract`
...
- 下游 child / future candidate：F2 / frontend-shell / backend-async-runner / A4 / release-gate-and-rollout / B1 / F1 / A2 / advanced audit

## 7 修订记录

| 日期 | 版本 | 变更 |
|------|------|------|
| 2026-07-07 | 1.8 | 对齐当前产品闭环：产品分析漏斗只覆盖导入、规划、练习、报告、复练当前轮和下一轮，题目回顾作为报告内信号。 |
| 2026-05-04 | 1.7 | 对齐 engineering-roadmap v3.0：删除当前影响范围中的原 C10-C12 / D7 / F4 / E4 编号口径，改为当前 P0 workstream 与 future candidate 表述。 |
| 2026-05-03 | 1.6 | 明确当前产品分析事件、灰度和 envelope 语义分别以 F2、A4/F2 与 B3 active contract 为准；相关 ADR 与 F1/B3 文档只作为当时决策背景记录。 |
| 2026-05-03 | 1.5 | 移除 ADR 当前段落中的原产品分析事件数量硬编码，明确 F2 必须按 product-scope / 当前 UI scope 重新锁定事件集。 |
