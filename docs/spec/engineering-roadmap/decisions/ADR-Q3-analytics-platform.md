...
- PostHog Kubernetes / Helm 路径已不再是官方支持主线（见 PostHog `charts-clickhouse` README）；不得把 W1/W4 gate 建在已废弃 chart 假设上
- P0 运维复杂度高于 Cloud，F2 / A2 / E4 必须先验证可运维 self-host path
...
5. **feature flags**：自托管 PostHog feature flags 是唯一灰度源；与 `01-technical-architecture.md` §「ai_fallback_model_enabled」等 flag 落到同一 console
6. **环境隔离**：dev / staging / prod 各自独立 project；dev 可使用 no-op / lightweight self-host profile，但 staging 必须跑与 prod 同等的 self-host 验证路径
7. **部署边界**：本 ADR 只锁定 self-host，不锁死 Helm / K8s chart；F2 / A2 / E4 必须在对应 child spec 中验证当前官方支持的部署路径、备份、升级、恢复、容量与告警。若唯一可行路径无法满足 Q4/E4 gate，必须新 ADR 推翻本决策
...
- **F2 `analytics-funnel`** —— 落地 adapter + 18 事件 + 3 漏斗 + feature flag 包装层，并验证 self-host PostHog 的 funnel / flag 能力
...
- **A2 `local-dev-stack`** —— 提供可选 self-host analytics profile 或 documented no-op profile，不能阻塞普通 `make dev-up`
...
- **E4 `release-gate-and-rollout`** —— W4 漏斗对账 gate 引用自托管 PostHog funnel 报表，并验证备份 / 恢复 / 升级 runbook
...
修订流程：本 ADR 状态由 `accepted` → `superseded`，新 ADR 显式标注 `supersedes: ADR-Q3-analytics-platform.md`。如果 F2 / A2 / E4 发现 self-host path 不可运维，不得静默降级到 Cloud，必须走新 ADR。
...
- 下游 child：F2 / D1 / C8 / A4 / F4 / E4 / B1 / F1 / A2
