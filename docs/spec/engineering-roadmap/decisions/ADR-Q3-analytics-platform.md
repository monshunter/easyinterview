...
1. **事件契约**：以 product-scope / 当前 UI scope 对齐后的 F2 产品分析事件集 + `06-event-contracts.md` 的 envelope 为唯一来源；前后端双发由 F2 `analytics-funnel` 提供 `idempotency_key` 去重
