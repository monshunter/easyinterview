...
- 含 feature flags；灰度字段以 A4/F2 当前 contract 为准，`F1 observability-stack` §「灰度发布」只作历史 seed
- 不引入 warehouse 维护成本
...
5. **feature flags**：自托管 PostHog feature flags 是唯一灰度源；具体 flag 字典由 A4/F2 当前 contract 决定，`engineering-roadmap decisions` §「ai_fallback_model_enabled」只作历史 seed
