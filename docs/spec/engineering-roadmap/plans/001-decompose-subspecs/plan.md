...
B2 `openapi-v1-contract` 在本阶段只锁定 spec 合同：v1.0.0 freeze 的 36 endpoint / 14 tag 清单、字段命名、additive-only 规则、privacy export P0 返回 501 例外与 fixtures 同源边界。`openapi/openapi.yaml`、codegen、fixtures 与 breaking-change linter 的真实落地由 B2 自身后续 `001` 系列 plan 承接；在该 plan 验证前，不把「OpenAPI v1.0.0 freeze」作为可执行 gate 已完成处理。
...
A2 `local-dev-stack` 在本阶段只锁定 7 个本地服务、端口、卷、`make dev-up` / `make dev-doctor` / `make dev-down` / `make dev-reset` 的行为契约与 JSON 健康检查口径。`deploy/dev-stack/docker-compose.yaml` 与 Make target 真实实现由 A2 自身后续 `001` plan 承接；依赖本地栈的 W2 implementation 在 A2 `001` 未通过前不得启动。
