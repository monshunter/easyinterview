...
B2 `openapi-v1-contract` 在本阶段只锁定 spec 合同：v1.0.0 freeze 的 37 endpoint / 14 tag 清单（含 `DELETE /api/v1/me` 账号删除别名）、字段命名、additive-only 规则、privacy export P0 返回 501 例外与 fixtures 同源边界。`openapi/openapi.yaml`、codegen、fixtures 与 breaking-change linter 的真实落地由 B2 自身后续 `001` 系列 plan 或原地 artifact remediation 承接；在该 plan 验证前，不把「OpenAPI v1.0.0 freeze」作为可执行 gate 已完成处理。
