...
- `debrief` / `debrief_full` / `profile` 不再是目标 route、workstream 或 scenario 正向对象；旧 route 只能归一到当前核心入口。
...
- [product-scope/001-core-loop-module-pruning](../product-scope/plans/001-core-loop-module-pruning/plan.md)：承接 D-22 复盘和用户画像跨层删除、场景替代与零残留 gate。
