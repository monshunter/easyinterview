...
每项决策必须在 W0 内产出一份 ADR（位置由后续 ADR 索引 spec 决定，不在本 spec 范围内）。ADR 通过即视为决策锁定，本 spec §3.2 同步更新。
...
- F4 `privacy-and-audit-runtime` 在 W3+ 进入，与 C12 `backend-privacy` 协同。
...
| C4 | `backend-targetjob` | TargetJob + JD 导入 + 解析状态 + 来源记录 + async job 编排 | B2, B3, B4, A3 | 5 | P0 |
...
| E2 | `e2e-scenarios-p0` | 跨前后端 P0 主漏斗 8 步：导入→工作台→练习→报告→错题→复练 | C4, C5, C6, C7, D2, D3, D4 | 1 | P0 |
| E3 | `e2e-scenarios-p1` | 真实复盘漏斗 + 简历定制漏斗 + 多语言场景 | C9, C10, C11, D5, D6, E2 | 1 | P1 |
| E4 | `release-gate-and-rollout` | 灰度开关、版本兼容、回滚 runbook、SLO 准入 | F1, F2, F3, F4, E2 | 1 | P0+持续 |
...
|------|---------|------|-----------|------------------|
| **W0** | 1 周 | 共识与骨架 | A1, B1，本 spec freeze | 5 项 ADR 全部签字；`make dev-up` 一键通 |
| **W1** | 1.5 周 | 基础设施 + 契约骨架（spec only，不写 impl plan） | A2, A3, A4, A5, B2, B3, B4, F1, F3 | 9 spec 通过 `/plan-review`；B2 OpenAPI v1.0.0 freeze |
...
```
F (横切)：F1 / F2 / F3 / F4 与 A→B→C/D 同步推进，不阻塞主路径
...
E4 release-gate ◄── F1, F2, F3, F4, E2
```

**3 个关键观察**：

1. **B2（OpenAPI）是 DAG 瓶颈节点**：C 全域和 D 全域都直接依赖。一旦 codegen 投产，破坏性变更会触发跨 spec 雪球。需要 B2 自带 breaking change linter，并在 W1 末做一次 v1.0.0 freeze。
...
| C-2 | 契约骨架就绪 | A1 + B1 完成 | W1 末 | A2-A5 / B2-B4 / F1 / F3 共 9 份 spec 通过 `/plan-review`；B2 OpenAPI v1.0.0 freeze | 001 Phase 3 |
...
| C-6 | 上线 gate | E4 release-gate 跑完 staging 灰度 + 回滚 | W5 末 | `04-metrics-observability.md` §15 最低上线门槛全勾；P0 准入 | 001 Phase 6 |
| C-7 | 收尾归档 | P0 全部上线 | W5 后 | 本 spec 状态由 `active` 调整为 `completed`；P1/P2 child 占位文件创建；触发 `/retrospective` | 001 Phase 7 |

## 7 关联计划

- [001-decompose-subspecs](./plans/001-decompose-subspecs/plan.md)：把 38 份 child subspec 按 6 wave 完整 spawn 并通过各自 review 的执行计划。这是本 spec 唯一挂的 plan；其他治理类 plan（灰度、release-gate、隐私）归入对应的 child（E4 / F4）。
