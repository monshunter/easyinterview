...
| Home / Job Picks / Parse | `frontend-home-job-picks-and-parse`、`backend-targetjob`、`backend-jobs-recommendations` | `frontend-home-job-picks-and-parse` completed；`backend-targetjob` active（001 import / parse completed）；`backend-jobs-recommendations` active（001 real backend baseline implementation in progress: Phase 0-5.5 + BDD scaffold landed; Phase 5.6-5.8 cmd/api scenarios + 6.5-6.8 BDD-Gate live verification deferred to next implement session） | 首页 JD 导入、岗位推荐、解析确认、目标岗位 / JD / 轮次假设 | B2、B3、B4、A3、F3、D1 |
