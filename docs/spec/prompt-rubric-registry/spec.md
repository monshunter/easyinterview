...
| D-15 | 评估框架与 judge 决策（`004` 锁定） | Promptfoo 消费同一份 registry prompt；默认使用固定 recorded outputs；judge contract 使用隔离的 `judge.default` profile 语义；当前 6 个 business prompt 均由 single-source registry 解析。真实 provider 只允许显式人工诊断，不作为完成 gate。`004` 的激活 marker 只能在当前 owner gate 实际重跑通过后写入 verified comment；completed-plan 压缩不得删除仍被下游 preflight 消费的 marker。 | 保证 single-source、确定性、provider 隔离与跨 owner 激活证据不漂移 |
...
- `target.import.parse` 只渲染 `{{jd_text}}` 与现有输出语言 token `{{language}}`；prompt/meta、active seed、resolved snapshot 与 runtime renderer 不得声明或消费 source URL、page heading、file metadata 或其它 source descriptor，也不得新增 `{{target_language}}` alias。
...
- Action schema `maxLength=200` code points只作fuse；English24 whitespace words/zh-CN64 Unicode code points为semantic/UX gate。Targeted action-label repair使用内部生成目标18/52，不改变UI边界。下游 UI owner 以真实浏览器验证 desktop/mobile 换行和 typed-invalid/no-raw。
...
| 离线评估集 ≥ 24（当前 6-key 基线） | F3 `004`（Promptfoo）+ 各 C 域 | 归 `004`；registry single-source；固定 recorded outputs 可重复执行 |
| 编辑 UI | 当前不在范围 | – |
...
| C-10 | 评估升级 | F3 `004` 维护每个 active feature key 至少 4 题 recorded/offline 评估集 + 独立 judge contract | 运行 `make eval-offline` | 当前 6-key 基线共 ≥ 24 题、逐维度 judge 结果与 fail-closed negative 均可由固定输出重复验证；真实 provider 不作为通过条件 | F3 `004` |
...
| C-17 | report reliability + UX | v0.2 +5 contexts | offline/recorded eval + downstream UI audit | 200只作fuse；24/64质量；18/52只作targeted-repair内部余量；desktop/mobile typed-invalid/no-raw 由 UI owner 验证 | F3 004 + downstream report UI owner |
| C-18 | report + practice v0.2 gated activation | storage、F3 002 structural 与 F3 004 rubric/eval gates 均已通过 | dev 8-status activate/rollback/re-activate + DB `000019` independent PostgreSQL up/down/up | 两个 feature 的 ResolveActive 均唯一返回 prompt/rubric v0.2.0；exact getters 仍可读取两组 v0.1.0；file rollback/reload 与 DB down 同时恢复两组 v0.1.0，任一 prerequisite 不成立时 mutation 前失败 | F3 002 + F3 004 + B4 + backend-practice 004 |
