...
- `002-output-schema-contract`：6 个语言无关 schema + prompt/example/struct consistency gate。
...
- `004-real-model-profile-and-evals`（completed）：真实 LLM Judge 实现（`Judge` 演进为逐维度 `[]Score`，`FailClosedJudge` 保留为安全默认）+ `judge.default` 激活 + runnable profile coverage 门禁；当前基线随 active feature key 集合保持每 key 4 个 recorded cases。
