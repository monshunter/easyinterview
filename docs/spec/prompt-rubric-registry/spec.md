...
### 3.1 已锁定决策（含 13 个 P0 feature_key 字典）
...
| D-8 | 13 个 P0 feature_key 字典 | 见 §3.1.1；新增必须 spec 修订 | – |
...
| `mistake.extract` | 错题候选抽取 | C6 | `report.assessment.default`（共享） |
...
- 启动时全量预加载 ≤ 1s（13 × 多 language baseline）。
...
|----|------|-------|------|------|-----------|
| C-1 | 13 个 baseline 全集 | F3 后续 001 完成 | `ls config/prompts/ \| wc -l` 与 `ls config/rubrics/ \| wc -l` | 各 13 个目录；每个目录至少 1 份 v0.1 baseline | F3 后续 001 |
...
- `001-baseline`（W1 末 / W2 初）：`internal/ai/registry/` + `config/prompts/` + `config/rubrics/` 13 份 baseline + Resolve 实现 + lint 规则。
