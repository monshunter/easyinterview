...
| D-15 | 评估框架与 judge 决策（`004` 锁定） | Promptfoo 消费同一份 registry prompt；默认使用固定 recorded outputs；judge contract 使用隔离的 `judge.default` profile 语义；当前 6 个 business prompt 均由 single-source registry 解析。真实 provider 只允许显式人工诊断，不作为完成 gate。 | 保证 single-source、确定性与 provider 隔离 |
