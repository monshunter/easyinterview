...
- 9 × 2 个 prompt body 的输出段从 prose 描述统一为 schema 渲染/校验的「输出契约 + complete example JSON output」写法；schema 是唯一字段真理源，prompt body 不手工维护第二份字段清单；example 必须是完整代表性 JSON output（覆盖 schema 声明的 required + optional 字段，使用业务形态值），且明确不是 JSON Schema / OpenAPI schema；改动的 `.md` 重算 `template_hash`。
...
| 日期 | 版本 | 变更 |
|------|------|------|
| 2026-07-07 | 1.7 | Wording cleanup：收敛 migration audit、seed net-state 与 parser alias 说明为当前 non-current gate 口径，不改变 prompt/rubric 可执行契约。 |
...
- **替代验证 gate**: ① `make lint-prompts`（schema 子集 + prompt 契约块可重渲染 + complete example JSON output schema-valid + 三向一致性 drift gate + negative fixtures）；② `go test ./backend/internal/ai/registry/... -race`（loader 加载 schema + `ResolveActive` OutputSchema 非空 + 语言无关单份 + fallback 仍返回同一 schema + seed migration 覆盖全部 active prompt/rubric 坐标）；③ `go test ./backend/internal/ai/aiclient/... -race`（`validateOutputSchema` enum/required fail-close）；④ `go test ./backend/internal/{targetjob,resume/jobs,review,practice}/... -race`（caller `OutputSchema` 透传 + 端到端 fail-close）；⑤ schema↔struct drift negative fixture；⑥ grep red-line：语音 feature_key（STT/TTS）不落 schema、不接 `OutputSchema`、不发 `response_format`；⑦ non-current feature_key negative lint；⑧ `python3 .agent-skills/implement/shared/scripts/validate_context.py` + `python3 .agent-skills/sync-doc-index/scripts/sync-doc-index.py --check`；⑨ 顶层 `make lint`；⑩ `DATABASE_URL=... make migrate-check`（Postgres dev-stack 下执行迁移 `up -> down -> up` + migration lint）。
...
Schema canonical key policy：`required` 只覆盖后端实际依赖字段；当前 parser 已兼容的 non-current alias（例如 `question`/`questionText`、`rewrite`/`suggestedBullet`、`id`/`jobMatchId`）只能作为 Phase 0 兼容事实记录，不得自动进入 prompt 新契约。若保留可选 prompt-only 字段，必须说明评估价值，并用 schema `description` + lint 证明不是第二份手写字段表。
...
> 语音 feature_key（`practice.voice.stt` / `practice.voice.tts` / `practice.dictation.stt`，对应 `doubao_speech` / `minimax_speech`）不产 JSON content，**不在本 plan 范围**，不落 schema、不接 `OutputSchema`。Debrief / JD Match feature_key 为 non-current，不再出现在当前 prompt/rubric truth source。
...
在 README / handoff 规则中明确：parser 为兼容既有 prompt 保留的 alias 不等于新契约字段；prompt-only 可选字段必须有 `description` 说明评估价值，并由 lint 证明与 schema/rendered block 一致。
...
若 0.2 确认当前 prompt 仍要求 parser 不再需要或评估价值不足的 non-current 字段：统一新 schema/prompt canonical key（默认使用 `originalBullet` / `suggestedBullet` / `reason`，保留 parser alias 兼容但不写入新 prompt contract），并补 round-trip 断言。
...
#### 7.4 Non-current feature_key negative gate
确认 non-current feature_key 不存在于 `config/prompts` / `config/rubrics`，且 `prompt_lint` / `rubric_lint` 对 non-current key 或非当前模块字段保持 fail-fast。
...
#### 9.2 non-current feature_key seed net-zero gate
对后续删除迁移中的 non-current feature_key 行执行净效果校验：non-current seed 可以作为 migration audit rows 保留，但当前迁移链落库后的 active prompt/rubric coordinate 必须只剩 9 个 feature_key × 2 language，non-current key 不得作为 active truth source 回流。
...
#### 10.1 Non-current feature_key rejection
删除当前 config truth source 中的 non-current feature_key，并补 lint 回归测试：non-current key 目录不存在；若测试 fixture 人为写入 non-current key，lint 必须返回 clear diagnostic，不允许被 seed coverage 或 schema renderer 当作 active coordinate。
...
- seed migration 覆盖从当前 `config/prompts` / `config/rubrics` truth source 反推的 9 个 active chat feature_key × 2 language 坐标；non-current seed rows 必须被后续删除迁移净化，不得作为 active prompt/rubric coordinate 回流；`make migrate-check` 在 dev-stack Postgres 下通过。
...
| `resume.tailor.gap_review` | `alignment_score`, `gaps[].{topic,why,severity}`, `strengths_to_amplify[].{topic,evidence}`, `risks` | `decodeTailorAIResponse` 优先消费 canonical `matchSummary.{strengths,gaps}`；否则把 non-current `strengths_to_amplify` / `gaps` 归一为 `matchSummary`；`alignment_score` / `risks` 不消费 | 新 schema/prompt 使用 canonical `matchSummary.{strengths,gaps}` required；non-current key 只作 parser compatibility；若保留 severity，仅作为 optional non-current annotation，不作为 required |
| `resume.tailor.bullet_suggestions` | `suggestions[].{rewrite,why_better,kept_facts}` | `normalizeSuggestions` canonical 消费 `originalBullet` / `suggestedBullet` / `reason`，兼容 `original_bullet`、`rewrite`、`why_better`；`kept_facts` 不消费 | 判定为新契约 drift；新 schema/prompt 使用 `suggestions[].{originalBullet,suggestedBullet,reason}`，non-current alias 保留在 parser，`kept_facts` 不进入新契约 |

> D-16 / D-22 后，Debrief 与 JD Match feature_key 为 non-current；本表只列当前 `config/prompts` / `config/rubrics` truth source 中仍存在的 9 个 feature_key。
...
- 兼容保留：parser 继续接受 `original_bullet`、`suggested_bullet`、`rewrite`、`why_better`、`whyBetter`，用于既有输出/fixture 回放。
...
- 跳过：`practice.voice.stt` / `practice.voice.tts` / dictation STT 不产 JSON content，不落 schema，不填 `OutputSchema`；`voiceFollowUpPayload` 使用 chat `practice.session.follow_up`，应随 chat 路径填 schema。
...
| `practice.session.first_question` | object | required `questionText` / `questionIntent`，与 parser 优先消费字段一致；non-current `question` / `intent` 不进 schema |
...
| `resume.tailor.gap_review` | object | required `matchSummary.{strengths,gaps}`，对齐 canonical parser path；non-current `strengths_to_amplify` / `gaps` 不进新 schema |
...
- Prompt 修复：`config/prompts/resume.tailor.bullet_suggestions/v0.1.0.md` 与 `v0.1.0.en.md` 输出字段改为 canonical `originalBullet` / `suggestedBullet` / `reason`；non-current `rewrite` / `why_better` / `kept_facts` 不再作为 prompt 输出字段。
...
- Drift red-line：`rg -n "Return strict JSON with keys|Return a strict JSON array|Return strict JSON only|follow_up_question|why_better|kept_facts|strengths_to_amplify|alignment_score" config/prompts` 无命中，non-current prose / prompt key 已从 prompt truth source 清除。
...
- Red phase：为 targetjob / resume/jobs 本地 `PromptResolution` 与 direct caller metadata 增加断言后先运行 focused tests：targetjob/resume 编译失败于本地 resolution 缺 `OutputSchema` 字段；review/practice 失败于 `CallMetadata.OutputSchema` 为空。Non-current caller 不再作为当前 red/green surface。
...
- non-current feature key guard：Debrief / JD Match 不再作为当前 caller surface；lint 和 config 目录负向断言防止其回流。
...
- 验证命令：
  ...
  - non-current feature key negative tests → pass
...
#### 8.2.15 L2 remediation: active seed migration coverage and non-current key net-zero

- Finding: output schema truth source and runtime parser cover the current 9 active chat feature_keys, but the seed gate must be derived from current `config/` truth source rather than hardcoded counts. Non-current feature_keys may still appear in older up/down migrations, but the final migration chain must delete them from active prompt/rubric tables.
...
- Fix:
  ...
  - Non-current key migration rows are treated as migration audit rows only; current net state is validated from active config plus delete migrations.
- Red/green evidence:
