...
| 日期 | 版本 | 变更 |
|------|------|------|
| 2026-07-10 | 1.8 | 删除非当前文本输入 STT schema 豁免口径；语音 schema 红线只承接电话模式 STT/TTS feature_key。 |
...
> 语音 feature_key（`practice.voice.stt` / `practice.voice.tts`，对应 `doubao_speech` / `minimax_speech`）不产 JSON content，**不在本 plan 范围**，不落 schema、不接 `OutputSchema`。Debrief / JD Match feature_key 为 non-current，不再出现在当前 prompt/rubric truth source。
