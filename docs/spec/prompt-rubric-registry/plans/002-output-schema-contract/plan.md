...
把 [prompt-rubric-registry spec](../../spec.md) v2.6 §3.1 D-13 锁定的 `output_schema` 契约端到端落地，关闭 §6 C-12。完成后：
...
- 13 × 2 个 prompt body 的输出段从 prose 描述统一为 schema 渲染/校验的「输出契约 + complete example JSON output」写法；schema 是唯一字段真理源，prompt body 不手工维护第二份字段清单；example 必须是完整代表性 JSON output（覆盖 schema 声明的 required + optional 字段，使用业务形态值），且明确不是 JSON Schema / OpenAPI schema；改动的 `.md` 重算 `template_hash`。
...
| 2026-05-24 | 1.4 | L2 remediation：修复 seed migration 静态门禁只覆盖 11 个历史 feature_key 的 false-green；新增动态 seed coverage test，从 `config/prompts` / `config/rubrics` 反推 active 坐标，补充 `jd_match.recommendation` / `jd_match.search` prompt/rubric seed migration，并用 `make migrate-check` 验证迁移链。 |
...
- **替代验证 gate**: ① `make lint-prompts`（schema 子集 + prompt 契约块可重渲染 + complete example JSON output schema-valid + 三向一致性 drift gate + negative fixtures）；② `go test ./backend/internal/ai/registry/... -race`（loader 加载 schema + `ResolveActive` OutputSchema 非空 + 语言无关单份 + fallback 仍返回同一 schema + seed migration 覆盖全部 active prompt/rubric 坐标）；③ `go test ./backend/internal/ai/aiclient/... -race`（`validateOutputSchema` enum/required fail-close）；④ `go test ./backend/internal/{targetjob,resume/jobs,review,practice,debrief,jdmatch}/... -race`（caller `OutputSchema` 透传 + 端到端 fail-close）；⑤ schema↔struct drift negative fixture；⑥ grep red-line：语音 feature_key（STT/TTS）不落 schema、不接 `OutputSchema`、不发 `response_format`；⑦ `python3 .agent-skills/implement/shared/scripts/validate_context.py` + `python3 .agent-skills/sync-doc-index/scripts/sync-doc-index.py --check`；⑧ 顶层 `make lint`；⑨ `DATABASE_URL=... make migrate-check`（Postgres dev-stack 下执行迁移 `up -> down -> up` + migration lint）。
...
在 README 固化 prompt body 输出段只能是 schema 渲染/校验的契约块：字段顺序来自 schema `required` + `properties`，字段说明来自 `description`，complete example JSON output 由 schema 生成并可解析，且必须覆盖 schema 声明的 required + optional 字段、使用业务形态值、明确不是 JSON Schema / OpenAPI schema；人手只编辑 schema 与业务 prompt 上下文，不在 26 个 `.md` 中独立维护第二份字段表。
...
把每个 `.md` 的 “Return strict JSON …” prose 段替换为 schema 渲染/校验的「输出契约 + complete example JSON output」块，与 Phase 2 schema 对齐；只改输出契约段，不改角色/任务/业务规则措辞。每个改动的 `.md` 重算 `template_hash` 写回同名 `.yaml`。

#### 4.2 prompt body drift gate
`make lint-prompts` 证明 schema、prompt contract block、complete example JSON output、YAML hash 与 seed migration hash（若涉及）全部一致；故意改一个 prompt contract 字段名或 example key 必须失败。L2 remediation 要求 example 不再是最小 required-only JSON；renderer 必须覆盖 schema 声明的 optional 字段，并用业务形态值替代 `string` / `1` 占位。
...
为 `scripts/lint/prompt_lint.py` 增加 focused regression：example JSON 必须包含 schema-declared optional properties，且不能使用 `string` / `1` 这类最小占位；先观察 red，再最小实现。

#### 8.2 prompt truth source regeneration
重渲染 13 × 2 个 prompt body 的 contract block；刷新 26 个 YAML `template_hash`；同步 `migrations/000002_seed_baseline_prompt_rubric_versions.up.sql` 中已有 prompt seed row 的 body/hash；保持 seed migration 与 on-disk truth source 一致。

#### 8.3 documentation and lifecycle closure
同步 `config/prompts/README.md`、spec v2.6、plan/checklist v1.3、context discovery 与 INDEX；执行 `make lint-prompts`、prompt lint 单测、context validation、docs/index gate 与 `git diff --check` 后再将 plan/checklist 恢复 `completed`。

### Phase 9: L2 seed migration coverage remediation（active truth source 全量覆盖）

#### 9.1 seed coverage static gate
重写 `backend/internal/ai/registry/db_integration_test.go` 的 seed coverage 断言：不再写死 11 个历史 feature_key / 22 行 seed，而是从 `config/prompts/*/v*.yaml` 的 `status: active` 与 `config/rubrics/*/v*.yaml` 反推当前 truth-source 坐标，并扫描所有 `migrations/*seed_baseline_prompt_rubric*.up.sql`。缺行、重复行、额外行或 prompt `template_hash` drift 都必须失败。

#### 9.2 jd_match seed migration rows
新增 `000010_seed_baseline_prompt_rubric_versions_jd_match.{up,down}.sql`，补齐 `jd_match.recommendation` / `jd_match.search` × `en` / `multi` 的 prompt_versions 与 rubric_versions seed rows。prompt body、template_hash 与 rubric schema JSON 必须来自当前 `config/` truth source；migration 继续使用 `ON CONFLICT DO NOTHING`，down migration 只删除这两个 feature_key 的 `v0.1.0` rows。
...
#### 10.1 JD-Match recommendation `posted` required drift
把 `jd_match.recommendation` 的 `posted` 从 schema required 集合与 `FEATURE_CONTRACTS` required paths 中移除，保持为 optional 字段；同步 multi/en prompt contract block、YAML `template_hash` 与 JD-Match seed migration body/hash。新增回归测试证明 `posted` 不再是 required，避免生产 `compactJDMatchJobsPool` 未提供 freshness 时促使模型编造字段或被 output schema fail-close。
...
- 13 个 chat feature_key 各有 1 份语言无关 schema；prompt body 输出契约块可由 schema 重渲染，complete example JSON output 可解析、schema-valid、覆盖 schema 声明的 required + optional 字段、使用业务形态值且明确不是 JSON Schema / OpenAPI schema；`ResolveActive` 输出非空 `OutputSchema`；故意制造 schema↔prompt↔struct 不一致 → `make lint-prompts` 失败。
- seed migration 覆盖从当前 `config/prompts` / `config/rubrics` truth source 反推的 13 个 active chat feature_key × 2 language 坐标；`jd_match.recommendation` / `jd_match.search` 不再只存在于配置与 runtime parser 中，Postgres baseline seed 同步落库；`make migrate-check` 在 dev-stack Postgres 下通过。
...
- `docs/spec/prompt-rubric-registry/plans/INDEX.md` 写入 002 行；`history.md` v2.6 entry；`spec.md` Header v2.6 与 `docs/spec/INDEX.md` 一致。
...
- Red/green evidence:
  ...
  - Regression: `go test ./backend/internal/ai/registry/... ./backend/internal/ai/aiclient/... ./backend/internal/targetjob/... ./backend/internal/resume/jobs/... ./backend/internal/review/... ./backend/internal/practice/... ./backend/internal/debrief/... ./backend/internal/jdmatch/... ./backend/cmd/api/... -race` → pass.
  - Repo lint: `make lint` → pass.
  - Bug record: [BUG-0095](../../../../bugs/BUG-0095.md).

#### 8.2.14 L2 remediation: complete prompt example JSON output

- Finding: prompt contract blocks rendered schema-valid examples, but examples were too thin: required-only JSON with generic `string` / `1` placeholders. This gave models a weak target and did not explicitly distinguish desired output from JSON Schema / OpenAPI schema.
- Fix:
  ...
  - 13 chat feature_key × 2 language coordinates were re-rendered; 26 YAML `template_hash` values and existing prompt seed rows in `migrations/000002_seed_baseline_prompt_rubric_versions.up.sql` were refreshed.
- Red/green evidence:
  ...
  - Prompt gate: `make lint-prompts` → `prompt_lint: 26 files clean`.
  - Placeholder negative search: `rg -n '"string"|: 1,|Example JSON:' config/prompts -g '*.md'` → 0 matches.
  ...
  - Repo lint: `make lint` → pass, including `prompt_lint: 26 files clean`.
  - Whitespace gate: `git diff --check` → pass.
  - Bug record: [BUG-0096](../../../../bugs/BUG-0096.md).

#### 8.2.15 L2 remediation: jd_match seed migration coverage

- Finding: output schema truth source and runtime parser now cover 13 active chat feature_keys, but the baseline prompt/rubric seed gate still hardcoded 11 historical feature_keys / 22 rows. `jd_match.recommendation` and `jd_match.search` therefore existed in `config/prompts`, `config/rubrics`, lint contracts, and runtime callers, but were absent from Postgres `prompt_versions` / `rubric_versions` baseline seed rows.
- Root cause: `TestSeedMigrationCoversBaselineFeatureKeys` asserted fixed counts instead of deriving expected rows from the current `config/` truth source. `prompt_lint.py` only verified hash drift for seed rows it could see, so a completely missing feature_key remained false-green.
- Fix:
  - `backend/internal/ai/registry/db_integration_test.go` now derives expected prompt rows from active prompt YAML files and expected rubric rows from rubric YAML files, scans all `migrations/*seed_baseline_prompt_rubric*.up.sql`, and fails on missing/extra/duplicate coordinates or prompt `template_hash` drift.
  - `migrations/000010_seed_baseline_prompt_rubric_versions_jd_match.{up,down}.sql` adds reversible `jd_match.recommendation` / `jd_match.search` × `en` / `multi` prompt_versions and rubric_versions rows, sourced from current `config/` prompt bodies, YAML hashes, and rubric schemas.
- Red/green evidence:
  - Red: `go test ./backend/internal/ai/registry -run TestSeedMigrationCoversBaselineFeatureKeys -count=1 -v` failed with missing `jd_match.recommendation|v0.1.0|{en,multi}` and `jd_match.search|v0.1.0|{en,multi}` for both prompt_versions and rubric_versions.
  ...
  - Static lint: `python3 scripts/lint/prompt_lint.py` → `prompt_lint: 26 files clean`; `python3 scripts/lint/rubric_lint.py` → `rubric_lint: 26 files clean`; `python3 scripts/lint/migrations_lint.py` → `migration lint: ok`.
