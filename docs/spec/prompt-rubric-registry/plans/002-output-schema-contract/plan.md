...
`practice.turn.lightweight_observe` canonical parser 改动必须同时验证 `backend/cmd/api/practice_http_scenario_test.go` 的确定性成功 fixture 输出 `cue` / `answerSummary`，alias-only `hint` 只能留在 invalid-output 负测。执行 P0.039 和 P0.048-P0.051 HTTP 场景，要求 canonical 成功路径不得降级为 `session_wait` 或额外写入 `AI_OUTPUT_INVALID` task run。
...
| Phase 13 | downstream scenario fixture | P0.039 + P0.048-P0.051 `cmd/api` HTTP tests; canonical success + alias-only negative coverage |
...
- Finding: after Phase 12 removed the `hint` alias, the `cmd/api` deterministic Practice AI success fixture still emitted `hint`, so P0.039/P0.048/P0.049/P0.050 degraded to `session_wait` and wrote an extra `AI_OUTPUT_INVALID` task run.
- Fix: the success fixture emits canonical `cue` / `answerSummary`; alias-only `hint` remains in the invalid-output negative case.
- Verification: focused P0.039 and P0.048-P0.051 tests plus the full Practice/internal AI/cmd-api package gate pass. F3/002 now requires downstream consumer scenario execution for canonical parser changes.
