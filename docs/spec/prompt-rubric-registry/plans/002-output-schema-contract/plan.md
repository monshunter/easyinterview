...
- 输出形状以自然语言写在每个语言变体 `.md`（prose + 双份重抄），无机器可读 schema，无 drift gate；历史 BUG-0065 即此类 drift 的案例。002 修复不能把 prose drift 变成多份手写字段表 drift；schema 必须成为唯一字段真理源，prompt 只承载由 schema 渲染/校验的可读契约块。
...
| 日期 | 版本 | 变更 |
|------|------|------|
| 2026-07-06 | 1.6 | D-16 / D-22 后裁剪复查：当前 prompt/rubric truth source 为 9 个 chat feature_key；Debrief / JD Match 仅保留历史迁移删除与 retired-negative lint 语境，不再作为 plan/context/checklist 当前正向 target surface。 |
...
- **替代验证 gate**: ① `make lint-prompts`（schema 子集 + prompt 契约块可重渲染 + complete example JSON output schema-valid + 三向一致性 drift gate + negative fixtures）；② `go test ./backend/internal/ai/registry/... -race`（loader 加载 schema + `ResolveActive` OutputSchema 非空 + 语言无关单份 + fallback 仍返回同一 schema + seed migration 覆盖全部 active prompt/rubric 坐标）；③ `go test ./backend/internal/ai/aiclient/... -race`（`validateOutputSchema` enum/required fail-close）；④ `go test ./backend/internal/{targetjob,resume/jobs,review,practice}/... -race`（caller `OutputSchema` 透传 + 端到端 fail-close）；⑤ schema↔struct drift negative fixture；⑥ grep red-line：语音 feature_key（STT/TTS）不落 schema、不接 `OutputSchema`、不发 `response_format`；⑦ retired feature_key negative lint；⑧ `python3 .agent-skills/implement/shared/scripts/validate_context.py` + `python3 .agent-skills/sync-doc-index/scripts/sync-doc-index.py --check`；⑨ 顶层 `make lint`；⑩ `DATABASE_URL=... make migrate-check`（Postgres dev-stack 下执行迁移 `up -> down -> up` + migration lint）。
...
> 语音 feature_key（`practice.voice.stt` / `practice.voice.tts` / `practice.dictation.stt`，对应 `doubao_speech` / `minimax_speech`）不产 JSON content，**不在本 plan 范围**，不落 schema、不接 `OutputSchema`。Debrief / JD Match feature_key 已退休，不再出现在当前 prompt/rubric truth source。
...
#### 7.4 Retired feature_key negative gate
确认已退休 feature_key 不存在于 `config/prompts` / `config/rubrics`，且 `prompt_lint` / `rubric_lint` 对退休 key 或旧模块字段保持 fail-fast。
...
重写 `backend/internal/ai/registry/db_integration_test.go` 的 seed coverage 断言：不再写死历史 feature_key / seed row 数，而是从 `config/prompts/*/v*.yaml` 的 `status: active` 与 `config/rubrics/*/v*.yaml` 反推当前 truth-source 坐标，并扫描所有 `migrations/*seed_baseline_prompt_rubric*.up.sql` 及后续删除迁移的净效果。缺行、重复行、额外行或 prompt `template_hash` drift 都必须失败。

#### 9.2 retired feature_key seed net-zero gate
对后续删除迁移中的 retired feature_key 行执行净效果校验：历史 seed 可以作为 migration audit trail 保留，但当前迁移链落库后的 active prompt/rubric coordinate 必须只剩 9 个 feature_key × 2 language，退休 key 不得作为 active truth source 回流。
...
#### 10.1 Retired feature_key rejection
删除当前 config truth source 中的退休 feature_key，并补 lint 回归测试：退休 key 目录不存在；若测试 fixture 人为写入退休 key，lint 必须返回 clear diagnostic，不允许被 seed coverage 或 schema renderer 当作 active coordinate。
...
- seed migration 覆盖从当前 `config/prompts` / `config/rubrics` truth source 反推的 9 个 active chat feature_key × 2 language 坐标；历史 retired seed rows 必须被后续删除迁移净化，不得作为 active prompt/rubric coordinate 回流；`make migrate-check` 在 dev-stack Postgres 下通过。
...
> D-16 / D-22 后，Debrief 与 JD Match feature_key 已退休；本表只列当前 `config/prompts` / `config/rubrics` truth source 中仍存在的 9 个 feature_key。
...
- Red phase：为 targetjob / resume/jobs 本地 `PromptResolution` 与 direct caller metadata 增加断言后先运行 focused tests：targetjob/resume 编译失败于本地 resolution 缺 `OutputSchema` 字段；review/practice 失败于 `CallMetadata.OutputSchema` 为空。历史已退休 caller 不再作为当前 red/green surface。
...
- retired feature key guard：Debrief / JD Match 不再作为当前 caller surface；lint 和 config 目录负向断言防止其回流。
...
- 验证命令：
  ...
  - retired feature key negative tests → pass
...
#### 8.2.15 L2 remediation: active seed migration coverage and retired key net-zero

- Finding: output schema truth source and runtime parser cover the current 9 active chat feature_keys, but the seed gate must be derived from current `config/` truth source rather than hardcoded historical counts. Historical retired feature_keys may still appear in older up/down migrations, but the final migration chain must delete them from active prompt/rubric tables.
...
- Fix:
  ...
  - Retired key migration rows are treated as historical audit trail only; current net state is validated from active config plus delete migrations.
- Red/green evidence:
