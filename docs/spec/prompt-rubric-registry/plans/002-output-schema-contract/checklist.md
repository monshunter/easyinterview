...
- [x] 14.6 OWNER-GATE: consume `REPORT_STORAGE_V18_PASS`, F3/004 `REPORT_RUBRIC_V020_PASS` and `REPORT_CONTEXT_AWARE_EVAL_PASS`; missing marker, non-unique active state, version parity drift or immutable-content drift fails before any status/DB mutation.
...
- [x] 14.8 Verify final `ResolveActive` returns v0.2 prompt/rubric for report and practice, exact getters still retrieve both v0.1 coordinates, report/practice data-source coordinates are exact, latest-migration seed/hash parity remains strict, schema/prompt/example/struct gates pass, and runtime rubric injection is absent. Emit `REPORT_PROMPT_V020_PASS` and `PRACTICE_SEMANTIC_FOCUS_PROMPT_V020_PASS`; backend semantic grounding still requires its own validator/repair/P0 markers.
  <!-- verified: 2026-07-12 markers="REPORT_PROMPT_V020_PASS PRACTICE_SEMANTIC_FOCUS_PROMPT_V020_PASS" commands="python3 -m pytest scripts/lint/prompt_lint_test.py scripts/lint/rubric_lint_test.py -q; make lint-prompts lint-rubrics lint-prompts-hardcode; cd backend && go test -race ./internal/ai/registry/... -count=1; cd backend && go test ./internal/ai/aiclient/outputschema ./internal/ai/aiclient/observability ./internal/ai/registry ./internal/migrations -count=1; make eval-offline" result="30 lint tests pass; 8 prompt and 8 rubric files clean; registry race and seed/latest-migration parity pass; final report/practice resolve v0.2/v0.2 with report-context.v1/registry.v1; exact v0.1 getters pass; no active runtime rubric placeholder; offline eval and Promptfoo both 28/28 pass." -->
- [x] 14.9 MAX4 RED-GREEN: schema200/semantic24-64/targeted18-52 unchanged。Evalkit generation and judge use independent max4 budgets；generation full-validates and dynamically selects action_labels/whole_report per round；judge retries only provider/protocol invalid and terminally rejects valid negative。Schema/example/seed/export/manifest contract records aggregate usage/latency + attempt_count/retry_count/reason/scope。Re-emit marker only after tests；P0.099 remains separate UX gate.
  <!-- verified: 2026-07-13 method=contract+offline+live evidence="independent max4 generation/judge budgets and dynamic full-validator repair scopes PASS; prompt/rubric/resolved offline 28/28 PASS; historical run e2e-p0-100-20260713T034811Z-35103 passed its then-current 5-case/11-attempt prompt; final-prompt run e2e-p0-100-20260713T101214Z-59381 produced mechanical 9/9, semantic judge 8/9 and fixed categories 4/5 while the strict scenario remained FAIL" -->
- [x] 14.10 EXAMPLE-GROUNDING RED-GREEN: retain the complete report JSON example, place a synthetic candidate input immediately before it, make every example fact/action traceable to that input, and require an anti-copy/current-context regeneration instruction. Synchronize template hash, migration body/hash, resolved prompt and active dev DB; do not add an example-omission mode.
  <!-- verified: 2026-07-13 marker=REPORT_PROMPT_V020_PASS evidence="paired prioritization/tie-breaking input + complete output + anti-copy present; focused lint 2/2 and prompt lint 24/24 PASS; 8 prompt files clean; hash e99faa33b00842c9320068faa2207f9dedb7af5e4742d7e51c2c81c4542e2fed; migration v19 up/down/up and active DB body/hash parity PASS" -->
