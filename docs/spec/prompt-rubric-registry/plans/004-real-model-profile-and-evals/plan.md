...
新增 `make eval-offline` target（`.PHONY` + help 描述；默认 fixture 模式；**不**纳入 `make test`）；落地 count≥50 断言与 registry-single-source drift check（Promptfoo 实际使用的 prompt 必须等于 registry resolved，漂移 exit 1）。Promptfoo tests、SQLite state、logs 等运行时产物必须统一写入 `.test-output/evals/`，不得写入或 gitignore `config/evals/.generated/`。
