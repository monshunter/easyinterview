...
| Phase 4 | eval 集 + Promptfoo | pinned Promptfoo dependency + lockfile；`config/evals` count≥50；`make eval-offline` fixture；drift gate；`EVAL_LIVE` 无网络断言；`lint-prompts-hardcode` green | PASS：4.1 `config/evals/<fk>/cases.yaml` 13 chat feature_key×4 档=52 用例（golden output 抽自 prompt .md 的 example output，judge fixture 逐 rubric dimension），含 1 个 follow_up en→multi fallback；real-suite 离线 grade 全绿（TestRealSuiteOfflineGreen，count≥50）。4.2 pinned `promptfoo@0.121.12` 根 devDependency + lockfile + `pnpm.onlyBuiltDependencies:[better-sqlite3]`；`pnpm exec promptfoo --version`=0.121.12。4.3 `internal/eval`+`cmd/evalkit` 经 RegistryClient.ResolveActive single-source 解析 + 单一 registry.LLMJudge grading；Promptfoo custom provider/assert 仅 exec 桥到 evalkit（不复制 prompt、不在 JS 重实现 judge）。4.4 录制 fixture 默认 + EVAL_LIVE=1 经 bootstrap opt-in；no-network 断言（TestRunOfflineMakesNoNetworkCall，env 指向黑洞仍绿）。4.5 `make eval-offline`（.PHONY+help，不入 make test）=drift-check + count≥50 + Promptfoo 52 passed；drift 负向 exit 1、正向 exit 0；`resolved-prompts.json` committed 为 drift baseline。4.6 `make lint-prompts-hardcode` exit 0。 |
