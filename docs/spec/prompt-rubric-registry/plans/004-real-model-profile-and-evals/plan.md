...
新增 `make eval-offline` target（`.PHONY` + help 描述；默认 fixture 模式；**不**纳入 `make test`）；落地 count≥50 断言与 registry-single-source drift check（Promptfoo 实际使用的 prompt 必须等于 registry resolved，漂移 exit 1）。Promptfoo tests、渲染后的 Promptfoo config、SQLite state、logs 等运行时产物必须统一写入 `$(EVAL_OUTPUT_DIR)`（默认 `.test-output/evals/`，允许 override 且所有路径必须同源移动），不得写入或 gitignore `config/evals/.generated/`。
...
- `config/evals/` ≥ 50 题；`make eval-offline` 默认录制 fixture 跑通；count≥50 断言通过；`EVAL_LIVE` 未设不打网络；`EVAL_LIVE=1` opt-in live 可运行；Promptfoo tests/state/logs 仅落 `.test-output/evals/`。
...
| Phase 1 | Judge []Score 演进 | `TestJudgeSignature` 新签名红→绿；`go build ./backend/...`；registry tests | PASS：1.1 red（接口返单 Score 编译失败）→1.2 `types.go` Judge 返回 `([]Score, Reasoning, error)`、Score 语义改逐 dimension→绿；1.3 `NotImplementedJudge` 返回 `(nil, Reasoning{}, ErrJudgeNotImplemented)`、`judge_test.go` 同步、无其他 caller（仅 doc.go 注释）；1.4 `go build ./backend/...` OK + `go test ./internal/ai/registry -count=1` ok。 |
| Phase 2 | judge dispatch + 真实 LLMJudge | A3 judge dispatch/adapter red→green；`judge_llm_test.go` 逐维度 + fail-close 红→绿；`go test ./backend/internal/ai/{aiclient,registry}` focused（adapter 测试名不匹配 `Test.*Judge|Test.*judge`，须独立包 `go test ./backend/internal/ai/aiclient/providers/judge_compatible -count=1` 选中） | PASS：2.1 red（CompleteJudge undefined）→2.2 `Client.CompleteJudge`（CapabilityJudge dispatch，不进 business AIClient 接口）+ `providers/judge_compatible` adapter（OpenAI-compatible 子集 wire，protocol/capability/meta 保持 judge，secret fail-fast，非 Complete 能力 fail-close）+ bootstrap materialize；judge dispatch/adapter/bootstrap tests 绿。提取 `aiclient/outputschema` 单一校验器（observability 与 LLMJudge 共用，行为保留）。2.3 red（NewLLMJudge undefined）→2.4 `LLMJudge`（注入 RubricProvider+JudgeModelClient 窄接口，judge.default profile，逐维度 []Score，business prompt 经 registry single-source，judge 指令注入不 hardcode）→绿；2.5 fail-close 7 子测（profile 不可用/output schema invalid/不可解析/维度数量/未知维度/越界/空 summary 均 ErrJudgeOutputInvalid，不静默补零）；2.6 `var _ Judge = (*LLMJudge)(nil)` + NotImplementedJudge 安全默认 + 构造注入。`make lint-prompts-hardcode` green。 |
| Phase 3 | judge.default 激活 + coverage | `config/ai-providers.yaml` 非 placeholder judge provider；`catalog_test.go` active；`ai_profile_coverage.py` 非 placeholder 断言 + 负向；`make lint-ai-profile-coverage` | PASS：3.1 red（catalog 期望 active 而 config unsupported；4 个 lint 测试期望新断言而缺失）；3.2 `judge-placeholder`→`judge-deepseek`（judge_compatible，复用 AI_PROVIDER_* env）、judge.default `unsupported→active` + `deepseek-v4-pro`，移除 unsupported_reason，13 chat profile status 未动（git diff 范围确认）；3.3 lint 加 placeholder 黑名单（judge.default + 全 chat 的 provider_ref/model 非 placeholder）+ judge.default active + 选中 provider protocol=judge_compatible 断言，已在 `make lint`；3.4 `make lint-ai-profile-coverage` OK + 11 lint 单测（含 4 负向 placeholder/unsupported/missing）+ `go test ./profile` green。 |
| Phase 4 | eval 集 + Promptfoo | pinned Promptfoo dependency + lockfile；`config/evals` count≥50；`make eval-offline` fixture；drift gate；`EVAL_LIVE` 无网络断言；`lint-prompts-hardcode` green；Promptfoo 运行产物位于 `$(EVAL_OUTPUT_DIR)`（默认 `.test-output/evals/`） | PASS：4.1 `config/evals/<fk>/cases.yaml` 13 chat feature_key×4 档=52 用例（golden output 抽自 prompt .md 的 example output，judge fixture 逐 rubric dimension），含 1 个 follow_up en→multi fallback；real-suite 离线 grade 全绿（TestRealSuiteOfflineGreen，count≥50）。4.2 pinned `promptfoo@0.121.12` 根 devDependency + lockfile + `pnpm.onlyBuiltDependencies:[better-sqlite3]`；`pnpm exec promptfoo --version`=0.121.12。4.3 `internal/eval`+`cmd/evalkit` 经 RegistryClient.ResolveActive single-source 解析 + 单一 registry.LLMJudge grading；Promptfoo custom provider/assert 仅 exec 桥到 evalkit（不复制 prompt、不在 JS 重实现 judge）。4.4 录制 fixture 默认 + EVAL_LIVE=1 经 bootstrap opt-in；no-network 断言（TestRunOfflineMakesNoNetworkCall，env 指向黑洞仍绿）。4.5 `make eval-offline`（.PHONY+help，不入 make test）=drift-check + count≥50 + Promptfoo 52 passed；tests/config/log/DB 运行产物统一位于 `$(EVAL_OUTPUT_DIR)`，覆盖 `EVAL_OUTPUT_DIR` 时 Promptfoo 读取同目录生成的 tests/config，`config/evals/.generated/` 不允许作为运行输出目录；drift 负向 exit 1、正向 exit 0；`resolved-prompts.json` committed 为 drift baseline。4.6 `make lint-prompts-hardcode` exit 0。 |
| Phase 5 | 验证与生命周期 | lint/test/build/eval/docs gates；active-scope zero-reference grep；profile status 范围断言；INDEX/work-journal/retrospective | PASS：5.1 `make lint`（golangci 0 issues + 全本地 gate OK）、`make test`（backend ./... 全绿 + frontend 1298 passed）、`make build`、`make eval-offline`（52 passed）、`make lint-ai-profile-coverage` 全绿。5.2 zero-reference grep = 0 matches。5.3 `git diff` 证明仅 judge.default `status` unsupported→active + judge provider/model 替换，13 chat 业务 profile status 未动。5.4 `validate_context` OK、`make docs-check` zero drift + links OK、`git diff --check` OK；004 plan/checklist 置 completed，同步 plans INDEX 与 docs/spec INDEX。附记：evalkit live 路径经 platform/config+platform/secrets bootstrap，`EVAL_LIVE`→`--live` flag，保持 `lint-getenv-boundary` green（零 os.Getenv）。 |
