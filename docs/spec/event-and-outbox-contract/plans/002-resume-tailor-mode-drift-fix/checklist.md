...
- [x] 1.3 运行 `make codegen-events` 重新生成 Go/TS 类型（验证：`backend/internal/shared/events/` 与 `frontend/src/lib/events/` 出现新字面量、旧字面量已清除）
- [x] 1.4 运行 `make codegen-check` PASS（验证：`git diff --exit-code` 0）
- [x] 1.5 新增或更新 `backend/internal/shared/events/resume_tailor_mode_test.go`，断言 `ResumeTailorModeGapReview` / `ResumeTailorModeBulletSuggestions` 存在且旧 `Inline` / `Rewrite` / `Mirror` 不在允许集合（验证：Red → Green）
- [x] 1.6 更新 `frontend/src/lib/events/events.test.ts`，断言 `ResumeTailorMode` typed payload 接受 `gap_review` / `bullet_suggestions` 并拒绝旧 `inline` / `rewrite` / `mirror`（验证：Red → Green）
