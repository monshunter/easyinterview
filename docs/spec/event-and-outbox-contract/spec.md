...
| 8 | `report.generated` | worker | report question-review / analytics | `feedback_report` | – |
| 9 | `report.generation.failed` | worker | analytics / alerting | `feedback_report` | – |
| 10 | `resume.parse.completed` | worker | retrieval upsert / analytics | `resume_asset` | `embedding_upsert`（可选） |
| 11 | `resume.tailor.completed` | worker | analytics | `resume_tailor_run` | – |
| 12 | `debrief.created` | api | debrief worker / analytics | `debrief` | `debrief_generate` |
| 13 | `debrief.completed` | worker | debrief practice planner / analytics | `debrief` | – |
| 14 | `source.refreshed` | worker | source cache / analytics | `source_record` | – |
| 15 | `privacy.request.created` | api | privacy worker / audit | `privacy_request` | `privacy_delete` / `privacy_export`（P1） |
| 16 | `privacy.request.completed` | worker | audit / notification（future） | `privacy_request` | – |
...
本 spec v1.5 按 product-scope v1.2 删除独立 `mistake` domain 事件，原错题本价值只保留在报告题目回顾 / 本轮复练字段中。后续如需新增事件 / 升级 eventVersion / 新增 canonical job_type / 调整 B2 API-facing subset：递增 spec 版本 + history；映射表 §3.1.1 / §3.1.2 与 payload schema §3.1.4 全文同步更新。
