...
- **DB/C8 canonical job_type 字典**（与 [03 §5.9 async_jobs.job_type](../../../easyinterview-tech-docs/03-db-definition.md) 一致并由本 spec 扩展）：`target_import` / `resume_parse` / `report_generate` / `resume_tailor` / `debrief_generate` / `source_refresh` / `embedding_upsert` / `privacy_export` / `privacy_delete` / `email_dispatch` 共 10 项。
...
B2 OpenAPI v1.0.0 的 `JobType` enum 只允许以下 7 项：`target_import` / `resume_parse` / `report_generate` / `resume_tailor` / `debrief_generate` / `privacy_export` / `privacy_delete`。`source_refresh` / `embedding_upsert` / `email_dispatch` 只能存在于 DB/C8 内部，不得出现在 `GET /api/v1/jobs/{jobId}` response、OpenAPI fixture 或前端 SDK 类型中；若未来需要对外暴露，必须先 additive 修订 B2 spec，再同步本 spec 与 B4 check constraint。
...
| 3 | `target.analysis.failed` | worker | analytics / alerting | `target_job` | – |
...
| 9 | `report.generation.failed` | worker | analytics / alerting | `feedback_report` | – |
| 10 | `mistake.created` | worker / review | growth / analytics | `mistake_entry` | – |
| 11 | `mistake.status.changed` | review | growth / analytics | `mistake_entry` | – |
...
| 14 | `debrief.created` | api | debrief worker / analytics | `debrief` | `debrief_generate` |
...
| `mistake.status.changed` | `mistakeId:uuidv7`, `fromStatus:MistakeStatus`, `toStatus:MistakeStatus`, `targetJobId:uuidv7` | – | B1 `MistakeStatus` | IDs/status only |
...
- `eventName` 一律 `dot.case`，允许 2 段或 3 段；最后一段必须是过去式动词；前缀 8 个固定 domain：`target / practice / report / mistake / resume / debrief / source / privacy`。多词状态必须拆为独立 dot segment，例如 `report.generation.failed`，禁止 `generation_failed` 这类 snake segment。
...
- generator 输入：`shared/events.yaml`（envelope schema + 18 事件清单 + §3.1.4 payload schema）+ `shared/jobs.yaml`（10 个 canonical job_type ↔ dotted name 映射 + API-facing subset 标记 + `email_dispatch` payload redaction policy）；B3 owns `backend/cmd/codegen/events`，只 import B1 已生成类型，不复用 B1 generator 进程。
...
| C-2 | jobType 常量生成 | `shared/jobs.yaml` 落地 | `make codegen-events` | Go `jobs.JobTypeTargetImport` 等 10 个 canonical 常量 + dotted task name 常量生成；TS 同步；`source_refresh` / `embedding_upsert` / `email_dispatch` 标记为 internal-only，不进入 B2 API-facing `JobType` | B3 后续 001 |
...
| C-6 | breaking change 拦截 | 故意把 `report.generated` 的 `mistakeCount` 改为 string 或删除 required 字段，或把事件名写成 `report.generation_failed` | 本地 `make lint-events` | `lint-events` 失败；提示 schema breaking 需 `eventVersion + 1`，事件名必须为真正 dot.case；新增 optional 字段通过 | B3 后续 001 |
