...
先扩展 `scripts/lint/scenario_env_contract_test.py` 建立 RED：`env-cleanup.sh` 必须复用 `local-dev-runtime.sh`，并在默认 `make dev-down` 或显式 `DEV_RESET_FORCE=1 make dev-reset` 前调用统一的 host runtime stop helper。GREEN 在共享 helper 中组合现有 PID/命令/repo cwd ownership 检查，停止 backend/frontend 进程组并清理 pidfile；陈旧 pidfile、PID 复用或手工/无关进程继续只清理 pidfile、不发送信号。

Live gate 通过当前 repo-managed backend/frontend pidfile 启动最小可观测进程，执行标准 cleanup 后验证两个进程组、pidfile、Compose 依赖均消失且命名卷保留；另用无关进程与陈旧 pidfile 证明安全边界。BDD 不适用：这是开发环境内部生命周期修复，替代 gate 为 focused Python contract、shell syntax、dry-run 顺序、隔离进程测试与真实标准 cleanup。

## 5 验收标准

- spec [§6 验收标准](../../spec.md#6-验收标准) C-1 到 C-23 全部成立，证据贴入工作日志或当前 `.test-output/`；raw NDJSON 本身不进入 evidence。
...
| 日期 | 版本 | 变更 | 关联 |
|------|------|------|------|
| 2026-07-20 | 1.29 | Scenario cleanup 在停止 Compose 依赖或 reset 数据卷前安全停止 repo-managed host-run backend/frontend，并保留无关进程与默认命名卷。 | cleanup lifecycle investigation |
