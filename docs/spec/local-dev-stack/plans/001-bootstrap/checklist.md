...
- [x] 4.2 落地 `deploy/dev-stack/README.md`：服务表（name/image/port/credentials/volume）、项目组件表（component/service/host port/health/metrics）、`make dev-*` 命令清单、AI provider 配置说明（本地部署使用真实 provider，stub 仅单测）、常见故障排查、与 `test/scenarios/` Kind 路径双轨说明、资源占用提示与默认依赖镜像下载体积估算（< 1.5GB），并声明默认本地栈不包含 OTel / Grafana / Loki / Prometheus / AI provider
