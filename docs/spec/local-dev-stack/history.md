...
| 2026-04-27 | 1.2 | 按 L1 plan-review 与用户确认修订本地开发栈边界：默认依赖收敛为 Postgres+pgvector / Redis / MinIO；`make dev-up` 改为启动最小依赖 + 当前项目可运行组件；本地观测改为应用 `/metrics` + 容器日志；默认排除 OTel Collector / Grafana / Loki / Prometheus 与 AI provider。 | [001-bootstrap](./plans/001-bootstrap/plan.md) |
...
| 2026-04-27 | 1.0 | 初始创建：锁定 `deploy/dev-stack/docker-compose.yaml` 落点、7 个本地依赖服务清单（Postgres+pgvector / Redis / MinIO / OTel / Grafana / Loki / Prometheus）、`make dev-up` / `dev-down` / `dev-doctor` / `dev-reset` 契约、健康检查口径与命名卷策略；承接 [engineering-roadmap §5.7](../engineering-roadmap/spec.md#6-实施顺序) 的 W1 dev-up spec-contract lock；引用 [01-technical-architecture.md §2.3](../../../easyinterview-tech-docs/01-technical-architecture.md#23-数据与基础设施) 的本地依赖列表。 | engineering-roadmap/001 Phase 3 |
