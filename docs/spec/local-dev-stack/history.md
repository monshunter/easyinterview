...
| 2026-04-27 | 1.0 | 初始创建：锁定 `deploy/dev-stack/docker-compose.yaml` 落点、7 个本地依赖服务清单（Postgres+pgvector / Redis / MinIO / OTel / Grafana / Loki / Prometheus）、`make dev-up` / `dev-down` / `dev-doctor` / `dev-reset` 契约、健康检查口径与命名卷策略；承接 [engineering-roadmap §5.7](../engineering-roadmap/spec.md#6-实施顺序) 的 W1 dev-up spec-contract lock；引用 `engineering-roadmap decisions §2.3` 的本地依赖列表。 | engineering-roadmap/001 Phase 3 |
