...
| getReportConversation | `Reports/getReportConversation.json`: ready/queued/generating/failed/missing/invalid-order/unsafe-markdown | report-conversation; ReportsScreen 传 current report 或每个不同 latest locator | reports handler/store | feedback_reports.session_id -> practice_messages ordered read | none | focused client/component/route tests + BDD.REPORT.CONVERSATION.001 + P0.099 click/load/back |
| regenerateFeedbackReport | `Reports/regenerateFeedbackReport.json` | ReportsScreen failed latest-attempt recovery | review regeneration handler/service/store | same report row + fresh job + idempotency/audit | async report.generate after 202 | focused client/component tests + BDD.REPORT.REGENERATE.UI.001 |
...
| Phase 14 | shared conventions + OpenAPI `regenerateFeedbackReport` codegen/fixture + backend-review Phase 13 PASS | failed-row UI 只消费 generated client，并继承同报告、幂等和 typed failure 合同 |
| Phase 15 | Phase 13 report-owned transcript + Phase 14 failed-row actions | conversation action follows each distinct report locator independently of report status；progress/regenerate remain sibling actions |
...
- **BDD 策略**: `BDD.REPORT.UI.001` 由代码层 owner tests 验证 generating/ready/failed、ready `4/2/2/2/1`、上下文链接、同行等高、replay/next/back 与 fail-closed 行为；`BDD.REPORT.REGENERATE.UI.001` 验证 failed-only 同报告重生成、会话恢复、双击抑制、稳定幂等 key 与 stale-result fencing。二者由仓库根 `make test` 统一回归；P0.099 仅作为真实 full-stack report/generating UI/API 与 exact-six screenshots 的独立 handoff，只有显式真实运行后才产生 PASS。
...
| spec D-16 | report conversation primary/failure | 13 | generated-client + component/route/Markdown tests; P0.099 real click/load/back handoff | formal `ReportConversationScreen` + ReportsScreen/Report entries | session list, live controls, third Header CTA, stale/partial transcript |
| spec C-16 | failed-report recovery | 14 | generated-client + ReportsScreen state/idempotency/a11y tests + BDD.REPORT.REGENERATE.UI.001 | failed latest-attempt action region | regenerate on non-failed/oversize, unstable IK after unknown outcome, double submit, wrong report/job navigation, raw error text |
| spec C-17 | completed-session conversation availability | 15 | ReportsScreen action matrix + localized a11y tests + scoped real Chrome acceptance | every distinct current/latest report locator | progress/regenerate replacing record action、same-ID duplication、empty-round fabrication |
...
RED-GREEN 覆盖两个 TargetJob 数据集、请求 target 精确绑定、跨用户/target mismatch、missing/extra/noncanonical rounds、target switch 首帧清旧链接与 stale response fence。每个 report locator 只归属一个 canonical round；同轮 current/latest 同 ID 只允许 latest ready，latest ready 不得缺 current，current 不得缺 latest。每轮只显示 current ready 和 latest attempt：queued/generating 链接 Generating；Phase 10 的 failed“无 Retry”是历史合同，Phase 14 已补充普通失败的同报告重新生成与查看记录，超限失败只保留查看记录；latest ready 与 current 同 ID 不重复；latest ready 与 current 不同时，仍只把 current 作为“查看报告”入口，但为不同 latest locator 保留独立“查看面试记录”，不扩展为历史报告版本列表。
...
将 generated `getReportConversation` 与 `/report-conversation?reportId=...` 接入正式 frontend，路由只允许 `reportId`。报告 Context Strip 的第四个同级子项保留主入口；Phase 13 当时只在 ReportsScreen current report 行保留快捷入口，当前 Phase 14 已扩展到 failed latest attempt；不改 Header 两 CTA 和 Phase 12 的 `4/2/2/2/1` 目标布局。
...
- **RED**: ReportsScreen tests require failed-only regenerate + conversation actions, old-ready/new-failed locator separation, oversize conversation-only behavior, pending/aria state, double-click suppression, stable IK after unknown outcome and fresh IK after explicit terminal rejection. ReportConversation route tests require failed Back to the trusted target-scoped ReportsScreen rather than terminal Generating.
- **GREEN**: use generated `regenerateFeedbackReport` only. Validate returned report/job identity and queued type before navigating to the same reportId Generating route；localized row errors never show raw API/provider text。`REPORT_INVALID_STATE_TRANSITION` / `REPORT_NOT_READY` from a stale tab refetch current TargetJob + overview and replace the stale failed row；unknown transport outcome keeps the same key and does not speculate state.
- **STALE/A11Y**: target switch invalidates old promises；current and failed conversation actions have distinguishable accessible names, while internal report IDs remain absent from visible/a11y text.
- **BDD-Gate**: `BDD.REPORT.REGENERATE.UI.001` plus generated-client and ReportsScreen tests pass；root `make test` is the aggregate regression. Current-run additionally uses the Chrome extension automation skill against the real local frontend/backend to verify the failed-row regenerate/conversation recovery path. This manual Chrome evidence does not mark P0.099 PASS；P0.099 may consume the real flow only when it can establish a failed precondition without mocks.

### Phase 15: Completed-session conversation action is status-independent

- **RED**: ReportsScreen tests require every distinct projected report pointer to expose its own conversation action. queued/generating must keep progress and conversation side by side；latest-ready-different-current must keep current/latest locators distinguishable；same-ID ready deduplicates and both-null exposes neither.
...
- Report 主入口与 ReportsScreen 快捷入口都打开同一 reportId-only 只读记录；ready 返回 Report、queued/generating 返回 Generating、failed 以可信 frozen target 直接返回 ReportsScreen，失败/隐私边界不泄露 partial transcript 或内部 ID。
- ReportsScreen 当前投影中的每个不同 report pointer 都独立提供面试记录；queued/generating 的进度动作、failed 的恢复动作不得替代记录，same-ID current/latest 只显示一次，empty round 不虚构入口。
- ReportsScreen 对普通 failed latest attempt 同时提供同报告“重新生成”和“查看面试记录”；超限失败只提供记录。重生成保持单飞，未知结果复用幂等 key，显式终端拒绝后才换 key；仅在 `202` 的 report/job identity 与 queued 类型全部匹配时进入同 reportId Generating。
