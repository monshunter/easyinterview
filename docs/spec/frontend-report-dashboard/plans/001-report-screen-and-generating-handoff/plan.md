...
| spec C-15 | command/read separation | 11 | ReportsScreen Back route tests + P0.059 + current-scope route negative | Reports Back control | Parse navigation/animation/import/poll; extra Workspace query params |
...
Report/Generating 恢复合同不变：当前/最后可信 API response 有 `targetJobId` 时 Back 到 `/reports?targetJobId=...`；missing reportId、404、首读网络失败或 invalid payload 无可信 identity 时回 `/workspace` 列表，route 仍为 reportId-only。更新 `E2E.P0.059` 覆盖 Reports Back 直达 Workspace 详情与浏览器 history，不经过 Parse；focused route/source gate 对当前实现、当前 spec/plan/checklist/test/BDD（排除显式 revision/history 记录）执行旧 Reports-to-Parse 正向引用零命中，并证明只读链没有 Parse animation/import/poll。
...
| 读取既有规划误入解析状态机 | Reports Back 固定为 targetJobId-only Workspace detail；P0.059 与 source negative 同时拒绝 Parse navigation、动画、import 和 polling |
