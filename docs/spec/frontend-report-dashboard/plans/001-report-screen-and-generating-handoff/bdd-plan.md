...
| `BDD.REPORT.GENERATING.VISUAL.003` | reportId 合法且状态为 queued/generating、可恢复读取失败或 terminal | 用户打开 Generating、等待轮询或返回父页 | 全局 TopBar/面试高亮保留；共享 report transition 只展示真实状态和 indeterminate 反馈，ready handoff/typed failure/trusted Back 不变，desktop/mobile 无横向溢出 | `GeneratingScreen.test.tsx` + shared route/responsive tests + current-run Chrome manual acceptance；根 `make test` 承接代码层回归 |
