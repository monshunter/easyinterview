...
- [x] 当前真实环境已通过显式 `/scenario-run` 完成 exact-six Chrome full-page capture、ready/generating no-OCR visual audit、Report → Conversation → Back 与 live API/PostgreSQL binding。
  <!-- verified: 2026-07-15 run_id="e2e-p0-099-20260715T021319Z-57232" result="PASS" evidence="six canonical desktop/mobile screenshots; manual-visual-audit.json; no request interception or mock backend" -->

## `BDD.REPORT.CONVERSATION.001` Report-owned transcript

- [x] Owner behavior tests 覆盖报告四状态、strict order/role、Markdown security、父报告 Back、reportId switch、missing/cross-user/error 与 no-ID/no-live-control negatives。
- [x] Prototype/formal source structure 与 visual geometry parity 分层通过；正式页面没有先于 `ui-design/` 自由设计。
- [x] 根 `make test` 执行对应 frontend/generated-client tests；该结果仍不是 E2E PASS。
- [x] P0.099 静态合同增加 real click/load/back + API/DB binding，且 exact-six screenshot manifest/directory/manual audit 行数保持不变。
