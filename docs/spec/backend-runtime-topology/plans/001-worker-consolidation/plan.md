...
`backend-auth` 实施中已经证明 email code 邮件派发可以由 backend-internal goroutine 完成本地闭环。继续保留独立后台进程入口、进程级 listen env 和独立 producer 标签会让尚未上线的产品承担不必要的进程、配置和观测复杂度。本计划把该架构选择落成一个独立 owner，避免散落在 backend-auth 或 roadmap 文档中。
...
修订 engineering-roadmap、ADR-Q2/Q4/Q5/Q1、secrets-and-config、event-and-outbox-contract、observability-stack、local-dev-stack、backend-auth、ai-provider handoff 等 active 文档，把独立 worker 前置改为 backend internal runner / background executor 口径；当前执行正文不得继续要求 worker 进程。

### Phase 2: Config and process cleanup

#### 2.1 删除 worker runtime config

从 `.env.example`、`deploy/dev-stack/.env.example`、`config/config.yaml`、`config/README.md`、A4 spec/plan/checklist、config bindings、validator 和 tests 中删除独立后台进程 listen 配置。

#### 2.2 删除独立 worker entrypoint

删除独立后台进程 entrypoint，更新 Makefile、lint allowlist、env dict scanner、A3 README/config comments、migrations comments、dev-stack docs，使 `go build ./cmd/...` 不再构建独立后台 binary。
