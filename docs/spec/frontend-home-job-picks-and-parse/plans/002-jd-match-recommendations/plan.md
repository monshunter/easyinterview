...
| Failure / recovery · searchJobs failed / slow-response | failed → inline error + 保留输入；slow-response → 5 步 AGENT panel（i18n + opacity 渐变 + accent 标签）持续显示直到 settle，不超时；不引入真步骤切换动画 | n/a | 4 | Vitest + E2E.P0.023 |
