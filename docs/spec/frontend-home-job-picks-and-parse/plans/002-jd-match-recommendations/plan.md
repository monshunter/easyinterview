...
在 plan `001-home-jd-import-and-parse` 已交付的 jd_match P1 placeholder shell 基础上，把 `ui-design/src/screen-jd-match.jsx` 完整三 tab（Recommended / Search / Watchlist）+ Profile snapshot chip 数据驱动 + AGENT scan status badge + Save / Mark not relevant / Confirm interview / Open source 闭环 + 自然语言 Search + Saved searches + Watchlist + Market signals 一次性源级复刻到正式 frontend；通过 OpenAPI `JobMatch` tag + 12 operationId + fixture-backed transport 完成 P0 UI/BDD 闭环；并在 `backend-jobs-recommendations/001-jd-match-real-backend-baseline` 已完成真实 handler 后，用 `VITE_EI_API_MODE=real` generated-client gate + 后端 E2E.P0.094-P0.097 证明前后端真实联调闭环。
...
3. Search tab：自然语言搜索框 + Run web search → `searchJobs`；in-flight 期间渲染 5 步 AGENT panel（i18n + opacity 渐变 + accent label）直到 results / failure；Saved searches 网格（list + create）；Results 2 列网格 + 4 个 chip filter 纯 client-side 切换
...
5. UI behavior 子用例继续通过 generated `JobMatch` client + fixture-backed mock transport 保持确定性；真实 API 模式通过 `src/api/jdMatch.realApiMode.test.ts` 在 `VITE_EI_API_MODE=real` 下覆盖 12 个 JobMatch operation 的 URL、cookie auth、5 个 `Idempotency-Key` side effect 与 `GenerationProvenance` roundtrip；所有隐私字段（query / saved-search label / watchlist label / sourceJobUrl / freeNote / linkedJobMatchId）不进 console / URL / localStorage / telemetry；i18n zh/en 完整切换；warm/light + dark + customAccent 三态可见变化；desktop + mobile pixel parity 通过
...
spec v1.2 D-1 曾把 jd_match 业务范围锁为「契约先行 + frontend fixture 消费」模式，与 plan 001 D-2 generated client + fixture-backed transport 模式一致：本 plan 在真实 backend handler 落地之前先扩展 OpenAPI `JobMatch` tag + 12 operationId + fixture，frontend 通过 generated client 一次性源级复刻 ui-design 三 tab 完整业务。2026-05-22 `backend-jobs-recommendations/001-jd-match-real-backend-baseline` 已完成 12 个真实 endpoint、Postgres persistence、AI runtime/provenance、agent scan、privacy delete 与 E2E.P0.094-P0.097 live scenarios；本次 L2 remediation 在原 plan 上补齐前端 `VITE_EI_API_MODE=real` 联调 gate，不再把 fixture PASS 误写为唯一完成依据。

ui-design 真理源由用户独立维护：用户决定 plan 002 实施期间不修订 `ui-design/src/screen-jd-match.jsx` SearchTab `searching=true` 区域，保留当前 5 步 AGENT panel + `● AGENT SCANNING` accent label + 1px accent 外边框 + 3px accent 左边框 + 5 行 step 文案 + `opacity: i <= 2 ? 1 : 0.4` 渐变作为真理源；plan 002 frontend 通过 i18n 5 个 step key 源级复刻，并把 ui-design 中的动态 JD 数字（`248 → 87 unique postings` / `248 → 87 条唯一岗位`）在前端 i18n 文案中替换为不含具体数字的静态语义文案；plan 002 不修改 ui-design 静态文件，未来 ui-design owner 任何 SearchTab `searching` 形态修订由后续 plan 接力。
...
- **BDD 策略**: Feature plan requires BDD；本 plan 在 `bdd-plan.md` 定义 5 个场景 `E2E.P0.027 / E2E.P0.028 / E2E.P0.029 / E2E.P0.030 / E2E.P0.031`，`bdd-checklist.md` 跟踪每个场景资产创建与执行；主 `checklist.md` 在每个 Phase 末尾保留 `BDD-Gate:` 项引用对应场景 ID；`p0-017-jd-match-placeholder` scenario 同步升级 verify 脚本（不再 grep placeholder，断言三 tab 数据驱动渲染）。
...
|------|----------|------------------|-------|---------|
| Primary path | 进入 jd_match → Hero/Profile/AGENT 渲染 → Recommended 列表 → JDDetail 详情 → Confirm interview → `nav("parse", { source: "jd_match", sourceJobMatchId })` | `screen-jd-match.jsx::JDMatchScreen` line 4 + `JobMatchCard` line 335 + `JDDetail` line 377 Action bar | 2+3 | E2E.P0.027 + Vitest `jd_match/RecommendedTab.test.tsx` + `jd_match/JDDetail.test.tsx` |
| Alternate · Save/Unsave | 点击 Save → `addToWatchlist` + Idempotency-Key + window.eiToast；再点 → `removeFromWatchlist` + toast | `JDMatchScreen::toggleSaved` line 181 | 3 | Vitest + E2E.P0.027 sub-case |
| Alternate · Mark not relevant | `markJobNotRelevant` + Idempotency-Key + 隐藏卡片 + 自动选下一张 + toast | `JDMatchScreen::markNotRelevant` line 196 | 3 | Vitest + E2E.P0.027 sub-case |
| Alternate · Open source | `window.open(url, "_blank", "noopener,noreferrer")` | `JDMatchScreen::openSource` line 213 | 3 | Vitest spy + E2E.P0.027 sub-case |
| Alternate · Search query + filter | 输入 query → Run → `searchJobs` + Idempotency-Key；4 chip filter 纯 client-side 切换 | `JDMatchScreen::runSearch` line 173 + `filterPredicate` line 226 | 4 | Vitest + E2E.P0.028 |
| Alternate · Saved search create | `saveCurrentAsWatch` → `createSavedSearch` + Idempotency-Key + toast | `JDMatchScreen::saveCurrentAsWatch` line 219 | 4 | Vitest + E2E.P0.028 |
| Alternate · Watchlist chevron handoff | `openJob(id)` → tab 切 Recommended + selected = `WatchlistItem.linkedJobMatchId` | `JDMatchScreen::openJob` line 235 + `WatchlistTab::findJobIdFor` line 594 | 5 | Vitest + E2E.P0.029 |
| Alternate · Auth pending action | 未登录 side-effect 操作触发 `requestAuth({ type: "jd_match_action", route: "jd_match", params })`；Recommended params 为 `{ tab: "recommended", selectedJobMatchId, action }`，Search params 为 `{ tab: "search", action, pendingJdMatchActionId }` 且 query/label 仅存当前 SPA 会话内存；登录后回到 jd_match 自动恢复 tab/selected 并重新触发 action | D1 `requestAuth` + plan 001 C-7 模式 | 3+4+5 | Vitest `jd_match/JDMatchAuthGate.test.tsx` + `SearchTabAuthGate.test.tsx` + E2E.P0.030 |
...
| Failure / recovery · addToWatchlist 4xx revert | save 调用失败 → button 状态 revert + error toast；卡片不进入 watchlist | n/a | 3 | Vitest + E2E.P0.027 sub-case |
| Failure / recovery · markJobNotRelevant 4xx revert | dismiss 失败 → 卡片重新显示 + error toast | n/a | 3 | Vitest + E2E.P0.027 sub-case |
| Failure / recovery · searchJobs failed / slow-response | failed → inline error + 保留输入；slow-response → 5 步 AGENT panel（i18n + opacity 渐变 + accent 标签）持续显示直到 settle，不超时；不引入真步骤切换动画 | n/a | 4 | Vitest + E2E.P0.028 |
| Failure / recovery · createSavedSearch 4xx | 失败 → toast + 不写入 saved searches | n/a | 4 | Vitest + E2E.P0.028 |
| Failure / recovery · getMarketSignals partial-data | 部分 signal 缺失 → 渲染部分卡 + 缺失值 fallback | n/a | 5 | Vitest + E2E.P0.029 |
...
| Boundary · empty watchlist | `listWatchlist` 返回 [] → empty state + 引导「去 Recommended 添加」 | n/a | 5 | Vitest + E2E.P0.029 |
...
| Boundary · unknown linkedJobMatchId | chevron handoff 时 `linkedJobMatchId` 不在当前 Recommended 列表 → tab 切但 selected fallback 第一张 + warning toast | `WatchlistTab::findJobIdFor` line 594 | 5 | Vitest + E2E.P0.029 |
| Preflight · UI truth source | `ui-design/src/screen-jd-match.jsx::SearchTab` 当前 `searching=true` 区域为 5 步 AGENT panel + opacity 渐变 + accent 标签；plan 002 不修改 ui-design 静态文件，前端按当前形态源级复刻并通过 i18n 5 step key 提供文案 | `ui-design/src/screen-jd-match.jsx` + `docs/ui-design/` | 0 | source grep 确认 5 步 panel 仍是真理源 + i18n key freeze |
...
| Privacy / security · query zero-leak | 自然语言搜索 query 不进 console / URL / localStorage / telemetry；mockTransport 仅记 call status | n/a | 4 | redact lint + Vitest spy + E2E.P0.028 |
...
| Privacy / security · auth gate | 未登录 API side-effect 操作触发 `requestAuth` + opaque pendingAction；Source button 与 Watchlist chevron 不触发 pendingAction；query / label 等不进 pendingAction params | D1 `requestAuth` + plan 001 模式 | 3+4 | Vitest + E2E.P0.030 |
...
| UX · loading state | Recommended/Search/Watchlist/MarketSignals/Profile/AgentStatus 6 处独立 loading；Search loading 是 5 步 AGENT panel（i18n + opacity 渐变 + accent label）持续到 settle | n/a | 2-5 | Vitest |
...
| UI source structure parity · SearchTab | 自然语言搜索框 + Run web search 按钮 + 数据源 tag 列 + 5 步 AGENT panel（`● AGENT SCANNING` accent label + 5 行 step 文案通过 i18n 5 step key 渲染 + `opacity: i <= 2 ? 1 : 0.4` 渐变 + 1px accent 外边框 + 3px accent 左边框）+ Saved searches 3 列网格（list + create，无 delete）+ Results 2 列网格 cap 6 + 4 chip filter (all/strong/remote/unseen) | `screen-jd-match.jsx::SearchTab` line 480 + line 515-531 | 4 | Vitest + testid `jdmatch-search-{input,run,sources,searching-panel,searching-step-${i},saved-grid,results,filter-${k}}` |
...
| UI visual geometry parity · focused screenshot smoke | clean checkout 不依赖 ignored screenshot baseline；用 DOM bounding box + non-empty element screenshot buffer 证明 focused JDMatch 区域可截图且非空 | n/a | 6 | Playwright focused screenshot smoke |
...
| UI dynamic-data negative · 动态 JD 数字 | ui-design `screen-jd-match.jsx::SearchTab` 第 3 步文案中的具体数字（`248` / `87` / `unique postings` / `唯一岗位` / `248 → 87`）在 `frontend/src/` 与前端 i18n locales 0 命中（仅展示不含数字的静态语义文案，如 `Dedup by JD hash · removing duplicate postings` / `按 JD 哈希去重 · 移除重复岗位`）；plan 002 不修改 ui-design 静态文件，仅约束前端 i18n + DOM | n/a | 4+6 | grep negative + Vitest |
...
本 plan 最初走 `docs/development.md` §2.2 Frontend-First Path：正式前端先对齐 `ui-design/` 并通过 generated `JobMatch` client + fixture-backed transport 完成 P0 UI/BDD。自 2026-05-22 起，真实 handler / store / agent scan pipeline / AI-backed search / candidate pool / market signals 已由 `backend-jobs-recommendations/001-jd-match-real-backend-baseline` 落地；frontend 仍保留 fixture-backed UI variants，同时必须用 `VITE_EI_API_MODE=real` generated-client gate 证明 12 个 operation 指向真实 backend base URL，并与 backend E2E.P0.094-P0.097 live scenarios 配对。

| operationId | path / method | fixture | frontend consumer | backend handler | persistence | AI dependency | scenario coverage |
|-------------|---------------|---------|-------------------|-----------------|-------------|---------------|-------------------|
| `getJobMatchProfile` | `GET /jd-match/profile` | `openapi/fixtures/JobMatch/getJobMatchProfile.json` scenarios: `default` / `unauthenticated` / `partial-profile` | `JDMatchScreen` `useJobMatchProfile()`；进入 jd_match 调一次；为空时 chip 显示 fallback | `backend/internal/jdmatch/handler/get_profile.go` + `cmd/api` `GET /api/v1/jd-match/profile` | backend aggregates 7 cross-owner profile sources; frontend none | none in frontend；backend 聚合 resume / target job / mock / debrief profile signals | E2E.P0.027, E2E.P0.030 + backend E2E.P0.094 |
| `getAgentScanStatus` | `GET /jd-match/agent-status` | `openapi/fixtures/JobMatch/getAgentScanStatus.json` scenarios: `idle` / `scanning` / `error` / `next-scan-soon` | `JDMatchScreen` `useAgentScanStatus()`；进入 jd_match + 切回 Recommended tab 各调一次；不引入 SSE/WebSocket/真实定时器 | `backend/internal/jdmatch/handler/get_agent_status.go` + `cmd/api` `GET /api/v1/jd-match/agent-status` | backend: `agent_scans` / scan runtime state | none in frontend | E2E.P0.027, E2E.P0.030 + backend E2E.P0.094/P0.097 |
| `listJobRecommendations` | `GET /jd-match/recommendations` | `openapi/fixtures/JobMatch/listJobRecommendations.json` scenarios: `empty` / `one` / `many` / `failed` | `RecommendedTab` `useJobMatchRecommendations()`；query 支持 cursor pagination；返回 `JobMatchRecommendation` 数组（含 score / reasons / risks / highlights / fit / posted / source / saved / seen / similarInterviewers / networkNote 等） | `backend/internal/jdmatch/handler/list_recommendations.go` + `cmd/api` route | backend: `jd_match_recommendations` cursor pagination + joins | backend-only score / reasons / risks 生成（GenerationProvenance 必带）；frontend 仅展示 | E2E.P0.027 + backend E2E.P0.094 |
| `getJobRecommendation` | `GET /jd-match/recommendations/{jobMatchId}` | `openapi/fixtures/JobMatch/getJobRecommendation.json` scenarios: `default` / `network-intel-empty` / `failed` | `JDDetail` `useJobRecommendation(jobMatchId)`；用户切卡时必须调；`listJobRecommendations` 只作为列表摘要来源，不作为详情 operation 的替代证据 | `backend/internal/jdmatch/handler/get_recommendation.go` + `cmd/api` route | backend: `jd_match_recommendations` detail lookup + owner isolation | 同上 | E2E.P0.027 + backend E2E.P0.094 |
| `addToWatchlist` | `POST /jd-match/watchlist` | `openapi/fixtures/JobMatch/addToWatchlist.json` scenarios: `default` / `4xx-validation` / `5xx-server-error` | `useToggleWatchlist()`；body `{ jobMatchId }`；带 `Idempotency-Key`；4xx → revert + error toast | `backend/internal/jdmatch/handler/add_watchlist.go` + `cmd/api` POST with session + IK middleware | backend: `watchlist_items` + `idempotency_keys` | none | E2E.P0.027 + backend E2E.P0.095 |
| `removeFromWatchlist` | `DELETE /jd-match/watchlist/{jobMatchId}` | `openapi/fixtures/JobMatch/removeFromWatchlist.json` scenarios: `default` / `4xx-not-found` | `useToggleWatchlist()`；带 `Idempotency-Key`；4xx → revert + error toast | `backend/internal/jdmatch/handler/remove_watchlist.go` + `cmd/api` DELETE with session + IK middleware | backend: `watchlist_items` delete + `idempotency_keys` | none | E2E.P0.027 + backend E2E.P0.095 |
| `markJobNotRelevant` | `POST /jd-match/recommendations/{jobMatchId}/dismiss` | `openapi/fixtures/JobMatch/markJobNotRelevant.json` scenarios: `default` / `4xx` | `useDismissRecommendation()`；body `{ reason: enum, freeNote?: string }`；reason enum 默认值由 frontend 透传，ui-design 不弹窗收集；带 `Idempotency-Key`；4xx → revert + error toast | `backend/internal/jdmatch/handler/mark_not_relevant.go` + `cmd/api` POST with session + IK middleware | backend updates `jd_match_recommendations.dismissed_at` / reason | backend-only learning loop；frontend 仅发送 | E2E.P0.027 + backend E2E.P0.094 |
| `searchJobs` | `POST /jd-match/search` | `openapi/fixtures/JobMatch/searchJobs.json` scenarios: `default` / `empty` / `failed` / `slow-response` | `SearchTab` `useSearchJobs()`；body `{ query, filters?, profileSnapshot? }`；带 `Idempotency-Key`；in-flight 期间渲染 5 步 AGENT panel；slow-response variant 验证 panel 持续渲染至 settle；query 不进 URL/localStorage/telemetry | `backend/internal/jdmatch/handler/search.go` + `cmd/api` POST with session + IK middleware | backend: `jd_match_search_runs` + recommendation joins / candidate pool payload | backend-only `jd_match.search` A3/F3 sync timeout + retryable=false invalid AI output path；frontend 仅展示；response 不再含 `progress` 字段（D-12） | E2E.P0.028 + backend E2E.P0.096 |
| `listSavedSearches` | `GET /jd-match/saved-searches` | `openapi/fixtures/JobMatch/listSavedSearches.json` scenarios: `default` / `empty` / `4xx` | `SearchTab` `useSavedSearches()`；切 Search tab 调一次 | `backend/internal/jdmatch/handler/list_saved.go` + `cmd/api` route | backend: `saved_searches` | none | E2E.P0.028 + backend E2E.P0.095 |
| `createSavedSearch` | `POST /jd-match/saved-searches` | `openapi/fixtures/JobMatch/createSavedSearch.json` scenarios: `default` / `4xx-validation` | `SearchTab` `useCreateSavedSearch()`；body `{ label, query, filters? }`；带 `Idempotency-Key`；label 不进 URL/localStorage/telemetry；ui-design 无 delete 入口故 plan 002 不实现 `deleteSavedSearch` | `backend/internal/jdmatch/handler/create_saved.go` + `cmd/api` POST with session + IK middleware | backend: `saved_searches` insert + `idempotency_keys` | none | E2E.P0.028 + backend E2E.P0.095 |
| `listWatchlist` | `GET /jd-match/watchlist` | `openapi/fixtures/JobMatch/listWatchlist.json` scenarios: `default` / `empty` / `few` / `4xx` | `WatchlistTab` `useWatchlist()`；切 Watchlist tab 调一次；返回 `WatchlistItem[]`，每项必含 `linkedJobMatchId`（chevron handoff 用） | `backend/internal/jdmatch/handler/list_watchlist.go` + `cmd/api` route | backend: `watchlist_items` join `jd_match_recommendations` | none | E2E.P0.029 + backend E2E.P0.095 |
| `getMarketSignals` | `GET /jd-match/market-signals?window=7d` | `openapi/fixtures/JobMatch/getMarketSignals.json` scenarios: `default` / `partial-data` / `failed` | `WatchlistTab` `useMarketSignals()`；切 Watchlist tab 调一次；返回 4 卡 `{ k, v, d, tone }`；partial-data variant → 缺失 fallback | `backend/internal/jdmatch/handler/get_market_signals.go` + `cmd/api` route | backend aggregation from watchlist, recommendations, and internal candidate pool | backend-only market-signal computation | E2E.P0.029 + backend E2E.P0.096 |
...
读取最新 `ui-design/src/screen-jd-match.jsx` 与 `docs/ui-design/` 对应文档，确认 `SearchTab` `searching=true` 区域当前形态为 5 步 AGENT panel：`● AGENT SCANNING` accent label + 1px accent 外边框 + 3px accent 左边框 + 5 行 step 文案（`Embedding your profile…` / `Querying LinkedIn · Boss · Maimai · Lagou in parallel…` / `Dedup by JD hash · 248 → 87 unique postings` / `Scoring each against profile · strengths · gaps · prefs` / `Drafting match reasoning…` 与对应中文）+ `opacity: i <= 2 ? 1 : 0.4` 渐变。plan 002 不修改 ui-design 静态文件；frontend 按当前 5 步 panel 形态源级复刻，并通过 i18n 5 个 step key（典型命名 `jdMatch.search.searchingStep1..5`，最终命名以 `messages.ts` typed helper 落地为准）提供 zh/en 文案；ui-design 第 3 步的动态 JD 数字（`248`/`87`/`unique postings`/`唯一岗位`）在前端 i18n 文案中替换为不含具体数字的静态语义文案（如 `→ Dedup by JD hash · removing duplicate postings` / `→ 按 JD 哈希去重 · 移除重复岗位`），并在 §3.5 Coverage Matrix `UI dynamic-data negative · 动态 JD 数字` 行中用负向 grep 锁定；保留 ui-design opacity 渐变作为唯一动画效果，不引入 setInterval / 真步骤切换 / 新 keyframes。
...
- Preflight Gate: Phase 0 evidence log 记录 UI truth-source 当前形态（5 步 AGENT panel 保留 + i18n 5 step key 列表 + 动态 JD 数字静态化策略）、B2 owner inventory 待同步清单、12 operation matrix freeze 与 auth action enum freeze、隐私字段负向清单。
...
- BDD-Gate: 验证 `E2E.P0.027 / E2E.P0.028 / E2E.P0.029 / E2E.P0.030 / E2E.P0.031` 资产构建到 ready 态前置 — fixture 存在性、generated client 可调用
...
- BDD-Gate: 验证 `E2E.P0.027` Recommended tab 主路径中 Hero/Profile/AGENT 阶段 + `E2E.P0.030` Profile/AGENT/auth 综合场景资产构建到 ready 态
...
- BDD-Gate: 验证 `E2E.P0.027` Recommended tab 主路径 + 4 button 闭环 + auth pending action + `E2E.P0.031` Confirm interview → parse 出口 params 完整性

### Phase 4: Search tab — 自然语言搜索 + Saved searches + Result filters + 5 步 AGENT panel

#### 4.1 新增 `SearchTab.tsx` + 子组件

按 `screen-jd-match.jsx::SearchTab` line 480 + line 515-531 源级复刻：`NATURAL LANGUAGE SEARCH` / `自然语言搜索` label + search icon + 搜索框 + Run web search 按钮 + `SOURCES` / `数据源` label + 5 个数据源 tag（LinkedIn / Boss 直聘 / 脉脉 / 拉勾 / Company sites 或 公司官网，保留原型展示 count）+ 5 步 AGENT panel（`jdmatch-search-searching-panel` testid + `● AGENT SCANNING` / `● AGENT 扫描中` accent label + 1px accent 外边框 + 3px accent 左边框 + 5 行 step 通过 i18n typed helper 渲染 `jdMatch.search.searchingStep1..5` 文案 + `jdmatch-search-searching-step-${i}` testid + `opacity: i <= 2 ? 1 : 0.4` inline style 渐变）+ Saved searches 3 列网格（list + create，无 delete）+ Results 2 列网格 cap 6 + 4 chip filter (all/strong/remote/unseen)；i18n 第 3 步文案不得包含 `248` / `87` / `unique postings` / `唯一岗位` 任意硬编码数字；testid 命名按 §3.5。

#### 4.2 `useSearchJobs()` hook

Run → `searchJobs({ query, filters?, profileSnapshot? }, { idempotencyKey })`；in-flight 期间 `searching=true` 渲染 5 步 AGENT panel（5 个 step 文案通过 i18n typed helper `jdMatch.search.searchingStep1..5` 渲染 + opacity 渐变前 3 步 active 1.0 / 后 2 步 dim 0.4 + accent label 与 accent 边框）；保持 ui-design 当前 opacity 渐变作为唯一动画效果，不引入 setInterval / 真步骤索引切换 / 新 keyframes；slow-response variant 验证 5 步 panel 持续渲染至 settle，不超时；query 不进 URL/localStorage/telemetry；Vitest 断言 request body / Idempotency-Key / 5 步 panel 渲染 + opacity 渐变 computed style + step i18n 文案 / 不调用 setInterval 与 setTimeout step-advance。
...
- slow-response variant → 5 步 AGENT panel（i18n + opacity 渐变 + accent 标签）持续渲染直到 settle
...
- BDD-Gate: 验证 `E2E.P0.028` Search tab 主路径 + Saved searches + filter + slow-response + failure + privacy

#### 4.11 L2 remediation — SearchTab source-level parity

补强 Search tab 源级 parity gate：Vitest / pixel parity / P0.028 verify 必须断言自然语言搜索 label、search icon、`SOURCES` / `数据源` label、5 个 source chips（含 Company sites / 公司官网）以及 i18n key 完整性；现有 4-source / `Data sources` 旧口径不得作为完成证据。
...
- BDD-Gate: 验证 `E2E.P0.029` Watchlist + Market signals + chevron handoff + empty + partial-data + privacy
...
- 聚焦截图 smoke：JDMatchScreen Hero + Profile chip + AGENT badge、Recommended 双列布局、JDDetail Action bar、SearchTab 输入区 + Saved searches、WatchlistTab 列表 + Market signals 必须有稳定 bounding box 与非空截图 buffer；不依赖被 `.gitignore` 排除的本地 `*-snapshots/` baseline
- 若后续要恢复 `toHaveScreenshot` baseline，必须先让 baseline 在 clean checkout / CI 可复现；不得把本地 ignored PNG 当作常规 PASS 证据
...
- `test/scenarios/e2e/p0-027-jd-match-recommended-and-confirm/`
- `test/scenarios/e2e/p0-028-jd-match-search-and-saved/`
- `test/scenarios/e2e/p0-029-jd-match-watchlist-and-signals/`
- `test/scenarios/e2e/p0-030-jd-match-profile-and-agent-status/`
- `test/scenarios/e2e/p0-031-jd-match-confirm-interview-handoff/`
...
`test/scenarios/e2e/INDEX.md` P0 表追加 5 行（P0.027/023/024/025/026），关联需求列指向 `frontend-home-job-picks-and-parse C-12～C-16`，状态 Ready，执行方式 automated；同步 P0.017 行（描述更新为「升级到三 tab 数据驱动 smoke」）。
...
- 动态 JD 数字 / 动态计数：`248`、`87`、`unique postings`、`唯一岗位`、`248 → 87`（出现在 ui-design 第 3 步原文里的硬编码数字与短语）在 `frontend/src/` 与 `frontend/src/app/i18n/locales/` 0 命中（前端只渲染不含数字的静态语义文案；ui-design 静态原型与本仓库 `ui-design/` 目录的 jsx 不在 grep 范围内）
...
- BDD-Gate: 验证 `E2E.P0.027 / E2E.P0.028 / E2E.P0.029 / E2E.P0.030 / E2E.P0.031` 全部 setup→trigger→verify→cleanup PASS + `E2E.P0.017` 升级后 PASS + D1+D2+D3+plan001 P0.001/002/004/005/006/014/015/016 regression 全 PASS

#### 6.13 L2 remediation — clean-checkout pixel gate

修复 jd_match pixel parity 在干净环境缺少 ignored screenshot baseline 时失败的问题：`frontend/tests/pixel-parity/jd_match.spec.ts` 不再用本地 ignored `*-snapshots/` 作为常规 PASS 条件；改为 DOM anchor + computed style + responsive geometry + focused screenshot smoke 组合，且常规 `playwright test tests/pixel-parity/jd_match.spec.ts` 必须在先 build、无 snapshot 文件的情况下通过。

#### 6.14 L2 remediation — real backend integration closure

原地关闭 plan 002 与 backend real implementation 的语义漂移：新增 `frontend/src/api/jdMatch.realApiMode.test.ts`，在 `VITE_EI_API_MODE=real` 下证明 `createAppClient` 使用 production generated `EasyInterviewClient` 和真实 backend base URL 调用 12 个 JobMatch operation；断言每个 request `credentials: "include"`、默认不带 fixture `Prefer` header、5 个 side-effect operation 带 `Idempotency-Key`、recommendation/search response 保留 `GenerationProvenance`。同步 P0.027-P0.031 trigger/verify/README/data，让每个 frontend scenario 都先跑 real-mode generated-client gate，再跑 fixture-backed UI variants；与 backend E2E.P0.094-P0.097 live Postgres route/persistence/auth/IK/privacy/AI provenance scenarios 配对。更新 OpenAPI/generated spec 描述与 owner spec/BDD/checklist，删除 `backend-jobs-recommendations` 仍为未来态的 stale 口径。

验证证据（2026-05-22）：`VITE_EI_API_MODE=real VITE_EI_API_BASE_URL=http://localhost:8080/api/v1 pnpm --filter @easyinterview/frontend exec vitest run src/api/jdMatch.realApiMode.test.ts` PASS；E2E.P0.027-P0.031 全部 setup→trigger→verify→cleanup PASS，且每个 trigger log 均包含 real-mode gate；backend E2E.P0.094-P0.097 全部 setup→trigger→verify→cleanup PASS；`make lint-openapi` PASS；二次 `make codegen-openapi` 后 generated diff 不再变化；`make codegen-check` PASS。

## 5 验收标准

- 本计划列出的 Phase 1-6 全部 checklist 项通过
- spec C-12 ~ C-16 全部覆盖且通过对应测试
- 关联 BDD-Gate（E2E.P0.027 / E2E.P0.028 / E2E.P0.029 / E2E.P0.030 / E2E.P0.031）全部通过，且每个场景 trigger log 都包含 `VITE_EI_API_MODE=real` + `src/api/jdMatch.realApiMode.test.ts` PASS；backend E2E.P0.094-P0.097 live API scenarios 全部通过；E2E.P0.017 升级后 PASS；D1+D2+D3+plan001 regression（P0.001/002/004/005/006/014/015/016）全部 PASS
...
- 负向搜索（plan 001 placeholder / 动态 JD 数字 / 旧 route alias / prototype data 直接 import / LLM/provider/外部招聘平台 / 隐私字段泄漏）全部 0 命中
...
| 风险 | 应对措施 |
|------|----------|
| ui-design `screen-jd-match.jsx` SearchTab `searching=true` 区域形态确认 | Phase 0.1 已锁定：plan 002 实施期间不修订 ui-design 静态文件，前端按当前 5 步 AGENT panel + opacity 渐变 + accent 标签源级复刻；i18n key `jdMatch.search.searchingStep1..5`（最终命名以 `messages.ts` typed helper 落地为准）；动态 JD 数字（`248`/`87`/`unique postings`/`唯一岗位`）在前端 i18n 替换为不含数字的静态文案；未来 ui-design owner 任何 SearchTab `searching` 形态修订（包括但不限于：简化为单一文案、加入真步骤切换动画、改写 step 文案）由后续 plan 接力，不影响本 plan 实施 |
...
| Save / Unsave / Dismiss optimistic 与 4xx revert 的 race condition | optimistic toggle 前缓存原状态；4xx revert 时按缓存恢复；并发多次点击时按最后一次 server 响应为准；Vitest fake timer 覆盖 race；E2E.P0.027 含连续点击子用例 |
| Auth pending action 在 jd_match side-effect 操作恢复时丢失 selectedJobMatchId 上下文 | pendingAction params 显式包含 `{ tab, selectedJobMatchId, action }`；登录恢复时按 selectedJobMatchId 重新选中卡片再触发 action；如卡片已被 dismiss 则 fallback 第一张 + 提示；E2E.P0.030 覆盖 |
| chevron handoff `linkedJobMatchId` 不在当前 Recommended 列表（如已被 dismiss / pagination 未加载） | 显式 fallback 第一张 + warning toast；不抛错；不强制重新调 `listJobRecommendations`；E2E.P0.029 含 unknown linkedJobMatchId 子用例 |
...
| Search query 残留导致跨 session 隐私泄漏 | 切 tab / unmount 时 query reset；不写 URL/localStorage；登录后从 pendingAction params 恢复时 params 不含 query 文本，仅含 action 类型；Vitest 隐私反查 + E2E.P0.028 |
...
| Pixel parity baseline 在三 tab × 三主题 × 双 viewport 组合下 baseline 文件爆炸 | 常规 gate 使用 DOM anchor + geometry + computed style + non-empty screenshot buffer，不依赖 ignored baseline PNG；若未来恢复 baseline，必须先保证 clean checkout / CI 可复现 |
...
| Search tab `slow-response` variant 验证 loading 持续渲染时 Vitest fake timer 行为偏差 | 用 `vi.useFakeTimers({ shouldAdvanceTime: true })` + 显式 advance；不依赖 real timer；E2E.P0.028 提供真实 mockTransport 延迟验证 |
...
## 7 Phase 0 Evidence Log

> 在 Phase 1 启动前由 `/implement` 在 feature branch `feat/frontend-home-job-picks-and-parse-002-jd-match-recommendations-0509` 上记录 Phase 0 preflight 状态与冻结清单。后续 Phase 1+ 的实施必须以本节为准。

### 7.1 UI truth-source 当前形态（Phase 0.1）

- 真理源文件：`ui-design/src/screen-jd-match.jsx::SearchTab` line 480-589（其中 `searching=true` 区域 line 515-531）
- 搜索输入区当前形态：`NATURAL LANGUAGE SEARCH` / `自然语言搜索` label + search icon + input placeholder + Run web search button + `SOURCES` / `数据源` label + 5 source chips（LinkedIn / Boss 直聘 / 脉脉 / 拉勾 / Company sites 或 公司官网，含原型静态 count）；正式前端必须逐项断言，不能退回 4-source 或 `Data sources` 旧口径
- 当前形态：保留 5 步 AGENT panel；`● AGENT SCANNING` accent label + 1px accent 外边框 + 3px accent 左边框 + 5 行 step 文案（zh + en）+ `<div style={{ opacity: i <= 2 ? 1 : 0.4 }}>` 渐变（前 3 步 active 1.0、后 2 步 dim 0.4）
- 5 行 step 原文案（按 ui-design 当前真理源 line 521-525）：
  1. `→ Embedding your profile…` / `→ 向量化你的画像…`
  2. `→ Querying LinkedIn · Boss · Maimai · Lagou in parallel…` / `→ 并行查询 LinkedIn · Boss · 脉脉 · 拉勾…`
  3. `→ Dedup by JD hash · 248 → 87 unique postings` / `→ 按 JD 哈希去重 · 248 → 87 条唯一岗位`（**含动态 JD 数字 `248` / `87` / `unique postings` / `唯一岗位` — 必须在前端 i18n 替换为不含数字的静态文案**）
  4. `→ Scoring each against profile · strengths · gaps · prefs` / `→ 按画像/优势/盲点/偏好逐条打分`
  5. `→ Drafting match reasoning…` / `→ 起草匹配理由…`
- 前端 i18n key 冻结：`jdMatch.search.searchingPanelLabel` + `jdMatch.search.searchingStep1..5`（最终命名以 `messages.ts` typed helper 落地为准；Phase 4.1 / 4.2 将在 plan 002 实施时按此 key 命名落地）
- 第 3 步动态 JD 数字静态化策略：英文 `→ Dedup by JD hash · removing duplicate postings`，中文 `→ 按 JD 哈希去重 · 移除重复岗位`（不含具体数字；最终中文/英文措辞以 typed helper 落地为准，唯一约束是 0 个具体数字 + 不出现 `248` / `87` / `unique postings` / `唯一岗位`）
- 动画效果策略：保留 ui-design 当前 `opacity: i <= 2 ? 1 : 0.4` 渐变作为唯一动画效果；不引入 setInterval / setTimeout 真步骤切换 / CSS keyframes / transition；前端不修改 ui-design 静态文件；未来 ui-design owner 任何 SearchTab `searching` 形态修订（包括但不限于：简化为单一文案、加入真步骤切换动画、改写 step 文案）由后续 plan 接力，不影响本 plan 实施

### 7.2 B2 owner inventory 待 additive 同步清单（Phase 0.2）

当前 B2 owner truth source 锁定 12 tag / 34 endpoint / 34-operation gate。Phase 1 必须在新增 `JobMatch` tag + 12 operationId 时同步把以下 7 个文件 additive 升级到 13 tag / 46 endpoint / 46-operation：

1. `docs/spec/openapi-v1-contract/spec.md` — §1（line 11）/ D-9 freeze 范围（line 63）/ §3.1.1 endpoint 表（line 111 总计行 + endpoint 列表）/ C-1 acceptance（line 221）/ §5.1 `001-bootstrap` 描述（line 238）
2. `openapi/README.md` line 65 — `make validate-fixtures` 描述中的 `34-operation coverage`
3. `openapi/fixtures/README.md` line 24 / 26 / 104 — 12 tag directories / 34 operationIds 文案 + `verifies all 34 operationIds are present` 描述
4. `scripts/lint/openapi_inventory.py` line 352 — `operation count must be 34 (spec §3.1.1); got {operation_count}` 硬编码 + 任何 tag 名单
5. `scripts/lint/validate_fixtures.py` line 16 / 160 — `34 operationIds frozen` / `34-operation inventory` 注释 + 任何 hardcode operation set
6. `docs/spec/mock-contract-suite/spec.md` line 22 / 68 — 12 tag / 34 operation fixtures 表述
7. `docs/spec/engineering-roadmap/spec.md` line 98 / 120 / 164 — 12 tag / 34 endpoint / 34 operation 表述

Phase 1.5 contract test 必须新增 validator 单测断言旧 34-operation gate 不再误拒新 `JobMatch` fixtures（spec D-1 D-1 已锁定）。

### 7.3 Operation matrix freeze（Phase 0.3）

`JobMatch` tag 12 operationId 冻结清单（与 plan §3.6 Frontend / Backend Operation Matrix 一致）：

| operationId | path / method | Idempotency-Key | 必调入口 |
|-------------|---------------|-----------------|---------|
| `getJobMatchProfile` | `GET /jd-match/profile` | n/a (GET) | 进入 jd_match 1 次 |
| `getAgentScanStatus` | `GET /jd-match/agent-status` | n/a (GET) | 进入 jd_match + 切回 Recommended tab |
| `listJobRecommendations` | `GET /jd-match/recommendations` | n/a (GET) | Recommended tab 1 次 + cursor pagination |
| `getJobRecommendation` | `GET /jd-match/recommendations/{jobMatchId}` | n/a (GET) | 用户切卡时必调（detail required，不得复用 list 摘要） |
| `addToWatchlist` | `POST /jd-match/watchlist` | **必带** | Save / Save current 入口 |
| `removeFromWatchlist` | `DELETE /jd-match/watchlist/{jobMatchId}` | **必带** | Unsave 入口 |
| `markJobNotRelevant` | `POST /jd-match/recommendations/{jobMatchId}/dismiss` | **必带** | Mark not relevant 入口 |
| `searchJobs` | `POST /jd-match/search` | **必带** | Run web search 入口 |
| `listSavedSearches` | `GET /jd-match/saved-searches` | n/a (GET) | 切 Search tab 1 次 |
| `createSavedSearch` | `POST /jd-match/saved-searches` | **必带** | Save current as watch 入口 |
| `listWatchlist` | `GET /jd-match/watchlist` | n/a (GET) | 切 Watchlist tab 1 次 |
| `getMarketSignals` | `GET /jd-match/market-signals?window=7d` | n/a (GET) | 切 Watchlist tab 1 次 |

5 个 side-effect operation 必带 `Idempotency-Key`：`addToWatchlist` / `removeFromWatchlist` / `markJobNotRelevant` / `searchJobs` / `createSavedSearch`。

`getJobRecommendation` 必调详情契约：用户切卡时必须调一次；`listJobRecommendations` 只作为列表摘要来源，不得复用作为详情 operation 的替代证据。

### 7.4 pendingAction action enum freeze（Phase 0.3）

未登录用户在 jd_match 触发 side-effect 时统一通过 D1 `requestAuth({ type: "jd_match_action", route: "jd_match", params, label })` 进入 auth pending action；params 严格使用以下 action enum：

- `save` — 触发 `addToWatchlist`
- `unsave` — 触发 `removeFromWatchlist`
- `dismiss` — 触发 `markJobNotRelevant`
- `confirm_interview` — 触发 `nav("parse", ...)`
- `run_search` — 触发 `searchJobs`
- `create_saved_search` — 触发 `createSavedSearch`

Source button (`window.open(noopener,noreferrer)`) 与 Watchlist chevron handoff 不进入 auth pending action（即使未登录也直接执行安全操作）。

### 7.5 隐私字段负向清单 freeze（Phase 0.3）

下列字段严禁出现在 `console.log` / URL query / `localStorage` / `sessionStorage` / telemetry payload / pendingAction params：

- `jobMatchId` — Recommended/JDDetail/Watchlist 上下文 ID
- `sourceUrl` / `sourceJobUrl` — JobMatchRecommendation.sourceUrl
- `freeNote` — markJobNotRelevant 自由备注（plan 002 不收集；仅 schema 兼容）
- `query` — Search tab 自然语言搜索 query
- saved-search `label` — createSavedSearch.label
- watchlist `label` — WatchlistItem.label
- `linkedJobMatchId` — WatchlistItem.linkedJobMatchId（chevron handoff 内部状态，不进 URL）

mockTransport spy 仅记录 status code + 调用次数，不记录 request / response body。`window.open(sourceUrl, "_blank", "noopener,noreferrer")` 调用必须带两个 flags 防 reverse tabnabbing（plan §3.5 Privacy / security · `window.open noopener,noreferrer` 行）。

### 7.6 Preflight Gate 通过条件（Phase 0.4）

- 7.1 UI truth-source freeze 已记录（5 步 panel + i18n + 动态数字静态化策略 + opacity 渐变保留）
- 7.2 B2 owner inventory 待 additive 同步清单已锁定（7 个文件 / 4 处硬编码 count gate）
- 7.3 12 operation matrix + 5 个 Idempotency-Key + getJobRecommendation 必调详情契约已 freeze
- 7.4 pendingAction action enum 6 项已 freeze + Source / chevron 不进入 auth pending action 已 freeze
