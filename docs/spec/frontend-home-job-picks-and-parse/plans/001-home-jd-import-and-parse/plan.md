...
| Boundary · listTargetJobs >3 items | 首页只展示最近 3 条按 updatedAt desc；超过 3 条通过 `更多` 跳到 `workspace` 列表页 | `screen-home.jsx::HomeScreen` recent section | 2+9 | Vitest + fixture variant + E2E.P0.014 |
...
| Cross-layer contract · listResumes for Home / Parse launch | Home 读取 ready 简历并要求通过下拉框显式选择后才能 import；Parse preview 继承首页 `resumeId` 或在缺失时兜底选择，禁止 `resume-unbound` 成功 handoff | OpenAPI `PaginatedResume` | 7+8+9 | Vitest `home/HomeResumeSelection.test.tsx` + `parse/ParseResumeBinding.test.tsx` + E2E.P0.015/P0.016 |
...
| UX · home / parse resume binding | Home 在 JD 输入卡下方渲染“选择已有简历”下拉框与“还没有简历？1 分钟创建 →”；有 ready 简历时必须等待用户显式选择；无 ready 简历时禁用 `立即面试` 并引导创建；Parse 继承首页显式选择或兜底阻断 | `screen-home.jsx::HomeScreen` + `screens-p0-complete.jsx::ParseScreen` Interview launch block + `screen-workspace.jsx::ResumePickerModal` | 7+8+9 | Vitest + Playwright home/parse confirm gate |
...
按 `updatedAt desc` 排序；当前首页只展示最近 3 条，超出部分由 Phase 9 的“更多”入口交给 `workspace` 列表页；fixture 与生产 API 端 cursor pagination（不在本 plan 内消费 cursor）可继续返回更多记录用于排序和 More 判断。
...
在 `ParseScreen` preview 阶段调用 generated `listResumes`，只保留 `parseStatus=ready` 且 `status !== archived` 的简历。即使存在 ready 简历，也不得默认选中最新项；必须渲染 `parse-launch` / `parse-resume-binding` / `parse-resume-picker-*` DOM 锚点，让用户显式点击一份简历后才展示名称、来源和更新时间。Red-Green: `parse/ParseResumeBinding.test.tsx` 先锁定选择前 Save/Start disabled 与无默认简历名称，再验证点击简历项后才启用。
...
用户显式选中简历后，`仅保存规划` 先执行原有 `updateTargetJob` 保存编辑字段，再 `navigate("workspace", interviewContextFromTargetJob(targetJob, { resumeId }))`；`立即面试` 同样保存编辑字段，再进入 `workspace` 并携带 `autoStartPractice=1`，由既有 workspace `useStartPractice` 链路创建 session 后进入 `practice`。params 必须包含 `planId` / `targetJobId` / `jdId` / 真实 `resumeId` / `roundId` / `roundName` / `practiceMode` / `mode` / `modality`，不得包含 `resume-unbound`。未显式选择 ready 简历时不得产生 pendingAction；若后续启动动作已有真实 `resumeId`，pendingAction 恢复到 `workspace autoStartPractice=1`，不再走 `confirm_interview -> workspace`。
...
更新 `ui-design/src/screen-home.jsx`、`docs/ui-design/jd-resume-management.md`、`docs/ui-design/ui-architecture.md`、`docs/ui-design/module-job-workspace.md` 与本 owner 文档：Home 的“选择已有简历”必须是 dropdown / combobox 控件，不再平铺每份简历；Recent mock interviews 最多展示 3 张卡片，超过 3 条时在 section header 右侧显示“更多”并跳转一级 `workspace` / `模拟面试` 列表。

#### 9.2 Red tests

更新 `home/HomeResumeSelection.test.tsx`：`home-resume-select` 必须是 `select`/combobox，用户通过 `selectOptions` 选择 ready 简历；旧的按钮式平铺 `home-resume-option-*` 不得作为可点击 button 列表出现。更新 `home/HomeRecentMocks.test.tsx`：`twelve-plus` fixture 只能渲染 3 张卡片，`home-recent-more` 可见并点击 `navigate({ name: "workspace", params: {} })`。

#### 9.3 实现

`ui-design/src/screen-home.jsx` 与 `frontend/src/app/screens/home/HomeScreen.tsx` 同步改为 dropdown；保留未选择时 submit disabled、无 ready 简历 empty/create CTA、pending import resumeId 约束。Recent mocks 仍按 `updatedAt desc` 从 `listTargetJobs` 结果排序，但 UI `slice(0, 3)`；`更多` 使用受保护路由跳转 `workspace`，未登录时沿用 `requestAuth(open_protected_route)`。

#### 9.4 BDD / parity gate

更新 E2E.P0.014 / E2E.P0.015 文档、verify 文案和 pixel parity：P0.014 证明 dropdown 锚点、3-card cap、`更多` 跳转；P0.015 继续证明 dropdown 选中的真实 `resumeId` 被传到 parse。`ui-design/ui-design-contract.test.mjs` 与 `frontend/tests/pixel-parity/home.spec.ts` 增加 dropdown / recent cap 的源级锚点。

## 5 验收标准

- 本计划列出的 Phase 1-9 全部 checklist 项通过
- spec C-1 ~ C-11、C-17、C-18 与 C-19 全部覆盖且通过对应测试
...
| 风险 | 应对措施 |
|------|----------|
| listTargetJobs / getTargetJob fixture 缺少 variants 导致 boundary / failure 路径无法测试 | Phase 2.4 / 4.2 / 9.2 先扩展或复用 fixture variants（empty / 12+ / processing / failed / hidden-signal-rich），通过 `mock-contract-suite` parity test 校验 |
