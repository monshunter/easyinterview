...
- **BDD 策略**: `BDD.HOME.JD.001/002` 保留 import/parse/Workspace handoff，`BDD.HOME.JD.003` 覆盖 Home 视觉层级、状态、响应式与可访问性，`BDD.HOME.JD.TEXTAREA.006` 覆盖 JD textarea 默认双倍高度与内容自动增高/回缩，`BDD.HOME.RESUME.OPTION.008` 覆盖只显示简历名称的下拉选项；均由 domain behavior tests 承接。Chrome manual acceptance 是辅助视觉证据，不是 E2E；`E2E.P0.098` 只覆盖 completion 后 Home/Workspace/TargetJob progress refresh/detail read。
