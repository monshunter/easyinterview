...
|---------|------|-----------|--------------|----------------------------|
| E2E.P0.014 | primary path · home 默认渲染 | Phase 1 + 2 + 9 | C-1, C-4, C-19 | Phase 1.5、Phase 2.6、Phase 9.4 |
| E2E.P0.015 | primary path · home 下拉选择已有简历 → paste/import → parse 主路径 + alternate path（upload / URL variants） + failure path（4xx / failed） | Phase 3 + 4 + 8 + 9 | C-2, C-3, C-6, C-18 | Phase 3.6、Phase 4.10、Phase 8.5、Phase 9.4 |
...
|---------|------|-------|------|------|----------|
| E2E.P0.014 | Home 默认渲染（empty + non-empty + 12+） | 用户打开 frontend dev server，未登录或已登录，listTargetJobs fixture 配置三种 variant：empty / one-job / 12+jobs | 切换三种 fixture variant 分别加载 `/#route=home` | （1）Hero `home-hero-{label,title}`、JD textarea + Submit、`home-resume-select` 下拉框、Resume create CTA 全部渲染并 testid 命中；旧 `home-hero-sub` 0 命中；（2）empty variant 显示 `HomeEmptyState`，点击「回到 JD 输入」按钮 focus textarea；（3）one-job variant 渲染 1 张 `home-recent-mock-card-${id}`，status pill computed background 对应 D2 token，MiniRoundRail 当前轮次圆点位置正确；（4）12+jobs variant 仅渲染最近 3 张卡片，按 `updatedAt desc` 排序，并展示 `home-recent-more` / `更多` 跳转 `workspace`；（5）TopBar `topbar-nav-home` 高亮；（6）切换 zh/en、warm→dark→customAccent，关键文本 / computed background 出现可见变化；（7）Home 主按钮文案为「立即面试」/ `Start interview now` | `test/scenarios/e2e/p0-014-home-default-render/` |

## Phase 3 + 4: Paste/Upload/URL → Import → Parse 主路径

| 场景 ID | 场景 | Given | When | Then | 验证入口 |
|---------|------|-------|------|------|----------|
| E2E.P0.015 | Home 下拉选择已有简历 → Paste/Upload/URL → import → parse loading → preview | 用户已登录，`listResumes` fixture 返回 ready 简历，importTargetJob fixture 返回 `{ targetJobId: "uuid", job }`，createUploadPresign fixture 返回 `fileObjectId`，getTargetJob fixture 配置 `analysisStatus` 序列 `queued → processing → ready` 与 `failed` 两种 variant | 用户在 home 先通过 `home-resume-select` 下拉框显式选择一份已有简历，再分别走三条路径：（A）粘贴 JD 文本点「立即面试」；（B）打开 upload modal 拖入 placeholder 文件后 Continue；（C）打开 URL modal 输入 URL 后 Continue；并在解析过程中切换 `failed` variant 触发失败态 | （1）未选择简历时「立即面试」disabled 且不调用 `importTargetJob` / `requestAuth`；（2）Home 不平铺所有简历为按钮列表；（3）A/C 路径分别提交 `source.type=manual_text\|url` 的 `ImportTargetJobRequest`；B 路径先调用 `createUploadPresign` `purpose=target_job_attachment` 并把返回 `fileObjectId` 写入 `source.type=file`；side-effect 调用均带 `Idempotency-Key`；（4）成功响应后 route 跳 `parse?targetJobId=…&source=…&resumeId=<selected ready resume id>`；（5）Parse 屏先渲染 `parse-loading-step-${0..3}` + `parse-loading-footer`，按 ≥600ms 节奏推进，footer 只展示 backend parse metadata / fixture metadata，不触发任何 LLM/provider 请求；（6）`analysisStatus=ready` 后切到 preview，渲染 fixture/backend response 中 title/companyName/locationText/requirements/summary.interviewHypotheses/summary.coreThemes/fitSummary.riskSignals，且 summary/fitSummary `GenerationProvenance` 可追溯，并继承 route `resumeId`；（7）`analysisStatus=failed` variant 下显示 failed UI（重新解析 / 返回首页 2 button），不展示伪造 preview；（8）JD raw text 在 console / URL / localStorage / telemetry 全部 0 命中；（9）4xx import / presign 响应触发 inline 错误并保留 textarea/modal 输入；（10）network/client spy 只允许 generated `listResumes`、`createUploadPresign`、TargetJobs client + existing shell runtime/auth 调用，不允许前端直连 AI provider、prompt registry 或 provider-specific endpoint | `test/scenarios/e2e/p0-015-jd-import-and-parse/` |
