...
4. 在自动化 journey 之后提供人工验收前所需的真实 provider UAT 材料包：真实前端、真实后端、真实 AI LLM、Mailpit 本地 magic-link 登录、双语输入材料、手工 checklist、证据归档和清理边界。
...
- 真实 provider manual UAT（`E2E.P0.100`）：`APP_ENV=dev` 后端 + `VITE_EI_API_MODE=real` 前端 + 真实 `AI_PROVIDER_BASE_URL` / `AI_PROVIDER_API_KEY` + Mailpit 本地 magic-link 登录 + synthetic JD / resume / answer materials。
...
- 真实 provider manual UAT 不替代 F3 eval 的模型质量基线，也不把具体 AI 输出文案固化为自动化断言；它只要求真实 provider 调用、用户可观察质量记录和脱敏证据。
...
| D-8 | manual UAT owner | 真实 provider 人工验收作为 `002-manual-uat-real-provider-full-funnel`，不改写 001 的 stub-AI 边界 | 防止把人工验收材料作为无 owner 文件或把 stub AI 自动化证明冒充真实 LLM 验收 |
| D-9 | manual UAT 账号入口 | 002 使用 `local-dev-stack` 提供的 Mailpit 本地 mailbox：synthetic `.example.test` 邮箱触发真实 `startAuthEmailChallenge`，Mailpit 收取 magic-link，`verifyAuthEmailChallenge` 签发 `ei_session`；不得直接写 `sessions` 表或新增正式 `backend/cmd` / Go helper | 支持用户验收时材料齐备，包括账号；本地测试不依赖真实外部邮箱服务或真实邮箱账号，同时避免场景依赖越界进入正式后端进程树 |
| D-10 | manual UAT AI provider | 后端必须以 `APP_ENV=dev` 连接真实 provider，默认 DeepSeek OpenAI-compatible endpoint；`APP_ENV=test`、deterministic stub、P0.099 test server 与 frontend fixture mock 都不能作为真实 UAT 完成证据 | 明确真实联调边界 |
...
| Q-3 | 真实 provider manual UAT 是否保留现场 | 默认保留 `.test-output/manual-uat/full-funnel/` 脱敏证据，DB cleanup 仅在用户确认后执行 |
...
- **真实 provider UAT 边界**：manual UAT 必须使用 `APP_ENV=dev` 后端真实进程、真实 `AI_PROVIDER_BASE_URL` / `AI_PROVIDER_API_KEY`、真实 PostgreSQL、真实 frontend real mode；不得用 `APP_ENV=test`、deterministic stub、fixture-backed frontend mock transport、`Prefer: example=<scenario>` 或 `EI_E2E_P0_099_SERVER=1` 冒充。
- **账号材料边界**：manual UAT 必须提供可重复的 synthetic account sign-in 材料；tracked 文件只能保存邮箱、Mailpit URL、验证步骤和 cleanup 说明，不得保存真实 session cookie、auth secret、AI key 或 magic-link token。`test/scenarios` 只允许 shell/Python 辅助；不得新增 `backend/cmd` / Go helper，也不得直接写 `sessions` 表绕过 auth flow。
...
| manual UAT companion | `test/scenarios/manual-uat/` | 人工验收 runbook、材料、checklist、Mailpit 登录说明；不是标准自动化 runner 套件 |
...
| C-9 | manual UAT 材料完整 | 用户准备人工验收 | 查看 `test/scenarios/manual-uat/full-funnel/` | Mailpit magic-link 登录说明、JD、简历、作答样例、checklist、环境变量模板、证据归档与 cleanup 说明齐备；tracked 文件无真实 secret / PII，且不存在 `backend/cmd/devsession` / `backend/internal/devsession` | 002 Phase 1-3 |
...
| C-11 | 无 mock/stub 冒充 | manual UAT runbook / checklist | 审查完成证据 | 不接受 `APP_ENV=test`、P0.099 test server、fixture mock transport、deterministic stub AI 或 `Prefer: example=` 作为真实 UAT 完成证据 | 002 Phase 2-4 |
| C-12 | dev-only 账号安全边界 | manual UAT 账号入口存在 | 运行 no-backend-cmd / no-Go scenario negative gate 与材料 secret scan | 只允许 synthetic `.example.test` 邮箱和本地 Mailpit；不输出 auth secret、AI key、session secret、magic-link token 或 cookie value；场景 helper 不进入正式 backend cmd / internal 包 | 002 Phase 1 |
...
- [002-manual-uat-real-provider-full-funnel](./plans/002-manual-uat-real-provider-full-funnel/plan.md)：真实 provider 人工验收材料包与启动路径（`E2E.P0.100`），覆盖 Mailpit magic-link 登录、真实前后端、真实 AI LLM、无 mock/stub 边界和脱敏证据。
