...
- P0.099：真实 frontend/backend/provider 下的 en/zh ready report 与 generating 页面、authenticated report API、只读 PostgreSQL 投影和精确六图人工核对。
...
| generating → report UI | primary | P0.099 | fixture-only 页面、伪 ready、跨 run evidence |
| ready report desktop/mobile | responsive/privacy | P0.099 | 少于/多于六图、裁剪、cookie/raw content、digest 未绑定 |
...
- P0.099 runbook 要求 exact-six evidence 绑定当前 run 的 DB/API/report/session/context/screenshot digest，且真实 desktop/390 页面完整显示 report/generating 状态。
...
- 本次静态重分类不启动场景环境，不把 `Ready` 误记为当前运行 PASS。
