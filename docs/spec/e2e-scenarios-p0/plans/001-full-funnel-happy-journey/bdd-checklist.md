...
- [ ] 099.C 编写 `scripts/setup.sh`：拉起真后端进程（dev-stack postgres + scenario stub / fixture AI）+ 前端 build/preview 通过 `VITE_EI_API_MODE=real` / `VITE_EI_API_BASE_URL=http://127.0.0.1:<backend-port>/api/v1` 指向真后端；health probe；通过 `registerResume` + `resume_parse` scenario stub/fixture AI seed user + ready resume
