...
| 2026-05-22 | 1.8 | 收窄 non-current-token gate：继续拦截独立 `/voice` route / `Voice` tag，但允许 practice-voice-mvp 拥有的 `createPracticeVoiceTurn`、`/practice/sessions/{sessionId}/voice-turns` 与 `PracticeVoiceTurn*` generated artifacts；`lint-mock-contract` 与 repo-wide `make lint` 通过。 | 001-fixture-backed-mock-runtime Phase 6 |
