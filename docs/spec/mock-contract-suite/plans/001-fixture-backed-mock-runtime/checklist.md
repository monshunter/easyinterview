...
- [x] 1.1 建立 operationId -> fixture registry；验证: 新增 focused unit test 覆盖至少 Auth、TargetJobs、PracticeSessions、Reports、Resumes、Privacy / runtime config 的当前 fixture lookup，未知 operationId 返回明确错误；Debriefs / Profile 只作为 retired operation 负向对象
...
- [x] 4.4 L2 remediation: `lint-mock-contract` 必须拒绝 `openapi/fixtures/` 下非当前 10 tag 的旧目录，即使目录为空或 Git 不跟踪；验证: focused Red-Green 先用当前 `Growth` / `Mistakes` 空目录证明 lint/test 失败，再删除旧目录并运行 mock runtime boundary tests、`make lint-mock-contract`、`make codegen-check`、`make docs-check`
...
## Phase 7: Post-pruning context reconciliation

- [x] 7.1 同步 completed plan/context 的当前 operation discovery；验证: `context.yaml` 的 `apiNames` 只列当前 35 个 OpenAPI operationId，不再列 `getMyProfile` / `updateMyProfile` / `listExperienceCards` / `createExperienceCard` / `updateExperienceCard` / `createDebrief` / `getDebrief` 等 D-22 已删除 operation；plan/checklist 中旧 34-operation / 12-tag / Debriefs 正向 fixture 口径已改为 35-operation / 10-tag / retired-negative 语义。
  <!-- verified: 2026-07-06 method=context-reconcile evidence="validate_context.py mock-contract-suite/001 tooling PASS; rg over context apiNames and plan positive wording shows no retired Profile/Debriefs operationId; openapi operationId list remains 35 current operations." -->
