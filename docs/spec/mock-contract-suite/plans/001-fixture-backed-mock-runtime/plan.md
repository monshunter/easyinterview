...
- **TDD 策略**: 通过 `/implement mock-contract-suite/001-fixture-backed-mock-runtime tooling` -> `/tdd` 执行；每个 checklist item 先补 focused test 或 lint fixture，再实现最小 runtime / gate；测试断言写在 checklist 的 `验证:` 后。Phase 5 必须先用 frontend focused tests 证明 dev client 默认 fixture-backed、真实 backend opt-out 保留、fixture coverage 覆盖全部 generated operationId，再接入 `main.tsx`。
