...
The registry reads fixture metadata from `openapi/fixtures/<tag>/<operationId>.json` and exposes operationId lookup for frontend and backend mock users. Coverage must match OpenAPI exactly: missing fixture, extra fixture, unexpected tag directory, mismatched operationId, or non-current API token is a gate failure.
...
- current mock/API token scan rejects non-current routes, tags, schema keys and config paths while allowing the current practice-voice contract.
...
| non-current token gate 误伤当前 voice turn API | boundary lint 只拦截独立 `/voice` route 与 `Voice` tag，允许 practice-voice owner operation |
