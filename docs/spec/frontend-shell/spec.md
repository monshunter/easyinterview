...
| D-7 | UI i18n | 正式前端至少支持 `zh` / `en` 两种 UI locale；每种语言必须有独立 locale 文件，聚合层只负责导入、类型约束和 helper；初始 UI 语言跟随浏览器 locale，未知或缺失时 fallback `en`；语言切换只关联前端显示偏好，不依赖 runtime config 或登录态 | 语言切换必须按 `ui-design/src/app.jsx` 源码复刻为 TopBar icon toggle，改变用户可见文案，并通过 `Accept-Language` 影响后续 API display hint |
...
- TopBar 语言切换 UI 必须按 `ui-design/src/app.jsx` 源码复刻为带 globe icon 的 toggle button（`中 · EN` / `EN · 中`），不得退化为 native `select`、分离按钮组或只切换状态的占位控件。
...
| C-7 | 中英 UI 切换 | 用户打开默认 App shell | 通过 TopBar language toggle 切换到 English / 中文 | TopBar、auth 入口和 D1 shell 静态文案即时切换；初始语言跟随浏览器 locale，不支持时 fallback English；登录态和 runtime locale 不覆盖前端语言设置；后续 API 请求带 `Accept-Language`；`zh` / `en` 文案分别来自独立 locale 文件；语言控件 DOM / 图标 / 文案节奏与 `ui-design/src/app.jsx` 一致 | 001-app-shell-auth-settings |
