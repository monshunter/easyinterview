...
|---|---|---|---|---|
| Email-code、资料补全与设置入口 | real frontend、backend 与 Mailpit 已运行 | 用户获取验证码、登录、补全资料，点击设置齿轮核对账号字段并退出 | shell 读取真实 session/profile，Settings 显示同一账号的姓名与完整邮箱，logout 清除 session；PASS/FAIL reporter 与落盘证据均不保存完整邮箱或 URL-encoded 等价值 | `E2E.P0.101`（原地扩展） |
