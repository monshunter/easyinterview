...
- [x] 实现 setup / trigger / verify / cleanup；verify 必须断言 Home、三个一级入口、单一登录入口、显示控制可见，welcome、注册入口、`jd_match`、`debrief`、`profile`、独立 `voice`、Growth / Mistakes / Drill 旧入口不可见
- [x] 执行并通过场景验证
- [x] 记录验证证据
...
- [x] 实现 setup / trigger / verify / cleanup；verify 必须断言语言切换控件是 TopBar language dropdown，切换到 English 后 TopBar、单一登录入口、用户菜单、auth/settings/placeholder shell 文案为英文，route/testid/params 未被 locale 改写，generated client 请求包含 `Accept-Language`，runtime locale 与登录态不覆盖前端语言设置，`profile` 不作为 live shell 出现
- [x] 执行并通过场景验证
- [x] 记录验证证据
...
- [x] 实现 setup / trigger / verify / cleanup；verify 必须断言默认非登录态、登录后头像 chip + dropdown、settings/logout 分流、logout 后非登录态、旧 inline 用户菜单、注册按钮、`profile` 菜单项和静态 authenticated default 负向约束
- [x] 执行并通过场景验证
- [x] 记录验证证据
...
- [x] 覆盖 `startAuthEmailChallenge` 真实 `202 Accepted` 空响应：generated client 不抛 JSON parse error，当前单一登录入口提交后进入 `auth_verify`；历史登录/注册双入口证据不得作为当前验收口径
