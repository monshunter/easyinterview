...
- [x] 26.1 RED-CONTRACT: OpenAPI/fixture/generated/backend/migration tests 要求 `AccountTheme` 与 `user_settings.theme` 接受 `ocean|plum|forest`、`updateMe#getMe` Forest round-trip，并继续拒绝 `warm`/unknown；运行 focused Go/Python/contract tests 记录目标失败。<!-- verified: 2026-07-20 method=focused-python-go-red evidence="OpenAPI enum remained [ocean,plum], themeForest fixture was absent, Forest PATCH returned 400, and migration 000022 was absent; all four new contract targets failed for the intended reason while warm rejection stayed covered." -->
- [x] 26.2 GREEN-CONTRACT: contract-first 更新 OpenAPI/fixture、执行 codegen，再扩展 migration/enum manifest 与 backend store/service/handler；profile/theme 事务、非法值零部分写入、migration up/down/up 和 codegen/fixture drift gates 通过。<!-- verified: 2026-07-20 method=focused-contract-and-disposable-postgres-green evidence="OpenAPI/generated enum + themeForest fixture, auth Forest PATCH/warm rejection, validate-fixtures, lint-openapi, migrate-check and the isolated PostgreSQL Forest v21->v22->v21->v22 integration all PASS; both disposable databases were removed and absence rechecked." -->
- [x] 26.3 RED-FRONTEND: token/Settings/runtime/dev-mock tests 要求三预设 metadata/i18n、六组 theme-mode selector、确认的 Ocean/Plum/Forest light/dark OKLCH accent matrix、Forest 选择退出 Custom、单次保存与 invalid fallback；旧实现产生目标失败。<!-- verified: 2026-07-20 method=focused-vitest-red evidence="43 focused tests produced 7 intended failures: two-theme keys/metadata, missing Forest palette/button, legacy hex Ocean/Plum accents, Forest runtime fallback to Ocean, and dev mock Forest rejection; 36 existing assertions remained green." -->
- [x] 26.4 GREEN-FRONTEND: 最小扩展 Theme union、palette/CSS、runtime normalizer、Settings metadata/locale 与 dev mock；Forest 立即预览且零请求，Save 一次 `updateMe`、成功零 follow-up GET、失败可重试、离开恢复确认值。<!-- verified: 2026-07-20 method=focused-vitest-green evidence="tokens/Settings/DisplayPreferences/dev-mock pass 43/43 with three metadata entries, six selectors, exact OKLCH matrix, Forest Custom exit, one updateMe, zero follow-up getMe, retry and fail-closed contracts." -->
- [x] 26.5 BDD-Gate: 执行 `BDD.SHELL.SETTINGS.PRESET.007` domain behavior tests；真实 Settings desktop/mobile Chrome 验证四个一级选项、Forest light/dark computed variables、Save 稳定与无横向溢出；`E2E.P0.101` 原地验证 Forest 保存/logout/relogin 恢复，不新增 E2E ID。<!-- verified: 2026-07-20 method=domain-tests-and-real-chromium-e2e evidence="Focused frontend 57/57 plus current-run E2E.P0.101 PASS on real frontend/backend/Mailpit: 1440 and 390 viewports, four options, Forest light/dark computed variables, Custom Save y-delta <=1px, zero overflow, one PATCH, zero follow-up/route-switch GET, relogin Forest, console/page/http failures 0; verify and cleanup PASS." -->
- [x] 26.6 REGRESSION: focused contract/backend/frontend、`make codegen-check`、`make validate-fixtures`、`make migrate-check`、frontend typecheck/build、根 `make test`、owner contexts、Header/INDEX/docs/diff/residual gates 通过；Phase 15.3 未完成时主 plan 保持 `active`。<!-- verified: 2026-07-20 method=full-regression-and-owner-gates evidence="make test PASS (Python 621, Go all packages, frontend 137 files/1119 tests); codegen-check, validate-fixtures, disposable migrate-check/integration, frontend build, both contexts, docs/Header/INDEX/links/contract IDs, diff checks and zero warm/two-theme active runtime references PASS. Plan remains active for independent Phase 15.3." -->
