...
- [x] 准备测试数据：workspace / practice / generating / report / resume workshop 的稳定 safe ID 组合、unauthenticated 与 authenticated mock session、direct-open frontend paths，以及 retired debrief/profile/jd_match 输入。Evidence: `data/seed-input.md` + `frontend/src/app/scenarios/p0-088-url-addressable-routing-canonical.test.tsx` 用例矩阵。
- [x] 实现 setup / trigger / verify / cleanup：trigger 覆盖 direct open、reload、App navigation、back、forward、unknown/malformed query fallback，以及 `autoStartPractice` / `reportId` / `reportStatus` / `tailorRunId` / `resumeId` / `sourceReportId` 当前 handoff keys；retired `debriefId` / `debriefJobId` / `selectedJobMatchId` / `pendingJdMatchActionId` 不作为 legal survival keys。
...
- [x] 准备隐私标记数据：raw JD、source URL、retired jd_match search query/label、resume text、guided answers、parsed summary、structured profile、suggestion text、question/answer text、retired debrief notes、AI prompt / response marker、auth secret marker（verify-side grep）。
- [x] 实现 auth-gated workflow：未登录打开 canonical workflow URL、触发 workspace auto-start / report replay / home import / resume workshop、完成 mock passwordless、恢复原 route；retired jd_match/debrief/profile 输入归一且不恢复旧 screen。Evidence: `frontend/src/app/scenarios/p0-089-url-routing-auth-privacy.test.tsx` 涵盖 redirect → verify → restore 流程 + hostile direct-open + hostile popstate restore。
- [x] 捕获 URL、history.state、pendingAction、localStorage、sessionStorage、console、mock transport logs，并对隐私标记做 zero-hit 断言，同时证明合法 retained handoff keys 未被 allowlist 误删、retired jd_match/debrief keys 被过滤；hostile popstate 后 URL/hash/history.state 必须被 scrub 到 canonical safe state。
