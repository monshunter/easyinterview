...
本 owner 固化正式前端 App shell 的视觉系统合同：维护 token、主题、暗色、`customAccent`、固定字体栈、TopBar、认证页、设置页与通用 screen shell，并保持路由、认证、pendingAction、i18n 和 mock runtime 行为一致。当前 Phase 24 原地新增 Forest，并按用户确认值重设三预设 accent matrix。

正式前端的可见样式只能来自 `frontend/src`，其架构、流程与交互约束来自 `docs/ui-design/`。本 owner 不创建业务页面细节、不扩大 route catalog、不引入外部设计系统、不复制 prototype data。

Phase 1-23 的已勾选内容只保留历史交付证据；Phase 24 是当前视觉 owner，并取代历史“只含 Ocean / Plum、Forest 为范围外”的正向描述。实现不得为历史证据保留兼容 UI、metadata、字体资产或 `warm` 入口。
...
- **BDD 策略**: `BDD.SHELL.PRESET.003` 覆盖 Ocean / Plum / Forest light/dark palette 与 Settings 四选项视觉行为；账号保存与恢复引用 `frontend-shell/001` 的 `BDD.SHELL.SETTINGS.PRESET.007`。本 owner 不新建重复 E2E，`E2E.P0.101` 仍由 001 原地扩展。
...
`frontend/src/app/theme/` 以 CSS variables 表达语义 token。TS 侧只导出语义键名，不能导出颜色字面量；`themes.css` 必须覆盖 ocean / plum / forest × light / dark 6 个组合；默认与 fallback 均为 `ocean`。三预设的 accent/accent-soft 使用确认的 OKLCH matrix：浅色 `58%/92%`，暗色沿同 hue/chroma 使用 `68%/28%`；Forest 的其它语义角色复用 Ocean 中性与状态基线。`--ei-radius-control: 8px` 是有框矩形/方形操作按钮的唯一共享圆角 token。Settings Appearance 暴露 `深海 / Ocean`、`梅子 / Plum`、`森林 / Forest` 和 `自定义 / Custom`，`暖陶 / Warm` 继续保持范围外。
...
TopBar 保持三入口 nav、pill-shaped 暗色 toggle、语言 dropdown 与登录/设置入口，不再渲染主题菜单或 custom accent row。desktop 使用 76px chrome 节奏；已登录账号区使用从 authenticated runtime `displayName` 派生首字符的圆形 initial mark 作为单一 settings button，名称为空显示 `?`；它不是用户头像，不渲染姓名、caret、backdrop、dropdown 或 TopBar logout。`CustomAccentPicker` 移入 Settings Appearance，只保留色相与饱和度控件；不渲染 preview/value/reset。设置按钮保持清晰 focus、>=40px 点击区域与 desktop/mobile viewport containment。
...
`themes.data.ts` contains six palette combinations and three predefined theme metadata entries. It does not own font-preset pairs. `THEME_METADATA` is consumed by Settings Appearance；`themes.css` is the checked-in runtime source verified against the data and UI design truth source。
...
`frontend-shell/001` owns behavior；本 owner 负责把 TopBar 账号区收敛为一个圆形 `E` initial-mark 设置按钮，并删除旧 chip/menu/backdrop/logout selectors。initial mark 不读取用户头像数据、不打开账号菜单。Settings 删除 tab rail、security/font/product/static-list layout，保留 Account/Privacy card rhythm、只读 value rows、unavailable label、destructive confirm/pending/error styles。字体资产只保留 Noto Serif SC、Inter、JetBrains Mono；删除 preset metadata、四个额外 packages/imports、matching tests/i18n/CSS，不增加隐藏 picker 或 fallback preset。

### 4.23 Framed action control radius

有明确背景或边框的矩形、方形 action 统一使用 `--ei-radius-control: 8px`。目标 inventory 覆盖 TopBar 登录/语言项、Auth 主操作、Home、Workspace、Parse、Practice、Reports/Report、Generating、Resume 与 Settings 的主次、危险、恢复及小型图标按钮；Settings 的保存、退出、注销与 dialog action 必须同圆角。圆形 initial、pill toggle、无边框 back/link、卡片、输入框、状态标签和装饰图形不是该 token 的 consumer。实现不得新增全局 `button` selector，也不得借修改 `--ei-radius-sm/md` 批量改变非目标 surface。


## 5 验收标准

- Token/theme tests 证明 Ocean / Plum / Forest 六组 theme-mode CSS variables 与 `ui-design` 的 OKLCH accent matrix 可追溯，且源码不引入 Tailwind、CSS-in-JS 或私有字体。
- Display wiring tests 证明 theme / dark / `customAccent` 切换即时更新根级属性和 computed variable。
- Custom accent picker 只显示色相/饱和度；旧 preview/value/reset UI 与 `onClear` / `active` 冗余 props 零引用；选择 Ocean / Plum / Forest 均能清晰退出 custom accent。
- TopBar/Auth/Settings visual tests 证明圆形用户名首字符单一设置入口、Account/Privacy 单页、destructive/unavailable states、className、i18n 和可访问性行为与当前 App shell 合同一致；旧固定 `E`、账号 menu/tab/static blocks 零引用。
- Token/source/visual tests 证明所有 inventory 中的有框 action 消费 `--ei-radius-control: 8px`，旧 `2px`/`--ei-radius-sm` 尖角 action 零残留；圆形、pill、无边框 link/back 与非按钮 surface 不被误改。
...
- `frontend/README.md` 同步为圆形 `E` 单一设置入口、无 tab Settings 与固定三字体栈的当前接入点，同时保留业务 owner 扩展规则。
...
RED 由 `TopBarVisual.test.tsx` 固化 76px desktop header、品牌/导航比例、active nav soft pill、sun + track dark control、语言 pill 与单一圆形 `E` settings entry，同时保留三个 route、40px hit target、accessible settings name、mobile content-driven wrap 和 no-overflow。GREEN 只修改正式 TopBar DOM/CSS，不恢复 theme menu、用户姓名/头像数据、dropdown 或 TopBar logout。Chrome 与 Home Phase 25 一起在 `1916x821` 和 `390x844` 验收。

### Phase 23: Framed action radius consistency

RED 先在 token/source contract 中要求 `--ei-radius-control: 8px`，并枚举正式前端有框 action consumer，复现 Settings 退出/注销、TopBar 未登录入口、Parse/Practice/Report/Generating 恢复动作等 `2px` 或页面私有 `6px/7px` 漂移。GREEN 最小增加语义 token，并把 inventory 中的 action 显式迁移到该 token；不使用全局 `button` selector，不修改 circular/pill、borderless back/link、card/input/status surface。focused/root/typecheck/build 通过后，在 Chrome desktop/mobile 验收 Settings 与跨页面样本的 computed `8px`、状态语义和 viewport containment。

### Phase 24: Ocean, Plum, and Forest preset palette

RED 先在 `tokens.test.ts`、DisplayPreferences/Settings tests 与 visual source contract 中要求 `THEME_KEYS=[ocean,plum,forest]`、三条 metadata/locale、六组 light/dark selector，以及用户确认的三套 accent/accent-soft OKLCH 值；现有两主题 hex palette、Forest invalid fixture 和三个一级选项必须目标失败。GREEN 最小扩展 Theme union、palette data、checked-in CSS、root wiring、Settings metadata/i18n 与 dev mock，保持语义 token、Custom overlay、Ocean fallback、primary row、Save 状态机和 viewport layout 不变。`BDD.SHELL.PRESET.003` 由 token/component/responsive tests 与 current-run Chrome light/dark desktop/mobile 验收承接；账号持久化和真实重登复用 001 的 `BDD.SHELL.SETTINGS.PRESET.007` / `E2E.P0.101`。

## 7 修订记录

| 日期 | 版本 | 说明 |
|------|------|------|
| 2026-07-20 | 3.8 | Complete Phase 24 after the six-selector OKLCH matrix, responsive real-Chromium acceptance, full regression and owner gates passed. |
| 2026-07-20 | 3.7 | Reopen Phase 24 to add Forest and replace the Ocean/Plum accents with the confirmed six-combination OKLCH preset matrix. |
| 2026-07-20 | 3.6 | Reopen Phase 23 to standardize framed rectangular and square actions on an 8px semantic control radius without changing circular, pill, borderless or non-button surfaces. |
| 2026-07-19 | 3.5 | Reopen Phase 22 to align Home-visible desktop chrome with the supplied reference while preserving the single settings action and all shell behavior. |
