...
本 owner 固化正式前端 App shell 的视觉系统合同：维护 token、主题、暗色、`customAccent`、固定字体栈、TopBar、认证页、设置页与通用 screen shell，并保持路由、认证、pendingAction、i18n 和 mock runtime 行为一致。Phase 23 已完成跨页面操作按钮圆角一致性修订。

正式前端的可见样式只能来自 `frontend/src`，其架构、流程与交互约束来自 `docs/ui-design/`。本 owner 不创建业务页面细节、不扩大 route catalog、不引入外部设计系统、不复制 prototype data。

Phase 1-22 的已勾选内容只保留历史交付证据；Phase 23 是当前视觉 owner，并取代历史固定 `E` initial 与页面私有尖角 action 的旧正向描述。实现不得为历史证据保留兼容 UI、metadata、字体资产或尖角按钮例外。
...
`frontend/src/app/theme/` 以 CSS variables 表达语义 token。TS 侧只导出语义键名，不能导出 hex 字面量；`themes.css` 必须覆盖 ocean / plum × light / dark 4 个组合；默认与 fallback 均为 `ocean`。`--ei-radius-control: 8px` 是有框矩形/方形操作按钮的唯一共享圆角 token，不得通过修改 `sm/md` 或全局 `button` selector 牵连卡片、输入框和状态 surface。Settings Appearance 只暴露 `深海 / Ocean`、`梅子 / Plum` 和 `自定义 / Custom`，不得再展示 `暖陶 / Warm` 或 `苔林 / Forest`。
