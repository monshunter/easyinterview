...
- lint、fixture、codegen、backend 与 frontend focused gate 全部通过后才允许原地 re-freeze v1.0.0 baseline。
- 任一 consumer 未迁移时整体回滚本 correction；不得把 `contentLimits` 改成 optional 来维持旧客户端形态。

## 4 验证边界

- OpenAPI 只持有 closed schema、fixture 与 generated artifact drift；A4 platform config 持有默认值、override 和非法值的单一 typed contract suite。
- consumer 只为 UTF-8 byte 错误语义或 UI 提交前提示保留一个小型 focused test，不在每层复制 fixture 默认值和 `limit / limit+1`。
- 本决策不拥有独立用户流程，不新增、扩展或重复运行 BDD / scenario 来证明配置注入或默认数值传播；domain 场景继续验证各自业务流程。
- 测试通过小型注入值或 file metadata 构造边界，不创建 10MiB/384KiB 等默认大小测试文件。
...
| 2026-07-14 | 1.1 | 收敛为 schema/codegen + 单一 typed config contract，不以跨层重复单测或场景环境证明配置传播。 |
