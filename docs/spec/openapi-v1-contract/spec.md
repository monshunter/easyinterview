...
- **endpoint 集**：38 端点，覆盖当前 P0 contract；本 spec §3.1.1 列出 v1.0.0 freeze 时的 endpoint 列表。任何新增 endpoint、tag、schema 或 fixture 都必须先修订本 spec、OpenAPI baseline、fixtures、generated artifacts 与 inventory lint。
...
- **fixtures**：每个 operation 对应一份默认 fixture（`scenario: default`）；其它 named scenario 由相应 consumer owner 在同一 fixture 中维护（与 [engineering-roadmap §4.3 mock-first](../engineering-roadmap/spec.md#43-契约与-mock-first-约束) 一致）。
...
| D-9 | v1.0.0 freeze 范围 | §3.1.1 列出当前 38 个 endpoint + 10 tag；Auth 使用 email-code challenge + session cookie；`DELETE /api/v1/me`、`getReportConversation`、`regenerateFeedbackReport`、`createPracticeVoiceTurn`、`updateMe`、扁平 Resume operations、`duplicateResume`、`getResumeSource` 和 `archiveTargetJob` 均属于当前 freeze。 | 任何 breaking change 必须 ADR + spec 修订；pre-launch correction 同步 history、baseline、fixtures、generated artifacts 和 diff-config |
...
| D-21 | Report-owned conversation / session-list removal | 用户于 2026-07-15 明确批准移除没有产品入口价值的 `GET /api/v1/practice/sessions` / `listPracticeSessions` 公共列表，并以 `GET /api/v1/reports/{reportId}/conversation` / `getReportConversation` 一对一替换；该 correction 当时保持 37 operations / 10 tags，当前由 D-40 additive 扩展为 38 / 10。response 为 closed `ReportConversation{reportId,reportStatus,context,messages}`，message 仅含 `sequence/role/content/createdAt`，其中 `content` 必须 `minLength: 1` 且匹配 `\S`，不得暴露 session/message/replay/anchor locator。保留 `startPracticeSession` 与 `getPracticeSession` 作为创建与 live recovery。 | 删除 path、fixture、`PaginatedPracticeSession`（若无其它引用）、generated method/server、inventory/mock/handler/service/store 正向 surface；不保留 deprecated、空列表或 redirect 兼容。新 endpoint 只按 `reportId + current user` 读取归属报告绑定的 session 与有序消息，不新增数据库关系表。OPENAPI-001 v1.7 承接 exact old-baseline correction。 |
...
| D-33 | Report action / retry responsibility | `ReportNextAction.label.minLength=1/maxLength=200`只作wire/schema malformed fuse。24/64与18/52由F3/runtime/frontend承接。单次 generation/judge 各自最多4调用、dynamic scope、attempt/retry/reason/scope与业务/infra backoff均是内部合同；HTTP只保留`queued/generating/ready/failed`和既有payload/error/provenance，不新增attempt/progress字段。D-40只增加 failed 后由用户显式触发的新生成动作，不能把内部 retry 暴露为 UI attempt。客户端窗口耗尽不能伪造server failed。 |
...
| D-39 | Settings current-user projection pruning | accepted [OPENAPI-007](./decisions/OPENAPI-007-settings-user-context-pruning.md)：`UserContext` 收敛为 closed required `{id,email,displayName,profileCompletionRequired}`，删除 `uiLanguage/preferredPracticeLanguage/emailMasked`，不保留 optional alias/default。 | `getMe/completeMyProfile` method/path/operationId/status、email-code/session/profile-completion 与 deleteMe 均不变；完整 email 只在 authenticated success body 中返回，日志/证据继续脱敏；001/002/003、backend-auth、frontend-shell、mock 与 B4 migration 同批迁移后才可 re-freeze。 |
| D-41 | Account theme and generic current-user update | accepted [OPENAPI-008](./decisions/OPENAPI-008-account-theme-update-me.md)：protected `PATCH /api/v1/me` 原地从 `completeMyProfile` 泛化为 `updateMe`，以 closed `UpdateMeRequest` 同时承接资料补全和账号 `displayPreferences`；`UserContext` required 新增 closed theme projection。 | 保持 path/method/200/session security、38/10 inventory 与 profile-completion 语义；账号主题由登录 bootstrap 的 `getMe` 一次取得，保存只发一次 PATCH 且无 follow-up GET；B2、backend-auth、B4、frontend-shell、mock 与 E2E.P0.101 同批迁移后才 re-freeze。 |
| D-40 | Failed report manual regeneration | additive 新增 protected `POST /api/v1/reports/{reportId}/regenerate`，operationId `regenerateFeedbackReport`，无 request body，必带 `Idempotency-Key`，成功返回 `202 + ReportWithJob`；`reportId` 与 `job.resourceId` 必须等于 path ID，job 必须为 queued `report_generate/feedback_report`。 | 只接受当前用户 owned、非 `REPORT_CONTEXT_TOO_LARGE` 的 failed report；同 key 重放同一响应，非 failed 返回 `409 REPORT_INVALID_STATE_TRANSITION`，旧 active job 短窗口返回 retryable `409 REPORT_NOT_READY`，missing/cross-user 为 hidden 404。原 report row 与 frozen session/context 保持不变，只重置生成结果并创建新 job。 |
| D-25 | Auth single-entry profile completion | 当前 38-operation freeze 包含 protected `PATCH /api/v1/me updateMe`；邮箱验证码入口不区分注册/登录；`profileCompletionRequired` 标识首次资料未完成，资料补全仍由完整字段对承接。 | D-41 泛化 operation 但不改变 profile-completion 语义；OpenAPI、fixtures、generated artifacts 与 consumers 同步 |
...
| D-27 | 核心闭环 contract boundary（product-scope D-22） | 当前 freeze 为 **38 endpoint / 10 tag**。账号资料补全与账号主题由 Auth `updateMe` 承接；practice 派生计划只接受 report-derived `retry_current_round` / `next_round`。 | OpenAPI source、fixtures、baseline、generated artifacts 与 consumers 同步；真实 API/UI 核心闭环由 product-scope owner 独立验收。 |
...
| D-28 | Resume PDF source preview | 当前 38-operation freeze 包含 protected `GET /api/v1/resumes/{resumeId}/source` operationId `getResumeSource`；仅用于 upload-backed PDF 原件 inline 预览，返回 `application/pdf` binary；paste、Markdown、TXT、missing、archived 和 cross-user 均返回 404。 | `openapi/openapi.yaml`、`openapi/fixtures/Resumes/getResumeSource.json`、`scripts/lint/openapi_inventory.py`、Go/TS generated artifacts、frontend-resume-workshop/001 Phase 8 与 backend-resume/001 Phase 12 保持同步 |
...
| 2 | Auth | PATCH | /api/v1/me | updateMe | UpdateMeRequest / UserContext |
...
| 23 | Reports | GET | /api/v1/reports/{reportId}/conversation | getReportConversation | ReportConversation |
...
| 38 | Reports | POST | /api/v1/reports/{reportId}/regenerate | regenerateFeedbackReport | ReportWithJob（IK 必带；同 reportId 重新排队） |

总计 38 个 endpoint，覆盖 10 tag。当前 freeze 包含 Auth 单入口资料补全、扁平 Resume operations、PDF source preview、TargetJob archive、ResumeTailor ephemeral suggestions、PracticeSessions live recovery / voice turn、report-owned conversation、failed report regeneration、Reports、Jobs 与 Privacy request contract。

> Auth current-user update (#2) 由 OPENAPI-008、backend-auth/001 与 frontend-shell/001 纳入 freeze：`PATCH /api/v1/me updateMe`、`UserContext.profileCompletionRequired/displayPreferences`、`UpdateMeRequest`、fixture 与 generated artifacts 同步；真实 handler 由 backend-auth 承接。

> `getReportConversation` (#23) 是报告详情的只读附件读取；Practice recovery 只保留 `startPracticeSession` 与 `getPracticeSession`。公共 session list 已按 D-21 删除，不存在兼容入口。

> Practice voice turn (#28) 由 `practice-voice-mvp/001-cascaded-stt-llm-tts` 纳入 v1.0.0 freeze：`PracticeSessions` tag 新增 side-effect `createPracticeVoiceTurn` operation、voice turn request/response schema、fixture、generated client/server artifact、inventory lint 与 fixture validator 已回填；真实 backend handler 由同计划 Phase 5 承接。
...
| Auth / runtime | `UserContext`、`AuthEmailStartRequest`、`UpdateMeRequest`、`AuthEmailVerifyQuery`、`Session`、`RuntimeConfig`、`ContentLimits`、`DeleteMeResponse`（alias `PrivacyRequestWithJob`） | `UserContext` closed required 精确含 `id/email/displayName/profileCompletionRequired/displayPreferences`；`UpdateMeRequest` closed 且支持 profile-only、theme-only 与 combined `updateMe`，empty/invalid 请求整体拒绝；`displayPreferences` closed required 为 `theme + customAccent`，非法服务端投影由前端 fail closed 到 `ocean + null`；`email` 是 authenticated 账号完整值，`emailMasked` 不存在；`ContentLimits` closed required 精确含五个 positive int64；runtime-config 不公开 report/HTTP/provider/profile limits；其余字段以 A4 D-2 为准 |
...
| Review | `FeedbackReport`、`ReportContextSnapshot`、`ReportConversation`、`ReportConversationMessage`、`DimensionAssessment`、`ReportHighlight`、`ReportIssue`、`ReportNextAction`、`TargetJobReportsOverview` 及其轻量 round summaries | closed report schemas；conversation 复用 frozen `ReportContextSnapshot`，message 只允许 `sequence/role/content/createdAt` 并按 sequence 严格递增；详情输出 grounded direct semantics，TargetJob overview 只输出 canonical round identity、current ready locator/timestamp 与 latest attempt status/error/timestamp，不输出 full report、provenance/model/rubric 或内部 locator |
...
- fixture named scenarios 由 `openapi/fixtures/` 直接维护，并由 `make validate-fixtures` 统一验证（B2 owner）。
...
|----|------|-------|------|------|-----------|
| C-1 | OpenAPI 文档结构 | `openapi/openapi.yaml` 已落地 | `npx -p @apidevtools/swagger-cli@4.0.4 swagger-cli validate openapi/openapi.yaml` + inventory lint | 通过；含 10 tag、38 endpoint；每个 endpoint 有 request/success 或 P0 例外/error schema；`PATCH /api/v1/me` 返回 `200 + UserContext`，`DELETE /api/v1/me` 返回 `202 + PrivacyRequestWithJob`；`archiveTargetJob` 返回 archived `TargetJob` 并要求 `Idempotency-Key`；`regenerateFeedbackReport` 返回 `202 + ReportWithJob` 并要求 `Idempotency-Key`；`ApiError` inner object / B1 shared schema 拓扑一致；Auth 路径与 ADR-Q1 `ei_session` 一致；fixture 完整性由 C-6 单独验证 | B2 001 / B2 004 / product-scope/001-core-loop-module-pruning / practice-voice-mvp 001 / backend-resume 002 / backend-auth 001 / backend-targetjob 001 Phase 12（contract/schema） |
...
| C-17 | report retry boundary | product/evalkit 使用 generation/judge max4 与内部 attempt audit；用户在 terminal failed 后显式发起新动作 | schema/inventory/fixture/generated + domain behavior | `FeedbackReport` 仍只有 queued/generating/ready/failed；无 attemptCount/retryCount/reason/scope/progress。唯一 client recovery endpoint 是 D-40 的 same-report regenerate；frontend polling timeout 不改 API terminal state | B2 001 + backend-review/001 + frontend-report-dashboard/001 |
| C-24 | failed report manual regeneration | owned failed report 与 frozen transcript 已存在 | `regenerateFeedbackReport` | 必需 IK；同一 report row 原子回到 queued 并返回 matching `ReportWithJob`；同 key replay；invalid state / active old job / context-too-large / cross-user typed fail closed 且零重复 active job | B1 + B2 001/002/003 + backend-review/001 + frontend-report-dashboard/001 |
...
| C-22 | Report-owned conversation replacement | owned report 处于 queued/generating/ready/failed，或 report/session/message 投影非法 | 调用 `getReportConversation(reportId)` | owned report row 存在即可按 report status 返回同一冻结 context 与严格递增 messages；missing/deleted/cross-user 统一 404；空 identity、未知 role、重复/非递增 sequence、额外 locator 或绑定不一致整份 fail closed。`listPracticeSessions` path/schema/fixture/generated/runtime/consumer 正向引用为零，`startPracticeSession` 与 `getPracticeSession` 不变，总量仍 37/10 | B2 001/002/003 + backend-review/001 + backend-practice/001 + frontend-report-dashboard/001 + mock-contract-suite/001 + E2E.P0.099 |
| C-23 | Settings current-user contract | authenticated/profile-incomplete 用户请求 `getMe` 或成功调用 `updateMe` | 校验 response、fixture、generated types 与 consumer | `UserContext` required 为 id/email/displayName/profileCompletionRequired/displayPreferences；登录 bootstrap 一次取得账号主题，跨路由不重复 `getMe`；主题保存单 PATCH、组合更新单事务、无 follow-up GET；旧语言字段和旧 operation 在 production surface 无正向引用 | OPENAPI-007 + OPENAPI-008 + B2 001/002/003 + backend-auth/001 + frontend-shell/001/002 + B4 001 |
...
- `001-bootstrap`：落地 `openapi/openapi.yaml` 框架、ADR-Q1 Auth 路径、`DELETE /api/v1/me` privacy deletion endpoint、privacy export 501 例外、B1 enum `$ref`、`GenerationProvenance`、`make codegen-openapi` 与本地 drift check；Phase 20 additive 新增 `regenerateFeedbackReport`，当前 freeze 为 10 tag / 38 endpoint。
- `002-fixtures-and-mock-source`：每个 operationId 一份 fixture + consumer-owned named scenarios；Phase 12 删除旧 session-list fixture 并补齐 report conversation fixture；E1 接入。
- `003-breaking-change-gate`：linter 规则集 + ADR 模板；Phase 11 承接 OPENAPI-001 v1.7 exact correction；远端 CI label workflow 仅在 A5 触发条件成立后再评估。
...
- `backend-auth/001-email-code-session-bootstrap` + `frontend-shell/001-app-shell-auth-settings`：Auth current-user contract 已泛化为 `PATCH /api/v1/me updateMe` + closed `UpdateMeRequest`；`UserContext` required 含 `profileCompletionRequired/displayPreferences`，profile-only、theme-only 与 combined update 共享 fixture、Go/TS generated artifacts 和后端事务 handler，账号主题由 frontend bootstrap 一次读取并单 PATCH 保存。
- `OPENAPI-007` + `backend-auth/001` + `frontend-shell/001` + `db-migrations-baseline/001`：D-39 settings simplification correction 删除 `UserContext.uiLanguage/preferredPracticeLanguage` 与后端无消费者 display-preference columns；001/002/003 Phase 19/13/12 承接 exact diff、fixture/codegen 与 guarded re-freeze。
- `product-scope/001-core-loop-module-pruning`：当前核心闭环范围 owner，负责 10 tag / 38 endpoint freeze、baseline、fixtures、generated artifacts、runtime route-negative tests 与 active spec zero-reference gates。
