...
本 spec 历史上由 `engineering-roadmap/001-decompose-subspecs` 的 contract lock 创建；当前执行口径以 roadmap active spec 的保留规则为准：`openapi/openapi.yaml` v1.0.0 freeze 范围为当前 55 endpoints / 13 tags / 字段命名 / additive-only 规则（D-17 JobMatch additive 升级 + D-18 Resume Workshop additive 升级均已落地）。真实 OpenAPI 文件、codegen、fixtures 与 breaking-change linter 由 B2 `001-bootstrap` / `002-fixtures-and-mock-source` / `003-breaking-change-gate` / `004-resume-additive-coverage` 分别验证；未通过前不得启动依赖 B2 的 implementation。
...
- **endpoint 集**：55 端点，覆盖当前 P0 contract；本 spec §3.1.1 列出 v1.0.0 freeze 时的 endpoint 列表。旧 `/mistakes`、`/mistakes/{mistakeId}/retest`、`/growth/overview` 已按当前 product-scope / UI 从 active contract 删除。
...
| D-9 | v1.0.0 freeze 范围 | §3.1.1 列出 55 个 endpoint + 13 tag；本 spec 锁定范围与 additive-only 规则，B2 `001` 落地 `openapi/openapi.yaml` 后强制执行（新增 endpoint / 新增可选字段 / 新增枚举值）；Auth tag 以 ADR-Q1 的 email magic link + session cookie 路径为准；`DELETE /api/v1/me` 按 ADR-Q5 纳入 P0 删除入口；D-17 additive 升级把 `JobMatch` tag + 12 operationId（jd_match 三 tab 业务）纳入 v1.0.0 freeze；D-18 additive 升级把 `Resumes` tag 9 个 Resume Workshop operation 纳入 v1.0.0 freeze | 任何 break change 必须 ADR + 本 spec 修订；本次删除旧 Mistakes / Growth 属开发期当前 product-scope / UI 授权的 freeze correction |
...
| 35 | JobMatch | GET | /api/v1/jd-match/profile | getJobMatchProfile | JobMatchProfile |
| 36 | JobMatch | GET | /api/v1/jd-match/agent-status | getAgentScanStatus | AgentScanStatus |
| 37 | JobMatch | GET | /api/v1/jd-match/recommendations | listJobRecommendations | PaginatedJobMatchRecommendation |
| 38 | JobMatch | GET | /api/v1/jd-match/recommendations/{jobMatchId} | getJobRecommendation | JobMatchRecommendation |
| 39 | JobMatch | POST | /api/v1/jd-match/recommendations/{jobMatchId}/dismiss | markJobNotRelevant | MarkNotRelevantRequest / MarkNotRelevantResult（IK 必带） |
| 40 | JobMatch | GET | /api/v1/jd-match/watchlist | listWatchlist | WatchlistResponse |
| 41 | JobMatch | POST | /api/v1/jd-match/watchlist | addToWatchlist | AddToWatchlistRequest / WatchlistItem（IK 必带） |
| 42 | JobMatch | DELETE | /api/v1/jd-match/watchlist/{jobMatchId} | removeFromWatchlist | 204（IK 必带） |
| 43 | JobMatch | POST | /api/v1/jd-match/search | searchJobs | SearchJobsRequest / SearchJobsResponse（IK 必带） |
| 44 | JobMatch | GET | /api/v1/jd-match/saved-searches | listSavedSearches | SavedSearchesResponse |
| 45 | JobMatch | POST | /api/v1/jd-match/saved-searches | createSavedSearch | CreateSavedSearchRequest / SavedSearch（IK 必带） |
| 46 | JobMatch | GET | /api/v1/jd-match/market-signals | getMarketSignals | MarketSignalsResponse |
| 47 | Resumes | GET | /api/v1/resumes | listResumes | PaginatedResumeAsset |
| 48 | Resumes | GET | /api/v1/resumes/{resumeAssetId}/versions | listResumeVersions | PaginatedResumeVersion |
| 49 | Resumes | GET | /api/v1/resume-versions/{resumeVersionId} | getResumeVersion | ResumeVersion |
| 50 | Resumes | POST | /api/v1/resume-versions | branchResumeVersion | BranchResumeVersionRequest / ResumeVersion（IK 必带；`ai_select` 可返回 202 + job） |
| 51 | Resumes | PATCH | /api/v1/resume-versions/{resumeVersionId} | updateResumeVersion | UpdateResumeVersionRequest / ResumeVersion（IK 必带） |
| 52 | Resumes | POST | /api/v1/resume-versions/{resumeVersionId}/suggestions/{suggestionId}/accept | acceptResumeTailorSuggestion | ResumeVersion（IK 必带） |
| 53 | Resumes | POST | /api/v1/resume-versions/{resumeVersionId}/suggestions/{suggestionId}/reject | rejectResumeTailorSuggestion | ResumeVersion（IK 必带） |
| 54 | Resumes | POST | /api/v1/resumes/{resumeAssetId}/archive | archiveResumeAsset | ResumeAsset（IK 必带） |
| 55 | Resumes | POST | /api/v1/resume-versions/{resumeVersionId}/exports | exportResumeVersion | ApiErrorResponse（P0 501 + `RESUME_EXPORT_NOT_AVAILABLE`；IK 必带） |

总计 55 个 endpoint，覆盖 13 tag。

> JobMatch (35–46) 由 `frontend-home-job-picks-and-parse/002-jd-match-recommendations` 在 D-17 additive 升级中纳入 v1.0.0 freeze；真实 backend handler / store / agent scan pipeline / 真实联网搜索 / 候选池抓取由独立未来 subspec `backend-jobs-recommendations` 承接。

> Resume Workshop additive (47–55) 由 `openapi-v1-contract/004-resume-additive-coverage` 在 D-18 additive 升级中纳入 v1.0.0 freeze：Resumes tag 9 个新 operation、schema、fixtures、generated client/server artifact 与 inventory lint 已回填；真实 backend handler 由独立未来 subspec `backend-resume` / `backend-upload` 承接。
...
| Uploads / resumes | `UploadPresignRequest`、`UploadPresign`、`RegisterResumeRequest`、`ResumeAsset`、`ResumeAssetWithJob`、`ResumeVersion`、`PaginatedResumeAsset`、`PaginatedResumeVersion`、`BranchResumeVersionRequest`、`UpdateResumeVersionRequest` | B2 owns request/response schema and fixture provenance；Resume Workshop D-18 新 schema 均位于 `Resumes` tag，不新建 `ResumeVersions` tag |
...
- `ApiError` schema 必须表示 B1 提供的 inner error object；`ApiErrorResponse` schema 必须是 `{error: ApiError}` envelope。`error.code` 字段定义为枚举（值集等于 [B1 D-5](../shared-conventions-codified/spec.md#31-已锁定决策) 全部错误码常量加本 spec D-12 的 `PRIVACY_EXPORT_NOT_AVAILABLE`），由 generator 自动同步；`RESUME_EXPORT_NOT_AVAILABLE` 已由 B1 D-10 落地。
...
|----|------|-------|------|------|-----------|
| C-1 | OpenAPI 文档结构 | `openapi/openapi.yaml` 已落地 | `npx -p @apidevtools/swagger-cli@4.0.4 swagger-cli validate openapi/openapi.yaml` + inventory lint | 通过；含 13 tag、55 endpoint；每个 endpoint 有 request/success 或 P0 例外/error schema；`DELETE /api/v1/me` 返回 `202 + PrivacyRequestWithJob`；`ApiError` inner object / B1 shared schema 拓扑一致；Auth 路径与 ADR-Q1 `ei_session` 一致；fixture 完整性由 C-6 单独验证 | B2 001 / B2 004（contract/schema） |
...
- `004-resume-additive-coverage`：D-18 Resume Workshop additive 升级落地：`Resumes` tag 扩容 9 operationId + 6 schema + `RegisterResumeRequest` additive 扩展 + 6 IK_REQUIRED + `ResumeVersion` 入 `AI_PROVENANCE_SCHEMAS`；同步 `RESUME_EXPORT_NOT_AVAILABLE` 错误码 + 3 新 enum `$ref` B1 D-10；fixtures（default / paginated / empty / processing / failed 多 variant）+ codegen drift + inventory lint 46 → 55 已完成。
