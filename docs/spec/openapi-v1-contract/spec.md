...
| 37 | JobMatch | GET | /api/v1/jd-match/profile | getJobMatchProfile | JobMatchProfile |
| 38 | JobMatch | GET | /api/v1/jd-match/agent-status | getAgentScanStatus | AgentScanStatus |
| 39 | JobMatch | GET | /api/v1/jd-match/recommendations | listJobRecommendations | PaginatedJobMatchRecommendation |
| 40 | JobMatch | GET | /api/v1/jd-match/recommendations/{jobMatchId} | getJobRecommendation | JobMatchRecommendation |
| 41 | JobMatch | POST | /api/v1/jd-match/recommendations/{jobMatchId}/dismiss | markJobNotRelevant | MarkNotRelevantRequest / MarkNotRelevantResult（IK 必带） |
| 42 | JobMatch | GET | /api/v1/jd-match/watchlist | listWatchlist | WatchlistResponse |
| 43 | JobMatch | POST | /api/v1/jd-match/watchlist | addToWatchlist | AddToWatchlistRequest / WatchlistItem（IK 必带） |
| 44 | JobMatch | DELETE | /api/v1/jd-match/watchlist/{jobMatchId} | removeFromWatchlist | 204（IK 必带） |
| 45 | JobMatch | POST | /api/v1/jd-match/search | searchJobs | SearchJobsRequest / SearchJobsResponse（IK 必带） |
| 46 | JobMatch | GET | /api/v1/jd-match/saved-searches | listSavedSearches | SavedSearchesResponse |
| 47 | JobMatch | POST | /api/v1/jd-match/saved-searches | createSavedSearch | CreateSavedSearchRequest / SavedSearch（IK 必带） |
| 48 | JobMatch | GET | /api/v1/jd-match/market-signals | getMarketSignals | MarketSignalsResponse |
| 49 | Resumes | GET | /api/v1/resumes | listResumes | PaginatedResumeAsset |
| 50 | Resumes | POST | /api/v1/resumes/{resumeAssetId}/structured-master | confirmResumeStructuredMaster | ConfirmResumeStructuredMasterRequest / ResumeVersion（201；409 `RESUME_STRUCTURED_MASTER_ALREADY_EXISTS`；IK 必带） |
| 51 | Resumes | GET | /api/v1/resumes/{resumeAssetId}/versions | listResumeVersions | PaginatedResumeVersion |
| 52 | Resumes | GET | /api/v1/resume-versions/{resumeVersionId} | getResumeVersion | ResumeVersion |
| 53 | Resumes | POST | /api/v1/resume-versions | branchResumeVersion | BranchResumeVersionRequest / ResumeVersion / BranchResumeVersionAccepted（IK 必带；`ai_select` 返回 202 + job） |
| 54 | Resumes | PATCH | /api/v1/resume-versions/{resumeVersionId} | updateResumeVersion | UpdateResumeVersionRequest / ResumeVersion（IK 必带） |
| 55 | Resumes | POST | /api/v1/resume-versions/{resumeVersionId}/suggestions/{suggestionId}/accept | acceptResumeTailorSuggestion | ResumeVersion（IK 必带） |
| 56 | Resumes | POST | /api/v1/resume-versions/{resumeVersionId}/suggestions/{suggestionId}/reject | rejectResumeTailorSuggestion | ResumeVersion（IK 必带） |
| 57 | Resumes | POST | /api/v1/resumes/{resumeAssetId}/archive | archiveResumeAsset | ResumeAsset（IK 必带） |
| 58 | Resumes | POST | /api/v1/resume-versions/{resumeVersionId}/exports | exportResumeVersion | ApiErrorResponse（P0 501 + `RESUME_EXPORT_NOT_AVAILABLE`；IK 必带） |
| 59 | PracticeSessions | GET | /api/v1/practice/sessions | listPracticeSessions | PaginatedPracticeSession |
| 60 | PracticeSessions | POST | /api/v1/practice/sessions/{sessionId}/voice-turns | createPracticeVoiceTurn | CreatePracticeVoiceTurnRequest / PracticeVoiceTurnResult（IK 必带） |
...
> JobMatch (37–48) 由 `frontend-home-job-picks-and-parse/002-jd-match-recommendations` 在 D-17 additive 升级中纳入 v1.0.0 freeze；真实 backend handler / store / agent scan pipeline / 真实联网搜索 / 候选池抓取由独立未来 subspec `backend-jobs-recommendations` 承接。
