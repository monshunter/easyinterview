...
- **schema 定义**：所有 endpoint request / success 或 P0 例外 response / async wrapper / error response 必须出现在 §4.2 schema inventory，或显式声明无 body / 无响应体；共享 `ApiError` / `PageInfo` / `PaginatedXxx` 与 14 个枚举类型引用 [B1 D-5/D-6](../shared-conventions-codified/spec.md#31-已锁定决策)，不得在 OpenAPI 内重复维护 enum 字面量。
...
|----|------|-------|------|------|-----------|
| C-1 | OpenAPI 文档结构 | `openapi/openapi.yaml` 已落地 | `npx @apidevtools/swagger-cli validate openapi/openapi.yaml` + inventory lint | 通过；含 14 tag、36 endpoint；每个 endpoint 有 request/success 或 P0 例外/error schema；共享 schema 全部 `$ref` 到 B1；Auth 路径与 ADR-Q1 一致；fixture 完整性由 C-6 单独验证 | B2 001（contract/schema） |
