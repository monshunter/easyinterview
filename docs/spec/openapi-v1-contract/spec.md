...
总计 37 个 endpoint，覆盖 10 tag。当前 freeze 包含 Auth 单入口资料补全、扁平 Resume operations、PDF source preview、TargetJob archive、ResumeTailor ephemeral suggestions、PracticeSessions live recovery / voice turn、report-owned conversation、Reports、Jobs 与 Privacy request contract。
...
- `001-bootstrap`：落地 `openapi/openapi.yaml` 框架、ADR-Q1 Auth 路径、`DELETE /api/v1/me` privacy deletion endpoint、privacy export 501 例外、B1 enum `$ref`、`GenerationProvenance`、`make codegen-openapi` 与本地 drift check；Phase 18 以 `getReportConversation` 一对一替换 `listPracticeSessions`，当前 freeze 仍为 10 tag / 37 endpoint。
