...
| 50 | Resumes | POST | /api/v1/resume-versions | branchResumeVersion | BranchResumeVersionRequest / ResumeVersion / BranchResumeVersionAccepted（IK 必带；`ai_select` 返回 202 + job） |
...
| Uploads / resumes | `UploadPresignRequest`、`UploadPresign`、`RegisterResumeRequest`、`ResumeAsset`、`ResumeAssetWithJob`、`ResumeVersion`、`BranchResumeVersionAccepted`、`PaginatedResumeAsset`、`PaginatedResumeVersion`、`BranchResumeVersionRequest`、`UpdateResumeVersionRequest` | B2 owns request/response schema and fixture provenance；Resume Workshop D-18 新 schema 均位于 `Resumes` tag，不新建 `ResumeVersions` tag |
