...
本 spec 历史上由 `engineering-roadmap/001-decompose-subspecs` 的 contract lock 创建；当前执行口径以 roadmap active spec 的保留规则为准：`openapi/openapi.yaml` v1.0.0 freeze 范围为当前 48 endpoints / 12 tags（2026-06-13 起：JobMatch tag 12 个 operation 随 product-scope v2.1 D-17 删除） / 字段命名 / additive-only 规则（D-17 JobMatch additive 升级 + D-18 Resume Workshop additive 升级 + D-20 Debrief suggestions additive 升级 + D-21 Practice sessions listing additive 升级 + D-22 Practice voice turn additive 升级 + D-23 Backend Resume structured master additive 升级 + D-25 Auth single-entry profile completion 均已落地）。真实 OpenAPI 文件、codegen、fixtures 与 breaking-change linter 由 B2 `001-bootstrap` / `002-fixtures-and-mock-source` / `003-breaking-change-gate` / `004-resume-additive-coverage` 分别验证；未通过前不得启动依赖 B2 的 implementation。
...
- **12 个 tag**：1. `Auth`、2. `Uploads`、3. `Profile`、4. `Resumes`、5. `TargetJobs`、6. `PracticePlans`、7. `PracticeSessions`、8. `Reports`、9. `ResumeTailor`、10. `Debriefs`、11. `Jobs`、12. `Privacy`。旧 `Mistakes` / `Growth` tag 已按当前 product-scope / UI 移除；`JobMatch` tag 与其 12 个 operation 已随 product-scope v2.1 D-17 删除（2026-06-13）。
- **endpoint 集**：60 端点，覆盖当前 P0 contract；本 spec §3.1.1 列出 v1.0.0 freeze 时的 endpoint 列表。旧 `/mistakes`、`/mistakes/{mistakeId}/retest`、`/growth/overview` 已按当前 product-scope / UI 从 active contract 删除。
...
| D-9 | v1.0.0 freeze 范围 | §3.1.1 列出 48 个 endpoint + 12 tag（D-17 JobMatch 已随 product-scope v2.1 删除）；本 spec 锁定范围与 additive-only 规则，B2 `001` 落地 `openapi/openapi.yaml` 后强制执行（新增 endpoint / 新增可选字段 / 新增枚举值）；Auth tag 以 ADR-Q1 的 email-code challenge + session cookie 路径为准，`startAuthEmailChallenge` 只接收邮箱和 `returnTo`，不得用 `purpose` 或 `displayName` 区分注册/登录；`verifyAuthEmailChallenge` 消费 query `token` 但其语义为 6 位 code；`DELETE /api/v1/me` 按 ADR-Q5 纳入 P0 删除入口；历史 D-17 additive 曾把 `JobMatch` tag + 12 operationId 纳入 freeze（2026-06-13 已随 product-scope v2.1 D-17 删除）；D-18 additive 升级把 `Resumes` tag 9 个 Resume Workshop operation 纳入 v1.0.0 freeze；D-20 additive 升级把 `Debriefs` tag 的 `suggestDebriefQuestions` 纳入 freeze；D-21 additive 升级把 `PracticeSessions` tag 的 `listPracticeSessions` 纳入 freeze；D-22 additive 升级把 `PracticeSessions` tag 的 `createPracticeVoiceTurn` 纳入 freeze；D-23 additive 升级把 `Resumes` tag 的 `confirmResumeStructuredMaster` 纳入 freeze；D-25 把 Auth 单入口登录和首次资料补全纳入 freeze | 任何 break change 必须 ADR + 本 spec 修订；本次删除旧 Mistakes / Growth 以及 pre-launch auth 注册/登录分离字段收敛属当前 product-scope / UI 授权的 freeze correction |
...
| 8 | Profile | GET | /api/v1/profiles/me | getMyProfile | CandidateProfile |
| 9 | Profile | PATCH | /api/v1/profiles/me | updateMyProfile | CandidateProfile |
| 10 | Profile | GET | /api/v1/profiles/me/experience-cards | listExperienceCards | PaginatedExperienceCard |
| 11 | Profile | POST | /api/v1/profiles/me/experience-cards | createExperienceCard | ExperienceCard |
| 12 | Profile | PATCH | /api/v1/profiles/me/experience-cards/{cardId} | updateExperienceCard | ExperienceCard |
| 13 | Resumes | POST | /api/v1/resumes | registerResume | ResumeAssetWithJob |
| 14 | Resumes | GET | /api/v1/resumes/{resumeAssetId} | getResume | ResumeAsset |
...
| 21 | PracticeSessions | POST | /api/v1/practice/sessions | startPracticeSession | PracticeSession |
| 22 | PracticeSessions | GET | /api/v1/practice/sessions/{sessionId} | getPracticeSession | PracticeSession |
| 23 | PracticeSessions | POST | /api/v1/practice/sessions/{sessionId}/events | appendSessionEvent | SessionEventResult |
| 24 | PracticeSessions | POST | /api/v1/practice/sessions/{sessionId}/complete | completePracticeSession | ReportWithJob |
| 25 | Reports | GET | /api/v1/reports/{reportId} | getFeedbackReport | FeedbackReport |
| 26 | Reports | GET | /api/v1/targets/{targetJobId}/reports | listTargetJobReports | PaginatedFeedbackReport |
...
| 30 | Debriefs | POST | /api/v1/debriefs/question-suggestions | suggestDebriefQuestions | SuggestDebriefQuestionsRequest / SuggestDebriefQuestionsResponse |
| 31 | Debriefs | GET | /api/v1/debriefs/{debriefId} | getDebrief | Debrief |
...
| 37 | Resumes | GET | /api/v1/resumes | listResumes | PaginatedResumeAsset |
| 38 | Resumes | POST | /api/v1/resumes/{resumeAssetId}/structured-master | confirmResumeStructuredMaster | ConfirmResumeStructuredMasterRequest / ResumeVersion（201；409 `RESUME_STRUCTURED_MASTER_ALREADY_EXISTS`；IK 必带） |
| 39 | Resumes | GET | /api/v1/resumes/{resumeAssetId}/versions | listResumeVersions | PaginatedResumeVersion |
| 40 | Resumes | GET | /api/v1/resume-versions/{resumeVersionId} | getResumeVersion | ResumeVersion |
| 41 | Resumes | POST | /api/v1/resume-versions | branchResumeVersion | BranchResumeVersionRequest / ResumeVersion / BranchResumeVersionAccepted（IK 必带；`ai_select` 返回 202 + job） |
| 42 | Resumes | PATCH | /api/v1/resume-versions/{resumeVersionId} | updateResumeVersion | UpdateResumeVersionRequest / ResumeVersion（IK 必带） |
| 43 | Resumes | POST | /api/v1/resume-versions/{resumeVersionId}/suggestions/{suggestionId}/accept | acceptResumeTailorSuggestion | ResumeVersion（IK 必带） |
| 44 | Resumes | POST | /api/v1/resume-versions/{resumeVersionId}/suggestions/{suggestionId}/reject | rejectResumeTailorSuggestion | ResumeVersion（IK 必带） |
| 45 | Resumes | POST | /api/v1/resumes/{resumeAssetId}/archive | archiveResumeAsset | ResumeAsset（IK 必带） |
| 46 | Resumes | POST | /api/v1/resume-versions/{resumeVersionId}/exports | exportResumeVersion | ApiErrorResponse（P0 501 + `RESUME_EXPORT_NOT_AVAILABLE`；IK 必带） |
| 47 | PracticeSessions | GET | /api/v1/practice/sessions | listPracticeSessions | PaginatedPracticeSession |
| 48 | PracticeSessions | POST | /api/v1/practice/sessions/{sessionId}/voice-turns | createPracticeVoiceTurn | CreatePracticeVoiceTurnRequest / PracticeVoiceTurnResult（IK 必带） |

总计 60 个 endpoint，覆盖 13 tag。
...
> 历史 JobMatch 12 个 operation（原行 37–48）已随 product-scope v2.1 D-17 删除（2026-06-13）；删除实施见 `backend-jobs-recommendations/001` Phase 9 与 `frontend-home-job-picks-and-parse/002` Phase 7。

> Resume Workshop / Backend Resume additive (49–58) 由 `openapi-v1-contract/004-resume-additive-coverage` 与 `backend-resume/002-versions-tailor-runs-and-save-v1` 纳入 v1.0.0 freeze：Resumes tag 10 个 additive operation、schema、fixtures、generated client/server artifact 与 inventory lint 已回填；真实 backend handler 由 `backend-resume` / `backend-upload` 承接。
...
> Practice sessions listing additive (#59) 由 `frontend-debrief/001-debrief-screen-and-handoff` Phase 0 cross-owner addendum 纳入 v1.0.0 freeze：`PracticeSessions` tag 新增 `listPracticeSessions` operation、`PaginatedPracticeSession` schema、fixture、generated client/server artifact 与 inventory lint 已回填；真实 backend handler 已由 debrief BUG-0069 remediation 补齐。

> Practice voice turn additive (#60) 由 `practice-voice-mvp/001-cascaded-stt-llm-tts` 纳入 v1.0.0 freeze：`PracticeSessions` tag 新增 side-effect `createPracticeVoiceTurn` operation、voice turn request/response schema、fixture、generated client/server artifact、inventory lint 与 fixture validator 已回填；真实 backend handler 由同计划 Phase 5 承接。
...
至少以下 schema 必须包含或可追溯到 `GenerationProvenance`：`TargetJob.summary` / `fitSummary`、`AssistantAction`、`FeedbackReport`、`ResumeTailorRun`、`Debrief`、`ResumeVersion`。`ResumeVersion` 覆盖 `structuredProfile` 与 tailor suggestion 字段。缺失 provenance 的 fixture 不得通过 `make validate-fixtures`。
...
|----|------|-------|------|------|-----------|
| C-1 | OpenAPI 文档结构 | `openapi/openapi.yaml` 已落地 | `npx -p @apidevtools/swagger-cli@4.0.4 swagger-cli validate openapi/openapi.yaml` + inventory lint | 通过；含 13 tag、60 endpoint；每个 endpoint 有 request/success 或 P0 例外/error schema；`PATCH /api/v1/me` 返回 `200 + UserContext`，`DELETE /api/v1/me` 返回 `202 + PrivacyRequestWithJob`；`ApiError` inner object / B1 shared schema 拓扑一致；Auth 路径与 ADR-Q1 `ei_session` 一致；fixture 完整性由 C-6 单独验证 | B2 001 / B2 004 / backend-debrief 001 Phase 0 / frontend-debrief 001 Phase 0 / practice-voice-mvp 001 / backend-resume 002 / backend-auth 001 Phase 8（contract/schema） |
...
- `001-bootstrap`：落地 `openapi/openapi.yaml` 框架 + 12 tag 占位 + 34 endpoint request/success 或 P0 例外/error schema + ADR-Q1 Auth 路径 + `DELETE /api/v1/me` privacy deletion endpoint + privacy export 501 例外 + B1 enum `$ref` + `GenerationProvenance` + `make codegen-openapi` + 本地 drift check。历史 D-17 additive 曾把 freeze 扩到 13 tag / 46 endpoint（JobMatch 已于 2026-06-13 随 product-scope v2.1 D-17 删除，当前 12 tag / 48 endpoint）。
