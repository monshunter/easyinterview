...
- **fixtures**：每个 operation 对应一份默认 fixture（`scenario: default`）+ `frontend/src` 折出来的 `scenario: prototype-baseline`（与 [engineering-roadmap §4.3 mock-first](../engineering-roadmap/spec.md#43-契约与-mock-first-约束) 一致）。
...
- `prototype-baseline` scenario 来自 `frontend/src`；维护方式：`make sync-fixtures-from-prototype`（B2 owner）。
