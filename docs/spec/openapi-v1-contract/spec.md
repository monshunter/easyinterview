...
| D-39 | Settings current-user projection pruning | accepted [OPENAPI-007](./decisions/OPENAPI-007-settings-user-context-pruning.md)：`UserContext` 收敛为 closed required `{id,emailMasked,displayName,profileCompletionRequired}`，删除 `uiLanguage/preferredPracticeLanguage`，不保留 optional alias/default。 | `getMe/completeMyProfile` method/path/operationId/status、email masking、email-code/session/profile-completion 与 deleteMe 均不变；001/002/003、backend-auth、frontend-shell、mock 与 B4 migration 同批迁移后才可 re-freeze。TopBar language 与 practice language 继续由各自 owner 承接。 |
...
| Auth / runtime | `UserContext`、`AuthEmailStartRequest`、`CompleteProfileRequest`、`AuthEmailVerifyQuery`、`Session`、`RuntimeConfig`、`ContentLimits`、`DeleteMeResponse`（alias `PrivacyRequestWithJob`） | `UserContext` closed required 仅含 `id/emailMasked/displayName/profileCompletionRequired`；`ContentLimits` closed required 精确含五个 positive int64；runtime-config 不公开 report/HTTP/provider/profile limits；其余字段以 A4 D-2 为准 |
...
| C-23 | Settings current-user minimal contract | authenticated/profile-incomplete 用户请求 `getMe` 或成功调用 `completeMyProfile` | 校验 response、fixture、generated types 与 consumer | `UserContext` 精确为四个 required 字段；email 仍脱敏；旧语言字段在 production schema/generated/backend/frontend/mock 中无正向引用；37/10 与 Auth methods/status/session/profile semantics 不变 | OPENAPI-007 + B2 001/002/003 + backend-auth/001 + frontend-shell/001 + B4 001 |
