...
本 plan 固化 OpenAPI 当前 flat Resume 覆盖面：`openapi/openapi.yaml` 保持 10 tag / 35 operation 合同，`Resumes` tag 只保留 flat `Resume` 资产读写、复制、归档、导出，`ResumeTailor` tag 只保留 tailor run 请求与读取。版本树、分支、suggestion accept/reject 和 version-scoped export 不再作为当前 OpenAPI operation、schema、fixture、generated client 或 inventory lint 正向面存在。

## 2 当前 Contract Surface

| Area | Current truth |
|------|---------------|
| OpenAPI inventory | `scripts/lint/openapi_inventory.py` enforces 10 tags and 35 operations |
...
| Resume fixtures | `openapi/fixtures/Resumes/{listResumes,registerResume,getResume,updateResume,duplicateResume,archiveResume,exportResume}.json` |
...
- `python3 scripts/lint/openapi_inventory.py openapi/openapi.yaml` → 10 tags / 35 operations
- `make lint-openapi`
- `make validate-fixtures`
- `make codegen-check`
- `make openapi-diff`
...
| 日期 | 版本 | 变更 |
|------|------|------|
| 2026-07-07 | 1.4 | Compress owner docs to current flat Resume / ResumeTailor OpenAPI contract, 35-operation inventory, fixtures, generated clients and 501 export gate. |
