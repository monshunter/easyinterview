...
- [x] `openapi/openapi.yaml` Resumes tag exposes only `listResumes / registerResume / getResume / updateResume / duplicateResume / archiveResume / exportResume`.
- [x] `ResumeTailor` tag exposes only `requestResumeTailor / getResumeTailorRun`.
- [x] `scripts/lint/openapi_inventory.py` enforces the current 10 tag / 35 operation inventory and current idempotency / 501 / provenance rules.
- [x] Resumes fixtures exist for all 7 current Resume operations and validate against OpenAPI.
...
- [x] OpenAPI README, fixtures README, B2 spec, mock-contract-suite and engineering-roadmap describe current 35-operation inventory.
