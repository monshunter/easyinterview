...
- [x] Trigger runs real-mode owner marker plus practice screen/event/action/body tests updated for no right panel, no dictation, no skip, no role switch and no visible strict switch.
- [x] Verify checks passing Vitest markers, current runtime testids, no prototype data helpers, no `getFeedbackReport` in practice runtime, no deleted-surface anchors and phone turn confined to owner hook.
...
- [x] Verify checks current goal visibility parity, append idempotency boundary, phone-mode user-visible labels, deleted-control absence and forbidden input markers.
...
- [x] Verify screenshots show no right panel, speech-to-text, skip, role switch, visible strict switch or voice-analysis panels.
