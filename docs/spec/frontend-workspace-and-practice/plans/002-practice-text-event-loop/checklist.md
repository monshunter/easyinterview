...
- [x] 11.1 RED-GREEN: add one shared `react-markdown + remark-gfm` message-body renderer and apply it to persisted user and assistant rows with `skipHtml` and no `rehypeRaw`.
  <!-- verified: 2026-07-14 method=focused-vitest-red-green evidence="Transcript semantic-GFM assertions failed against plain text/missing shared renderer, then passed 2/2 after both persisted roles adopted PracticeMessageBody with react-markdown, remark-gfm and skipHtml; source negative asserts no rehypeRaw." -->
- [x] 11.2 SECURITY-GATE: raw HTML is inert, remote images create no requestable `<img>`, `javascript:`/unsafe URIs are rejected, safe external links use hardened `rel`, and script/event handlers cannot execute.
  <!-- verified: 2026-07-14 method=focused-vitest-red-green evidence="Hostile-content test first failed on a requestable Markdown img; it then passed with raw HTML skipped, img renderer null, explicit protocol allowlist, unsafe anchors unwrapped, safe HTTP(S) links target=_blank plus rel=noopener noreferrer, zero fetch calls and no handler execution." -->
...
- [x] 11.6 BDD-Gate: extend existing E2E.P0.044/P0.046 for user+assistant GFM, malicious-content negatives, exact raw retry, current Workspace-detail terminal CTA and fresh desktop/mobile evidence；do not create a sibling scenario.
  <!-- verified: 2026-07-14 method=serial-scenario-run evidence="Current-source P0.044 run c71ceb11-300f-4b35-b68c-2ec726b8d4f7 and P0.046 run 04fd68b6-1ed7-49d7-b271-a37c236ea541 passed setup/trigger/verify/cleanup against one source SHA 85f46c92fd79aa61d6894b86d4b80bc8ac58d1acda9efac9cb8cc1419f761fdf; each recorded six current desktop/mobile PNGs, exact GFM/security/raw-retry/Workspace-route markers, source fingerprint PASS and isolated PostgreSQL residual=0." -->
