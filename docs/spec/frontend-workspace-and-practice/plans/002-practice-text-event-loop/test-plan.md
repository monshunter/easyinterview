...
|------|------------------|------------|
| Screen parity | `src/app/screens/practice/PracticeScreen.test.tsx`, `src/app/screens/practice/__tests__/PracticeScreenIntegration.test.tsx`, `frontend/tests/pixel-parity/practice.spec.ts` | current DOM anchors, text/phone segmented controls, no right panel, global finish CTA, responsive shell, theme/customAccent smoke |
...
| Deleted surfaces | `PracticeScreen.test.tsx`, `__tests__/nonCurrentNegative.test.ts`, scenario verify scripts | no `practice-rightpanel*`, `practice-input-dictate`, `practice-input-skip`, RoleDropdown, strict switch, voice expression metrics, manual transcript fallback UI |
...
| `E2E.P0.045` | `test/scenarios/e2e/p0-045-practice-text-loop-mode-policy-display/scripts/` | text/phone policy, current goal parity, deleted-control negative gates |
