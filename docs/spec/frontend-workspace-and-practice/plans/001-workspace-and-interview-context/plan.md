...
| `practiceMode` | startContext 默认 `strict`（与 ui-design line 99 一致） | spec D-3 二值；正式前端只允许 `assisted` / `strict`，`legacy debrief replay value` 作为旧 generated enum 负向搜索 |
