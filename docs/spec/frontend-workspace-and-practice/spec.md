...
`report` 不并入本 subspec。`report` 的 dashboard、复练当前轮、进入下一轮与报告详情交互由 `frontend-report-dashboard` / future `backend-review` owner 承接。`company_intel` 独立详情页与 route 已由 product-scope D-18 删除；本 subspec 只在 `workspace` 中保留公司轻情报摘要卡片，旧 `company_intel` route 由 shell 归一回 `workspace`，不再有外部 company-intel owner handoff。
...
- `workspace` 屏（`route=workspace`）：
  ...
  - Main Left：公司轻情报摘要卡片（嵌入式，只停留在 workspace，不跳转独立详情页）+ JD 拆解。
...
- `CompanyIntelScreen` / `getCompanyIntel` 数据源 / 合规来源详情页：已随 product-scope D-18 删除；本 spec 只保留 workspace 内嵌摘要卡片。
- Home / Parse 与 JD 导入解析业务，由 `frontend-home-job-picks-and-parse` 承接；JD Match 已随 product-scope D-17 删除。
...
| ID | 决策 | 锁定值 | 影响 |
|----|------|--------|------|
| D-1 | Route owner 范围 | 本 subspec 只接管 `workspace / practice / generating`；`report` 是外部 owner handoff；`company_intel` 独立 route 已删除并归一回 `workspace` | 消除与 `frontend-report-dashboard` 的边界冲突，避免 plan 把报告详情或旧公司情报详情混入 workspace/practice |
...
| D-3 | practice 三轴分离 | 形式 `mode/modality∈{text,voice}`、辅助度 `practiceMode∈{assisted,strict}`、数据来源 `practiceGoal/goal∈{baseline,retry_current_round,next_round}` 分离；旧 `goal='debrief'` 已退役且不得作为正向组合 | 与 backend-practice D-22 / product-scope D-22 一致；正式前端不得产出旧 `legacy debrief replay value` 或 `goal='debrief'` |
...
| D-5 | Route 最小上下文 | 本 spec owner route 使用完整 InterviewContext；外部 `report` 最小键是 `sessionId/reportId`；旧 `company_intel` 输入只作为归一到 `workspace` 的 legacy route | 不把可继承字段误判为所有 route 必填，避免无谓空态 |
...
| D-8 | company intel 嵌入边界 | workspace 可显示公司轻情报摘要和刷新/查看提示按钮，但按钮停留在 `workspace`；详情页、刷新 API、数据源、`getCompanyIntel` operation 不存在 | 防止前端在无 contract 的情况下恢复独立公司情报模块 |
...
- workspace 中的公司轻情报只复刻 `CompanyIntelEmbed` 卡片；按钮只保留当前 workspace safe params，不得导航到 `company_intel`，不得实现 `CompanyIntelScreen` 或 `getCompanyIntel` 数据源。
...
| `company_intel` | legacy alias | `targetJobId`；推荐携带 `jdId` | 归一回 `workspace`，不渲染独立详情页 |
...
| Company Intel 详情 | 已删除 | `CompanyIntelScreen`、`getCompanyIntel`、独立刷新与公开来源详情页不属于当前产品范围；仅 workspace 内嵌摘要卡片保留 |
...
| Home / Parse | `frontend-home-job-picks-and-parse` | parse confirm 跳转 workspace 与 JD 导入；JD Match 已删除 |
...
| `listPracticeSessions` | `openapi/fixtures/PracticeSessions/listPracticeSessions.json` (`default`) | Workspace session history handoff owner | backend-practice real handler | `practice_sessions` | none | workspace/history + real-mode gate |
...
| `getCompanyIntel` | N/A | Removed; workspace card consumes `getTargetJob` summary only | no handler | none | none | D-18 legacy-negative gate |

## 6 验收标准

| ID | 场景 | Given | When | Then | 对应 Plan |
|----|------|-------|------|------|-----------|
| C-1 | 三条 owner route 专属 Screen 接管 | `frontend-shell` D1 已交付，owner route 当前由 PlaceholderScreen 占位 | 进入 `workspace` / `practice` / `generating` | 三条 route 渲染正式 Screen；`practice/generating` 仍隐藏 chrome；`report` 不由本 spec 实现；旧 `company_intel` 归一回 `workspace` | 001 / 002 / 004 |
...
| C-7 | Downstream handoff 参数 | workspace 公司轻情报卡片存在；generating 成功；session history 仍缺 typed contract | 用户点击情报按钮 / 生成完成 / 查看 history 占位 | 情报按钮保持在 `workspace` 并携带 safe `targetJobId/jdId`；generating 对 `report` handoff 携带 `sessionId/reportId`；plan 001 的 workspace history 行保持 disabled placeholder，不伪造 `sessionId/reportId`；报告目标屏渲染归外部 owner | 001 / 004 |
...
- `002-practice-text-event-loop` — PracticeScreen 文本 surface + `getPracticeSession/appendSessionEvent/completePracticeSession` 消费 + assisted/strict 辅助度策略 + current core-loop goals 显隐回归 + generating 入口；2026-05-23 L2 remediation 将 P0.044-P0.047 trigger 接入 `frontendOwners.realApiMode.test.ts`。
...
不再预留本 subspec 内的 `report` plan；旧 `company_intel` 独立详情页已删除，不再创建外部 owner plan。
