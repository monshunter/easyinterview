...
- Auth operation shape 只允许按 B2 additive rule 增加非必填字段；需要 breaking change 时先修订 `openapi-v1-contract`。
...
| D-8 | 注册身份唯一性 | 邮箱是唯一账号标识，注册邮箱就是后续登录邮箱；`AuthEmailStartRequest.purpose` 显式区分 `login` / `signup`，注册页同时传 optional `displayName`；C1 将 purpose + trimmed displayName 暂存到 `auth_challenges`，verify 时 `signup` 只能创建尚未注册邮箱，`login` 只能登录已注册邮箱；displayName 不唯一、不参与账号唯一性判断 | 注册后 `/me.displayName` 与 TopBar 必须显示用户提交的显示名；重复注册同一邮箱不得覆盖既有 displayName 或隐式登录；未知邮箱登录不得隐式创建账号 |
