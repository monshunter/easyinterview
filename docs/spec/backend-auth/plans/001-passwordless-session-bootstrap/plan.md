...
- **BDD 策略**: 需要 BDD。本 plan 引入认证 API 行为和 session cookie 工作流，必须维护 `bdd-plan.md`、`bdd-checklist.md`，并在主 checklist 中使用 `BDD-Gate:` 引用 `E2E.P0.003`、`E2E.P0.101`。
...
接收邮箱和可选 `returnTo`，创建 6 位数字 challenge code hash、记录 IP / UA hash，并通过 C1 backend-internal mail dispatcher 发送一次性验证码到 dev mail sink / provider adapter。明文 code 只允许短暂存在于一次性发送边界；dev/test 读取必须通过 mail sink retrieval，不允许 log-only code delivery。发码阶段不区分注册/登录，不接收或持久化 displayName，也不得泄露邮箱是否已存在。
...
在 backend API 进程内实现 C1-owned 后台派发器：handler 只入队并返回 B2 `202` accepted，后台 goroutine / 线程 drain 队列并写入 dev mail sink 或邮件 provider adapter；测试覆盖不启动独立 worker 进程时仍可读取 challenge link、队列关闭 / graceful shutdown、派发失败不会泄露邮箱或 token。后续接入 backend internal runner 时必须沿用 2.3 的 B3 payload redaction contract。
...
#### 7.1 OpenAPI additive contract and generated clients

`AuthEmailStartRequest` 增加可选 `purpose`（`login` / `signup`，默认 `login`）与 `displayName` 字段；`verifyAuthEmailChallenge` 继续使用 query 名 `token` 以保持 wire shape，但 schema / 描述必须说明它承载 6 位数字验证码。运行 `make codegen-openapi`，禁止手改 generated Go / TS artefacts。

#### 7.2 Six-digit code + 5-minute TTL

C1 challenge token generator 默认改为 cryptographically random 6-digit numeric code，`ChallengeTTL` 固定 5 分钟；session token 仍使用 32-byte secure opaque token。focused tests 必须断言 code 格式、TTL、hash 存储和错误路径。

#### 7.3 Registration display-name persistence

`StartEmailChallengeInput.purpose` 与 `displayName` 写入 `auth_challenges.purpose` / `auth_challenges.display_name`；`ConsumeChallenge` 返回这些字段；`signup` start 时必须先检查邮箱唯一性，重复邮箱直接返回错误、不创建 challenge、不发送 code；verify 时 `signup` 创建新用户并写入 displayName，数据库冲突仍返回错误且不覆盖既有用户；`login` 只查找已注册用户，未知邮箱返回错误且不创建账号。登录页不传 displayName 时不得产生样例姓名。

#### 7.4 Code-only Mailpit / SMTP email

SMTP writer 和 dev mail sink 只暴露 6 位验证码 retrieval；邮件标题、正文和 HTML/text 样式必须强调 5 分钟有效期，不包含 magic link、`/auth/verify?token=`、完整 URL 或内部 delivery secret。`email_dispatch` payload 仍只传 redacted helper 允许字段。

#### 7.5 BDD-Gate: 验证 E2E.P0.101 通过

该历史 gate 曾把 real frontend/backend/Mailpit 登录与注册场景从 mail-link callback 改为 email-code flow。v1.7 Phase 8 已将当前验收口径改为单一邮箱登录入口 + 首次资料补全：不再使用注册页、`purpose=signup/login` 或 displayName-before-verify 作为完成证据。
...
`startAuthEmailChallenge` 不再检查或暴露账号是否存在，只创建同一类 email challenge 并发送 6 位 code。`verifyAuthEmailChallenge` 消费 challenge 后按 normalized email 查找用户：既有用户直接 mint session；新邮箱创建资料未补全账号、user_settings 和 session。旧 `signup/login` purpose 不再参与业务分支，不得出现重复注册 409 或未知邮箱登录拒绝语义。
...
#### 8.5 Privacy, metrics, and old-scope negative gates

日志、audit、metric label 仍不得包含 raw code、session cookie、完整邮箱或 PII。新增 focused negative tests / grep gate：`purpose=signup/login`、duplicate-register start rejection、displayName stored before verify、password/OAuth auth wire、magic-link URL callback 和旧 `auth_register` live contract 不得作为当前后端完成证据。

#### 8.6 BDD-Gate: 验证 E2E.P0.101 通过

更新 real frontend/backend/Mailpit 场景：新邮箱从单一登录入口完成 code verify 后 `/me.profileCompletionRequired=true`；关闭浏览器、换浏览器或退出后用同一邮箱重新登录仍返回资料未补全状态；`PATCH /me` 完成资料后 `/me.profileCompletionRequired=false`；后续同邮箱登录直接进入正常账号。场景必须证明邮箱唯一、displayName 不唯一且不参与账号唯一性判断。
...
- B3 `email_dispatch` payload helper gate 通过，raw code / URL / 邮箱明文 / 邮件正文不会进入 in-process queue、dev sink、future outbox、async payload、log 或 audit。
...
- BDD-Gate `E2E.P0.003` 与当前单入口资料补全版 `E2E.P0.101` 通过。
...
| 过早依赖独立 worker 阻塞本地验证 | 本 plan 在 C1 内实现 backend-internal goroutine / 后台线程派发器与 dev mail sink；backend internal runner 是未来替换边界，不是 P0 auth 闭环前置 |
