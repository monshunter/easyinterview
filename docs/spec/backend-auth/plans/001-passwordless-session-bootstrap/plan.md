...
接收邮箱、purpose 与可选 displayName，创建 6 位数字 challenge code hash、记录 IP / UA hash，并通过 C1 backend-internal mail dispatcher 发送一次性验证码到 dev mail sink / provider adapter。明文 code 只允许短暂存在于一次性发送边界；dev/test 读取必须通过 mail sink retrieval，不允许 log-only code delivery。注册页传入 `purpose=signup` 与 displayName，登录页传入或默认 `purpose=login`；purpose 与 displayName 必须随 challenge 持久化。
...
验证一次性 6 位验证码，根据 challenge purpose 执行账号路径：`signup` 只能创建尚未注册邮箱并写入 displayName，重复邮箱返回错误且不得覆盖既有 displayName；`login` 只能读取已注册邮箱，未知邮箱不得隐式创建账号。成功后写入 server-side session，并通过 `Set-Cookie` 返回 opaque session。
...
把 real frontend/backend/Mailpit 登录与注册场景从 mail-link callback 改为 email-code flow：先用注册页提交唯一邮箱 + displayName -> Mailpit 提取 6 位 code -> 前端 `auth_verify` 输入 code -> `/me` 为 authenticated 且 TopBar 展示注册 displayName；退出后使用同一邮箱从登录页再次提交并完成 code verify，仍进入同一账号。场景还必须覆盖重复注册同一邮箱在 start 阶段被拒、不再发送 code、不覆盖 displayName 或隐式登录，且不出现 `刘哲` / `Liu Zhe` / `liuzhe@example.com` fallback。
...
- BDD-Gate `E2E.P0.003` 与 `E2E.P0.101` 通过。
