...
- [x] Owner behavior tests 覆盖 challenge/verify、含 displayPreferences 的 `/me`、generic updateMe、组合更新原子性、session、profile completion、logout/relogin、重放与隐私。
- [x] 根 `make test` 执行对应 Go tests；该结果是代码层行为证据，不是 E2E PASS。

## `E2E.P0.101` 静态 handoff

- [x] 静态资产原地扩展真实设置齿轮、主题保存、跨页无重复 `/me`、重登恢复与 logout，且证据不保存完整邮箱；不另建场景。
