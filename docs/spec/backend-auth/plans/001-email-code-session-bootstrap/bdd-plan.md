...
|-------------|-------|------|------|----------|
| `BDD.AUTH.EMAIL.001` | 新邮箱或已有账号发起 email-code 登录 | challenge、verify、读取 `/me`、通过 `updateMe` 补全 profile 或保存主题、logout/relogin | session/profile/theme 按账号持久化；UserContext 含 closed display preferences；组合更新原子；完整 email 只在 authenticated response 中返回且不进入日志；非法/重放请求 fail closed | backend Auth contract/integration tests，由根 `make test` 承接 |
...
|--------|-------|------|------|-------|
| `E2E.P0.101` | real frontend、backend 与 Mailpit 已运行；新邮箱尚无已补全账号 | 用户登录、补全资料，在设置保存主题，跨页面切换，再退出并重新登录 | session/profile/theme 真实持久化；页面切换不重复读取 `/me`；重登恢复主题；账号证据脱敏 | `e2e-scenarios-p0/001`；本 plan 只登记业务 handoff |
