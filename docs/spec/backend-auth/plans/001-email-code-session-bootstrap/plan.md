...
- **替代验证 gate**: 不适用；BDD gate 是 API 行为验证入口。补充 gate 包括 focused Go tests、OpenAPI generated contract tests、migration dry-run / table contract、config lint、B3 `email_dispatch` helper tests、`EmailDispatchEnqueuer` / `EmailDispatchHandler` tests、F1 auth metric registry preflight、privacy log/audit grep、auth metric label checks、`make docs-check`。
...
按 `bdd-plan.md` 和 `bdd-checklist.md` 验证 email challenge -> `email_dispatch` runner delivery -> dev mail sink retrieval -> verify -> `/me` -> runtime-config session resolver -> logout -> repeated logout / deleteMe idempotency / error paths 行为。
...
- `startAuthEmailChallenge`、`email_dispatch` enqueuer / handler delivery、`verifyAuthEmailChallenge`、session middleware、`getMe`、`deleteMe` idempotent auth handoff、`logout`、A4 runtime-config session resolver integration 的 focused Go tests 通过。
