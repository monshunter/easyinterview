...
| 2026-04-27 | 1.0 | 初始创建：锁定 `golang-migrate` 工具、`migrations/` 目录与 `NNNNNN_<verb>_<noun>.{up,down}.sql` 文件命名、27 张 P0 应用表 + `schema_migrations` 元数据 + pgvector 扩展、B4 索引 inventory、`make migrate-{up,down,status,create}` target、可逆 + 数据回填策略、prod 防呆与 enum 与 B1 同源约束。 | engineering-roadmap/001 Phase 3 |
