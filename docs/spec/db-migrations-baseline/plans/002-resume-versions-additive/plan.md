...
#### 6.1 flatten up migration（含 in-migration SQL UPDATE backfill，DROP 之前）

新增 `migrations/000015_resume_flatten.up.sql`（单一事务，顺序固定）：
...
- **SQL UPDATE backfill（必须在 DROP `resume_versions` 之前）**：`UPDATE resumes SET structured_profile = rv.structured_profile, display_name = rv.display_name FROM (SELECT DISTINCT ON (resume_asset_id) resume_asset_id, structured_profile, display_name FROM resume_versions WHERE version_type = 'structured_master' AND deleted_at IS NULL ORDER BY resume_asset_id, updated_at DESC) rv WHERE resumes.id = rv.resume_asset_id;`（无 structured_master 的 resume 保留默认 `{}` / NULL）；
- `ALTER TABLE resumes DROP COLUMN guided_answers`；`source_type` CHECK 由 {`upload`, `paste`, `guided`} 改为 {`upload`, `paste`}（`DROP CONSTRAINT resume_assets_source_type_check` + `ADD CONSTRAINT resumes_source_type_check CHECK (source_type IS NULL OR source_type IN ('upload','paste'))`）；保留 `original_text` / `parsed_text_snapshot` / `raw_text` / `file_object_id`（D-20 只读保留原始来源 + 解析快照）；
- `ALTER TABLE practice_plans RENAME COLUMN resume_asset_id TO resume_id`（FK 随 `resume_assets`→`resumes` RENAME 自动指向 `resumes`，本步仅列名更名）；
...
#### 6.2 backfill 机制：in-migration SQL UPDATE（不走 Go registry）

**设计修正（实现期发现）**：`RunBackfills`（`backend/internal/migrations/backfill.go`，注释 "runs manifest-defined backfills after schema migration up"）在**所有 SQL up migration 完成之后**统一运行；因此 Go backfill registry 无法读取已被 000015 `DROP` 的 `resume_versions`。原计划的 `backfills/000015/` Go backfill 不可行。改为在 000015 up.sql 内以 **SQL `UPDATE ... FROM`**（见 6.1，`DROP` 之前）完成 structured_master → `resumes.structured_profile` 的回填，单一事务保证源表可读；**不新增 `backfills/000015/` Go 条目、不改 `migrations/backfill/manifest.yaml`**。该 copy 是简单一次性 in-migration 数据搬移，受 transactional ordering 约束（D-3 的 Go registry 面向复杂 / 可 dry-run / 大表 backfill）。

（验证：backfill 正确性由 6.1 的 schema test 覆盖——`go test ./internal/migrations/... -run TestResumeFlatten*` 断言 structured_master content 落入 `resumes.structured_profile`、无 master 留空；无 `DATABASE_URL` 时 live 明确 skip，static lint 仍 PASS）
...
`scripts/lint/migrations_lint.py::extract_sql_checks` 静态扫描**全部** up.sql 的 CHECK（按文件名排序、同 key 后号 migration 值胜出，**无 net-state**）。沿用 **product-scope D-17 处理 jd_match 的既定先例**——D-17 drop 5 张 jd_match 表（000014）后，其 CREATE migration（000009）的 CHECK 文本仍被扫描，故 `agent_scans.status` / `watchlist_items.tone` enum-sources 条目**保留至今**（已核实 enum-sources.yaml line 55-56 仍在，lint green）。本 phase 同款处理：

- **新增** `migrations/enum-sources.yaml` 一行 `resumes.source_type`（source `openapi-v1-contract.RegisterResumeRequest.sourceType`，values `[upload, paste]`，checksum = `checksum_values(["upload","paste"])` = `sha256("upload|paste").hexdigest()[:16]`），匹配 000015 新增的 `resumes.source_type` CHECK。
- **保留**所有历史 resume 条目不删（`resume_assets.parse_status` / `resume_assets.source_type` / `resume_versions.version_type` / `resume_versions.seed_strategy` / `resume_version_suggestions.status` / `resume_tailor_runs.mode` / `resume_tailor_runs.status`）——它们匹配 000001 / 000005 未变的 CHECK 文本，lint 仍要求注册（与 jd_match 同款）。**不改 `migrations_lint.py` / `B1_SOURCE_MAP`**。
- **D-20 zero-residue 豁免** enum-sources 历史 migration CHECK 条目（如 D-17 zero-residue 豁免迁移文件 + 负向断言）；zero-residue 针对 runtime schema / generated / 业务代码，不针对历史 migration 文件与其 enum-source 映射。

check negative test：插入 `resumes.source_type='guided'` 必须被 PG 拒绝（23514）。

（验证：`python3 scripts/lint/migrations_lint.py --repo-root .` PASS + `go test ./internal/migrations/... -run TestResumeFlatten*` PASS（无 DATABASE_URL 时 live skip））
...
#### 6.9 L2 hardening - legacy-row narrowing cleanup

Migration contract tests 必须覆盖 D-17 / D-20 收窄 check constraint 前的历史合法行 cleanup：`000014_drop_jd_match_module.up.sql` 在新增 narrowed `async_jobs.job_type` CHECK 前删除 `jd_match_agent_scan` / `jd_match_search` rows；`000015_resume_flatten.up.sql` 在新增 `resumes.source_type` CHECK 前把 `source_type='guided'` rows 转换为允许值。该 gate 防止开发库 / UAT 库中旧合法行让 migration 在 constraint narrowing 阶段失败。

（验证：`go test ./backend/internal/migrations -run 'TestResumeFlattenMigrationContract|TestDropJDMatchMigrationDeletesRetiredAsyncJobsBeforeNarrowingCheck' -count=1` PASS）
...
- migration narrowing 前的历史合法行 cleanup 已被 contract test 固化：`source_type='guided'` rows 先转为 `paste`，retired `jd_match_agent_scan` / `jd_match_search` async jobs 先删除，再添加 narrowed CHECK
