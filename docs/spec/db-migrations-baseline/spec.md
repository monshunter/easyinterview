...
[engineering-roadmap spec §5.1](../engineering-roadmap/spec.md#51-当前已存在的-active-spec) 将 B4 `db-migrations-baseline` 定义为当前 active Contract spec（依赖 [B1 `shared-conventions-codified`](../shared-conventions-codified/spec.md) 与 [A2 `local-dev-stack`](./../local-dev-stack/spec.md)）。当前迁移 baseline 由本 spec、`migrations/` 与 product-scope 当前范围决定：26 张当前应用表、[ADR-Q1](../engineering-roadmap/decisions/ADR-Q1-auth.md) 锁定的 3 张 auth / session 支撑表、迁移元数据表、索引与 pgvector 扩展契约落到迁移文件层，决定了：
...
4. **索引策略锁定**：当前 B-Tree、ivfflat 向量索引与可选 GIN 全文索引 inventory 由本 spec 直接承接；性能查询路径在后续业务域加表时不得遗漏索引。
...
- **26 张 P0 应用表**：与 product-scope 当前范围一致：
...
- **索引**：覆盖本 spec 索引 inventory 中的全部 B-Tree 索引 + `retrieval_chunks.embedding` 上的 `ivfflat`（默认 lists=100）+ 可选 `target_jobs` GIN 全文索引（dev 默认启用，prod 由 C4 在自己 spec 决策是否启用）。
...
| D-7 | 索引覆盖 | 当前 26 张应用表所需 B-Tree 索引 + `ivfflat` + 可选 GIN 全文（按 D-9 决策） | 性能 baseline |
...
| C-2 | 索引覆盖 | C-1 完成 | `select indexname from pg_indexes where schemaname='public'` | 本 spec 当前索引 inventory 中的 B-Tree 索引 + `idx_retrieval_chunks_embedding`（ivfflat） + 可选 `idx_target_jobs_fts`（GIN）存在 | B4 后续 001 |
