...
- **26 张 P0 应用表**：与 product-scope v1.2 当前范围一致：
...
- **B3 outbox operational columns**：`outbox_events` 在 03 §5.9 baseline 字段外，必须纳入 [B3 v1.3](../event-and-outbox-contract/spec.md) 锁定的 retry / dead-letter 字段与 due-query 索引。
...
| 03-db-definition 中尚未提升到 B1/B2/B3 的 DB-local enum | [03-db-definition.md §5](../../../easyinterview-tech-docs/03-db-definition.md#5-核心-ddl) + B4 `migrations/enum-sources.yaml` | 001-bootstrap 必须逐列登记 `table.column -> source -> values checksum`；后续若该 enum 需要跨 API / event 复用，再提升到 B1/B2/B3 | SQL 中出现未登记 check list 时 `migrations/lint.sh` 失败 |
