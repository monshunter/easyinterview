...
- **CI 校验**：CI 中跑 `make migrate-up && make migrate-down && make migrate-up` 验证 idempotency 与可逆性（`migrate-down` 仅在 dev / CI 模式下生效，prod 模式禁止）。
...
| C-5 | enum 与 B1 同源 | B1 modify enum 增加新值 | `make codegen-conventions && make migrate-up`（在干净 DB） | 新增枚举值出现在对应 `text + check (col in (...))` 中；CI drift 通过 | B4 后续 001 + B1 |
| C-6 | prod 安全 | `APP_ENV=prod` | `make migrate-down` | exit 非 0；stderr 提示需 `MIGRATE_DOWN_FORCE=1` | B4 后续 001 |
| C-7 | 数据回填 | 某 migration 需要回填 `users.display_name` | up SQL + Go backfill 联动 | up SQL 加列；backfill Go 在本机 dry-run 通过；CI 中执行成功 | B4 后续 001 + 实际场景 |
...
| C-10 | CI 完整闭环 | PR 修改 schema | A5 CI | `migrate-up && migrate-down && migrate-up` 全部成功；diff 检查通过 | B4 后续 001 + A5 |
...
后续业务域加表 / 改 schema：各 C 域在自己 plan 中提交 migration 文件，PR 必须由 B4 owner approve schema 变更；本 spec 不需逐次修订（仅在跨域 schema 决策变更时修订，如分区 / multi-tenant 升格）。
