...
1. **30 张 baseline 应用 / 支撑表 + 2 张迁移元数据表**：覆盖 03 §4 全集 27 张表，外加 ADR-Q1 指派给 B4 的 `auth_challenges` / `sessions` / `external_identities` 3 张支撑表；迁移系统自身使用 `schema_migrations` 与 `schema_backfills`。`make migrate-up` 后 public schema 至少有 32 张表。
...
| `question_assessments` / `feedback_reports` / `mistake_entries` | hard delete | 证据片段、报告正文、错题原因均视为用户内容 |
...
|----|------|-------|------|------|-----------|
| C-1 | 干净 DB baseline 落地 | 干净 Postgres 16（A2 dev stack 启动，pgvector 可已预启用或未启用） | `make migrate-up` | 27 张 03 表 + `auth_challenges` / `sessions` / `external_identities` + `schema_migrations` + `schema_backfills` 全部存在；`vector` 扩展已启用；`select count(*) from information_schema.tables where table_schema='public'` ≥ 32 | B4 后续 001 |
...
- 落地一组严格递增的 `migrations/000001_*.up.sql` / `.down.sql`，覆盖 `vector` 扩展、30 张应用 / auth 支撑表、B3 outbox retry operational columns、A3/F1 AI call meta typed columns、`schema_backfills`、索引与 `schema_migrations` 工具表协同；不强制一表一文件，但编号必须稳定、可 review。
