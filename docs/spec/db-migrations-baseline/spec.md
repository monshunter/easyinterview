...
[engineering-roadmap spec §5.1](../engineering-roadmap/spec.md#51-当前已存在的-active-spec) 将 B4 `db-migrations-baseline` 定义为当前 active Contract spec（依赖 [B1 `shared-conventions-codified`](../shared-conventions-codified/spec.md) 与 [A2 `local-dev-stack`](./../local-dev-stack/spec.md)）。当前迁移 baseline 由本 spec、`migrations/` 与 product-scope 当前范围决定：22 张当前应用表（product-scope D-17 已删 5 张 JD-Match 表，D-20 已把简历版本树扁平化为单一 `resumes`，D-22 已删除真实复盘和候选人画像表）、[ADR-Q1](../engineering-roadmap/decisions/ADR-Q1-auth.md) 锁定的 3 张 auth / session 支撑表、迁移元数据表与索引契约落到迁移文件层，决定了：
...
1. **25 张 baseline 应用 / 支撑表 + 2 张迁移元数据表**：覆盖当前产品范围内的 22 张应用表，外加 ADR-Q1 指派给 B4 的 `auth_challenges` / `sessions` / `external_identities` 3 张支撑表；迁移系统自身使用 `schema_migrations` 与 `schema_backfills`。`make migrate-up` 后 public schema 至少有 27 张表。旧 `mistake_entries`、5 张 JD-Match 表（product-scope D-17）、`resume_versions` / `resume_version_suggestions` / `resume_tailor_runs`（D-20 扁平化）、`candidate_profiles` / `experience_cards` / `debriefs`（product-scope D-22）与向量检索表已删除；报告内题目回顾由 `question_assessments` / `feedback_reports` 承载，简历结构化内容由扁平 `resumes` 承载。
2. **统一迁移工具**：当前选型 `golang-migrate`（轻量、社区成熟），不与 `atlas` / `goose` 共存；后续如需切换迁移工具，必须先修订本 spec。
...
- **22 张 P0 应用表**：与 product-scope 当前范围一致：
  1. `users` / 2. `user_settings` / 3. `file_objects` / 4. `resumes` / 5. `target_jobs` / 6. `target_job_requirements` / 7. `target_job_sources` / 8. `practice_plans` / 9. `idempotency_records` / 10. `practice_sessions` / 11. `practice_session_events` / 12. `practice_turns` / 13. `question_assessments` / 14. `feedback_reports` / 15. `source_records` / 16. `prompt_versions` / 17. `rubric_versions` / 18. `ai_task_runs` / 19. `async_jobs` / 20. `outbox_events` / 21. `privacy_requests` / 22. `audit_events`。D-20 简历扁平化（product-scope）：`resume_assets`→`resumes` 单一扁平资产承载原始来源（`original_text` / `parsed_text_snapshot` / `raw_text` / `file_object_id`）+ 结构化内容（`structured_profile` / `display_name`），删除 `resume_versions`（版本树 / 结构化主版本 / 岗位定制版本）/ `resume_version_suggestions`（逐条改写状态）/ `resume_tailor_runs`（改写 run 持久化）三表，`source_type` 收敛为 {`upload`, `paste`}（删除 `guided` 轻量问答 + `guided_answers` 列），`practice_plans.resume_asset_id`→`resume_id`。product-scope D-17 已删除 5 张 JD-Match 表（`jd_match_recommendations` / `watchlist_items` / `saved_searches` / `agent_scans` / `jd_match_search_runs`，由 `000014_drop_jd_match_module`）。product-scope D-22 已删除 `candidate_profiles` / `experience_cards` / `debriefs`，当前 baseline 不再为真实复盘或候选人画像保留表。具体 migration up/down + check constraint + idx + backfill 覆盖由 [db-migrations-baseline/002-resume-versions-additive](./plans/002-resume-versions-additive/plan.md)（含 D-20 flatten phase）与 [product-scope/001-core-loop-module-pruning](../product-scope/plans/001-core-loop-module-pruning/plan.md) 落地。旧 `mistake_entries`、`candidate_profiles`、`experience_cards`、`debriefs` 与向量检索表已删除；不得作为 P0/P1/P2 后续表恢复。
...
| D-7 | 索引覆盖 | 当前 22 张应用表所需 B-Tree 索引 + 可选 GIN 全文（按 D-9 决策） | 性能 baseline |
...
| D-17 | Resume Workshop additive 表与字段（已落地；`resume_versions` / `resume_version_suggestions` 已由 D-22 删除、`resume_assets`→`resumes` 扁平化、`source_type` 去 `guided`、`guided_answers` 列删除） | 本次修订落地 Resume Workshop 阶段 0 contract additive 升级所需 DB 范围：（1）新增 `resume_versions` 表（id / user_id / resume_asset_id FK / parent_version_id FK self / version_type [structured_master, targeted] / target_job_id FK NULLABLE / display_name / seed_strategy NULLABLE [copy_master, blank, ai_select] / focus_angle NULLABLE / structured_profile jsonb / match_score numeric NULLABLE / prompt_version / rubric_version / model_id / provider / created_at / updated_at / deleted_at + idx `(user_id, updated_at DESC)` + idx `(resume_asset_id, version_type)` + idx `(parent_version_id) WHERE parent_version_id IS NOT NULL`）；（2）新增 `resume_version_suggestions` 表（id / resume_version_id FK ON DELETE CASCADE / tailor_run_id FK / original_bullet / suggested_bullet / reason / status [pending, accepted, rejected] / decided_at + idx `(resume_version_id, status)` + idx `(tailor_run_id)`）；（3）`resume_assets` additive 字段（`source_type` text NULL CHECK ∈ {`upload`, `paste`, `guided`} / `original_text` text NULL / `guided_answers` jsonb NULL / `parsed_text_snapshot` text NULL，保持向后兼容）；（4）`resume_version_edits` 表（手动编辑历史）归 P1 延后；`migrations/enum-sources.yaml` 同步登记 3 个 B1 enum check 来源（B1 D-10 `ResumeVersionType` / `ResumeSeedStrategy` / `ResumeTailorSuggestionStatus`）+ 1 个 B2 API-facing sourceType 来源（B2 D-18 `RegisterResumeRequest.sourceType`）；具体 SQL 与 idx 由 `migrations/000005_resume_versions.{up,down}.sql` 承接 | `migrations/000005_resume_versions.up.sql` + `.down.sql`、`migrations/enum-sources.yaml`、§2.1 表 inventory（28 张 P0 应用表）、§3.1.2 privacy deletion matrix、`docs/spec/openapi-v1-contract` D-18 同步、`docs/spec/event-and-outbox-contract` D-14 同步、§6 验收（baseline 表数 28）|
| D-18 | Practice session event replay snapshot（已落地） | `practice_session_events` 在 pre-launch baseline 中新增 nullable `replay_payload jsonb`，用于保存 `appendSessionEvent` clientEventId replay 的 client-visible result snapshot；`payload` 继续作为事件 / 隐私红线载体，必须保持 sanitized，不写 `hint_text` / question / answer / prompt / response 明文；`replay_payload` 只服务同 `(session_id, client_event_id)` replay，随 `practice_session_events` 级联删除。 | backend-practice/003 L2 replay 修复；`migrations/000001_create_baseline.up.sql`；`backend/internal/store/practice/append_event.go` |
| D-19 | Report generation baseline rebase 授权 | backend-review/001 Phase 0 实施时必须在 B4 pre-launch baseline 中补齐：`ai_task_runs.task_type` 新增 `report_assessment`，`feedback_reports` 新增 `language` / `feature_flag` / `data_source_version` / `retry_focus_turn_ids`，并同步 enum-source、migration lint 与 `AITaskRunCapability` allowlist；该项是 report dashboard baseline 的 owner gate，未落地前 backend-review worker 不得进入依赖 report assessment task type 或 feedback report provenance 单表 round-trip 的实现阶段。 | backend-review/001-report-generation-baseline Phase 0.3 / 0.5 |
| D-20 | JD-Match baseline tables cross-owner additive（已由 product-scope D-17 整体退役：5 张 JD-Match 表由 `000014_drop_jd_match_module` 删除，§2.1 inventory / §3.1.2 matrix / §6 C-1 计数已同步移除） | backend-jobs-recommendations/001 携带 baseline migration 创建 5 张 JD-Match 表：(1) `jd_match_recommendations`（id PK / user_id FK / title / company / score smallint 0-100 / fit jsonb / reasons text[] / risks text[] / highlights text[] / seen bool / dismissed_at / dismiss_reason / dismiss_free_note / source_url / source_label / network_note / similar_interviewers / interview_hypotheses text[] / prompt_version / rubric_version / model_id / language / feature_flag / data_source_version / recommended_at / updated_at / deleted_at；含 partial index `(user_id, score DESC, recommended_at DESC, id DESC) WHERE dismissed_at IS NULL AND deleted_at IS NULL`）(2) `watchlist_items`（id PK / user_id / linked_job_match_id FK→jd_match_recommendations ON DELETE CASCADE / label / tone CHECK ∈ {ok, warn, muted} / change_note / added_at + UNIQUE(user_id, linked_job_match_id)）(3) `saved_searches`（id / user_id / label / query / filters jsonb / new_jobs_count / last_run_at / created_at / updated_at）(4) `agent_scans`（id / user_id / status CHECK ∈ {idle, scanning, error} / started_at / finished_at / last_scan_at / next_scan_at / error_message / recommendation_count）(5) `jd_match_search_runs`（id / user_id / search_run_id UNIQUE / query / filters jsonb / result_count / prompt_version / rubric_version / model_id / data_source_version / created_at）；`migrations/enum-sources.yaml` 同步登记 2 个新 enum source（`agent_scans.status` / `watchlist_items.tone`）；baseline 应用表数从 28 升至 33；§3.1.2 privacy deletion matrix 同步追加 5 表删除顺序 watchlist_items → saved_searches → jd_match_search_runs → jd_match_recommendations → agent_scans；不接入外部招聘平台（LinkedIn / Boss / 脉脉 / 拉勾），baseline 仅在内部 jobs 池中推荐 / 搜索。 | `migrations/000009_jd_match_baseline.{up,down}.sql`、`migrations/enum-sources.yaml`、§2.1 表 inventory（33 张 P0 应用表）、§3.1.2 privacy deletion matrix、§6 验收 C-1 / C-13、`backend-jobs-recommendations/001-jd-match-real-backend-baseline` |
| D-21 | Privacy request tombstone FK | `migrations/000011_privacy_requests_user_tombstone.{up,down}.sql` 将 `privacy_requests.user_id` 改为 nullable 且 FK `ON DELETE SET NULL`；`privacy_delete` runner 完成时先保留 request id / status / completed_at / metadata tombstone，再 hard delete `users` 行，避免 account identity cleanup 级联删除 request tombstone | 支撑 ADR-Q5 / §3.1.2 的 `privacy_requests` audit tombstone 语义与 BUG-0106 修复；down migration 删除 `user_id IS NULL` tombstone 后恢复 `NOT NULL + ON DELETE CASCADE` |
| D-22 | 简历资产扁平化 flatten migration（product-scope D-20） | 新增 `migrations/000015_resume_flatten.{up,down}.sql`：（1）`ALTER TABLE resume_assets RENAME TO resumes` + 新增 `structured_profile jsonb NOT NULL DEFAULT '{}'::jsonb` / `display_name text`；in-migration SQL `UPDATE` 从 structured_master version 回填 `structured_profile` / `display_name`（在 DROP 之前；不走 Go backfill registry——`RunBackfills` 在所有 SQL up 之后运行、无法读已 drop 的源表；无 master 留空）；`source_type` CHECK 收敛为 {`upload`, `paste`}（删除 `guided`），DROP `guided_answers` 列；保留 `original_text` / `parsed_text_snapshot` / `raw_text`（D-20 要求只读保留原始来源 + 解析快照）。（2）`practice_plans.resume_asset_id` → `resume_id`（REFERENCES `resumes` ON DELETE SET NULL，是唯一存活的跨表 resume FK）。（3）`DROP TABLE resume_version_suggestions` → `resume_tailor_runs` → `resume_versions`（按 FK 依赖反序）。（4）`migrations/enum-sources.yaml` 移除 3 个 resume version enum check 来源（`resume_versions.version_type` / `resume_versions.seed_strategy` / `resume_version_suggestions.status`），更新现 `resumes.source_type` check 来源为 {`upload`, `paste`}。`resume_tailor` async job_type 与 `resume.tailor.*` ai task_type **保留**（AI 改写能力延续；改写建议改为 ephemeral 即时返回，不再落 `resume_tailor_runs`，最终由 backend-resume D-20 确认）。down migration 恢复被删表的结构骨架（数据入 backfill log），并把 `resumes`→`resume_assets`、`resume_id`→`resume_asset_id` 反向重命名。应用表数 28→25。 | `migrations/000015_resume_flatten.{up,down}.sql`、`migrations/enum-sources.yaml`、§2.1 表 inventory（25 张）、§3.1.2 privacy matrix、§6 C-1（表数 25 / ≥30）、[shared-conventions-codified](../shared-conventions-codified/spec.md) D-20（3 enum 退役）、[openapi-v1-contract/004](../openapi-v1-contract/plans/004-resume-additive-coverage/plan.md) D-20、[backend-resume](../backend-resume/spec.md) D-20、[backend-practice](../backend-practice/spec.md)（resume_id binding）；由 [db-migrations-baseline/002](./plans/002-resume-versions-additive/plan.md) 的 D-20 flatten phase 落地 |
| D-23 | product-scope D-22 复盘 / 候选人画像表删除 | 当前 baseline 不再创建、恢复或要求 privacy runner 覆盖 `candidate_profiles` / `experience_cards` / `debriefs`；`idempotency_records` 仍是当前用户关联表，必须保留在 privacy matrix 中并按 `user_id` hard delete。应用表数 25→22；应用 / auth 支撑表数 28→25；public schema count gate 30→27。 | [product-scope/001-core-loop-module-pruning](../product-scope/plans/001-core-loop-module-pruning/plan.md)、`backend/internal/migrations/privacy.go`、`backend/internal/migrations/cli_test.go` |
...
| DB / event / outbox job、publish 状态与 retry 字段 | [B3 `event-and-outbox-contract`](../event-and-outbox-contract/spec.md) | `async_jobs.job_type` 必须跟随当前 B3 canonical 8 项（其中 `source_refresh` / `email_dispatch` 为 internal-only，`privacy_export` 为 contract-only）；`outbox_events.publish_status`、retry operational columns、事件字段约束跟随 B3 | B4 lint 检查 B3 job manifest 与 DB check 一致，确认 B2 API-facing 子集不被扩大，并确认 outbox due-query 索引存在 |
...
| `user_settings` | hard delete | 删除账号设置；候选人画像 / Experience Card 已随 product-scope D-22 删除，不再作为 privacy matrix 表项 |
| `file_objects` / `resumes` | hard delete + object storage delete | 先删除对象存储文件，再删 DB 行；`resumes` 含 `original_text` / `parsed_text_snapshot` / `raw_text` 原始来源快照与 `structured_profile` 结构化内容，均视为用户内容一并删除；失败时保留 retryable job 状态 |
...
| `privacy_requests` | audit tombstone | 完成后保留 request id、request type、status、completed_at、duration bucket；`user_id` 必须可置空并在用户最终 hard delete 前/中移除，不保留可识别内容；FK 语义由 `000011_privacy_requests_user_tombstone` 固定为 `ON DELETE SET NULL` |
...
| 边界 | Owner | 说明 |
|------|-------|------|
| `migrations/` 目录与 baseline 文件 | B4 | 25 张当前应用 / auth 支撑表 + 2 张迁移元数据表 + 索引 |
...
|----|------|-------|------|------|-----------|
| C-1 | 干净 DB baseline 落地 | 干净 Postgres 18（A2 dev stack 启动） | `make migrate-up` | 22 张当前应用表（含扁平 `resumes`）+ `auth_challenges` / `sessions` / `external_identities` + `schema_migrations` + `schema_backfills` 全部存在；`select count(*) from information_schema.tables where table_schema='public'` ≥ 27；不存在 `mistake_entries`、`resume_versions` / `resume_version_suggestions` / `resume_tailor_runs`、5 张 JD-Match 表、`candidate_profiles` / `experience_cards` / `debriefs` 或向量检索表 | B4 后续 001 + 002（含 D-20 flatten phase）+ product-scope/001 |
...
| C-13 | P0 privacy deletion matrix 可执行 | B4 baseline migration 已完成，测试用户产生覆盖 27 个当前 public baseline 表项的样本数据（含 `idempotency_records`，不含已删除的 JD-Match / debrief / profile 表） | 触发 C8 `privacy_delete` dry-run / apply | dry-run 输出 §3.1.2 每表 disposition；apply 后用户可识别内容已 hard delete / cascade / tombstone；`prompt_versions` / `rubric_versions` / migration metadata 保留 | B4 后续 001 + 002 + C8 + product-scope/001 |
...
- [001-bootstrap](./plans/001-bootstrap/plan.md)（已完成）：
  - 落地一组严格递增的 `migrations/000001_*.up.sql` / `.down.sql`，覆盖当前 baseline 所需应用 / auth 支撑表、B3 outbox retry operational columns、A3/F1 AI call meta typed columns、`schema_backfills`、索引与 `schema_migrations` 工具表协同；当前表数与删除后的零残留要求以 §2.1、§3.1.2 与 §6 为准；不强制一表一文件，但编号必须稳定、可 review。
...
- [002-resume-versions-additive](./plans/002-resume-versions-additive/plan.md)：D-17 Resume Workshop additive 表与字段落地（历史 baseline）。新增 `migrations/000005_resume_versions.{up,down}.sql` 引入 `resume_versions` / `resume_version_suggestions` 表 + `resume_assets` 字段补充（`source_type` / `original_text` / `guided_answers` / `parsed_text_snapshot`）；同步 `migrations/enum-sources.yaml` 与 §3.1.2 privacy deletion matrix；提供 migration up/down 幂等测试、check constraint 验证、idx 覆盖测试；Phase 5 已补 live test cleanup 顺序，固定 UUID 测试必须 rerun-safe。`resume_version_edits` 表归 P1 延后。**D-20 flatten phase（Phase 6）**：`migrations/000015_resume_flatten.{up,down}.sql` 把 `resume_assets`→`resumes`（+ `structured_profile` / `display_name` + structured_master backfill），drop `resume_versions` / `resume_version_suggestions` / `resume_tailor_runs`，`practice_plans.resume_asset_id`→`resume_id`，`enum-sources.yaml` 移除 3 个 version enum 来源、`source_type` 去 `guided`、drop `guided_answers` 列；§2.1 baseline 应用表 28→25（叠加 product-scope D-17 jd_match drop 计数回填）。product-scope D-22 后续删除 debrief/profile 表与 privacy matrix 收敛由本 spec v1.24 与 [product-scope/001-core-loop-module-pruning](../product-scope/plans/001-core-loop-module-pruning/plan.md) 承接。
