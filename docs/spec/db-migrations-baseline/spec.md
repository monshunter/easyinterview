...
- **B3 outbox operational columns**：`outbox_events` 在 03 §5.9 baseline 字段外，必须纳入 [B3 v1.2](../event-and-outbox-contract/spec.md) 锁定的 retry / dead-letter 字段与 due-query 索引。
...
| C-5 | enum/check 来源矩阵 | B1 修改 shared enum，或 B3 修改 `shared/jobs.yaml` jobType | `make codegen-conventions && make migrate-check`（在干净 DB）+ B4 lint | 对应 `text + check (col in (...))` 与 §3.1.1 来源一致；B2 API-facing `JobType` 子集未被 DB-only job 扩大；本地 drift 通过 | B4 后续 001 + B1/B2/B3 |
...
- 落地一组严格递增的 `migrations/000001_*.up.sql` / `.down.sql`，覆盖 `vector` 扩展、30 张应用 / auth 支撑表、B3 outbox retry operational columns、`schema_backfills`、索引与 `schema_migrations` 工具表协同；不强制一表一文件，但编号必须稳定、可 review。
