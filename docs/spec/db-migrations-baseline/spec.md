...
[engineering-roadmap spec §5.1](../engineering-roadmap/spec.md#51-当前已存在的-active-spec) 将 B4 `db-migrations-baseline` 定义为当前 active Contract spec（依赖 [B1 `shared-conventions-codified`](../shared-conventions-codified/spec.md) 与 [A2 `local-dev-stack`](./../local-dev-stack/spec.md)）。当前迁移 baseline 由本 spec、`migrations/` 与 product-scope 当前范围决定：25 张当前应用表（product-scope D-17 已删 5 张 JD-Match 表，D-20 已把简历版本树扁平化为单一 `resumes`）、[ADR-Q1](../engineering-roadmap/decisions/ADR-Q1-auth.md) 锁定的 3 张 auth / session 支撑表、迁移元数据表与索引契约落到迁移文件层，决定了：
...
1. **28 张 baseline 应用 / 支撑表 + 2 张迁移元数据表**：覆盖当前产品范围内的 25 张应用表，外加 ADR-Q1 指派给 B4 的 `auth_challenges` / `sessions` / `external_identities` 3 张支撑表；迁移系统自身使用 `schema_migrations` 与 `schema_backfills`。`make migrate-up` 后 public schema 至少有 30 张表。旧 `mistake_entries`、5 张 JD-Match 表（product-scope D-17）、`resume_versions` / `resume_version_suggestions` / `resume_tailor_runs`（D-20 扁平化）与向量检索表已删除；报告内题目回顾由 `question_assessments` / `feedback_reports` 承载，简历结构化内容由扁平 `resumes` 承载。
...
- **25 张 P0 应用表**：与 product-scope 当前范围一致：
  1. `users` / 2. `user_settings` / 3. `candidate_profiles` / 4. `experience_cards` / 5. `file_objects` / 6. `resumes` / 7. `target_jobs` / 8. `target_job_requirements` / 9. `target_job_sources` / 10. `practice_plans` / 11. `idempotency_records` / 12. `practice_sessions` / 13. `practice_session_events` / 14. `practice_turns` / 15. `question_assessments` / 16. `feedback_reports` / 17. `debriefs` / 18. `source_records` / 19. `prompt_versions` / 20. `rubric_versions` / 21. `ai_task_runs` / 22. `async_jobs` / 23. `outbox_events` / 24. `privacy_requests` / 25. `audit_events`。D-20 简历扁平化（product-scope）：`resume_assets`→`resumes` 单一扁平资产承载原始来源（`original_text` / `parsed_text_snapshot` / `raw_text` / `file_object_id`）+ 结构化内容（`structured_profile` / `display_name`），删除 `resume_versions`（版本树 / 结构化主版本 / 岗位定制版本）/ `resume_version_suggestions`（逐条改写状态）/ `resume_tailor_runs`（改写 run 持久化）三表，`source_type` 收敛为 {`upload`, `paste`}（删除 `guided` 轻量问答 + `guided_answers` 列），`practice_plans.resume_asset_id`→`resume_id`。product-scope D-17 已删除 5 张 JD-Match 表（`jd_match_recommendations` / `watchlist_items` / `saved_searches` / `agent_scans` / `jd_match_search_runs`，由 `000014_drop_jd_match_module`）。具体 migration up/down + check constraint + idx + backfill 覆盖由 [db-migrations-baseline/002-resume-versions-additive](./plans/002-resume-versions-additive/plan.md)（含 D-20 flatten phase）落地。旧 `mistake_entries` 与向量检索表已删除；不得作为 P0/P1/P2 后续表恢复。
...
| D-7 | 索引覆盖 | 当前 25 张应用表所需 B-Tree 索引 + 可选 GIN 全文（按 D-9 决策） | 性能 baseline |
...
| `debriefs` | hard delete | P0 debrief replay 随用户删除；简历改写（tailor）输出 D-20 后为 ephemeral，不再单表持久化，结构化内容随 `resumes` 删除（`resume_versions` / `resume_version_suggestions` / `resume_tailor_runs` 已由 D-22 删除，JD-Match 5 表已由 product-scope D-17 删除） |
...
| 边界 | Owner | 说明 |
|------|-------|------|
| `migrations/` 目录与 baseline 文件 | B4 | 36 张当前应用 / auth 支撑表 + 2 张迁移元数据表 + 索引 |
...
|----|------|-------|------|------|-----------|
| C-1 | 干净 DB baseline 落地 | 干净 Postgres 18（A2 dev stack 启动） | `make migrate-up` | 25 张当前应用表（含扁平 `resumes`）+ `auth_challenges` / `sessions` / `external_identities` + `schema_migrations` + `schema_backfills` 全部存在；`select count(*) from information_schema.tables where table_schema='public'` ≥ 30；不存在 `mistake_entries`、`resume_versions` / `resume_version_suggestions` / `resume_tailor_runs`、5 张 JD-Match 表或向量检索表 | B4 后续 001 + 002（含 D-20 flatten phase） |
...
| C-13 | P0 privacy deletion matrix 可执行 | B4 baseline migration 已完成，测试用户产生覆盖 36 张当前应用 / auth 支撑表的样本数据（含 5 张 JD-Match 表） | 触发 C8 `privacy_delete` dry-run / apply | dry-run 输出 §3.1.2 每表 disposition；apply 后用户可识别内容已 hard delete / cascade / tombstone；`prompt_versions` / `rubric_versions` / migration metadata 保留 | B4 后续 001 + 002 + C8 + backend-jobs-recommendations/001 |
...
- [002-resume-versions-additive](./plans/002-resume-versions-additive/plan.md)：D-17 Resume Workshop additive 表与字段落地（历史 baseline）。新增 `migrations/000005_resume_versions.{up,down}.sql` 引入 `resume_versions` / `resume_version_suggestions` 表 + `resume_assets` 字段补充（`source_type` / `original_text` / `guided_answers` / `parsed_text_snapshot`）；同步 `migrations/enum-sources.yaml` 与 §3.1.2 privacy deletion matrix；提供 migration up/down 幂等测试、check constraint 验证、idx 覆盖测试；Phase 5 已补 live test cleanup 顺序，固定 UUID 测试必须 rerun-safe。`resume_version_edits` 表归 P1 延后。**D-20 flatten phase（Phase 6，本次新增）**：`migrations/000015_resume_flatten.{up,down}.sql` 把 `resume_assets`→`resumes`（+ `structured_profile` / `display_name` + structured_master backfill），drop `resume_versions` / `resume_version_suggestions` / `resume_tailor_runs`，`practice_plans.resume_asset_id`→`resume_id`，`enum-sources.yaml` 移除 3 个 version enum 来源、`source_type` 去 `guided`、drop `guided_answers` 列；§2.1 baseline 应用表 28→25（叠加 product-scope D-17 jd_match drop 计数回填）。
