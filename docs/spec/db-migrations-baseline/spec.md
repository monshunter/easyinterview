...
[engineering-roadmap spec §5.1](../engineering-roadmap/spec.md#51-当前已存在的-active-spec) 将历史 B4 `db-migrations-baseline` 保留为当前 active Contract spec（依赖 [B1 `shared-conventions-codified`](../shared-conventions-codified/spec.md) 与 [A2 `local-dev-stack`](./../local-dev-stack/spec.md)）。它以 [03-db-definition.md §5](../../../easyinterview-tech-docs/03-db-definition.md) 的历史 DB 输入为 seed，但当前迁移 baseline 由本 spec、`migrations/` 与 product-scope v1.4 决定：26 张当前应用表、[ADR-Q1](../engineering-roadmap/decisions/ADR-Q1-auth.md) 锁定的 3 张 auth / session 支撑表、迁移元数据表、索引与 pgvector 扩展契约落到迁移文件层，决定了：
...
1. **29 张 baseline 应用 / 支撑表 + 2 张迁移元数据表**：覆盖当前产品范围内的 26 张应用表，外加 ADR-Q1 指派给 B4 的 `auth_challenges` / `sessions` / `external_identities` 3 张支撑表；迁移系统自身使用 `schema_migrations` 与 `schema_backfills`。`make migrate-up` 后 public schema 至少有 31 张表。旧 `mistake_entries` 表已随独立错题本删除；报告内题目回顾由 `question_assessments` / `feedback_reports` 承载。
2. **统一迁移工具**：`golang-migrate` 或 `atlas` 之一；本 spec §3.1 选型 `golang-migrate`（轻量、社区成熟），在 B4 自身 plan 落地时如发现限制可在本 spec 修订中切换。
3. **可逆 + 数据回填策略**：所有 migration 必须说明是否可逆 / 是否需数据回填 / 是否阻塞写入；[03 §9](../../../easyinterview-tech-docs/03-db-definition.md#9-迁移策略) 仅作历史 seed，当前执行口径以本 spec 为准。
4. **索引策略锁定**：从 03 §7 的有效历史 seed 收敛出当前 B-Tree + ivfflat 向量索引 + 可选 GIN 全文索引；性能查询路径在后续业务域加表时不得遗漏索引。
5. **AI 与隐私承载完整**：`ai_task_runs` 必须显式承载 A3/F1 需要检索的 call meta；隐私删除必须按 per-table matrix 执行，避免 C8 worker 漏删或误删全局配置表。
...
- **26 张 P0 应用表**：与 product-scope v1.4 当前范围一致：
  1. `users` / 2. `user_settings` / 3. `candidate_profiles` / 4. `experience_cards` / 5. `file_objects` / 6. `resume_assets` / 7. `target_jobs` / 8. `target_job_requirements` / 9. `target_job_sources` / 10. `practice_plans` / 11. `practice_sessions` / 12. `practice_session_events` / 13. `practice_turns` / 14. `question_assessments` / 15. `feedback_reports` / 16. `resume_tailor_runs` / 17. `debriefs` / 18. `source_records` / 19. `retrieval_chunks` / 20. `prompt_versions` / 21. `rubric_versions` / 22. `ai_task_runs` / 23. `async_jobs` / 24. `outbox_events` / 25. `privacy_requests` / 26. `audit_events`。旧 `mistake_entries` 已删除；不得作为 P0/P1/P2 后续表恢复。
- **B3 outbox operational columns**：`outbox_events` 在 03 §5.9 历史 baseline 字段外，必须纳入 [B3 current event-and-outbox contract](../event-and-outbox-contract/spec.md) 锁定的 retry / dead-letter 字段与 due-query 索引。
- **A3/F1 AI call meta typed columns**：`ai_task_runs` 在 03 §5.8 baseline 字段外，必须显式增加 `model_family` / `model_profile_name` / `model_profile_version` / `fallback_chain` / `route` / `validation_status` / `output_schema_version` typed columns，避免核心观测字段只落 JSONB。
...
- **扩展**：B4 migration 必须执行 `CREATE EXTENSION IF NOT EXISTS vector`；A2 dev stack 可提前启用 pgvector 以保证空实例可用，但 B4 仍负责 staging / prod 与任意干净 DB 的幂等启用。默认 down migration 不 drop `vector` 扩展，除非 dev 环境显式设置 `MIGRATE_DROP_EXTENSIONS=1`。
...
- **privacy deletion matrix**：§3.1.2 锁定 P0 `privacy_delete` worker 的每表处理策略（hard delete / cascade / retain / audit tombstone），C8 实现必须逐项验证。
...
| D-5 | 阻塞操作约束 | 不在高频表（`practice_session_events` / `outbox_events` / `audit_events`）执行阻塞式大表重写；优先 additive；若必须重写需先在本 spec 中登记 | [03 §9.1](../../../easyinterview-tech-docs/03-db-definition.md#91-原则) 仅作历史 seed，当前 gate 以本 spec 与 B4 tooling 为准 |
...
| D-7 | 索引覆盖 | 当前 26 张应用表所需 B-Tree 索引 + `ivfflat` + 可选 GIN 全文（按 D-9 决策）；03 §7 仅作历史 seed | 性能 baseline |
...
| D-12 | pgvector extension 生命周期 | up 永远幂等 `CREATE EXTENSION IF NOT EXISTS vector`；down 默认保留扩展；仅 dev + `MIGRATE_DROP_EXTENSIONS=1` 允许 drop | 避免回滚 schema 时破坏共享 DB extension |
...
| DB / event / outbox job、publish 状态与 retry 字段 | [B3 `event-and-outbox-contract`](../event-and-outbox-contract/spec.md) | `async_jobs.job_type` 可包含 B3 canonical 10 项（含 internal-only `email_dispatch`）；`outbox_events.publish_status`、retry operational columns、事件字段约束跟随 B3 | B4 lint 检查 B3 job manifest 与 DB check 一致，确认 B2 API-facing 子集不被扩大，并确认 outbox due-query 索引存在 |
...
| B4 DB-local enum（尚未提升到 B1/B2/B3） | B4 `migrations/enum-sources.yaml`；[03-db-definition.md §5](../../../easyinterview-tech-docs/03-db-definition.md#5-核心-ddl) 仅作历史 seed | 001-bootstrap 必须逐列登记 `table.column -> source -> values checksum`；后续若该 enum 需要跨 API / event 复用，再提升到 B1/B2/B3 | SQL 中出现未登记 check list 时 `migrations/lint.sh` 失败 |
...
| `source_records` / `retrieval_chunks` | hard delete | 外部 source 摘要、embedding chunk 与 owner 关联一并删除 |
| `ai_task_runs` | hard delete after audit summary | 删除前只允许聚合 token/cost/SLA 计数进入非用户维度指标；不保留 prompt/response 摘要行 |
| `async_jobs` / `outbox_events` | hard delete or redacted terminal tombstone | 与用户资源关联的 payload/result 必须删除；若为正在执行的 `privacy_delete` job，仅保留 redacted terminal 状态直到 worker 完成 |
...
- pgvector 索引 `lists` 参数：默认 100；后续 retrieval workstream 按数据量调整。
...
- 表名 / 列名严格 `snake_case`；与 B1 当前命名约定和 B4 migration lint 一致，[03 §2.1](../../../easyinterview-tech-docs/03-db-definition.md#21-基本约定) 仅作历史 seed；外键命名 `fk_<from_table>_<column>` / `idx_<table>_<purpose>` / `uq_<table>_<columns>`。
...
| 边界 | Owner | 说明 |
|------|-------|------|
| `migrations/` 目录与 baseline 文件 | B4 | 29 张当前应用 / auth 支撑表 + 2 张迁移元数据表 + 索引 + pgvector |
| `backend/cmd/migrate/main.go` 与 `make migrate-*` target | B4 | A1 占位 `make migrate` 的真实实现；包装 `golang-migrate` 与 Go backfill registry |
| Postgres 实例与 pgvector 启用（dev） | A2 + B4 | A2 可预启用空实例；B4 仍通过 migration 幂等 `CREATE EXTENSION IF NOT EXISTS vector`，down 默认不 drop |
...
|----|------|-------|------|------|-----------|
| C-1 | 干净 DB baseline 落地 | 干净 Postgres 16（A2 dev stack 启动，pgvector 可已预启用或未启用） | `make migrate-up` | 26 张当前应用表 + `auth_challenges` / `sessions` / `external_identities` + `schema_migrations` + `schema_backfills` 全部存在；`vector` 扩展已启用；`select count(*) from information_schema.tables where table_schema='public'` ≥ 31；不存在 `mistake_entries` | B4 后续 001 |
| C-2 | 索引覆盖 | C-1 完成 | `select indexname from pg_indexes where schemaname='public'` | 本 spec 当前索引 inventory 中的 B-Tree 索引 + `idx_retrieval_chunks_embedding`（ivfflat） + 可选 `idx_target_jobs_fts`（GIN）存在；03 §7 仅作历史 seed | B4 后续 001 |
| C-3 | 迁移可逆 | C-1 完成 | `make migrate-down` （dev） | B4 应用 / auth 支撑表与 `schema_backfills` 按 down 语义回滚；`schema_migrations` 可作为工具元数据保留；`vector` 扩展默认保留，只有 `MIGRATE_DROP_EXTENSIONS=1` 时才 drop；exit 0 | B4 后续 001 |
...
| C-9 | retrieval_chunks 向量索引 | 1 万行 embedding 数据 | top-K 查询 | ivfflat 命中；P95 < 50ms（lists=100，`ivfflat.probes=10` 测试设置） | B4 后续 001 + C11 |
...
| C-12 | A3/F1 AI call meta 承载 | 干净 DB baseline 落地 | `select column_name from information_schema.columns where table_name='ai_task_runs'` | `model_family` / `model_profile_name` / `model_profile_version` / `fallback_chain` / `route` / `validation_status` / `output_schema_version` typed columns 存在；核心 dashboard 查询不依赖 JSONB path scan | B4 后续 001 + A3 + F1 |
| C-13 | P0 privacy deletion matrix 可执行 | B4 baseline migration 已完成，测试用户产生覆盖 29 张当前应用 / auth 支撑表的样本数据 | 触发 C8 `privacy_delete` dry-run / apply | dry-run 输出 §3.1.2 每表 disposition；apply 后用户可识别内容已 hard delete / cascade / tombstone；`prompt_versions` / `rubric_versions` / migration metadata 保留 | B4 后续 001 + C8 |

## 7 关联计划

B4 当前暂无 active impl plan；后续由 B4 自身的 `001-bootstrap` 承接：

- 落地一组严格递增的 `migrations/000001_*.up.sql` / `.down.sql`，覆盖 `vector` 扩展、29 张应用 / auth 支撑表、B3 outbox retry operational columns、A3/F1 AI call meta typed columns、`schema_backfills`、索引与 `schema_migrations` 工具表协同；不强制一表一文件，但编号必须稳定、可 review。
