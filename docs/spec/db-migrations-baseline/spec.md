...
|----|------|-------|------|------|-----------|
| C-1 | 干净 DB baseline 落地 | 干净 Postgres 18（A2 dev stack 启动） | `make migrate-up` | 25 张当前应用表 + `auth_challenges` / `sessions` / `external_identities` + `schema_migrations` + `schema_backfills` 全部存在；`select count(*) from information_schema.tables where table_schema='public'` ≥ 30；不存在 `mistake_entries` 或向量检索表 | B4 后续 001 |
