...
- Markdown 正文区域只渲染 resume body 本身，不得额外注入 `displayName`、详情 header 名称、summary 或来源元数据；PDF 页面栈与 Markdown page surface 直接位于详情画布，外层 `article` 不得恢复共享背景板样式。
...
- **TDD 策略**: Phase 26 先扩展 `ResumeListView.test.tsx` 与共享 destructive-dialog component test，证明首次点击仍会立即调用 `archiveResume`、且缺少可访问确认/取消/pending/retry 边界；GREEN 后由相同 tests 验证确认前零请求、单次确认、焦点、Escape/遮罩、same-key retry 与成功/失败卡片状态。
- **BDD 策略**: `BDD.RESUME.DELETE.CONFIRM.006` 使用 `ResumeListView.test.tsx` + shared dialog tests 作为 domain behavior evidence；真实 Chrome 仅截图验收正式 UI，不创建或声明 E2E ID。
...
- Markdown body 不额外注入详情 header `displayName` / 名称 / summary；PDF 页面栈和 Markdown page surface 直接位于详情画布，外层语义 `article` 不形成额外背景板。
