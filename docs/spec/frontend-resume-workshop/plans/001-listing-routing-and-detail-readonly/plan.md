...
- **TDD 策略**: Phase 27 先扩展 `ResumeWorkshopCssParity.test.ts`，要求 PDF/Markdown page surface 共用 `794px` A4 纸宽并拒绝旧 `1150px`；PDF rule 必须保留 `210 / 297`，Markdown rule 必须没有 `aspect-ratio` 与固定/最小高度。GREEN 后由相同 source contract 与现有 renderer component tests 验证只改变纸张几何，不改变 PDF.js、Markdown DOM 或只读边界。
- **BDD 策略**: `BDD.RESUME.DETAIL.A4.007` 使用 `ResumeWorkshopCssParity.test.ts` + `ResumePreviewTab.test.tsx` + `PdfPageStackPreview.test.tsx` 作为 domain behavior evidence；真实 Chrome 验收 desktop PDF/Markdown bbox 与 mobile no-overflow，不创建或声明 E2E ID。
