...
- `ResumeListView` 使用 `listResumes` 的 closed `ResumeSummary` 渲染单层响应式卡片网格、唯一 Header 创建入口、每卡片打开/删除入口、loading / empty / retry / pagination 状态；desktop 固定最大卡片列宽并从左排列，mobile 收敛为同序单列；列表 consumer 不得读取完整 `Resume` 详情字段，且底部“上传或粘贴另一份简历”重复 CTA 必须删除。
