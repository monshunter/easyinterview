...
- queued/processing 且无可读正文的解析等待态不渲染内联 Back action；failed 且无正文的失败态和普通详情页仍保留返回 Workshop 控件。
...
- **TDD 策略**: Phase 29 先把 `ResumeDetailView.test.tsx` 的 queued/processing Back 正向断言改为负向，同时在 failed-without-body case 中明确锁定 `resume-detail-parse-back` 仍存在。RED 必须只命中运行中 action；GREEN 只删除 shared resume transition 的 `action` prop，不改 sequential polling、pending data、ready/failed 替换、shared component 或普通详情。
- **BDD 策略**: `BDD.RESUME.PARSE.VISUAL.005` 使用 `ResumeDetailView.test.tsx` 与 shared scene/CSS tests 作为 domain behavior evidence；真实 Chrome 验收等待态无内联 action/遗留空白、failed recovery 保持和 desktop/mobile containment，不创建或声明 E2E ID。
...
先修改 `ResumeDetailView.test.tsx` 建立 RED：queued/processing shared `resume` transition 仍保留 busy semantics、插画、稳定 DOM 与后台 poll，但 `resume-detail-parse-back`、`.ei-transition-scene__action-wrap` 和内联“返回 / Back”均不进入等待态；failed 且无正文的 `ResumeParseState` 继续渲染 `resume-detail-parse-back`。随后只从 `ResumeDetailView` 的 `isParsing` 分支删除 `AsyncTransitionScene.action`，保留 `onBack` 供失败态和普通详情使用。`BDD.RESUME.PARSE.VISUAL.005` 由 owner tests 与 current-run Chrome desktop/mobile UI acceptance 承接；运行 focused Resume tests、frontend typecheck/build、根 `make test`、owner context、Header/INDEX/docs/diff/pruning gates，不修改 generated client、fixture、backend、route、locale 或 E2E 资产。
...
- queued/processing 简历解析等待画布不渲染 `resume-detail-parse-back`、共享 action wrapper 或其他内联返回按钮；failed/detail 返回、sequential polling、pending data 与终态替换保持不变。
...
| 日期 | 版本 | 变更 |
|------|------|------|
| 2026-07-21 | 4.8 | Reopen Phase 29 to remove the inline Back action from queued/processing resume parsing while preserving failed/detail recovery and sequential polling. |
