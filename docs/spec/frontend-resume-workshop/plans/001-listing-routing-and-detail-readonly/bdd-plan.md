...
| `BDD.RESUME.PARSE.VISUAL.005` | queued/processing Resume 尚无可读正文且后台轮询可能跨多个 request | 用户停留在详情等待或等待 ready/failed | 页面持续显示共享 resume transition 且不闪现通用 loading；运行中不渲染内联返回按钮或 action wrapper，failed-without-body 仍保留返回 Workshop 控件；ready/failed 原子替换，desktop/mobile 无横向溢出 | `ResumeDetailView.test.tsx` + polling/CSS parity tests + current-run Chrome manual acceptance；根 `make test` 承接代码层回归 |
