...
- [ ] 6.1 store/adapter `ResumeAsset`/`ResumeVersion`→`Resume`、route `versionId`→`resumeId`；`ResumeListView` 平铺（删 TreeView/FlatView/ViewSwitcher）、`ResumeDetailView` 去 Breadcrumb/版本图（验证：vitest 组件/adapter/route hook + pixel parity PASS）
- [ ] 6.2 收口：`pnpm --filter @easyinterview/frontend test` + typecheck + build + 零 `ResumeTreeView`/`resumeAssetId`/`resumeVersionId`/`versionId` 残留 grep（generated adapter 除外）（验证：全 gate PASS + 负向 grep）
