...
- [ ] 3.2 默认 tab 选择：按 `resumeDefaultTab(version)` MASTER→preview / TARGETED→rewrites；001 阶段 rewrites 内容可为 `<ComingSoonTab>`，但 active tab 和 URL `tab` 保持 rewrites（验证：Vitest 单测 3 case）
