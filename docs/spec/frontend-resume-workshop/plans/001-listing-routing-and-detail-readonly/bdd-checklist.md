...
- [x] `trigger.sh` 执行 `src/app/scenarios/p0-037-resume-detail-preview-readonly.test.tsx`，覆盖 read-only original-content detail、generic-name negative、legacy rewrites tab ignored、removed action negative 和 not-found fallback。
- [x] `verify.sh` 检查 trigger log 的当前测试数 passed、测试文件 marker、non-current testid negative、fallback-text negative、generic upload/paste name negative 和 error-code no-echo。
