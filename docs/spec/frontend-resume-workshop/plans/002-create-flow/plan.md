...
- Paste 路径通过 `registerResume` 完成注册。
- `ResumeParseFlow` 轮询 `getResume`，ready 后进入 `ResumePreviewConfirm`。
- Preview confirm 通过 `updateResume` 写回 flat Resume 的结构化内容，然后回到 Resume Workshop list。
...
当前产品已采用 flat Resume IA：简历是平铺资产，创建流程只需要 upload / paste、解析进度、预览确认和保存回列表。正式前端必须源级复刻 `ui-design/src/screen-resume-workshop.jsx` 的当前 CreateFlow / ParseFlow / PreviewConfirm 构图，同时通过 generated client 与 backend-resume / backend-upload 合同集成。

当前实现事实：

- `frontend/src/app/screens/resume-workshop/ResumeWorkshopScreen.tsx` 将 `flow=create` dispatch 到 `ResumeCreateFlow`。
- `frontend/src/app/screens/resume-workshop/create/` 包含 create-flow 容器、tabs、upload/paste hooks、parse polling、preview confirm 和 negative tests。
- `frontend/src/app/screens/resume-workshop/create/PreviewStage.tsx` 使用 `useResumeSave().overwrite` 调 `updateResume` 保存 flat structured profile。
- P0.081 / P0.082 / P0.083 场景资产覆盖主路径、失败恢复、CTA handoff 和隐私边界。
...
- **BDD 策略**: 适用。当前 BDD gate 为 E2E.P0.081、E2E.P0.082、E2E.P0.083。
- **替代验证 gate**:
...
`ResumeCreateFlow` owns the `input -> parsing -> preview` state machine. The input state shows upload and paste tabs, preserves route params through auth pending action without raw content, and keeps TopBar / route highlighting stable.
...
- Paste tab validates non-empty text and calls `registerResume`.
...
`ResumeParseFlow` shows the parser progress state while `useResumeParsingPolling` calls `getResume`. Ready state enters preview. Failure, timeout and cancel paths preserve user input and do not leak resume content into URL, pending action, storage or logs.

### Phase 4: Preview Confirm Save

`ResumePreviewConfirm` renders parsed structured content. `PreviewStage` builds the flat structured-profile payload and uses `updateResume` through `useResumeSave().overwrite`; success navigates back to Resume Workshop list, validation errors remain inline.
...
| C-3 | Paste path | Non-empty text | Submit | `registerResume` receives paste payload and enters parse stage | hook / component tests |
| C-4 | Parse recovery | Polling fails, times out or is cancelled | User retries or returns | State transitions preserve input and avoid content leaks | parse hook tests |
| C-5 | Preview save | Ready resume enters preview | Confirm | `updateResume` writes structured profile and navigates to list | preview tests |
...
| C-7 | BDD gates | P0.081 / P0.082 / P0.083 assets | Scenario verify | Main path, recovery path and CTA handoff are covered | BDD docs + scenario scripts |
