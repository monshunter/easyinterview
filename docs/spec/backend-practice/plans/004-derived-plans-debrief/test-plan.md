...
| T3 | stale debrief source field returns | retired-token grep over OpenAPI, generated artifacts, fixtures, backend runtime, scenario docs | no current `sourceDebriefId`, `source_debrief_id`, `PracticeGoalDebrief`, `goal='debrief'`, or debrief first-turn scenario |
| T4 | stale BDD no-op gate | `test/scenarios/e2e/p0-070-*`, `test/scenarios/e2e/p0-072-*`, backend test search | P0.070 and P0.072 scripts target existing tests; P0.071/P0.073 are not current owner gates |
...
Expected current hits are limited to retired-negative text in scenario verifiers or product-scope gates, not positive runtime/generated/fixture contract.

```bash
python3 .agent-skills/implement/shared/scripts/validate_context.py \
  --context docs/spec/backend-practice/plans/004-derived-plans-debrief/context.yaml \
  --docs-root docs \
  --target backend
make docs-check
git diff --check
...
- Historical debrief-only `E2E.P0.071` / `E2E.P0.073` are retired with product-scope D-22 and must not be used as current proof.
