...
- [x] 1.1 `sourceReportId` remains the only current derived-plan source field for `CreatePracticePlanRequest` / `PracticePlan`; verification: OpenAPI and generated Go/TS expose `sourceReportId` for retry / next-round and do not expose `sourceDebriefId`.
  <!-- verified: 2026-07-06 method=current-state-search evidence="rg over openapi/openapi.yaml, backend/internal/api/generated, frontend/src/api/generated, backend/internal/practice, backend/internal/store/practice, backend/internal/api/practice shows sourceReportId/source_report_id current hits and no sourceDebriefId/source_debrief_id positive hits outside retired-negative scenario text." -->
...
- [x] 2.1 Remove debrief-derived positive contract from plan/checklist/test/BDD/context; verification: this plan no longer lists `goal='debrief'`, `sourceDebriefId`, `source_debrief_id`, `PracticeGoalDebrief`, or raw debrief first-turn seeding as current implementation items.
  <!-- verified: 2026-07-06 method=doc-rewrite evidence="plan/test/BDD docs rewritten to report-derived only; context keywords use retired wording only." -->
- [x] 2.2 Retire historical debrief scenario gates from this owner; verification: active BDD docs retain only `E2E.P0.070` and `E2E.P0.072`, and no P0.071/P0.073 scenario directories exist under `test/scenarios/e2e`.
...
- [x] 2.3 Run docs/context gates after reconciliation; verification: `validate_context.py`, `make docs-check`, `git diff --check`, and retired-token grep pass.
  <!-- verified: 2026-07-06 method=focused-go-doc-gates evidence="cd backend && go test ./internal/practice ./internal/store/practice ./internal/api/practice ./cmd/api -run 'Derived|Source|TestE2EP0070|TestE2EP0072' -count=1 PASS; retired runtime/generated/fixture grep for sourceDebriefId/source_debrief_id/PracticeGoalDebrief/goal='debrief'/debrief-derived/raw_questions returned no matches; validate_context.py backend-practice/004 backend PASS; sync-doc-index --check PASS; git diff --check PASS." -->
