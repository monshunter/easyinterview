...
- [ ] 准备 seed-input：用户 A + session + N（≥10）个独立 clientEventId
- [ ] 实现 setup.sh：创建 session 并把 currentTurn 推进到 `answered` 之外的安全状态供并发新事件写入
- [ ] 实现 trigger.sh：用 GNU parallel / xargs / curl 并发发送 N 个 `POST events`（每个 clientEventId 唯一）
- [ ] 实现 verify.sh：断言所有请求 200；DB `seq_no` 严格连续无重无丢；UNIQUE(session_id, seq_no) 零违反；`practice_turns` / `outbox_events` 行数 1:1
- [ ] 实现 cleanup.sh
- [ ] 执行并通过场景验证
- [ ] 记录验证证据并同步 INDEX
