...
|--------|-------------------|-------|----------|------------|
| shared/jobs.yaml `triggerEventSemantic` enum + lint | R13 | Phase 0 + Phase 4 | drift + 单元 | `python3 scripts/lint/events_inventory.py` + generated jobs Go const + `IsSourceEventOnly` 谓词单元测试 |
...
| openapi fixtures extension (events / complete) | R14 / R15 | Phase 0 | contract + drift | `python3 scripts/lint/validate_fixtures.py`（或当前 fixture validator）+ Prism / contract 测试 |
...
| async_jobs dedupe_key UNIQUE | R12 / R15 | Phase 3 | 集成 | `complete_session_integration_test.go` |
| Privacy redaction | R17 | Phase 2 + Phase 3 + Phase 4 | 单元 + scenario verify.sh grep | `redaction_test.go` + scenario assertions |
| Metric label allowlist | R18 | Phase 2 + Phase 3 | 单元 | `observability_test.go` |
...
| 旧 wire turn enum 压缩 grep | repo grep gate（CI 或 scenario verify.sh） | Red: 旧 mapping 函数残留；Green: `compressTurnStatus` / `mapInternalToWire` / 类似命名零出现 |
| openapi fixtures 扩展（events + complete 命名场景） | `python3 scripts/lint/validate_fixtures.py`（或当前 fixture validator）+ contract test | Red: 命名场景缺失 / fixture 不匹配 generated schema；Green: 13 个命名场景齐全 + Prism / contract 测试 PASS |
...
| 任务 | 测试文件 / 命令 | 预期 Red/Green 证据 |
|------|----------------|---------------------|
| Repository AppendSessionEvent 主流程 | `backend/internal/store/practice/append_event_test.go` (`TestAppendSessionEvent_AnswerSubmitted`) | Red: tx 分裂 / outbox 缺失 / audit 缺失；Green: 单事务写 event / turn / session / outbox / audit；`SELECT FOR UPDATE` 起效 |
| Repository concurrent seq_no | 同上（`TestAppendSessionEvent_ConcurrentSeqNo`，用真实 Postgres + goroutine） | Red: seq_no 重号 / 丢序；Green: 100 并发 insert 后 seq_no 单调连续；UNIQUE(session_id, seq_no) 约束生效 |
...
| Service AppendSessionEvent（含 F3 follow_up） | `backend/internal/practice/append_session_event_service_test.go` | Red: AI 调用在事务内 / AI 失败阻塞用户；Green: AI 在事务外、失败时退化到 `ask_question` placeholder 并写 `failure_code` |
| Handler AppendSessionEvent | `backend/internal/api/practice/append_session_event_test.go` | Red: 接受 `Idempotency-Key` header / 错误码映射错；Green: 拒绝 header → 400；正常返回 200 + B2 wire shape |
| Error mapping | `backend/internal/practice/error_mapping_test.go` | Red: 新增映射缺失；Green: `clientEventIdMismatchToConflict` / `idempotencyHeaderNotAllowed` 映射 PASS |
...
| 任务 | 测试文件 / 命令 | 预期 Red/Green 证据 |
|------|----------------|---------------------|
| Repository CompleteSession 主流程 | `backend/internal/store/practice/complete_session_test.go` (`TestCompleteSession_FirstTime`) | Red: 单事务漏写表 / outbox 缺失 / async_jobs 缺失；Green: 单事务写 session / event / feedback_reports / async_jobs / outbox / audit |
...
| async_jobs dedupe_key | `complete_session_integration_test.go` | Red: dedupe_key 为空 / 不写 INDEX；Green: dedupe_key=sessionId 写入；UNIQUE INDEX 阻止二次创建 |
...
| Handler CompletePracticeSession + idempotency middleware | `backend/internal/api/practice/complete_practice_session_test.go` | Red: middleware 未挂接 / 双 key 行为错；Green: 正常返回 202 + ReportWithJob；replay 返回首次 response；mismatch 返回 409；cross-user 返回 404；D-35 双 key 走 service replay 路径 |
...
| Error mapping | `error_mapping_test.go` | Red: 缺映射；Green: session not found → 404 PRACTICE_SESSION_NOT_FOUND；middleware mismatch → 409 |

## 7 Phase 4: 隐私 / 观测 / Legacy-Negative

| 任务 | 测试文件 / 命令 | 预期 Red/Green 证据 |
|------|----------------|---------------------|
| Redaction 单元测试 | `backend/internal/store/practice/redaction_test.go` 或同等 | Red: outbox / audit / log 出现明文；Green: `question_text` / `answer_text` / `hint_text` / prompt / response / secret 在 fixture 输入下零出现 |
| Metric label allowlist | `backend/internal/practice/observability_test.go` | Red: metric label 含 `feature_key` / prompt-rubric version / provider raw model id；Green: 命中 F1 allowlist |
| Repo-wide legacy-negative grep | CI grep script 或 scenario verify.sh | Red: `report_generate` 二次创建 / 旧 wire turn enum 压缩 / `warmup` 等 retired 术语出现；Green: 全部零出现 |
