...
| `E2E.P0.039` | append replay / mismatch / kind router / header policy | boundary + failure | `clientEventId` replay/mismatch、4 kind routing、legacy strict hint returns `show_hint`、append header refusal、cross-user 404 |
...
| `E2E.P0.039` | 用户 A 拥有 running session；用户 B 独立；矩阵覆盖 replay、mismatch、kind、header 和 cross-user | 同 payload 重放、变更 payload 重放、提交 4 种当前 text kind、携带 `Idempotency-Key`、用户 B 访问用户 A session | replay 返回原结果且无重复 side effect；mismatch 返回 409；4 kind 均路由；legacy strict hint 返回 `show_hint`；header 返回 400；cross-user 返回 404 | `TestE2EP0039PracticeEventIdempotencyKindRouterAndHeaderPolicy` |
