...
| 场景 ID | 名称 | 类别 | 覆盖 |
|---------|------|------|------|
| `E2E.P0.038` | append answer flow | primary | answer_submitted 三分支、server-owned follow-up state、turn status 5 值、turn-completed outbox |
| `E2E.P0.039` | append replay / mismatch / kind router / header policy | boundary + failure | `clientEventId` replay/mismatch、5 kind routing、hint strict conflict、append header refusal、cross-user 404 |
...
| `E2E.P0.043` | privacy and runtime boundary | privacy + regression | redaction, bounded AI metrics/task rows, source-event-only job boundary, 5-value turn status drift check |

## 3 Given / When / Then

| 场景 ID | Given | When | Then | 验证入口 |
|---------|-------|------|------|----------|
| `E2E.P0.038` | 已登录用户拥有 ready practice plan、running session、asked current turn 和 fake follow-up AI | 连续提交 `answer_submitted` 事件覆盖 follow-up、next-question 和 session-completed 分支 | HTTP 200；assistant action 类型正确；client payload 不驱动 `follow_up_count`；accepted events `seq_no` 连续；turn status 暴露当前 5 值；`practice.turn.completed` outbox 一次/turn；provenance 仅含 wire 字段 | `TestE2EP0038PracticeEventLoopAnswerFlow` |
| `E2E.P0.039` | 用户 A 拥有 running session；用户 B 独立；矩阵覆盖 replay、mismatch、kind、header 和 cross-user | 同 payload 重放、变更 payload 重放、提交 5 种 kind、携带 `Idempotency-Key`、用户 B 访问用户 A session | replay 返回原结果且无重复 side effect；mismatch 返回 409；5 kind 均路由；hint 返回 strict conflict；header 返回 400；cross-user 返回 404 | `TestE2EP0039PracticeEventIdempotencyKindRouterAndHeaderPolicy` |
...
| `E2E.P0.043` | P0.038-P0.042 的 API 行为已可执行；日志、metric、audit、outbox 和 task-run payload 可检查 | 触发 follow-up AI 和 strict hint conflict，并运行 runtime boundary lint / generated drift gate | question/answer/hint/prompt/response/secret 文本零泄露；append 不写 audit；source-event-only `report_generate` boundary 成立；turn status 5 值无 drift；runtime boundary lint 通过 | `TestE2EP0043PracticeEventLoopPrivacyAndNonCurrentNegativeSurface` |
