...
- 外部 shell 场景资产: 002 不新增 `test/scenarios/e2e/p0-NNN-*` 目录；若未来需要部署级 BDD，可由 scenarios owner 按当前编号把同一 Given / When / Then 提升为 repo-tracked local runner 或 release-owned scenario 资产，不改变本 plan 的 API 行为语义
