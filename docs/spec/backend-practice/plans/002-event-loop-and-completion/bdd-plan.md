...
| `E2E.P0.038` | appendSessionEvent 并发 seq_no 序列化 | 用户 A 拥有 `practice_sessions(status='running')` 与 currentTurn；scenario runner 准备两个并发客户端请求，均携带不同 `clientEventId`，但都针对同一个 currentTurn | 两个客户端近同时 `POST /practice/sessions/{sessionId}/events kind='answer_submitted'` | 服务端通过 `SELECT FOR UPDATE` 序列化：恰好一个请求被接受并返回 200 + `SessionEventResult.acknowledged:true`，另一个返回 409 `PRACTICE_SESSION_CONFLICT` 或等价 stale-turn conflict，且 envelope 不泄露另一请求 payload；DB `practice_session_events.seq_no` 对已接受事件严格连续无重号，UNIQUE(session_id, seq_no) 零违反；完成的 turn 数与 `outbox_events WHERE event_name='practice.turn.completed' AND aggregate_id=sessionId` 行数 1:1。N≥10 全成功的 seq_no 压力验证留在 repository integration test，不作为 BDD 用户流 gate | `test/scenarios/e2e/p0-038-practice-event-concurrent-seq-no/` |
