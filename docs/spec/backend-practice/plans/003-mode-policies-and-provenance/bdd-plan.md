...
| `E2E.P0.049` | strict hint 拒绝 × goal 矩阵 | alternate + failure | `backend/cmd/api/practice_http_scenario_test.go::TestE2EP0049PracticeHintStrictRefusalAcrossGoals` |
...
### E2E.P0.049 strict hint 拒绝

| Given | When | Then |
|-------|------|------|
| 用户 A 拥有三类 current goal 的 strict running session | 用户对每个 session 发起 `hint_requested` | 返回 409 + `PRACTICE_SESSION_CONFLICT`；details 包含 `policy='hint_disabled_in_mode'` 和 `mode='strict'`；hint_text 保持 NULL；reserved event finalized 为 sanitized conflict payload；replay 同一 409；AI/outbox/audit/task-run 不增加 |

### E2E.P0.050 provenance and task-run boundary

| Given | When | Then |
|-------|------|------|
| 用户 A 拥有 assisted running session；fake F3/A3 支持 follow-up 与 lightweight observe；task-run writer 可观测 | 用户依次触发 answer_submitted、hint_requested、turn_skipped、session_paused、最终 answer_submitted | 每个 AssistantAction provenance JSON 只含 `promptVersion` / `rubricVersion` / `modelId` / `language` / `featureFlag` / `dataSourceVersion`；runtime 字段不出现在 wire；task-run writer 按实际 AI action 记录 typed columns |
