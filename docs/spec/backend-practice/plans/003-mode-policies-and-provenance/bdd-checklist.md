...
- [x] 在 `backend/cmd/api/practice_http_scenario_test.go` 新增 `TestE2EP0049PracticeHintStrictRefusalAcrossGoals`：4 个 user/plan/session 命名空间（goal 分别为 baseline / retry_current_round / next_round / debrief，mode 全部 strict）
- [x] 准备 fake F3 RegistryClient + fake AIClient：fake AIClient 设置调用计数器，断言**不**被调用
- [x] 实现 setup：写入 4 个 ready plan + 4 个 running session
- [x] 实现 trigger：对每个 session 发起 1 次 `POST /practice/sessions/{sessionId}/events kind='hint_requested'`
- [x] 实现 verify：
...
- [x] 执行 `cd backend && go test ./cmd/api -run TestE2EP0049PracticeHintStrictRefusalAcrossGoals -count=1`
- [x] 记录验证证据
...
- [x] 在 `backend/cmd/api/practice_http_scenario_test.go` 新增 `TestE2EP0051PracticeHintDegradeAndPrivacy`
- [x] 准备 fake F3 RegistryClient + fake AIClient + fake AITaskRunWriter；按场景分支配置：① F3 `registry.ErrPromptUnsupported`；② A3 secret missing；③ A3 timeout；④ A3 invalid output；外加 1 个 strict session 用于隐私 baseline
- [x] 实现 setup：写入 4 个 assisted plan/session 分别绑定 4 种失败注入 + 1 个 strict plan/session
- [x] 实现 trigger：对每个 assisted session 发起 1 次 `hint_requested`；对 strict session 发起 1 次 `hint_requested`
- [x] 实现 verify：
...
- [x] 实现 cleanup
- [x] 执行 `cd backend && go test ./cmd/api -run TestE2EP0051PracticeHintDegradeAndPrivacy -count=1`
- [x] 记录验证证据

## 收口

- [x] `cd backend && go test ./cmd/api -run 'TestE2EP0048|TestE2EP0049|TestE2EP0050|TestE2EP0051' -count=1` 全绿
- [x] `python3 scripts/lint/backend_practice_legacy.py --repo-root . --phase all` 通过（含 003 scoped legacy 反查项）
- [x] `python3 -m pytest scripts/lint/backend_practice_legacy_test.py -q` 通过
