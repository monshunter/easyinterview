...
|-----------|---------------------|----------------------------------|
| Mode dispatch | assisted and strict mode produce `show_hint` AI path; goal is orthogonal | `cd backend && go test ./internal/practice -run 'Test.*Hint.*StrictMode|TestServiceAppliesHintAIForAssisted' -count=1` |
...
- Boundary lint lives under `scripts/lint/backend_practice_out_of_scope.py` and its pytest coverage.
