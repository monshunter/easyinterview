...
| D-9 | 当前 UI 产品范围下的练习 / 报告枚举 | `PracticeMode = assisted / strict`；`PracticeGoal = baseline / retry_current_round / next_round`；原 `MistakeStatus` 改为 `QuestionReviewStatus = open / queued_for_retry / resolved` | 对齐 product-scope 当前范围与 `docs/ui-design`：mode 只表达辅助度；D-22 后复盘面试来源不再由 `goal='debrief'` 表达；移除热身、单题深钻、反问专练、独立错题本、独立成长中心和真实复盘模块；报告内部题目回顾与本轮复练仍保留 |
| D-10 | Resume Workshop additive vocabulary 升级（已落地） | 新增 3 个生成枚举与 2 个错误码常量：`ResumeVersionType = structured_master / targeted`；`ResumeSeedStrategy = copy_master / blank / ai_select`；`ResumeTailorSuggestionStatus = pending / accepted / rejected`；`RESUME_EXPORT_NOT_AVAILABLE` 错误码常量（前缀 `RESUME_*`，类比 D-12 `PRIVACY_EXPORT_NOT_AVAILABLE`，用于 B2 D-18 `exportResumeVersion` P0 `501` 响应）；`RESUME_STRUCTURED_MASTER_ALREADY_EXISTS` 错误码常量（前缀 `RESUME_*`，用于 `backend-resume/002` 的 `confirmResumeStructuredMaster` 409 响应）；术语映射决策：UI 真理源 `docs/ui-design/resume-module.md` 中的 `ResumeSource` ≡ OpenAPI `ResumeAsset`（后端真理源不重命名），UI `ResumeVersion` ≡ OpenAPI 新 schema `ResumeVersion`（B2 D-18 新增），前端通过 adapter 层 wrap UI 命名；`shared/conventions.yaml` 字面量、Go/TS generated 类型、parity fixture 与 B1 lint 已由 [openapi-v1-contract/004-resume-additive-coverage](../openapi-v1-contract/plans/004-resume-additive-coverage/plan.md) Phase 1 与 [backend-resume/002](../backend-resume/plans/002-versions-tailor-runs-and-save-v1/plan.md) Phase 1 落地 | 业务代码 (`backend-resume` / `frontend-resume-workshop`) 不得绕过 B1 私造同义字符串；B2 D-18/D-23 schema 必须通过 `$ref` 引用本 spec 锁定的枚举 / 错误码字面量；shared-conventions-codified 当前枚举类型（注：3 个 resume version 枚举与 `RESUME_STRUCTURED_MASTER_ALREADY_EXISTS` 错误码已由 D-20 退役，term mapping 见 D-20） |
| D-20 | 简历资产扁平化 vocabulary 退役（contract rename + 删表，product-scope v2.1 D-20 锁定） | 退役 D-10 的 3 个生成枚举（`ResumeVersionType` / `ResumeSeedStrategy` / `ResumeTailorSuggestionStatus`，sourceSection 5.14/5.15/5.16）与 `RESUME_STRUCTURED_MASTER_ALREADY_EXISTS` 错误码（`confirmResumeStructuredMaster` 随版本树删除）；`shared/conventions.yaml` 生成枚举类型 19→16；保留 `RESUME_EXPORT_NOT_AVAILABLE`（export 改为资产级 `exportResume` `501`）；术语映射重写：resume 扁平为单一实体，物理表 `resume_assets`→`resumes`、列 `resume_asset_id`→`resume_id`，OpenAPI `ResumeAsset`/`ResumeVersion` schema 与 `resumeAssetId`/`resumeVersionId` 字段统一坍缩为 `Resume`/`resumeId`，UI resume ≡ OpenAPI `Resume`（取代 D-10 的 `ResumeSource≡ResumeAsset` / `ResumeVersion≡ResumeVersion` 映射） | 由 [openapi-v1-contract/004](../openapi-v1-contract/plans/004-resume-additive-coverage/plan.md) 与 [backend-resume](../backend-resume/spec.md) 的 D-20 phase 同步 `shared/conventions.yaml`、Go/TS generated enums/errors、B2 `ApiErrorCode` 与 parity fixtures；零 `ResumeVersionType`/`ResumeSeedStrategy`/`ResumeTailorSuggestionStatus`/`resumeVersionId`/`resumeAssetId` 残留 |
...
|----|------|-------|------|------|-----------|
| C-1 | 真理源生成 Go 类型 | `shared/conventions.yaml` 已落地 | 执行 `make codegen-conventions`（B1 持有） | `backend/internal/shared/types/*.go` 中 16 个枚举类型常量、`PageInfo` 结构与共享常量按 D-5 / D-6 命名生成；Go `APIError` 结构在 `backend/internal/shared/errors/` 手写，generator 补齐全部错误码常量（含 A3 `AI_*` baseline、`RESUME_EXPORT_NOT_AVAILABLE`；`RESUME_STRUCTURED_MASTER_ALREADY_EXISTS` 已由 D-20 退役）；`go vet ./backend/...` 通过 | 001-bootstrap + A3 spec remediation + openapi-v1-contract/004 + backend-resume/002 + D-20 contract phase |
| C-2 | 真理源生成 TS 类型 | 同 C-1 | 同 C-1 | `frontend/src/lib/conventions/*.ts` 中 16 个 union string literal 类型、`ApiError` / `PageInfo` interface 与全部错误码常量（含 A3 `AI_*` baseline、`RESUME_EXPORT_NOT_AVAILABLE`；`RESUME_STRUCTURED_MASTER_ALREADY_EXISTS` 已由 D-20 退役）按 D-5 / D-6 生成；`pnpm tsc --noEmit` 通过 | 001-bootstrap + A3 spec remediation + openapi-v1-contract/004 + backend-resume/002 + D-20 contract phase |
...
| 日期 | 版本 | 变更 | 关联计划 |
|------|------|------|----------|
| 2026-06-13 | 1.21 | D-20 简历扁平化：退役 D-10 新增的 3 个 resume version 生成枚举（`ResumeVersionType` / `ResumeSeedStrategy` / `ResumeTailorSuggestionStatus`）与 `RESUME_STRUCTURED_MASTER_ALREADY_EXISTS` 错误码（`confirmResumeStructuredMaster` 随版本树删除）；保留 `RESUME_EXPORT_NOT_AVAILABLE`；术语映射重写为 resume 单一扁平实体（物理表 `resume_assets`→`resumes`，`ResumeAsset`/`ResumeVersion`/`resumeAssetId`/`resumeVersionId` 坍缩为 `Resume`/`resumeId`）。同步修正历史计数漂移：`shared/conventions.yaml` 实际生成枚举类型为 19（spec 旧述 17 为陈旧值），退役 3 个后为 16；§1/§3.1 D-6/§6 C-1/C-2 计数同步为 16。enum/errors 实际删除由 openapi-v1-contract/004 与 backend-resume 的 D-20 phase 落地。 | openapi-v1-contract/004 + backend-resume D-20 phase |
