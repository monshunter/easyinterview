...
- `debrief` 屏（正式 route=`debrief`；宽松入口 `debrief_full` 先经 route normalization 归一为 `debrief`；保留默认 App chrome / TopBar；TopBar 一级导航 `debrief` 入口高亮）：
  ...
  - 3 个 in-page picker modal（`<DebriefContextPickerModal>` lines 434-518）：
    ...
    - Resume picker：列出用户已有 `resume_versions`（generated `listResumeVersions`），单选确认
  - 文本模式 AI 推荐问题流程：
...
- Mock Session picker 是否支持 `listPracticeSessions(targetJobId, status='completed')` server-side filter：plan 001 Phase 0 验证 B2 现状；如不支持，前端 client-side filter 并记录 fallback；不阻塞主流程。
...
| `listResumeVersions` | `openapi/fixtures/Resumes/listResumeVersions.json`（既有） | DebriefContextPickerModal Resume picker | backend-resume 既有 handler | `resume_versions` read | none | `E2E.P0.065` |
| `listPracticeSessions` | `openapi/fixtures/PracticeSessions/listPracticeSessions.json`（既有，如不支持 status filter 走 client-side） | DebriefContextPickerModal Mock Session picker | backend-practice 既有 handler | `practice_sessions` read | none | `E2E.P0.065` |
