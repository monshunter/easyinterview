...
- `debrief` 屏（正式 route=`debrief`；宽松入口 `debrief_full` 先经 route normalization 归一为 `debrief`；保留默认 App chrome / TopBar；TopBar 一级导航 `debrief` 入口高亮）：
  ...
  - 文本模式 AI 推荐问题流程：
    ...
    - 用户交互：「遇到过」→ 写入 entries 一行 source='ai_confirmed'；「没问到」→ 跳过当前 guide；「改成真实问题」→ 编辑后写入 entries source='ai_edited'；「手动添加」→ 写入 entries source='manual'
...
- `PracticeScreen` 任何 UI / 状态机消费 / 文本 surface / voice surface / 完成动作 / 复盘面试 session 启动逻辑：由 [frontend-workspace-and-practice](../frontend-workspace-and-practice/spec.md) 承接；本 spec 只在 step 2 CTA 中 nav 回 practice。
...
| D-8 | Context picker 范围 | 3 个 in-page modal（JD / MockSession / Resume）；所有 modal 在当前 debrief 页打开（不跳转）；JD picker 调 generated `listTargetJobs(user_id)` filtered by status='ready'；Mock Session picker 依赖 Phase 0 backend-practice/B2 addendum 生成 `listPracticeSessions({targetJobId,status:'completed'})`，未生成前 BLOCK，实现后如不支持 status filter 则前端 client-side filter；Resume picker 调 generated `listResumes()` 列资产，再对选中 asset 调 `listResumeVersions(resumeAssetId)` 展开 ready versions；用户可"暂不关联模拟面试"（Mock 是 optional） | 与 review-module.md §5.1 "三个上下文动作都不得跳出复盘页" 一致 |
...
- Mock Session picker 所需 `listPracticeSessions({targetJobId,status:'completed'})` 目前在 generated client 中缺失；plan 001 Phase 0 必须先由 backend-practice/B2 addendum 落地 operation + fixture + generated client。只有 operation 已生成但缺少 status filter 时，才允许前端 client-side filter；operation 缺失本身阻塞 Phase 1。
...
| `listPracticeSessions` | `openapi/fixtures/PracticeSessions/listPracticeSessions.json`（Phase 0 backend-practice/B2 addendum 新增；至少支持 targetJobId，status filter 可由 server 或 client 处理） | DebriefContextPickerModal Mock Session picker | backend-practice Phase 0 addendum handler | `practice_sessions` read | none | `E2E.P0.065` |
...
| `createPracticePlan` / `startPracticeSession` | — | 本 spec **不**调用；由 frontend-workspace-and-practice 在 practice 路由 mount 时调用 | — | — | — | 负向断言（plan 001 Phase 5 negative grep + Vitest spy 确认在 frontend/src/app/screens/debrief/ 0 调用） |
