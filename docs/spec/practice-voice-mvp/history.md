...
| 日期 | 版本 | 变更 | 关联计划 |
|------|------|------|----------|
| 2026-05-22 | 1.5 | Repo lint follow-up：收窄 backend-practice legacy lint 的 retired standalone voice route 匹配，继续拦截独立 `/voice` route / alias，但允许 `createPracticeVoiceTurn`、`/voice-turns`、`voice-turn://` persisted ref 与 `practice.voice.*` profile / feature key；focused lint test、`make lint-backend-practice-legacy` 与 `make lint` 通过。 | [001-cascaded-stt-llm-tts](./plans/001-cascaded-stt-llm-tts/plan.md) |
