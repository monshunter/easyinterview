...
- [ ] 实现 setup / trigger / verify / cleanup；verify 断言 transcript、assistant text、TTS playback state、voice turn event、profile meta 摘要和下一题入口
- [ ] 执行并记录验证证据
...
- [ ] 实现 setup / trigger / verify / cleanup；verify 断言未播放 draft 不在 committed context / prompt 中，interruption note 存在
- [ ] 执行并记录验证证据
...
- [ ] 实现 setup / trigger / verify / cleanup；verify 断言 fail-fast / 文本 fallback / 不走 stub 或 realtime / privacy grep 无明文
- [ ] 执行并记录验证证据
