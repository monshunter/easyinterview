...
| 日期 | 版本 | 变更 | 关联计划 |
|------|------|------|----------|
| 2026-05-21 | 1.2 | plan 001 完成：5 个 Profile HTTP handler + `backend/internal/profile` store/service/handler + `cmd/api` route wiring（含 IK middleware 仅作用于 experience card CUD）+ B2 cross-owner additive D-24（`createExperienceCard` / `updateExperienceCard` IK + `CandidateProfile` nullable headline/yearsOfExperience/currentRole/region）+ B1 cross-owner additive（`RESOURCE_NOT_FOUND` 错误码）+ privacy delete chain + audit tombstone（PII redline）+ cross-owner `GetCandidateProfileForUser` / `CountExperienceCardsBySource` / `DeleteCandidateProfileForUser` internal API；3 个 BDD scenarios E2E.P0.091 / E2E.P0.092 / E2E.P0.093 全 PASS（本地 dev-stack Postgres）；下游 backend-jobs-recommendations/001 aggregation 与 backend internal privacy runner 可消费。 | 001-candidate-profile-and-experience-cards |
