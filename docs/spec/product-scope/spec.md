...
| D-20 | 简历资产扁平化 | 简历是平铺列表中的独立资产，不区分原始简历 / 结构化主版本 / 岗位定制版本，不做版本树、分叉、继承或版本管理；创建仅上传 / 粘贴；改写建议每条仅有`采纳`，默认不动作；采纳后确认前预览整份结果，由用户选择`覆盖原简历`或`保存为新简历` | 每份简历仍只读保留原始来源与解析文本快照；面试绑定对象为简历（`resumeId`）；改写建议不提供逐条`拒绝 / 编辑` |
