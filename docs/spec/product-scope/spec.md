...
| D-14 | JD 导入单次确认 | `parse` 解析确认页同时承载面试启动决策：核对解析结果、绑定 ResumeVersion、确认 InterviewRound、立即面试；首次导入链路只允许一次全页确认 | `workspace` 不再是首次导入的必经第二确认页，定位为回访枢纽（最近面试、切换/新建规划、公司轻情报、会话历史、再次发起面试）；不得在 parse 与 session 之间恢复平行确认页 |
| D-15 | 复盘上下文选一带二 | 复盘上下文任一项选定后，可推导的其余上下文自动带入并保持可更换；选择仍全部在复盘页弹窗内完成 | 选关联模拟面试自动带出其绑定 JD 与简历；选目标 JD 自动带出该规划默认简历与最近关联面试；自动带入项必须可见可改 |
...
| 模拟面试 | 回访并管理既有面试规划，再次发起 session | 当前面试规划（回访枢纽）、切换/新建规划、公司轻情报、会话历史、立即面试 | 不作为泛岗位资产管理中心；不作为首次 JD 导入的必经第二确认页 |
...
| 页面 | 归属 | 进入方式 | 关键上下文 |
|------|------|----------|------------|
| `parse` | JD 解析确认 + 面试启动 | 首页 JD 导入或岗位推荐确认 | `jdId / targetJobId / resumeVersionId / roundId` |
...
- 模拟面试规划（回访枢纽）必须明确 `TargetJob / JD / ResumeVersion / InterviewRound`。
...
- 选择关联模拟面试。
- 绑定简历。
- 上下文选一带二：任一上下文选定后，可推导的其余上下文自动带入并保持可更换。
- 文本添加真实问题。
- 语音连续对话复盘。
...
- 三个上下文选择都在复盘页弹窗内完成。
- 任一上下文选定后，可推导的其余上下文自动带入，标注“已自动带入”，且仍可逐项更换。
...
| C-14 | 复盘上下文联动 | 用户在复盘页尚未选择上下文 | 选定关联模拟面试或目标 JD | 可推导的其余上下文自动带入、标注“已自动带入”且可逐项更换 | `docs/ui-design/review-module.md` |
