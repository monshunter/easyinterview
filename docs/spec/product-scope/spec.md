...
| D-21 | 全局呈现收敛 | 设置页删除`通知`与`订阅`P1 占位 tab；主题删除`自定义 accent`（色相 / 饱和度滑杆），保留四个预设主题 + 暗色模式 + 语言下拉 + 字体预设 | 设置页只保留个人资料与隐私数据两个 tab；占位能力未来按需重新设计，不以空 tab 形式预留 |
...
| 主题自定义 accent | 移除（D-21） | 保留四个预设主题 + 暗色模式即可覆盖呈现需求 |
