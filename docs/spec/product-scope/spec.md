...
- 简历边界：平铺简历列表、上传 / 粘贴创建、解析预览确认、简历详情（预览、改写建议、手动编辑）、采纳改写后覆盖原简历或另存为新简历。
...
- 不恢复旧版独立成长中心、多轮计划、经历库、追问树、单题 Drill、独立错题队列、报告时间线、刊物式报告页、真实面试复盘或用户画像。
...
| D-6 | 语音边界 | 语音是面试形式，不是独立业务模块；当前目标路由只允许 `practice` 显式携带 `mode=voice` / `modality=voice` | 删除旧 `voice` route alias 和独立语音页面骨架；文本输入框麦克风只表示语音转文字 |
...
| D-9 | 真实面试复盘 | 已被 D-22 取代：复盘不再是 P0 模块或后续默认恢复能力 | 不保留 `debrief` route、API、DB、AI feature key 或场景 |
...
| D-12 | Roadmap 对齐 | engineering-roadmap active spec 已删除 `docs/spec/INDEX.md` pending child 占位模型，只保留真实 active spec，并要求后续 child 按当前 Home / Mock Interview / Practice / Report / Resume / Settings/Auth 等保留能力 on-demand 创建 | 后续 child spec / plan 不得用旧 pending 名称或旧 route 恢复已丢弃模块 |
...
| D-14 | JD 导入单次确认 | `parse` 解析确认页同时承载面试启动决策：核对解析结果、绑定简历、确认 InterviewRound、立即面试；首次导入链路只允许一次全页确认 | `workspace` 不再是首次导入的必经第二确认页，定位为回访枢纽（最近面试、切换/新建规划、公司轻情报、会话历史、再次发起面试）；不得在 parse 与 session 之间恢复平行确认页 |
| D-15 | 复盘上下文选一带二 | 已被 D-22 取代：复盘上下文选择不再保留为产品能力 | 相关 UI / API / 场景全部删除 |
| D-16 | 无密码认证唯一流 | 邮箱验证码是唯一登录方式；不保留独立“重置登录 / 密码重置”页面，`auth_reset` 移除 | 验证码重发与更换邮箱在 `auth_verify` 内完成；旧 `auth_reset` route 归一回 `auth_login`，文档与 UI 不得再出现“忘记密码 / 密码 / 两步验证”口径 |
| D-17 | 删除岗位推荐模块 | `岗位推荐 / Job Picks / jd_match` 一级模块整体删除，含推荐列表、联网搜索、关注列表与画像摘要呈现；JD 获取唯一入口是首页导入 | 一级导航曾收敛为四项，D-22 后继续收敛为三项；主流程 B（岗位推荐来源）删除；Q-2 关闭；`全球多平台搜岗` 规划例外随模块一并丢弃 |
| D-18 | 公司情报仅保留轻量嵌入卡片 | 删除 `company_intel` 独立详情页与 route；模拟面试规划页内嵌轻量卡片是公司情报唯一呈现 | 轻量卡片仍展示一句话画像、近期公开信号、反问建议与合规来源说明；P2 生产化仍以嵌入卡片为呈现边界，不得恢复独立情报页 |
...
| D-20 | 简历资产扁平化 | 简历是平铺列表中的独立资产，不区分原始简历 / 结构化主版本 / 岗位定制版本，不做版本树、分叉、继承或版本管理；创建仅上传 / 粘贴（删除轻量问答）；改写建议每条仅有`采纳`，默认不动作；采纳后确认前预览整份结果，由用户选择`覆盖原简历`或`保存为新简历` | 每份简历仍只读保留原始来源与解析文本快照；面试绑定对象从 ResumeVersion 改为简历（`resumeId`）；改写建议不再提供逐条`拒绝 / 编辑` |
| D-21 | 全局呈现收敛 | 设置页删除`通知`与`订阅`P1 占位 tab；主题保留四个预设 + `自定义 accent`（色相 / 饱和度滑杆）+ 暗色模式 + 语言下拉 + 字体预设，默认主题为`深海` | 设置页只保留个人资料与隐私数据两个 tab；占位能力未来按需重新设计，不以空 tab 形式预留；主题自定义按用户决策保留（v2.1 回调首版裁剪） |
| D-22 | 核心闭环裁剪方案 B | 删除真实面试复盘和用户画像：`debrief` / `profile` 不再是目标 route，`Debriefs` / `Profile` OpenAPI tags、`debriefs` / `candidate_profiles` / `experience_cards` DB 表、debrief/profile AI feature key、shared event/job 和正向场景全部清理 | P0 只保留 JD / 简历 -> 模拟面试 -> 报告 -> 复练当前轮 / 进入下一轮；账号资料补全和设置隐私保留，但不承担候选人画像语义 |
...
> Q-2（岗位推荐数据来源策略）已随 D-17 删除岗位推荐模块而关闭，不再是待确认事项。
...
- `debrief` / `debrief_full` / `profile` 不再是目标 route；旧路径或 hash 输入必须归一到当前核心入口，不渲染旧页面。
...
| M2 · Target Job Workspace | 围绕 JD 管理岗位要点、简历绑定、轮次假设、公司轻情报和会话历史 | 首页 JD 导入、模拟面试规划 |
...
| 模拟面试 | 回访并管理既有面试规划，再次发起 session | 当前面试规划（回访枢纽）、切换/新建规划、公司轻情报嵌入卡片、会话历史、立即面试 | 不作为泛岗位资产管理中心；不作为首次 JD 导入的必经第二确认页 |
| 简历 | 管理可被岗位和面试消费的简历资产 | 平铺简历列表、上传 / 粘贴创建、解析预览确认、简历详情（预览 / 改写建议 / 手动编辑）、采纳改写后覆盖或另存 | 不做版本树 / 主版本 / 岗位定制继承；不做复杂排版设计器 |
...
| 成长中心 | 已决策移除为独立模块 | 只保留嵌入报告或面试规划的趋势信号 |
| 多轮计划 | 移除独立系统 | 轮次节点在模拟面试规划和报告 CTA 中表达 |
| 经历库 / STAR 编辑器 | 移除独立模块 | 经历证据由简历和面试上下文承载 |
| 报告时间线 / 刊物式报告 | 移除 | 报告统一为 Dashboard |
| 岗位推荐一级模块 | 移除（D-17） | 超出 MVP 闭环；JD 获取唯一入口是首页导入 |
| 公司情报独立详情页 | 移除（D-18） | 轻量情报由模拟面试规划页嵌入卡片承载 |
| 简历版本树 / 主版本 / 岗位定制版本 | 移除（D-20） | 简历按平铺资产管理，不做版本继承 |
| 轻量问答建档 | 移除（D-20） | 创建简历只保留上传 / 粘贴 |
| 设置页通知 / 订阅占位 tab | 移除（D-21） | 占位能力未来按需重新设计，不以空 tab 预留 |
| 真实面试复盘 / Debrief | 移除（D-22） | 不再是 P0 核心闭环的一环，避免把用户从 JD / 简历 / 模拟 / 报告主线拉到平行记录系统 |
| 用户画像 / CandidateProfile / ExperienceCard | 移除（D-22） | 画像页和候选人画像数据模型增加前后端复杂度，但不直接缩短当前核心训练闭环；账号资料和设置隐私保留 |
...
首次导入链路只有一次全页确认：解析确认页同时完成核对与启动决策。`Mock Interview Plan` 不再插在首次导入与 session 之间，它服务回访场景：从首页最近模拟面试、报告或会话历史回到既有规划，切换/新建规划，并再次发起面试。
...
真实面试复盘已随 D-22 删除，不再作为 P0 主流程、一级入口、后续默认 workstream 或场景验收对象。用户完成模拟面试后的下一步只保留报告内的 `复练当前轮` 与 `进入下一轮`。

### 6.5 已删除 M1-lite：渐进式画像

#### 目标

候选人画像页、画像纠偏、Experience Card 和独立画像数据模型已随 D-22 删除。上下文仍来自简历、JD、面试 session 和报告，但只作为当前任务输入和证据来源，不沉淀为用户可见画像模块。
...
- 岗位推荐 / 全球多平台搜岗：已随 D-17 整体丢弃，不再是规划例外。
- 复杂职位收藏生态。
- 大规模外部公司情报拼图与独立情报详情页。
- 把 `当前岗位` 做成独立一级模块。
...
- 上传 / 粘贴创建，Agent 解析，预览确认后保存。
- 每份简历只读保留原始来源（文件或粘贴文本）和解析文本快照。
- 简历详情：预览、改写建议、手动编辑。
- 改写建议每条仅提供 `采纳`；默认不动作即忽略，不提供逐条 `拒绝 / 编辑`。
- 采纳改写后，确认前预览整份结果，由用户选择 `覆盖原简历` 或 `保存为新简历`。
- 原件预览、导出、复制等操作反馈。
...
- 采纳改写后必须先看到整份预览，再选择覆盖原简历或保存为新简历；未采纳的建议不产生任何变更。
- 模拟面试规划可以绑定列表中的任意一份简历。

### 6.10 已删除 M4 扩展：真实面试复盘

真实面试复盘、复盘分析、复盘面试和 debrief-derived practice plan 已随 D-22 删除。`debrief` 不再是 route、OpenAPI tag、DB 表、AI feature key、shared event/job 或 E2E 场景。

### 6.11 已丢弃能力

本节不是“P0 暂不做”清单，而是已决策丢弃的旧能力记录。

| 能力 | 当前判断 | 原因 |
|------|----------|------|
| 岗位推荐一级模块 | 已丢弃（D-17） | 超出 MVP 闭环；JD 获取唯一入口是首页导入 |
| 全球多平台搜岗 | 已丢弃（随 D-17） | 岗位推荐模块删除后失去承载方向；如未来重新引入须按 §2.4 重新设计审批 |
| 公司情报独立详情页 | 已丢弃（D-18） | 轻量情报由模拟面试规划页嵌入卡片承载 |
| 简历版本树 / 主版本 / 岗位定制版本 / 轻量问答 | 已丢弃（D-20） | 简历按平铺资产管理；创建只保留上传 / 粘贴 |
...
| 独立成长中心 | 已丢弃 / 长期不恢复 | 当前只保留嵌入报告和面试规划的准备度 / 趋势信号 |
| 独立多轮计划 | 已丢弃 / 长期不恢复 | 面试轮次只在模拟面试规划和报告 CTA 中表达 |
| 独立经历库 / STAR 编辑器 | 已丢弃 / 长期不恢复 | 经历证据由简历和面试上下文承载 |
| 独立错题本 / 单题 Drill / 追问树 | 已丢弃 / 长期不恢复 | 题目问题留在报告题目回顾和本轮复练，不形成单题流程 |
| 报告时间线 / 刊物式报告 | 已丢弃 / 长期不恢复 | 报告统一为 session-scoped Dashboard |
| 真实面试复盘 / Debrief | 已丢弃（D-22） | 当前核心闭环收敛到模拟面试报告后的复练 / 下一轮，不维护平行复盘系统 |
| 用户画像 / CandidateProfile / ExperienceCard | 已丢弃（D-22） | 账号资料和设置隐私保留，但不再沉淀独立候选人画像产品或数据模型 |
...
- 准备度和趋势信号只嵌入报告和岗位规划，不恢复独立成长中心或用户画像。
...
| C-16 | 岗位推荐与情报独立页零入口 | 岗位推荐模块与公司情报独立页已删除 | 走查导航、首页与静态原型路由 | 不存在 `jd_match` / `company_intel` 目标 route、岗位推荐入口或独立情报详情页；公司情报只出现在模拟面试规划页嵌入卡片 | `docs/ui-design/removed-modules-and-scope.md` |
...
| C-19 | 复盘和用户画像零入口 | D-22 已生效 | 走查 TopBar、用户菜单、URL/hash route、OpenAPI、DB、shared、config 和场景索引 | 不存在 `debrief` / `profile` 目标 route、`Debriefs` / `Profile` OpenAPI tag、`debriefs` / `candidate_profiles` / `experience_cards` 表或正向场景 | [001-core-loop-module-pruning](./plans/001-core-loop-module-pruning/plan.md) |
...
本 spec 当前挂载 [001-core-loop-module-pruning](./plans/001-core-loop-module-pruning/plan.md) 作为 D-22 跨层删除 owner。后续如果要调整 P0/P1 阶段、恢复旧能力或新增用户可见行为，必须先在本 subject 下创建或原地修订 plan / checklist / context.yaml，再进入 `/implement`。

## 11 修订记录

| 版本 | 日期 | 修订内容 |
|------|------|----------|
| 2.2 | 2026-06-29 | 用户确认方案 B：删除真实面试复盘和用户画像，将 P0 收敛为 JD / 简历 -> 模拟面试 -> 报告 -> 复练当前轮 / 进入下一轮；新增 D-22、C-19 和 001-core-loop-module-pruning owner plan。 |
| 2.1 | 2026-06-12 | 用户回调第二批裁剪中的主题部分：恢复主题`自定义 accent`模式，默认主题改为`深海`；D-21 改写为仅删除设置占位 tab，§5.4 移除表删去主题自定义 accent 行 |
| 2.0 | 2026-06-12 | UX 冗余第二批裁剪（如无必要勿增实体）：新增 D-17（删除岗位推荐模块，一级导航收敛为四项，关闭 Q-2，全球搜岗规划例外随之丢弃）、D-18（删除 company_intel 独立页，公司情报仅保留 workspace 嵌入卡片）、D-19（报告 CTA 单点收敛）、D-20（简历资产扁平化：删除版本树 / 主版本 / 岗位定制 / 轻量问答，改写建议仅采纳，采纳后覆盖或另存）、D-21（设置占位 tab 与主题自定义 accent 删除）；删除主流程 B（岗位推荐来源）并重排 §6，重写 M5，更新 §5.1-§5.4、§7、C-3/C-9 并新增 C-16/C-17/C-18 |
| 1.9 | 2026-06-12 | UX 冗余裁剪首批方案：新增 D-14（JD 导入单次确认，parse 承载启动决策、workspace 回访枢纽化）、D-15（复盘上下文选一带二）、D-16（无密码认证唯一流，移除 auth_reset）；同步主流程 A/B、§5.2/§5.3 模块与页面表、M2/M4 扩展验收标准与 C-4/C-14/C-15 验收场景 |
