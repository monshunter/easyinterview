...
| D-29 | 报告归属的只读会话记录 | 一份报告附属一份完成会话记录；用户只以 `reportId` 进入 `/report-conversation?reportId=...`，不提供会话历史列表或 `sessionId` 用户路由 | 现有 `feedback_reports.session_id` 唯一外键已经表达未完成会话 `0` 份报告、可报告完成会话 `1` 份报告；不新增关系表。报告资源存在时，无论 queued / generating / ready / failed 均可查看只读 Markdown transcript；无报告则无记录入口 |
...
- `report-conversation` 是 Report 的只读附属页，只以 `reportId` 定位；不得暴露 `sessionId`、创建会话历史一级入口或让 Workspace 直接消费会话列表。主入口位于 Report Dashboard 的 Context Strip 下方，ReportsScreen 的同轮 current report 行可以提供快捷入口。
...
| M2 · Target Job Workspace | 围绕 JD 管理岗位要点、简历绑定、轮次假设、公司轻情报和报告入口 | 首页 JD 导入、面试规划列表与统一面试规划详情 |
...
| 面试 | 浏览并回访既有面试规划，再次发起 session | `/workspace` 面试规划列表、`/workspace?targetJobId=...` 当前面试规划只读详情（回访枢纽）、公司轻情报嵌入卡片、报告入口、立即面试 | 不作为泛岗位资产管理中心；不在 `/parse` 复刻 ready 详情或解析动画；不建立会话历史列表 |
...
| `report` | Report Dashboard | 面试结束、ReportsScreen 当前报告入口 | `reportId` |
| `report-conversation` | 报告附属只读会话记录 | Report Dashboard 主入口、ReportsScreen 同轮 current report 快捷入口 | `reportId`；不得使用 `sessionId` |
...
- Header：目标岗位、轮次、绑定简历和报告归属说明。
- Context Strip：目标岗位、轮次、简历；`sessionId` / `reportId` 不作为用户字段展示。
...
Evidence Surface 展示会话级 highlights / issues 的 dimension label、证据摘要和本地化置信度，不把完整 transcript 复制进报告正文，也不按题号或 turn 分组。完整 ordered transcript 作为该报告的只读附属页单独呈现，复用 Practice Markdown 消息渲染但移除 Composer、结束、暂停、重试和继续面试能力。每个主要判断在后端保留候选人消息 grounding anchor；未回答的 assistant 追问只能表达为“未覆盖 / 证据不足”，不能据此形成负面能力结论。复练当前轮由后端从 source report 投影可选的 report-local dimension focus；没有可支持 focus 时创建空 focus 的通用同轮复练，不使用题目 ID 或客户端 focus。

#### 当前不进入本模块

- 报告全局一级导航或跨规划中心。
- 无 session 上下文的报告页。
- 独立会话历史一级模块、会话历史列表、`sessionId` 深链或新建会话—报告关系表。
- 时间线报告。
- 刊物式报告页。
...
- 报告资源存在时，用户可从报告详情或 ReportsScreen 同轮 current report 行打开只读会话记录；queued / generating / ready / failed 共享同一 report-owned 访问边界，跨用户或缺失 report 统一 hidden 404。
...
| 版本 | 日期 | 修订内容 |
|------|------|----------|
| 2.23 | 2026-07-15 | D-29：会话记录归属于单份报告，以 reportId-only 只读 Markdown 页呈现；报告资源存在即允许查看，不新增 session 历史列表、sessionId 用户路由或关系表，并以现有 feedback_reports.session_id 唯一外键作为数据边界。 |
