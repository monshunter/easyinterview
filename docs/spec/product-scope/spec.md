...
EasyInterview 当前产品形态由 `docs/spec/product-scope/spec.md`、`docs/ui-design/`、`ui-design/` 和各工程 owner spec 共同约束。产品工作台只围绕 `首页 / 面试 / 简历` 三个一级入口组织；规划详情内容区可进入 target-scoped ReportsScreen 索引当前规划轮次报告，但它不属于全局导航或第二种报告内容形态。报告内容只有 `Report Dashboard(reportId)` 一种形态，并且必须区分 `复练当前轮` 与 `进入下一轮`。
...
- 报告索引边界：规划详情右上角进入 `/reports?targetJobId=...`，只展示当前规划 canonical rounds 的 current report 与 latest attempt；不做跨规划中心或完整历史版本列表。
...
| D-21 | 全局呈现收敛 | 设置页只保留个人资料与隐私数据两个 tab；主题保留四个预设 + `自定义 accent`（色相 / 饱和度滑杆）+ 暗色模式 + 语言下拉 + 字体预设，默认主题为`深海` | 通知、订阅等范围外能力未来按需重新设计，不以空 tab 形式预留；主题自定义按用户决策保留 |
...
| D-23 | 面试入口 landing | 一级导航 `面试` 默认进入面试规划列表；带 `targetJobId` / `planId` 等上下文时进入当前面试规划详情 | 避免用户从一级入口进入后看到无上下文死胡同；列表使用当前 `listTargetJobs` 规划候选，不新增独立多轮计划或泛岗位资产中心 |
...
| D-27 | 当前规划报告入口 | Parse 内容区右上角进入 `/reports?targetJobId=...`；ReportsScreen 只索引当前 TargetJob canonical rounds 的 current report 与 latest attempt；Report/Generating trusted Back 返回该列表 | 不加入 TopBar，不嵌入 Parse，不显示其他规划或完整历史，不改数据库/OpenAPI Schema；无可信 target 时安全回 workspace |
...
- “面试报告”只允许作为规划详情内容区的页面级入口和 target-scoped 上下文页面；不得进入 TopBar 或成为无上下文全局中心。
- `workspace` 的产品语义是 `面试 / 面试规划列表 / 当前面试规划`，不是一级岗位资产管理模块；无上下文一级进入展示规划列表，缺数据时给出导入 JD 的友好空态。
...
| 面试 | 浏览并回访既有面试规划，再次发起 session | 面试规划列表、当前面试规划（回访枢纽）、切换/新建规划、公司轻情报嵌入卡片、会话记录、立即面试 | 不作为泛岗位资产管理中心；不作为首次 JD 导入的必经第二确认页 |
...
| `reports` | 当前规划报告索引 | 面试规划详情内容区右上入口、Report/Generating trusted Back | `targetJobId` |
...
首次导入链路只有一次全页确认：解析确认页同时完成核对、持久化规划与直接启动，不提供“仅保存规划”第二 CTA。已创建的 `Mock Interview Plan` 仍服务回访场景：用户以后可从首页最近模拟面试、面试一级列表、报告或会话记录回到既有规划，切换/新建规划并再次发起面试；这不是首次 Parse 页的分叉。
...
- ReportsScreen：仅按当前 TargetJob canonical rounds 显示 `currentReport/latestAttempt`，覆盖 loading/empty/error/identity mismatch；它是规划范围索引，不是报告内容页，不展示完整历史。
...
| C-23 | 当前规划报告隔离 | D-27 已生效，用户从规划详情进入报告列表或从 Report/Generating 返回 | 打开 `/reports?targetJobId=...` | 只显示当前规划 current/latest；TopBar 无报告入口、Parse 无嵌入列表/section；跨 target/mismatch/stale fail closed，无完整历史 | frontend-home 001 + frontend-report 001 + frontend-shell 004 |
