...
EasyInterview 当前产品形态由 `docs/spec/product-scope/spec.md`、`docs/ui-design/`、`ui-design/` 和各工程 owner spec 共同约束。产品工作台只围绕 `首页 / 面试 / 简历` 三个一级入口组织，报告只有 `Report Dashboard(reportId)` 一种形态，并且必须区分 `复练当前轮` 与 `进入下一轮`。
