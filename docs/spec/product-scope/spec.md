...
| D-7 | 报告形态 | 报告只有 session-scoped conversation Dashboard 一种形态 | 报告按准备度、能力维度、会话证据和下一步组织，不按题目/turn |
| D-8 | 复练价值承载 | 复练当前轮基于能力缺口和会话证据 | 不设置题目回顾、逐题评分、独立错题队列、Drill builder 或 turn-based retry |
...
| `generating` | 报告生成过渡态 | 面试结束 | `sessionId / reportId` |
| `report` | Report Dashboard | 面试结束、会话记录、相关入口 | `sessionId / reportId` |
...
- Summary Cards：准备度、能力维度、会话证据、下一动作。
- Detail Surface：准备度、能力维度、会话证据、下一步。
- Next Actions：Header 的 `复练当前轮` 和 `进入下一轮`，是报告唯一一对开练 CTA（D-19）；复练计划详情只承载路径说明与复练清单。

#### 会话证据

Evidence Surface 展示会话级 highlights / issues 的能力维度、证据摘要和置信度，不复制完整 transcript，也不按题号或 turn 分组。复练当前轮使用能力重点，不使用题目 ID。
...
- 报告的每个主要判断能回溯到会话证据、JD、简历或 session 来源。
