...
- 当前 UI 已收敛为 `首页 / 模拟面试 / 简历 / 复盘` 四个一级入口。
...
| D-12 | Roadmap 对齐 | engineering-roadmap active spec 已删除 `docs/spec/INDEX.md` pending child 占位模型，只保留真实 active spec，并要求后续 child 按当前 Home / Mock Interview / Practice / Report / Resume / Debrief / User Menu 等保留能力 on-demand 创建 | 后续 child spec / plan 不得用旧 pending 名称或旧 route 恢复已丢弃模块 |
...
| D-17 | 删除岗位推荐模块 | `岗位推荐 / Job Picks / jd_match` 一级模块整体删除，含推荐列表、联网搜索、关注列表与画像摘要呈现；JD 获取唯一入口是首页导入 | 一级导航收敛为四项；主流程 B（岗位推荐来源）删除；Q-2 关闭；`全球多平台搜岗` 规划例外随模块一并丢弃；用户画像不再用于推荐排序，画像呈现仅保留用户菜单画像页 |
...
2. **目标岗位优先**：练习、报告、简历建议、复盘都必须围绕 `TargetJob / JD / Resume / Round / Session` 建立上下文。
...
| 能力层 | 当前职责 | 当前 UI 承载 |
|--------|----------|--------------|
| M1 · Progressive Profile | 通过简历、JD、模拟面试、复盘和用户修正逐步沉淀候选人画像 | 用户画像、简历、模拟面试和复盘的共享信号 |
...
|----------|----------|---------|--------------|
| 首页 | 快速开始一次岗位准备 | JD 粘贴 / 上传 / URL 导入，最近模拟面试，创建简历入口，复盘辅助入口 | 不做登录前营销页 |
...
| `practice` | Interview Session | 模拟面试规划、报告复练、进入下一轮、复盘面试 | `sessionId / targetJobId / resumeId / roundId / mode / modality / practiceMode` |
...
用尽量少的前置成本，获得足够支撑面试定制、简历建议和复盘分析的上下文。
...
| C-3 | 一级导航一致 | 当前 UI 文档已锁定四个一级入口 | 查看 TopBar | 只出现首页、模拟面试、简历、复盘 | `docs/ui-design` |
...
| C-16 | 岗位推荐与情报独立页零入口 | 岗位推荐模块与公司情报独立页已删除 | 走查导航、首页、画像页与静态原型路由 | 不存在 `jd_match` / `company_intel` 目标 route、岗位推荐入口或独立情报详情页；公司情报只出现在模拟面试规划页嵌入卡片 | `docs/ui-design/removed-modules-and-scope.md` |
...
- API 和前端必须按用户身份隔离 TargetJob、Resume、InterviewSession、Report 和 DebriefRecord。
