...
EasyInterview 当前产品形态由 `docs/spec/product-scope/spec.md`、`docs/ui-design/`、`ui-design/` 和各工程 owner spec 共同约束。产品工作台只围绕 `首页 / 面试 / 简历` 三个一级入口组织，报告只有 `Report Dashboard(sessionId)` 一种形态，并且必须区分 `复练当前轮` 与 `进入下一轮`。
...
- 当前一级 UI 模块：`首页`、`面试`、`简历`。
- 用户菜单能力：`设置与隐私`、`认证流程`。
- 模拟面试会话边界：文本面试、电话模式、带提示练习、严格模拟、面试官角色、问题推进、结束并生成报告。
...
| D-6 | 语音边界 | 语音是面试形式，不是独立业务模块；当前电话模式路由只允许 `practice` 显式携带 `mode=phone` / `modality=phone` | 文本输入框麦克风只表示语音转文字；不设置独立语音 route 或旧 `voice` query 入口 |
...
| ID | 待确认事项 | 影响 | 默认处理 |
|----|------------|------|----------|
| Q-1 | 生产版电话模式语音能力的 P0 发布门槛 | UI 已有电话模式目标，但后端 STT、媒体留存和隐私链路可能晚于文本闭环 | 产品层保留电话模式形式；工程 release gate 可用 feature flag 控制上线 |
...
- 电话模式只能通过 `practice?mode=phone&modality=phone` 或等价显式 phone 参数进入；不得保留或新增 `voice` 目标 route / route alias / query 入口。
...
- 文本面试：用户主要通过文本回答。
- 电话模式：用户按电话面试心智与 AI 交流；底层可使用 STT / LLM / TTS 级联实现。
...
| 独立错题本 / 单题 Drill / 追问树 | 范围外 | 题目问题留在报告题目回顾和本轮复练，不形成单题流程 |
