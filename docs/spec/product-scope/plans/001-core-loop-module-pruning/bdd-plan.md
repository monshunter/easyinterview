...
| E2E.P0.088 | regression/non-current-negative | canonical route 不再包含 debrief/profile | 用户访问 canonical route 集 | 深链到 workspace/practice/generating/report/resume-versions 与非当前 `/debrief`、`/profile` | 核心 route 保持 safe params；非当前路径归一到当前核心入口，不渲染非当前页面 | `test/scenarios/e2e/p0-088-url-addressable-routing-canonical/` |
| E2E.P0.090 | regression/non-current-negative | hash non-current route 不 materialize non-current modules | 用户输入 `#route=debrief`、`#route=debrief_full`、`#route=profile` | App 解析 route | 不生成非当前 route state，不出现 non-current testid 或 screen title | `test/scenarios/e2e/p0-090-url-routing-hash-non-current-negative/` |
| E2E.P0.102 | failure/recovery | 未登录保护路由不把非当前模块当业务目标 | 未登录用户直接打开非当前业务 route | App 执行 route/auth gate | 不创建 pendingAction 到 `debrief` 或 `profile`；核心面试 route 仍按登录 gate 工作 | `test/scenarios/e2e/p0-102-auth-gated-interview-routes/` |
...
## Phase 3: Non-current 场景删除矩阵

| Non-current 场景 | 处理 | 替代覆盖 |
|--------------|------|----------|
