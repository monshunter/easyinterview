...
| E2E.P0.088 | regression/out-of-scope-negative | canonical route 不再包含 debrief/profile | 用户访问 canonical route 集 | 深链到 workspace/practice/generating/report/resume-versions 与范围外 `/debrief`、`/profile` | 核心 route 保持 safe params；范围外路径归一到当前核心入口，不渲染范围外页面 | `test/scenarios/e2e/p0-088-url-addressable-routing-canonical/` |
| E2E.P0.090 | regression/out-of-scope-negative | hash out-of-scope route 不 materialize out-of-scope modules | 用户输入 `#route=debrief`、`#route=debrief_full`、`#route=profile` | App 解析 route | 不生成范围外 route state，不出现 out-of-scope testid 或 screen title | `test/scenarios/e2e/p0-090-url-routing-hash-out-of-scope-negative/` |
