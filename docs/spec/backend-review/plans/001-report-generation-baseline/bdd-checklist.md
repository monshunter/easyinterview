...
- [x] 在 `backend/cmd/api/reports_http_scenario_test.go` 新增 `TestE2EP0052ReportGenerationHappyPath`：1 个 user/report/session/target_job 命名空间（不与 backend-practice 002+003 已占用编号冲突）
- [x] 准备 fake F3 RegistryClient + fake AIClient + fake AITaskRunWriter；fake AIClient 为 `report.generate` 返回合法 JSON（含 highlights/issues/next_actions 各 ≥ 1 行 + content_summary），为 `report.question_assessment` 对每 turn 返回 dimension_results map (4 维度 × score_level；写入 wire 前映射为 B2 DimensionStatus)
- [x] 实现 setup：用真实 SQL fake / mem store 写入 1 个 user + 1 个 target_job + 1 个 practice_plan + 1 个 practice_session（status='completed', turn_count=3, language='en'）+ 3 个 practice_turns（turn_index=1/2/3, status='assessed', answer_summary='...'）+ 1 个 feedback_reports(status='queued') placeholder + 1 个 async_jobs(report_generate, status='queued', dedupe_key=session_id, attempts=0, locked_at=null)
- [x] 实现 trigger：启动 review runner（用 fake clock 触发 1 次 poll loop）
- [x] 实现 verify：
...
- [x] 执行 `cd backend && go test ./cmd/api -run TestE2EP0052ReportGenerationHappyPath -count=1`
...
- [x] 执行 `cd backend && go test ./cmd/api -run TestE2EP0053ReportReadAndListing -count=1`
- [x] 记录验证证据

## E2E.P0.054 AI 失败 graceful + retry policy

- [x] 在 `backend/cmd/api/reports_http_scenario_test.go` 新增 `TestE2EP0054ReportAIFailureAndRetry`：7 个 user/report/session/async_job 命名空间（每个对应一种失败模式）
- [x] 准备 fake F3 + fake AIClient + fake AITaskRunWriter + fake clock；按场景分支配置：
...
- [x] 实现 setup：写入 7 套 user/session/turns/feedback_reports(status='queued')/async_jobs(status='queued', attempts=0, locked_at=null)；report G 的 `async_jobs.attempts=4`
- [x] 实现 trigger：启动 review runner（fake clock 触发 poll loop 多次以覆盖所有 7 个）
- [x] 实现 verify：
...
- [x] 实现 cleanup
- [x] 执行 `cd backend && go test ./cmd/api -run TestE2EP0054ReportAIFailureAndRetry -count=1`
- [x] 记录验证证据

## E2E.P0.055 cross-user + privacy + legacy-negative

- [x] 在 `backend/cmd/api/reports_http_scenario_test.go` 新增 `TestE2EP0055ReportPrivacyAndLegacy`
- [x] 准备 fake F3 RegistryClient + fake AIClient + fake AITaskRunWriter + log / metric / audit 收集器
...
- [x] 实现 cleanup
- [x] 执行 `cd backend && go test ./cmd/api -run TestE2EP0055ReportPrivacyAndLegacy -count=1`
- [x] 记录验证证据

## 收口

- [x] `cd backend && go test ./cmd/api -run 'TestE2EP0052|TestE2EP0053|TestE2EP0054|TestE2EP0055' -count=1` 全绿
- [x] `python3 scripts/lint/backend_review_legacy.py --repo-root . --phase all` 通过（含 001 scoped legacy 反查项）
- [x] `python3 -m pytest scripts/lint/backend_review_legacy_test.py -q` 通过
