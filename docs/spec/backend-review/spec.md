...
- 不实现独立 worker / Asynq dispatcher / 生产级 outbox consumer / 集中式 retry orchestrator；本 spec 仅在 backend-review 包内实现 inline runner，沿用 `backend/internal/privacy/runner/` 先例；未来 `backend-async-runner` plan 接管时统一收干，本 spec D-13 显式说明此为临时形态。
...
| D-13 | Inline runner | 本 spec P0 在 `backend/internal/review/runner.go` 内实现 SELECT FOR UPDATE polling worker，沿用 `backend/internal/privacy/runner/` 包结构与 lifecycle（poll loop + lease + retry + graceful shutdown）；poll 周期默认 5s，max worker concurrency 默认 1，lease timeout 默认 5min；未来 `backend-async-runner` plan 接管时统一抽象 + 收干 | 不阻塞 P0；spec 显式记录"未来由 backend-async-runner 收干"避免后续重构忘记 |
...
- 本域 P0 实现 inline review runner + 消费 `async_jobs(report_generate)` queued 行；与 backend-practice / privacy/runner 共存于同一 backend 进程；未来 `backend-async-runner` plan 接管时必须读取 D-32 forward-binding 语义并保持当前 worker 不重复消费。
