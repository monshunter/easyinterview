...
| D-14 | parse-derived displayName | `resume.parse` 成功后从 LLM 结构化结果中派生可读 `display_name`，优先组合候选人姓名与标题 / 岗位 / 项目名称；不得把通用“上传的简历 / 粘贴的简历”作为 ready 完成态名称写回 | 支撑前端列表和只读详情展示可识别简历名称，避免用户看到无意义标题 |
