...
- [ ] 1.5 返回 202 + `ResumeAssetWithJob{resumeAssetId, job(jobType=resume_parse, status=queued)}`，与 [B2 fixture `registerResume.json`](../../../mock-contract-suite/spec.md) `default` / `paste-text` / `guided-answers` 三个 scenario 字节一致（验证：fixture parity test）
...
- [ ] 5.2 mock-first 对齐：`registerResume` (`default` / `paste-text` / `guided-answers`)、`getResume` (`default` / `not-found`)、`listResumes` (`default` / `empty` / `paginated`) 真实响应与对应 fixture 字节比对 PASS
