...
- [x] 3.1 实现 `backend/internal/resume/handler/get_version.go`：generated server interface `GetResumeVersion`；cross-user 返回 404 不暴露存在（验证：集中落地于 `backend/internal/resume/handler/version_read.go`；`cd backend && go test ./internal/resume/handler/... -run TestResumeVersionReadFixtureParity -count=1` PASS；`cd backend && go test ./internal/resume/... -count=1` PASS）
- [x] 3.2 实现 `backend/internal/resume/handler/list_versions.go`：generated server interface `ListResumeVersions`；asset ownership 404；cursor pagination 按 `updated_at DESC, id DESC` 唯一稳定序；invalid cursor → 422 `VALIDATION_FAILED`（验证：集中落地于 `backend/internal/resume/handler/version_read.go`；`cd backend && go test ./internal/resume/handler/... -run TestResumeVersionReadFixtureParity -count=1` PASS；`cd backend && go test ./internal/resume/... -count=1` PASS）
- [x] 3.3 实现 store `resume_versions.GetByID` + `ListByAsset(userID, resumeAssetID, cursor, pageSize)`，复用 B4 002 索引（验证：`cd backend && go test ./internal/resume/store/... -run 'TestGetVersionByID|TestListVersionsByAsset' -count=1` PASS；`cd backend && DATABASE_URL='postgres://easyinterview:dev@localhost:5432/easyinterview?sslmode=disable' go test ./internal/resume/store/... -tags=integration -run 'TestStructuredMasterUnique|TestResumeVersionListPagination' -count=1 -v` PASS）
- [x] 3.4 handler unit test：getResumeVersion 200 / 404 cross-user / 404 not exist；listResumeVersions empty / 25 行 + 第二页 + `hasMore=false` / asset 不属本人 404 / invalid cursor 422（验证：`cd backend && go test ./internal/resume/handler/... -run 'Test.*ResumeVersion' -count=1` PASS；`cd backend && go test ./internal/resume/... -count=1` PASS）
...
- [x] 4.1 实现 `backend/internal/resume/handler/update_version.go`：generated server interface `UpdateResumeVersion`；PATCH 语义；不可编辑字段（`versionType` / `resumeAssetId` / `parentVersionId` / `targetJobId` / `seedStrategy` / `provenance.promptVersion` 等 server-owned 字段）出现在请求体即 422（验证：`cd backend && go test ./internal/resume/handler/... -run TestUpdateResumeVersion -count=1` PASS；`cd backend && go test ./internal/resume/... -count=1` PASS）
- [x] 4.2 structured_profile partial merge：客户端 partial jsonb 与现有值合并；`provenance` 字段强制由 server 重置；`displayName` / `focusAngle` / `matchScore` 直接覆盖（验证：`cd backend && go test ./internal/resume -run TestUpdateResumeVersion -count=1` PASS；`cd backend && go test ./internal/resume/... -count=1` PASS；service 当前负责修剪字段并剥离 client provenance，事务内 merge/update 由 4.4 store gate 落地）
- [x] 4.3 IK 必带（D-8）；同 IK + 同 fingerprint 走 idempotency middleware replay；同 IK + 不同 fingerprint 409（验证：`cd backend && go test ./internal/resume/handler/... -run 'TestUpdateResumeVersion(RequiresIdempotencyKey|Idempotency|$)' -count=1` PASS；`cd backend && go test ./internal/resume/handler/... -run TestUpdateResumeVersion -count=1` PASS）
- [x] 4.4 实现 store `resume_versions.UpdatePatch`：单事务 UPDATE + 返回更新行；cross-user 404；deleted_at 行 404（验证：`cd backend && go test ./internal/resume/store/... -run 'TestUpdateVersionPatch|TestRepositoryExposesResumeAssetMethods' -count=1` PASS；`cd backend && DATABASE_URL='postgres://easyinterview:dev@localhost:5432/easyinterview?sslmode=disable' go test ./internal/resume/store/... -tags=integration -run 'TestResumeVersionUpdatePatch|TestStructuredMasterUnique|TestResumeVersionListPagination' -count=1 -v` PASS；`cd backend && go test ./internal/resume/... -count=1` PASS）
- [x] 4.5 handler unit test：happy（包括 structured_profile partial merge）/ IK replay / IK mismatch 409 / 422 不可编辑字段 / cross-user 404 / 404 deleted_at（验证：`cd backend && go test ./internal/resume/handler/... -run TestUpdateResumeVersion -count=1` PASS）
- [x] 4.6 在 `cmd/api` 挂载 `PATCH /api/v1/resume-versions/{resumeVersionId}` 含 IK middleware（验证：RED `cd backend && go test ./cmd/api -run 'TestBuildAPIHandlerMountsResumeRoutesBehindSessionMiddleware|TestResumeUpdateVersionHTTPScenario' -count=1` 失败于 405 Method Not Allowed；GREEN 同命令 PASS）
- [x] 4.7 字节比对 [B2 fixture `updateResumeVersion.json`](../../../mock-contract-suite/spec.md) `default` / `validation-error-422`，并补齐 `idempotency-replay` variant（验证：RED `cd backend && go test ./internal/resume/handler/... -run TestUpdateResumeVersionFixtureParity -count=1` 失败于 422 `details.field` 缺失；GREEN 同命令 PASS；`make validate-fixtures` PASS，`validate-fixtures: OK — 59 fixtures`；`cd backend && go test ./internal/resume/handler/... -run 'TestUpdateResumeVersion|TestUpdateResumeVersionFixtureParity' -count=1` PASS）
- [x] 4.8 BDD-Gate: 验证 `E2E.P0.075` 通过（验证：`test/scenarios/e2e/p0-075-resume-update-version-merge-and-ik/scripts/setup.sh && .../trigger.sh && .../verify.sh && .../cleanup.sh` PASS；trigger 覆盖 `make validate-fixtures`、`TestResumeUpdateVersionHTTPScenario`、`TestUpdateResumeVersionFixtureParity`、service/store focused tests、live integration `TestResumeVersionUpdatePatchMergeCrossUserAndDeleted`；verify 输出 `method=cmd-api-http`、fixture parity、DB merge / cross-user / deleted-row isolation、privacy grep 0；shell `LC_ALL=C.UTF-8` locale warning 非阻塞）
