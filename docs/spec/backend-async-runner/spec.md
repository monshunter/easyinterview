...
`backend/internal/runner/` 是唯一 backend in-process runtime kernel；`cmd/api` 通过 `buildRunnerKernel` 创建 `runner.Runtime`，并用 `Runtime.SetOutboxDispatcher` 挂接 `runner.OutboxDispatcher`。当前可执行 handler 集合为 `target_import` / `privacy_delete` / `resume_parse` / `resume_tailor` / `report_generate` / `email_dispatch`；所有业务 handler 直接实现 `runner.Handler`，所有 async job lease/finalize/reaper SQL 由 kernel 单点持有。`privacy_export` 保留为 DB / OpenAPI contract 但不注册 handler。
...
- `backend/internal/runner/` package 提供单一 `runner.Runtime`，负责 handler registry、lease（`SELECT ... FOR UPDATE SKIP LOCKED`）、finalize、business-job retry、reaper（lease timeout 回收）和 graceful shutdown drain；outbox/infra delivery 使用独立退避策略。
- 所有业务 handler 直接实现 `runner.Handler{Handle(ctx, runner.ClaimedJob) runner.JobOutcome}`；不保留第二套 async job 类型或 handler adapter。
- 接管当前可执行 canonical job_type 的运行：`target_import` / `privacy_delete` / `resume_parse` / `resume_tailor` / `report_generate` / `email_dispatch`；handler 业务实现仍由各 owner 域持有，本 subject 只承接注册与生命周期。`privacy_export` 只保留为 DB / OpenAPI contract，本计划不注册 handler。
...
| D-3 | Lease SQL contract + fencing | kernel claim返回递增后的`attempts`作为lease generation；所有kernel finalize必须匹配`id + status='running' + attempts=claimed.Attempts`且恰好更新1行，reaper后旧worker generation立即失效。当前直接写`async_jobs`的业务事务中，report status/success/failure与`resume_tailor` result/outbox必须先锁定并校验同一running+attempt generation，再写各自业务副作用 | 全局阻止旧worker覆盖新lease的kernel终态；当前报告链路与`resume_tailor`直接事务额外阻止stale worker写入report/result/outbox/audit/job，不把本条扩张为其他handler业务副作用已完成fencing的声明 |
| D-4 | Retry ownership 分层 | Business job基础设施重排默认`[10s,20s,40s,80s]` cap80，row `max_attempts`只决定lease/finalize终态；outbox独立保留`[30s,2m,10m,1h,6h]`。`report_generate`的单次用户动作在`GenerateReport`调用内部拥有`1+3`与`10s/20s/40s`，动作返回即销毁；runner不得用`async_jobs.attempts/max_attempts`编码产品重试次数或等待表 | 普通business job、report产品动作与infra投递三层独立；新用户动作清零，job lease重放不继承或恢复产品retry context |
| D-5 | Reaper 覆盖 | 单一 reaper，覆盖全部已注册 job_type；lease timeout 默认 `async.leaseTimeoutSeconds`（typed config 提供，默认 300s）；reaper interval 默认 `async.reaperIntervalSeconds`（默认 60s） | review.Reaper 删除；`target_import` / `privacy_delete` / `resume_parse` / `resume_tailor` / `report_generate` / `email_dispatch` 死锁 row 可恢复 |
...
| D-16 | TargetJob paste-only runner contraction | 删除 refresh canonical job 的 handler、kernel registration、lease loop、queue assignment、metrics expectation 与 cmd/api builder wiring；`target_import` 继续读取持久化 TargetJob paste text。独立 `source_records` 表/模型/查询保留，但不再拥有 refresh job/event/runtime | 消费 B3 2.15 的 7-job generated contract；当前 runtime 收敛为 6 handlers，禁止兼容注册或 dormant queue |
...
- 列名仅允许 `async_jobs.locked_at` / `attempts` / `max_attempts` / `available_at` / `status`；claim后的`attempts`就是lease generation，不新增同义token列。所有kernel finalize必须使用`status='running' AND attempts=claimed.Attempts` fencing并检查rows=1；当前report status/success/failure与`resume_tailor` result/outbox直接事务使用同一谓词，在业务写入前完成校验。
...
- `async_jobs` 业务失败使用 `DefaultBackoffPolicy()`：attempt1/2/3/4 后分别等待 `10s/20s/40s/80s`，更高 attempt 继续 cap80s。retryable outcome 在 `attempts < max_attempts` 时 finalize 为 `queued` 并推进 `available_at`，在 `attempts >= max_attempts` 时 finalize 为 `dead`；non-retryable outcome finalize 为 `failed`；不得让各域 handler 私自选择不同终态。
- `report_generate`沿用普通business job的基础设施lease/finalize契约；`async_jobs.attempts/max_attempts`只统计基础设施执行，不是产品调用计数，也不拥有`10s/20s/40s`。单次`GenerateReport`动作内的provider调用上限与等待由backend-review内存retry context负责；动作返回后不持久化，新的独立动作从initial重新开始。当前合同明确不保证进程crash/replay跨动作全局最多4次provider调用。
- Lease 超时回收**不**算 attempt 次数；回收后 `available_at` 仍按 business-job policy 前移避免 thrash。
- Outbox/infra delivery 必须显式使用 `DefaultOutboxBackoffPolicy()`，保留 `30s/2m/10m/1h/6h` 与 attempt5 failed；不得使用 business 10s/20s/40s/80s policy。两套 policy 共用无状态映射类型，不共享 schedule。
...
| C-2 | 分层退避表 | kernel business/outbox policy 默认值 | 分别调用 `DefaultBackoffPolicy().Next` 与 `DefaultOutboxBackoffPolicy().Next` | business返回10s/20s/40s/80s并cap80；outbox返回30s/2m/10m/1h/6h；runtime/outbox各自注入正确policy，retryable耗尽与non-retryable终态不变 | 001 Phase 1 |
...
| C-5 | target_import runtime | handler 已直接实现 `runner.Handler` | `Runtime.RunOnce` lease 并调用 handler | TargetJob paste import contract / cmd-api HTTP integration tests PASS；不存在 refresh handler fallback | 001 Phase 2 / Phase 7 |
...
| C-7 | Contract-only job negative | `privacy_export` 是 B3/B2/B4 contract-only job_type | 构建 runtime kernel | `runtime.Handles("privacy_export") == false`；6 个可执行 handler均已注册 | product-scope / backend runtime gates |
| C-8 | resume_parse / resume_tailor runtime | resume handlers 直接实现 `runner.Handler` 并注册到 kernel | 触发 resume 注册 + tailor run | `resume/jobs/*_test.go` + `cmd/api/resume_parse_runner_integration_test.go` / `resume_tailor_runner_integration_test.go` PASS | 001 Phase 2 / Phase 4 |
| C-9 | report_generate 迁移；review.Runner/Reaper 删除 | report job row queued | kernel lease 接管 | runner只负责lease/finalize；一次handler调用内部由review执行initial+最多3次retry及10s/20s/40s等待，返回后不留产品retry状态；outbox仍走独立infra policy | 001 Phase 2 / Phase 6 |
...
| C-18 | 全局代码回归 | runner/domain/cmd-api owner tests 已覆盖各 handler | P2 / P3 / P4 完成后从仓库根执行 `make test` | backend/frontend 全量回归 PASS；focused package 结果不替代全局门禁 | 001 Phase 4 |
...
| C-21 | L2 scheduler/backoff review invariants | critical/default handler长耗时、report action内provider/protocol失败、handler执行时间超过首档 | runtime/review处理失败 | email_dispatch不被starvation；fresh timestamp计算available/completed；report action内等待10s/20s/40s且新动作清零；runner business policy仍可到80s cap，outbox保持30s/2m/10m/1h/6h；任何跨层混用均失败 | 001 Phase 4 / Phase 6 |
...
| C-23 | report retry ownership | 同一用户动作发生retryable provider/protocol失败，随后用户再次独立触发`GenerateReport` | review执行动作内retry；runner只处理job lease/finalize | 首次动作最多4次调用且等待10s/20s/40s；动作返回销毁上下文；新动作从0重置；job attempts/max_attempts变化不等于产品重试次数 | 001 Phase 6 |
| C-24 | lease generation fencing | worker A attempt1被reap，worker B claim attempt2 | A/B迟到result交错 | kernel success/retry/failure finalize均令A得到typed stale且不覆盖attempt2；report stale success/failure对`feedback_reports`/outbox/audit/job零副作用，`resume_tailor` stale transaction对result/outbox零副作用；不执行report pre-call产品计数预留 | 001 Phase 5 / Phase 6 |
| C-25 | TargetJob refresh runner 删除 | B3 2.15 已生成 7-job contract，现有 runtime 仍可能注册旧 refresh handler/queue | 启动 kernel、运行 runner/targetjob focused tests并扫描 current surface | runtime 只注册 6 handlers；旧 refresh job 不可 enqueue/lease/handle、无 low queue assignment或 metric label expectation；`target_import` 仍通过；独立 `source_records` persistence 查询继续存在 | 001 Phase 7 |

## 7 关联计划

- [001-internal-job-outbox-runner](./plans/001-internal-job-outbox-runner/plan.md)：本 spec 的 active 原地修订计划。Phase 1-5保留单一kernel、outbox与lease-generation fencing的历史证据；Phase 6负责删除runner对report产品重试次数/等待表的所有权，并重新验证action-local retry与基础设施attempt分离。
