...
| C-5 | target_import runtime | handler 已直接实现 `runner.Handler` | `Runtime.RunOnce` lease 并调用 handler | TargetJob paste import pipeline / cmd-api HTTP scenario PASS；不存在 refresh handler fallback | 001 Phase 2 / Phase 7 |
