...
| P2-1 | HISTORICAL-SUPERSEDED: Phase 2 当时的 target_import / source_refresh handler 迁移 | 2 | 历史代码层回归 evidence；current contract 由 §2.11 覆盖 | historical regression |
...
| P2-4 | spec C-8 resume_parse / resume_tailor | 2 | `backend/internal/resume/jobs/*_test.go` + `backend/cmd/api/resume_parse_runner_integration_test.go` + `resume_tailor_runner_integration_test.go` rerun | unit + integration |
| P2-5 | spec C-9 report_generate；review.Runner/Reaper 删除 | 2 | 有价值断言迁移到 kernel / `GenerateHandler` / report contract tests | unit + integration |
...
| P4-3 | canonical runner contract cleanup | 4 | `scripts/lint/runner_out_of_scope_test.py` + native handler compile assertions + targetjob/resume cmd/api integration tests | contract + integration |
...
| X-6 | spec C-9 / C-21 `report_generate`产品retry使用动作内10/20/40且新动作清零；runner attempts/max_attempts只作基础设施lease；outbox保留infra schedule | 6 | runner backoff/runtime/outbox tests + backend-review action-retry/store integration | unit + integration (PG) |
| X-7 | spec C-24 kernel finalize与当前直接事务generation fencing | 5 | `runner/lease_integration_test.go::TestFinalizeAsyncJob_RejectsStaleLeaseGenerationAfterTakeover` + `store/review/report_persistence_integration_test.go` + `resume/store/tailor_runs_integration_test.go::TestCompleteTailorRunSuccessFencesStaleTakeover` | integration (PG) |
...
- Action schedule/reset and async-attempt separation remain code-level unit/integration assertions; they do not emit E2E markers.
...
| R-2 | 全量代码回归 | 4 | 仓库根 `make test` | backend/frontend unit regression |
...
| P7-3 | TargetJob paste import 与失败恢复代码回归 | 7 | targetjob/cmd-api focused tests；阶段收口根 `make test` | unit/integration regression |
...
- 阶段完成与 CI 回归统一从仓库根执行 `make test`；focused 命令仅用于开发反馈。

## 4 收口标准

- 所有 §2 行有对应通过证据（unit / integration / contract / lint / doc reconcile）。
- §2.7 R-1 lint 与 R-2 根 `make test` 通过。
