...
| P2-3 | D-22 retired module guard | 2 | Retired module negative search / current package absence check | regression-negative |
...
| P2-6 | D-22 retired jobs-recommendations guard | 2 | `make lint-runner-legacy` + retired module negative search | regression-negative |
...
| B-6 | retired async job names 不回流 | 2 / 4 | `make lint-runner-legacy` 与 active-doc negative grep | lint |
...
| X-5 | B3 D-18 / `shared/jobs.yaml` 8 canonical job_type 当前事实 | 2 / 4 | `python3 scripts/lint/events_inventory.py shared/events.yaml shared/jobs.yaml` + `make lint-runner-legacy` + `backend/internal/shared/jobs/jobs_test.go` | generated contract + lint |
