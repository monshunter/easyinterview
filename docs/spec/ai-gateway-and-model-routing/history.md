...
| 日期 | 版本 | 变更 | 关联计划 |
|------|------|------|----------|
| 2026-04-29 | 1.5 | 修复 L1 review 发现的一致性与完备性问题：`AICallMeta` 明确归 A3 runtime 拥有，B1 只提供 `AI_*` 错误码；fallback 执行层改为 AI routing layer（本地直连由 A3 client 执行，外部 gateway route 可由 gateway 执行）；metric 语义区分 per-call 与 event-only counter；STT / Audio Transcription 降为 C14 P2 预留；补齐 `Stream` 事件合同边界。 | plan-review remediation |
