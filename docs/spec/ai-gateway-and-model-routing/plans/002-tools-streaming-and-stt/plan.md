...
- **BDD 策略**: BDD 不适用。本 plan 打开的能力仍是内部 AI gateway / provider protocol / observability 契约；任何把 streaming、STT 或 tool 调用暴露到用户可见 UI / API workflow 的后续 workstream 必须在自身 plan 维护 BDD gate。
