...
在 [A2 dev-stack](../../../engineering-roadmap/spec.md#51-layer-a--foundation5-份全部-p0) docker compose 配置与 Kind manifest 中确认未默认设置 stub fallback；当 `AI_GATEWAY_BASE_URL` / `AI_GATEWAY_API_KEY` 缺失时启动失败。本 plan 不修改 A2 / Kind 资产本身（由各自 owner 维护），但需要在 `backend/internal/ai/aiclient/README.md`（包级 README）中明确记录上述 fail-fast 协议和本地部署要求（接真实 OpenAI-compatible LLM 服务），并写出本 plan smoke 验证步骤示意（导出真实 endpoint env 后跑 `go test -tags smoke ./backend/internal/ai/aiclient/...`，本 plan 不开 smoke build tag 的 CI 集成）。本地 smoke 测试时绝不在测试代码或 fixture 中嵌入真实 API key（[spec §7](../../spec.md#7-关联计划)）。
...
把 plan / checklist Header 在所有验收通过后由 active 切到 completed，运行 `/sync-doc-index --check` 与 `/sync-doc-index --fix-index` 同步 [ai-gateway-and-model-routing/plans/INDEX.md](../INDEX.md) 与根 [docs/spec/INDEX.md](../../../INDEX.md)。不修改 [engineering-roadmap/001-decompose-subspecs Phase 3.2](../../../engineering-roadmap/plans/001-decompose-subspecs/checklist.md#phase-3-wave-1基础设施--契约骨架)（W1 review gate 已闭合，C-8 不在本 plan 范畴）。把 Phase 5.1 / 5.2 / 5.3 命令输出贴入工作日志。给 [F1 observability-stack](../../../engineering-roadmap/spec.md#56-layer-f--quality-横切4-份) / [F3 prompt-rubric-registry](../../../engineering-roadmap/spec.md#56-layer-f--quality-横切4-份) / [B4 db-migrations-baseline](../../../db-migrations-baseline/spec.md) / 各 C 域 owner 留出 handoff 备注：A3 已暴露 `AITaskRunWriter` / `AuditEventWriter` / `prometheus.Registerer` 三个 DI 入口，依赖注入由各 owner 在自己 plan 中绑定真实实现。

## 4 验收标准

- spec [§6 验收标准](../../spec.md#6-验收标准) 中 C-1 / C-2 / C-3 / C-4 / C-5 / C-6 / C-7 / C-9 全部成立；C-8 引用 [engineering-roadmap/001-decompose-subspecs Phase 3.2](../../../engineering-roadmap/plans/001-decompose-subspecs/checklist.md#phase-3-wave-1基础设施--契约骨架) 的 W1 review gate，本 plan 不重复关闭。
