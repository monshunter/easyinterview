...
- [x] 3.4 落地 `config/feature-flags.yaml` current baseline：6 项 P0 flag 为 `practice_hint_enabled` / `report_evidence_v2_enabled` / `report_retry_plan_enabled` / `readiness_signals_enabled` / `ai_fallback_model_enabled` / `practice_assistance_mode_enabled`；显式标注 `public: true|false`；`ai_fallback_model_enabled` 必须 `public: false`；三项 out-of-scope key 仅作为负向测试输入
