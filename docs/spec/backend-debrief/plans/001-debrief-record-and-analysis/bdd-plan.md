...
执行入口：`bash test/scenarios/e2e/p0-0NN-<slug>/run.sh`；wrapper 必须保留真实 `go test` exit code，verify.sh 含 `--- PASS` + `ok` + 旧口径 grep 反查（参考 backend-practice/003 wrapper 规范）。
...
| 执行入口 | `bash test/scenarios/e2e/p0-061-debrief-get-and-cross-user/run.sh` |
...
| 执行入口 | `bash test/scenarios/e2e/p0-064-debrief-privacy-and-legacy-negative/run.sh` |
