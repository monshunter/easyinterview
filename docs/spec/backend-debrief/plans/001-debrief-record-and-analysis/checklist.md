...
- [ ] 0.1 B1 addendum：`shared/conventions.yaml` 新增 `DEBRIEF_NOT_FOUND` 错误码 + `DebriefRoundType` enum + `DebriefQuestionSource` enum；`make codegen-check` 同步 generated Go + TS + fixture parity 通过；commit `feat(shared-conventions): add debrief enums and error code` 落地（验证：`grep -rn "DEBRIEF_NOT_FOUND\|DebriefRoundType\|DebriefQuestionSource" shared/go shared/ts shared/fixtures` 命中所有 3 个标识符）
- [ ] 0.2 B2 addendum：`openapi/openapi.yaml` 新增 `POST /debriefs/question-suggestions` operation + `SuggestDebriefQuestionsRequest`/`Response` schema + 修复 `Debrief.roundType` / `CreateDebriefRequest.roundType` 引用 `$ref DebriefRoundType`；新增 fixtures `Debriefs/createDebrief.json` + `Debriefs/getDebrief.json` + `Debriefs/suggestDebriefQuestions.json`；`make codegen-check` + `make validate-fixtures` 通过；commit `feat(openapi): add suggestDebriefQuestions operation and align debrief round type` 落地（验证：generated `frontend/src/api/generated/` 含 `suggestDebriefQuestions`；generated `backend/internal/api/openapi/` 含 `SuggestDebriefQuestions` handler interface）
