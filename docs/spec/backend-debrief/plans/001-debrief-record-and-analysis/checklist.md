...
- [ ] 6.6 BDD-Gate: 执行 P0.060 scenario 四段脚本（`scripts/setup.sh` → `scripts/trigger.sh` → `scripts/verify.sh` → `scripts/cleanup.sh`）通过；verify.sh 含 backend runner pass marker + 旧口径 grep 反查
- [ ] 6.7 BDD-Gate: 执行 P0.061 scenario 四段脚本通过
- [ ] 6.8 BDD-Gate: 执行 P0.062 scenario 四段脚本通过
- [ ] 6.9 BDD-Gate: 执行 P0.063 scenario 四段脚本通过
- [ ] 6.10 BDD-Gate: 执行 P0.064 scenario 四段脚本通过
