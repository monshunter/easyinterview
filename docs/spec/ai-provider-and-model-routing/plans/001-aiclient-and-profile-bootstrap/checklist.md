...
- [x] 16.1 RED: recorder tests require `ai.complete.raw.v1` two-phase NDJSON, UUIDv7 `callId == ai_task_runs.id`, cross-restart non-collision, invalid/provider-failure evidence, append/concurrency, ConfigDir-parent resolution, symlink/non-regular rejection and existing-mode tightening to 0700/0600.<!-- verified: 2026-07-16 method=raw-io-focused-red evidence="compilable recorder tests fail on absent provider-before request+Sync, shared capture API, UUIDv7 task-run correlation, schema-invalid/provider-error, 100-call concurrency, append/reopen, privacy and hardened path behavior" -->
