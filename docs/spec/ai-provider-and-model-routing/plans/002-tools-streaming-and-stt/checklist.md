...
- [ ] 1.3 把 spec 版本从当前基线递增到下一版本（当前基线 2.3 时为 2.4+；若已继续递增则使用下一版本）并同步 history.md；验证: `docs/spec/ai-provider-and-model-routing/spec.md`、`history.md` 与 `docs/spec/INDEX.md` 版本一致，`python3 .agent-skills/sync-doc-index/scripts/sync-doc-index.py --check` 通过
...
- [ ] 3.3 HTTP wire（SSE 或 chunked）选型落地，并把决策写回 spec §3.1；验证: spec/history 更新通过 `make docs-check`，adapter/handler contract tests 证明 wire 形态一致，且后续 D3/B2 用户可见入口仍需自身 BDD gate

## Phase 4: STT provider adapter

- [ ] 4.1 与 C14 spec 联合锁定 `Transcribe` 入参形态后，更新 spec §4.1；验证: A3 与 C14 owner docs 均引用同一 audio payload contract，`make docs-check` 与 `sync-doc-index --check` 通过
...
- [ ] 4.4 复核 realtime fail-closed：只实现 STT 时不得打开 `practice.voice.realtime.default`；验证: `make lint-ai-profile-coverage` 断言 realtime profile 仍为 `unsupported`，除非 C14 / practice voice owner 已完成联合修订并记录触发证据

## Phase 5: 接入 F1 / F3 / B1

- [ ] 5.1 F1 metric / log / dashboard 字段扩展同步；验证: F1 spec / generated lint gate 与 focused observability tests 对新增字段、allowed labels、forbidden labels 均通过
