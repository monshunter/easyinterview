...
- **STT (`Transcribe(ctx, profile, audio) → (transcript, meta)`)**：新增 OpenAI-compatible `/v1/audio/transcriptions` provider adapter，使 `capability=stt` 具备可执行 adapter；tracked `practice.voice.stt.default` 的启用仍取决于 secret provisioning 与真实 provider smoke。provider-specific speech / TTS 底座归 004，用户可见电话模式与媒体生命周期归 `practice-voice-mvp`。
...
当前 24 项 checklist 中 23 项已完成；唯一未完成项为 6.3 真实 provider 非测试 smoke。`E2E.P0.007` / `E2E.P0.009` 已覆盖确定性级联与失败隔离，但其 in-process Go/Vitest runner 不使用真实 provider secret，不能替代该 gate，因此本 plan 保持 `active`。
...
- **提前激活边界**: Header `状态: active` 只承接 Tools / provider-side streaming / OpenAI-compatible STT transcription 与真实 provider smoke；provider-specific speech / TTS 归 004，媒体留存与 UI release gate 归 `practice-voice-mvp`，realtime multimodal / judge 继续 fail-closed。
...
落地 audio transcription provider 适配，复用 plan 003 的 provider registry / capability profile / fail-fast / 隐私红线机制；`capability=stt` adapter 可执行，tracked voice profile 在 secret provisioning 与真实 provider smoke 完成前保持 `disabled`。
...
Replace the duplicate malformed-chunk and provider-error stream tests with one table-driven contract test. Keep named cases, exact SSE chunks, one terminal error event, `AI_OUTPUT_INVALID` for malformed JSON and `AI_PROVIDER_TIMEOUT` for provider error events. This phase does not change the pending real-provider smoke or plan active status.

## 5 验收标准

- 本 plan 在 6.3 真实 provider smoke 完成前保持 `active`；其余 23 项 checklist 已勾选并有当前实现证据。
...
本 plan 已于 2026-05-06 根据用户明确确认提前激活。当前 owner 范围只包含 Complete tools payload、provider-side streaming SSE consumer、OpenAI-compatible STT Audio Transcriptions、metadata / observability / profile / registry / shared vocabulary gate，以及未完成的真实 provider smoke。provider-specific speech/TTS 由 004 承接，媒体留存与电话模式 UI/API 由 `practice-voice-mvp` 承接；realtime multimodal 与 judge adapter 必须由各自 owner spec / plan 打开。
...
| 2026-07-10 | 1.3 | 对齐 spec 2.22 与 003/004 当前 owner 边界，删除 cost-cap 范围，更新真实 provider smoke blocker，并统一 out-of-scope 术语。 | tech-debt pruning |
| 2026-05-22 | 1.2 | 将 6.3 从 Kind 部署 smoke 改为 host-run app 或 repo-tracked runner 的真实 provider smoke。 | local-dev-stack contract sync |
