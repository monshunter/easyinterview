...
- **STT (`Transcribe(ctx, profile, audio) → (transcript, meta)`)**：新增 OpenAI-compatible `/v1/audio/transcriptions` provider adapter，把 spec §2.1 中保留的 `capability=stt` 从 fail-closed profile 升级为可执行 adapter，归 [production voice future candidate](../../../engineering-roadmap/spec.md#53-future-candidates不自动-spawn) / practice voice owner P2。
...
[spec §3.2 待确认事项](../../spec.md#32-待确认事项) 列出三项尚未锁定的边界：`Tools(...)` 是否扩展、`model_profile_version` 是否独立 SemVer、`Stream` HTTP wire 选型。其中 `Tools(...)` 与 stream wire 选型已在 [ADR-Q6 §5](../../../engineering-roadmap/decisions/ADR-Q6-ai-provider-and-model-routing.md#5-失效与修订条件) 明确为「触发即修订」。STT `Transcribe(...)` 在 spec §2.2 与 §3.1 D-9 中显式归 [production voice future candidate](../../../engineering-roadmap/spec.md#53-future-candidates不自动-spawn) / practice voice owner P2；当前 A3 spec v2.4 仅允许 `capability=stt` / `capability=realtime` profile 以 `disabled` / `unsupported` 方式存在，运行时必须 fail-closed，直到本 plan 或 production voice / practice voice owner 联合修订后打开可执行 adapter。
...
- **Draft 实施阻断**: Header `状态: draft` 只表示文档占位，不是可编码许可。若 `/implement` no-arg 候选列表展示本 plan，leader 不得选择它；必须先完成 Phase 1、切换 Header 为 `active`，或修复 `/implement` candidate filter / lifecycle hard-stop 后再进入编码。
...
> - 后续业务域（典型为 [backend-practice](../../../engineering-roadmap/spec.md#52-当前-p0-实施-workstream-候选) 的 followup planner）显式要求 tool-based 结构化抽取；
> - [frontend-workspace-and-practice](../../../engineering-roadmap/spec.md#52-当前-p0-实施-workstream-候选) 或 [backend-practice](../../../engineering-roadmap/spec.md#52-当前-p0-实施-workstream-候选) 要求实时 token streaming 进入产品 UX；
> - production voice / practice voice owner 进入 P2 设计或实施；
...
把 [ai-provider-and-model-routing spec](../../spec.md) 版本从当前基线递增到下一版本（当前基线为 `2.4` 时即 `2.5+`，若 A3 已继续递增则使用当时的下一版本），在 §2.1 / §3.1 / §4 / §6 中追加被打开能力对应的接口契约、决策项、设计约束与 AC。同步更新 [history.md](../../history.md)。
...
> **激活条件**: Phase 1 已收口，且触发记录中包含「frontend-workspace-and-practice / backend-practice 实时 token streaming UX 需求」。
...
按 spec §3.2 待确认事项最终决策结果，落地 SSE 或 chunked 实现，并把决策写回 spec §3.1 决策表。前端消费由 [frontend-workspace-and-practice](../../../engineering-roadmap/spec.md#52-当前-p0-实施-workstream-候选) 自行 plan，不由本 plan 承担。

### Phase 4: STT provider adapter

> **激活条件**: Phase 1 已收口，且 production voice / practice voice owner 进入 P2 设计或实施。

#### 4.1 `Transcribe` 接口

新增 `Transcribe(ctx, profile, audio) → (transcript, meta)`；`audio` 入参形态（multipart / object key / 字节流）需先在 spec §4.1 + production voice / practice voice owner spec 联合锁定，本 plan 不预设。
...
激活后的收口必须执行 active-scope negative search，确认 A3-owned 代码、配置、deploy、generated artifacts、active docs 与被本 plan 激活并修订过的 owner docs 不含 `task_type`、`default.provider`、`config/ai-profiles/` 作为运行时 truth source、retired AI provider-route 术语、独立 `voice` route、独立 Mistakes / Growth / Drill 口径；历史 work journal / reports / bugs 可作为只读历史例外。对仍由其他 owner 保持的 referenced active spec（例如 F1 在完成迁移前的 `task_type` label），必须回到 Phase 5.1 owner handoff 后再计入 pass。
...
| 风险 | 应对措施 |
|------|----------|
| 在 trigger 未成立时被业务侧推动直接进入 /implement | 本 plan Header 锁 `状态: draft`；§7 治理章节明确禁止；进入需先按 Phase 1 完成 ADR / spec 修订 |
...
| draft plan 被 `/implement` 候选误当作可编码计划 | §3.1 与 §7 要求先完成 trigger evidence + ADR/spec 修订并切 `active`；若 `/implement` no-arg 仍展示 draft plan，leader 不得选择它，并应修复 candidate filter / lifecycle hard-stop 后再继续 |
...
| 日期 | 版本 | 变更 | 关联 |
|------|------|------|------|
| 2026-05-06 | 0.5 | L1 plan-review remediation：对齐当前 roadmap subject 命名、F1 `task_type` label owner handoff、draft `/implement` 候选阻断与 active-scope negative search 范围。 | plan-review --fix |
