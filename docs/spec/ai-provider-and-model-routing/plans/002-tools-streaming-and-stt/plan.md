...
激活后的收口必须执行 active-scope negative search，确认代码、配置、deploy、generated artifacts 与 active docs 不含 `task_type`、`default.provider`、`config/ai-profiles/` 作为运行时 truth source、retired AI provider-route 术语、独立 `voice` route、独立 Mistakes / Growth / Drill 口径；历史 work journal / reports / bugs 可作为只读历史例外。
