...
| `createPracticeVoiceTurn` downstream handoff | `openapi/fixtures/PracticeSessions/createPracticeVoiceTurn.json` owned by `practice-voice-mvp/001` | no active voice UI while disabled | `backend/internal/practice` leading disabled guard, owned by `practice-voice-mvp/001` | none while disabled | A3 004 supplies profile names, provider/client error semantics, privacy summary, and cost metadata | downstream handler/component contract tests; no E2E while voice is disabled |
