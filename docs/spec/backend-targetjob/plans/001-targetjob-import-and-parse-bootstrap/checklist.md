...
- [x] 11.3 `cmd/api` handler integration tests prove parse failure is not admitted into TargetJob read/list assets；这些 Go tests 属于代码层，focused run 只作开发反馈，阶段完成由仓库根 `make test` 承接。
...
- [x] 17.1 RED: Get/List store tests require typed completion/ready-plan facts, `session_completed` event filtering, pair non-null filtering and one list query for multiple cards；focused run only provides development feedback.
...
- [x] 17.5 BDD-Gate: `BDD.TARGETJOB.IMPORT.001` 由 [BDD checklist](./bdd-checklist.md) 关联 import/parse/Get/List owner behavior tests。
- [x] 17.5a E2E-HANDOFF: `E2E.P0.098` 仅承接真实 completion 后 progress refresh；本轮未运行，current-run 状态仍为 `Ready`。
- [x] 17.6 Repository-root `make test` provides frontend/backend unit regression；OpenAPI/migration, query-count, context/docs/index/diff/privacy remain separate gates.
...
- [x] 18.6 HISTORICAL-SUPERSEDED: 上一轮最终汇总 gate 不作为 Phase 19 重开后的当前完成证据；当前 root unit regression、zero-ref 与生命周期恢复由 19.5/19.6 承接。
...
- [x] 19.6 UNIT-GATE: 仓库根 `make test` 完成前后端全量单测回归；real PostgreSQL integration 与 schema drift 作为独立 gates。

## Phase 20: Injected raw JD text guard

- [x] 20.1 RED/GREEN: import service 注入小型 byte limit；ASCII/多字节 focused test 证明超限在 TargetJob/job/outbox/provider 前 typed reject，不构造默认大小输入。
- [x] 20.2 OWNER-GATE: missing/default/override/invalid 与跨字段配置只由 A4 typed contract 覆盖；本 owner 不复制 loader/composition/RuntimeConfig 传播测试。
