...
- [x] 4.5 Internal-only `source_refresh` follow-up job；验证: tests 覆盖 `target.parsed` 触发后写入 internal-only `async_jobs(job_type=source_refresh)`（B3 dotted task `source.refresh`）、payload 不含 source URL 完整路径、drainer 端 `SourceRefreshHandler` 标记 `target_job_sources.freshness_status='stale'`
