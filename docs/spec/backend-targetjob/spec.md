...
| C-2 | URL 导入与抓取 | 用户提交合规 `https` JD URL | `POST /targets/import` 后 runner kernel 处理 | URL 抓取受 SSRF / 长度 / timeout 守护，snapshot 写入 `target_job_sources.fetched_at` / `freshness_status`，敏感 query 串不入库 / 不入日志 | 001 |
...
| C-5 | 解析失败 non-retryable | 解析返回 `AI_OUTPUT_INVALID` 或 `TARGET_IMPORT_SOURCE_INVALID` | runner kernel 处理 job | 失败事务写入 `target.analysis.failed.retryable=false` 并删除失败 TargetJob 资产；失败 JD 原文、空标题、source snapshot 不作为可继续规划持久化，用户重试必须重新 import 创建新 `targetJobId` | 001 |
