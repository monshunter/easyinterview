...
//   - ApiError, PageInfo (B1 structures)
...
func renderB1AutoBlock(conv *Conventions) (string, error) {
	...
	sb.WriteString("    # The block tracks: ApiError, ApiErrorCode, PageInfo, JobStatus, and the\n")
	sb.WriteString("    # 14 enums catalogued in `shared/conventions.yaml#enums`.\n")
	...
	sb.WriteString("      description: |\n")
	sb.WriteString("        Canonical error envelope per spec D-4. All 4xx/5xx (and the P0 501\n")
	sb.WriteString("        privacy-export example) responses use this schema. Mirrors B1\n")
	sb.WriteString("        `structures.ApiError`.\n")
