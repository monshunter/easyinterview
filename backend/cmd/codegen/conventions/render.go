...
const reportContextTooLargeCode = "REPORT_CONTEXT_TOO_LARGE"
const reportContextTooLargeMessage = "report context exceeds supported generation size"
...
func RunFromBytes(data []byte, repoRoot string, verbose bool) error {
	...
	if err := validateReportContextTooLargeError(spec.Errors); err != nil {
		return err
	}
...
func validateReportContextTooLargeError(errors []ErrorSpec) error {
	matches := 0
	for _, entry := range errors {
		if entry.Code != reportContextTooLargeCode {
			continue
		}
		matches++
		if entry.Message != reportContextTooLargeMessage || entry.Retryable {
			return fmt.Errorf(
				"%s must use message %q and retryable=false",
				reportContextTooLargeCode,
				reportContextTooLargeMessage,
			)
		}
	}
	if matches != 1 {
		return fmt.Errorf("%s must appear exactly once, got %d", reportContextTooLargeCode, matches)
	}
	return nil
}
