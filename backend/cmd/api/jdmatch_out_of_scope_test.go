...
// This negative gate keeps the out-of-scope surface from coming back: no /api/v1/jd-match/*
...
`)
	...
	for _, p := range paths {
		...
		if rec.Code != http.StatusNotFound {
			t.Fatalf("expected 404 for out-of-scope route %s, got %d body=%s", p, rec.Code, rec.Body.String())
		}
	}
}
...
func TestGeneratedRouteCatalogHasNoJobMatchOperations(t *testing.T) {
	for _, route := range generated.AllRoutes {
		...
		if strings.Contains(lowerOp, "jobmatch") || strings.Contains(lowerOp, "watchlist") ||
			...
			t.Fatalf("generated route catalog still knows out-of-scope JD-Match surface: %s %s", route.OperationID, route.Path)
		}
	...
	for _, outOfScopeOperation := range []string{
