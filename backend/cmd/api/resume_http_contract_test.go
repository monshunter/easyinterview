...
func TestResumeRegisterListHTTPContract(t *testing.T) {
	dir := t.TempDir()
...
`)
	...
	resumeSvc := newResumeContractService()
	...
	registerRaw := doResumeJSON(t, handler, true, http.MethodPost, "/api/v1/resumes", "resume-contract-register", api.RegisterResumeRequest{
	...
	replayRaw := doResumeJSON(t, handler, true, http.MethodPost, "/api/v1/resumes", "resume-contract-register", api.RegisterResumeRequest{
		Title:      "Contract Resume",
		Language:   "en",
		SourceType: strPtr("paste"),
		RawText:    strPtr("Private resume body that must not leak."),
...
func TestResumeRegisterListHTTPValidationContract(t *testing.T) {
	dir := t.TempDir()
	writeAPIFile(t, filepath.Join(dir, "config.yaml"), `
runtime:
  appVersion: "1.2.3"
...
`)
	...
	handler := buildAPIHandler(
		...
		uploadRoutes{},
		resumeRoutes{Handler: resumehandler.New(resumehandler.Options{
			Service: &resumeValidationContractService{},
			Session: currentUserFromContext,
		})},
		reportRoutes{},
		jobsRoutes{},
...
type resumeValidationContractService struct {
	resumeContractService
}

func (s *resumeValidationContractService) RegisterResume(context.Context, resume.RegisterInput) (api.ResumeWithJob, error) {
	return api.ResumeWithJob{}, resume.ErrValidationFailed
}

func (s *resumeValidationContractService) ListResumes(context.Context, resume.ListRequest) (api.PaginatedResume, error) {
	return api.PaginatedResume{}, resumestore.ErrInvalidCursor
}

type resumeContractService struct {
...
func newResumeContractService() *resumeContractService {
	return &resumeContractService{
		resumes:   map[string]api.Resume{},
		summaries: map[string]api.ResumeSummary{},
		byKey:     map[string]api.ResumeWithJob{},
	}
...
func (s *resumeContractService) RegisterResume(_ context.Context, in resume.RegisterInput) (api.ResumeWithJob, error) {
...
func (s *resumeContractService) GetResume(_ context.Context, _ string, resumeID string) (api.Resume, error) {
...
func (s *resumeContractService) ListResumes(_ context.Context, in resume.ListRequest) (api.PaginatedResume, error) {
