...
// This negative gate keeps the retired version-tree routes and operation IDs
...
func TestResumeVersionRoutesAreGonePerD20(t *testing.T) {
	dir := t.TempDir()
	...
	for _, tt := range tests {
		t.Run(tt.method+" "+tt.path, func(t *testing.T) {
			...
			if rec.Code != http.StatusNotFound {
				t.Fatalf("expected 404 for retired route %s %s, got %d body=%s", tt.method, tt.path, rec.Code, rec.Body.String())
			}
		})
	}
}
...
func TestGeneratedRouteCatalogHasNoResumeVersionOperations(t *testing.T) {
	...
	for _, route := range generated.AllRoutes {
		...
		for _, retired := range retiredOperations {
			if lowerOp == strings.ToLower(retired) {
				t.Fatalf("generated route catalog still knows retired operation %q", retired)
			}
		}
		if strings.Contains(lowerPath, "resume-versions") || strings.Contains(lowerPath, "structured-master") {
			t.Fatalf("generated route catalog still knows retired resume version path: %s %s", route.OperationID, route.Path)
		}
	}
	for _, retired := range retiredOperations {
