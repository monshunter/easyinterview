...
// This negative gate keeps the non-current version-tree routes and operation IDs
	...
	for _, tt := range tests {
		t.Run(tt.method+" "+tt.path, func(t *testing.T) {
			...
			if rec.Code != http.StatusNotFound {
				t.Fatalf("expected 404 for non-current route %s %s, got %d body=%s", tt.method, tt.path, rec.Code, rec.Body.String())
			}
		})
	}
}
...
func TestGeneratedRouteCatalogHasNoResumeVersionOperations(t *testing.T) {
	...
	for _, route := range generated.AllRoutes {
		...
		for _, nonCurrent := range nonCurrentOperations {
			if lowerOp == strings.ToLower(nonCurrent) {
				t.Fatalf("generated route catalog still knows non-current operation %q", nonCurrent)
			}
		}
		if strings.Contains(lowerPath, "resume-versions") || strings.Contains(lowerPath, "structured-master") {
			t.Fatalf("generated route catalog still knows non-current resume version path: %s %s", route.OperationID, route.Path)
		}
	}
	for _, nonCurrent := range nonCurrentOperations {
