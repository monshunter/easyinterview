...
func TestValidTaskTypesIncludesReportAssessment(t *testing.T) {
	ctx := AITaskRunContext{
		UserID:       "01918fa0-0010-7a00-8a00-000000000001",
		Capability:   AITaskRunTaskReportAssessment,
		ResourceType: AITaskRunResourceFeedbackReport,
		ResourceID:   "01918fa0-0020-7a00-8a00-000000000002",
	}
	if err := ctx.Validate(); err != nil {
		t.Fatalf("Validate should accept report_assessment: %v", err)
	}
	if _, ok := allowedAITaskRunCapabilities[AITaskRunTaskReportAssessment]; !ok {
		t.Fatalf("allowedAITaskRunCapabilities missing %q", AITaskRunTaskReportAssessment)
	}
}
