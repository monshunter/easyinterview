...
// D-12: the legacy in-process BackgroundMailDispatcher must not reappear in the
...
func TestNoBackgroundDispatcher(t *testing.T) {
	...
	for _, e := range entries {
		...
		if strings.Contains(string(data), "BackgroundMailDispatcher") {
			t.Fatalf("%s still references BackgroundMailDispatcher; legacy in-process dispatcher must be removed", name)
		}
	}
}
