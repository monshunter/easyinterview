...
func TestE2EP0003EmailCodeSessionCookie(t *testing.T) {
	now := time.Date(2026, 5, 6, 12, 0, 0, 0, time.UTC)
	store := newEmailCodeScenarioStore()
...
type emailCodeScenarioStore struct {
...
func newEmailCodeScenarioStore() *emailCodeScenarioStore {
	return &emailCodeScenarioStore{
		challenges:      map[string]*scenarioChallenge{},
		usersByEmail:    map[string]auth.UserContext{},
		usersByID:       map[string]auth.UserContext{},
		sessionsByHash:  map[string]auth.SessionRecord{},
...
func (s *emailCodeScenarioStore) CountRecentChallenges(_ context.Context, email string, ipHash string, since time.Time) (int, error) {
	count := 0
...
func (s *emailCodeScenarioStore) CreateChallenge(_ context.Context, rec auth.ChallengeRecord) error {
	s.challenges[rec.TokenHash] = &scenarioChallenge{record: rec, status: "pending"}
	return nil
}

func (s *emailCodeScenarioStore) ConsumeChallenge(_ context.Context, tokenHash string, now time.Time) (auth.ChallengeRecord, error) {
...
func (s *emailCodeScenarioStore) CreateUserByEmail(_ context.Context, email string, displayName string, userID string, _ time.Time) (auth.UserContext, error) {
...
func (s *emailCodeScenarioStore) FindUserByEmail(_ context.Context, email string) (auth.UserContext, error) {
...
func (s *emailCodeScenarioStore) CreateSession(_ context.Context, rec auth.SessionRecord) error {
	s.sessionsByHash[rec.SessionHash] = rec
	s.sessionsByID[rec.ID] = rec.SessionHash
	return nil
}

func (s *emailCodeScenarioStore) GetSessionByHash(_ context.Context, sessionHash string, _ time.Time) (auth.SessionRecord, error) {
...
func (s *emailCodeScenarioStore) GetUserContext(_ context.Context, userID string) (auth.UserContext, error) {
...
func (s *emailCodeScenarioStore) TouchSession(_ context.Context, sessionID string, now time.Time, expiresAt time.Time) error {
...
func (s *emailCodeScenarioStore) RevokeSession(_ context.Context, sessionID string, now time.Time) error {
	hash, ok := s.sessionsByID[sessionID]
	if !ok {
		return auth.ErrSessionInvalid
	}
...
func (s *emailCodeScenarioStore) CreatePrivacyDeleteHandoff(_ context.Context, userID string, idempotencyKey string, privacyRequestID string, jobID string, now time.Time) (auth.PrivacyDeleteHandoff, error) {
...
func (s *emailCodeScenarioStore) privacyHandoffCount() int {
	return len(s.privacyHandoffs)
}

func (s *emailCodeScenarioStore) sessionStatus(sessionID string) auth.SessionStatus {
