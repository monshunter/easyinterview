...
func writeResumeServiceError(w http.ResponseWriter, err error, fallbackMessage string) {
	if errors.Is(err, resume.ErrValidationFailed) || errors.Is(err, resume.ErrInvalidCursor) || errors.Is(err, resume.ErrResumeLimitExceeded) || errors.Is(err, resumestore.ErrInvalidCursor) {
