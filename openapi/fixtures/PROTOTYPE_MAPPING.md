...
- **缺失数据节直接跳过**：data.jsx 缺哪一节，对应 fixture 的 `prototype-baseline` 就不写入；不强制 36/36 覆盖。但本表列出的 P0 关键 8 个 endpoint 必须存在数据节并写入 prototype-baseline。
...
| `user` | `getMyProfile` | 1:1 | Profile | `years` → `yearsOfExperience`；`title` → `currentRole` / `headline`；`locale` → `uiLanguage` / `preferredPracticeLanguage`；`region` 取 prototype 隐含值。 |
...
| `report` | `getFeedbackReport` | 1:1 | Reports | `report.readiness` → `preparednessLevel`（按 §4 翻译）；`highlights/issues/perQuestion` → `highlights/issues/questionAssessments`；`dimensions` 折成 `questionAssessments[i].dimensionResults`。`provenance` 由同步工具按 §4 默认填入。 |
| `mistakes[]` | `listMistakes` | 1:N | Mistakes | `ability` → `competencyCode`（lower-snake）；`question` → `questionText`；`status` 翻译到 `MistakeStatus` enum；`priority` 从中文等级翻成数字；`provenance` 由 §4 默认填入。 |
| `growth` | `getGrowthOverview` | 1:1 | Growth | `last30d` → `summary`（字段重命名）；`weeks` + `trendDim` 折成 `dimensionTrend`；`recent` 折成 `preparednessTrend`（`readiness` 数值翻译为 `ReadinessTier`）。 |
| `debriefs[0]` | `getDebrief` | N:1 | Debriefs | 第一个 debrief → `Debrief`：`company/round` → `roundType` 推导；`hotQuestions` → `questions[].questionText`；`nextAction` → `nextRoundChecklist[].label`。 |
| `resume` | `getResumeTailorRun` | 1:1 | ResumeTailor | `version` → `id` 派生；`match` → `matchSummary.strengths` / `gaps`；`suggestions[]` → `suggestions[]`（`before/after/reason`）。 |
...
- **enum 翻译**:
    ...
    - `mistakes[].status` 中文 → `MistakeStatus`：`未解决→open`、`改善中→improving`、`已攻克→mastered`。
    - `mistakes[].priority` 中文 → 数字：`高→80`、`中→50`、`低→20`。
